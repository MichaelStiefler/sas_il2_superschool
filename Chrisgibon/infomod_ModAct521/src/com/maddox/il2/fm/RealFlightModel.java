...
*/
        ...
        if(Config.isUSE_RENDER() && showFM && actor == Main3D.cur3D().viewActor())
        {
            ...
            TextScr.output(5, 235, "VATOR TR = " + CT.trimElevator);
            TextScr.output(5, 210, "RUDDR TR = " + CT.trimRudder);
            TextScr.output(5, 110, "Vtas " + (int)getSpeedKMH() + " km/h");
            for(int i1 = 0; i1 < super.EI.getNum(); i1++)
            {
                hpOld[i1] = hpOld[i1] * 0.95F + (0.05F * super.EI.engines[i1].w * super.EI.engines[i1].engineMoment) / 746F;
                TextScr.output(650, 485, "---ENGINES (" + super.EI.getNum() + ")---");
            ...
            int m = 5;
            int l = 700;
            for(int i1 = 0; i1 < CT.Weapons.length; i1++)
            {
                if(CT.Weapons[i1] != null)
                {
                    for(int j1 = 0; j1 < CT.Weapons[i1].length; j1++)
                    {
                        BulletEmitter bulletemitter = CT.Weapons[i1][j1];
                        if(null != bulletemitter)
                        {
                            int k1 = bulletemitter.countBullets();
                            float bulletmassa = bulletemitter.bulletMassa();
                            TextScr.output(m, l, bulletemitter.getHookName() + " " + k1 + " x " + bulletmassa + " = " + (float)k1 * bulletmassa + " kg");
                            if((l -= 20) <= 300)
                            {
                                l = 700;
                                m += 400;

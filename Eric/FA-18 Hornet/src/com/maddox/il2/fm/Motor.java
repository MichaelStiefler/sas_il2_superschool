...
public class Motor extends FMMath
{
 ...
 // --------------------------------------------------------
		...
		if(stage > 0 && stage < 6 && l > given)
		{
			...
			if((type == 0 || type == 7) && starter != 1){
				...
				if((starter == 0 || starter == 3 || starter == 5) && stage > 3){
					...
					if(World.Rnd().nextFloat(0.0F,10.0F) < 3F)
						stage = 3;
				}
			}
			//Radial engines
			...
			{
				if(starter == 2 ){
				if(stage == 3 && World.Rnd().nextFloat(0.0F,10.0F) < 2F)
					stage = 2;
				else
					if(stage > 3 && World.Rnd().nextFloat(0.0F,10.0F) < 3F)
						stage = 3;
				}
				if(starter == 0 || starter == 3 || starter == 5){
					if(stage > 3 && World.Rnd().nextFloat(0.0F,10.0F) < 0.25F && reference.isStationedOnGround())
						stage = 0;
					else
					if(stage == 3 && World.Rnd().nextFloat(0.0F,10.0F) < 1F)
						stage = 2;
					else
					if(stage > 3 && World.Rnd().nextFloat(0.0F,10.0F) < 3F)
						stage = 3;
			...
			if((starter == 1) || (type == 9))
			{
				if(stage > 3 && World.Rnd().nextFloat(0.0F,10.0F) < 2F && reference.isStationedOnGround())
					stage = 0;
				else
				if(stage == 3 && World.Rnd().nextFloat(0.0F,10.0F) < 2F)
					stage = 2;
    ...
    public void computeFuel(float f)
    {
        tmpF = 0.0F;
        if(stage == 6)
        {
            ...
            case 2: // '\002'
                ...
                float heightOffset = reference.HofVmax - reference.getAltitude();
                if (heightOffset < 0.0F) heightOffset *= -1.0F;
                float optimumHeightFactor = 0.5F; // Percentage of Fuel Consumption at opt. Alt, adjust as requested
                float heightFactor = optimumHeightFactor + (heightOffset / reference.HofVmax * (1.0F - optimumHeightFactor));
                tmpF *= heightFactor;
                break;
    ...
    private void computeTemperature(float f)
    {
        ...
        float f4 = 0.8F + 60F / (50F + f1);
        float f5 = 0.45F;
        if(engineBoostFactor > 1.0F)
            f3 = (controlThrottle + Math.max(controlThrottle - 1.0F, 0.0F) * (engineBoostFactor - 1.1F) * 10F) / engineBoostFactor;
        if(!controlAfterburner && engineAfterburnerBoostFactor > 1.0F)
            f3 /= engineAfterburnerBoostFactor;
        float f6 = f3;
        if(type == 0 || type == 1 || type == 7)
            f5 = 0.45F / Atmosphere.density((float)reference.Loc.z);
        else
        if(type == 2)
            f5 = 0.35F + 0.1F / Atmosphere.density((float)reference.Loc.z);
        float f7 = coolMult * f3;
        if(World.cur().diffCur.ComplexEManagement && reference.isPlayers() && ((RealFlightModel)reference).isRealMode())
            f6 = w / wWEP;
        if(stage == 6)
        {
            float f8 = f2 + tOilOutMaxRPM * (1.0F - 0.15F * controlRadiator) * (0.65F + 0.45F / Atmosphere.density((float)reference.Loc.z)) * (1.0F + (float)reference.AS.astateOilStates[number] * 0.35F) * 1.2F * f6 * f6 * (0.7F + 0.3F * f7) * f4;
            tOilOut += (f8 - tOilOut) * f * tChangeSpeed;
            float f12 = tOilOut * (0.75F - 0.05F * controlRadiator);
            tOilIn += (f12 - tOilIn) * f * 0.5F;
            f12 = f2 + tWaterMaxRPM * (1.0F - 0.15F * controlRadiator) * (0.65F + f5) * (0.6F + 0.4F * f6) * (0.1F + 1.1F * f7) * f4 * (1.1F - 0.1F * controlMix);
            tWaterOut += (f12 - tWaterOut) * f * tChangeSpeed;
        } else
        {
            float f9 = f2;
            tOilOut += (f9 - tOilOut) * f * tChangeSpeed * (0.2F + 0.2F * controlRadiator);
            float f13 = tOilOut;
            tOilIn += (f13 - tOilIn) * f * 0.5F;
            f13 = f2;
            tWaterOut += (f13 - tWaterOut) * f * tChangeSpeed * (0.2F + 0.2F * controlRadiator);
        }
        if(!reference.isPlayers())
            return;
        if(World.cur().diffCur.Engine_Overheat && (tWaterOut > tWaterCritMax || tOilOut > tOilCritMax))
        {
            ...
            {
                float f10 = tWaterOut / tWaterCritMax - 1.0F;
                f10 *= f10 * f10 * f * 1000F;
                float f14 = World.Rnd().nextFloat() * timeOverheat;
                if(f10 > f14)
                    switch(type)
                    {
                    default:
                    ...
                    case 7: // '\007'
                        ...
                        {
                            reference.AS.setEngineCylinderKnockOut(reference.actor, number, 1);
                            reference.AS.setSootState(reference.actor, number, 3);
            ...
            {
                float f11 = tOilOut / tOilCritMax - 1.0F;
                f11 *= f11 * f11 * f * 1000F;
    ...
    private void computeForces(float f)
    {
        ...
        case 1: // '\001'
        case 9: // '\009'
			//TODO: Stand-in code for turboprop
		case 10: // '\002'
            ...
            case 9: // '\t'
                if(bControlPropAuto)
                {
                    ...
                    if(isPropAngleDeviceOperational())
                    {
                        if(w < f22)
                        {
                            f22 = Math.min(1.0F, 0.01F * (f22 - w) - 0.012F * aw);
                            propTarget -= f22 * getPropAngleDeviceSpeed() * f;
                        } else
                        {
                            f22 = Math.min(1.0F, 0.01F * (w - f22) + 0.012F * aw);
                            propTarget += f22 * getPropAngleDeviceSpeed() * f;
                        }
        ...
        case 2: // '\002'
            ...
            float f32 = f30 * f30;
            float f33 = 1.0F - 0.006F * (Atmosphere.temperature((float)reference.Loc.z) - 290F);
        ...
        case 6: // '\006'
            ...
            if(f8 > 208.333F)
                if(f8 > 291.666F)
                    ...
                    propForce *= (float)Math.sqrt((291.666F - f8) / 83.33299F);
            engineForce.set(engineVector);
            engineForce.scale(propForce);
            engineTorque.cross(enginePos, engineForce);
            rpm = toRPM(w);
            ...
            f8 = Aircraft.cvt(propForce, 0.0F, thrustMax, 0.0F, 0.21F);
    ...
    public boolean isAllowsAutoProp()
    {
        if(reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel)reference).isRealMode())
            if(World.cur().diffCur.ComplexEManagement)
                ...
                case 1: // '\001'
                case 2: // '\002'
                    return (reference.actor instanceof SPITFIRE9) || (reference.actor instanceof SPITFIRE8) || (reference.actor instanceof SPITFIRE8CLP);
...
label0:
                    ...
                    if(f2 > f5)
                    {
                        ...
                        for(int j = 1; j <= controlCompressor; j++)
                            if(compressorAltMultipliers[controlCompressor] >= 1.0F)
                                f46 *= compressorBaseMultipliers[controlCompressor];
                            else
                                f46 *= compressorBaseMultipliers[controlCompressor] * compressorAltMultipliers[controlCompressor];
                ...
                do
                {
                    ...
                    if(f2 > f6)
                    {
                        ...
                        for(int k = 1; k <= controlCompressor; k++)
                            if(compressorAltMultipliers[controlCompressor] >= 1.0F)
                                f49 *= compressorBaseMultipliers[controlCompressor];
                            else
                                f49 *= compressorBaseMultipliers[controlCompressor] * compressorAltMultipliers[controlCompressor];
                    ...
                    } else
                    {
                        ...
                        f38 = (f34 * f2 * f15 * (compressorPAt0 + (1.0F - compressorPAt0) * w * _1_wMax)) / (f6 * f29);
                    }
                    if(f38 > f25)
                    {
                        f25 = f38;
	...
	{
		float f1 = 0.0F;
		if(type == 10) //TODO: Turboprops spin-down like jets
		{
			...
			if(!bPropHit && (bIsInoperable || stage == 0 || controlMagneto == 0))
			{
				...
				if((((Interpolate) (reference)).actor instanceof BF_109) || (((Interpolate) (reference)).actor instanceof BF_110) || (((Interpolate) (reference)).actor instanceof JU_87) || (((Interpolate) (reference)).actor instanceof JU_88) || (((Interpolate) (reference)).actor instanceof JU_88NEW) || ((((Interpolate) (reference)).actor instanceof FW_190) && type == 0))
				{
					...
					if((((Interpolate) (reference)).actor instanceof P_51) || (((Interpolate) (reference)).actor instanceof P_47) || (((Interpolate) (reference)).actor instanceof P_38) || (((Interpolate) (reference)).actor instanceof P_39) || (((Interpolate) (reference)).actor instanceof P_40) || (((Interpolate) (reference)).actor instanceof P_40SUKAISVOLOCH) || (((Interpolate) (reference)).actor instanceof SPITFIRE) || (((Interpolate) (reference)).actor instanceof Hurricane) || ((((Interpolate) (reference)).actor instanceof FW_190) && type == 1) || type == 1)
					{
						...
						if(type == 9) //TODO: Spin-down behaviour for rotaries. Great rotating inertial mass, hence longer spin-down
						{
							fricCoeffT += 0.07F * f;
							if(fricCoeffT > 0.05F)
								fricCoeffT = 0.05F;

...
public class Ballistics
{
    ...
    public static void updateSpinningRocket(Actor actor, float f, float f1, float f2, boolean flag, float f3)
    {
        float f4 = Time.tickLenFs();
        float f5 = (float)actor.getSpeed(v);
        actor.pos.getAbs(pos, or);
        dir.set(1.0D, 0.0D, 0.0D);
        or.transform(dir);
        float f6;
        if(f5 > 330F)
            f6 = KF(f5) * KD((float)pos.z) * f1;
        else
            f6 = (0.2F * f1 * 1.225F * KD((float)pos.z) * f5 * f5) / 2.0F;
        float f7 = (f2 - f6) / f;
        dir.scale(f7 * f4);
        v.add(dir);
        if(flag)
            v.z -= (f4 * Atmosphere.g()) / (f3 + 1.0F);
        actor.setSpeed(v);
        pos.x += v.x * (double)f4;
        pos.y += v.y * (double)f4;
        pos.z += v.z * (double)f4;

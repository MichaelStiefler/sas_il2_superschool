    ...
    implements MsgCollisionRequestListener, MsgCollisionListener
{
    class Interpolater extends Interpolate
    {
        ...
        public boolean tick()
        {
            ...
            {
                if(interpolateStep())
                    Ballistics.updateSpinningRocket(actor, M, S, P, timeBegin + noGDelay < Time.current(), spinFactor);
            } else
            if(interpolateStep())
                Ballistics.update(actor, M, S, P, timeBegin + noGDelay < Time.current());
            return true;
        }
    ...
    protected void endSmoke()
    {
        ...
        Eff3DActor.finish(smokeStart);
        Eff3DActor.finish(smokeTile);
        destroy(flame);
        stopSounds();
    }
    ...
    public void start(float f, int i)
    {
        ...
        if(World.cur().diffCur.RealisticRocketSpread)
        {
            float f2 = -1F + 2.0F * rangerandom.nextFloat();
            f2 *= f2 * f2;
            float f4 = -1F + 2.0F * rangerandom.nextFloat();
            f4 *= f4 * f4;
            init(f1, Property.floatValue(class1, "massa", 6.8F), Property.floatValue(class1, "massaEnd", 2.52F), Property.floatValue(class1, "timeFire", 4F) / (1.0F + 0.1F * f2), Property.floatValue(class1, "force", 500F) * (1.0F + 0.1F * f2), f + f4 * 0.1F);
            ...
            float f5 = 0.68F * Property.floatValue(class1, "maxDeltaAngle", 3F);
            f2 = -1F + 2.0F * rangerandom.nextFloat();
            f4 = -1F + 2.0F * rangerandom.nextFloat();
            f2 *= f2 * f2 * f5;
            f4 *= f4 * f4 * f5;
            Aircraft.tmpOr.increment(f2, f4, 0.0F);
        } else
        {
            ...
            pos.setAbs(pos.getCurrent());
            pos.getAbs(Aircraft.tmpOr);
            float f3 = 0.5F * Property.floatValue(class1, "maxDeltaAngle", 0.0F);
            Aircraft.tmpOr.increment(rangerandom.nextFloat(-f3, f3), rangerandom.nextFloat(-f3, f3), 0.0F);
        }
        if(getOwner() == World.getPlayerAircraft())
            setName("MyRocket_" + World.getPlayerAircraft().FM.CT.getWeaponCount(2));
        ...
        s = Property.stringValue(class1, "smokeStart", null);
        if(s != null)
        {
            if(hook == null)
                hook = findHook("_SMOKE");
            smokeStart = Eff3DActor.New(this, hook, null, 1.0F, s, -1F);
            if(smokeStart != null)
                smokeStart.pos.changeHookToRel();
        }
        s = Property.stringValue(class1, "smokeTile", null);
        if(s != null)
        {
            if(hook == null)
                hook = findHook("_SMOKE");
            smokeTile = Eff3DActor.New(this, hook, null, 1.0F, s, -1F);
            if(smokeTile != null)
                smokeTile.pos.changeHookToRel();
        }
    ...
    {
        Class class1 = getClass();
        String s = Property.stringValue(class1, "friendlyName", "Rocket");
        return s;
    }
    ...
    protected Eff3DActor smokeStart;
    protected Eff3DActor smokeTile;

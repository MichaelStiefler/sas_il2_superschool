// Decompiled by DJ v3.10.10.93 Copyright 2007 Atanas Neshkov  Date: 07.09.2015 13:57:52
...
//            Cockpit, TypeStormovik, AircraftLH
    ...
    public void msgShot(Shot paramShot)
    {
      ...
      if (tmpBonesHit > 0)
      {
        ...
        for (int j = 0; j < tmpBonesHit; j++) {
          hierMesh(); String str1 = HierMesh.collisionNameMulti(j, 1);
          if (str1.length() == 0) { hierMesh(); str1 = HierMesh.collisionNameMulti(j, 0); }
          if (paramShot.power > 0.0F) {
            hierMesh(); Pd.interpolate(tmpP1, tmpP2, HierMesh.collisionDistMulti(j));
            tmpLoc3.transformInv(Pd);

            debuggunnery("Hit Bone [" + str1 + "], E = " + (int)paramShot.power);
            hitBone(str1, paramShot, Pd);
            if (!str1.startsWith("xx")) {
            ...
            if ((this.FM.CT.bHasGearControl) && ((str1.toLowerCase().startsWith("xgear")) || (str1.toLowerCase().startsWith("gear"))))
            {
              if ((getEnergyPastArmor(2.0F, paramShot) > 0.0F) && (paramShot.power > 2500.0F))
              {
                k = -1;
                String str2 = "";
                if (str1.toLowerCase().startsWith("x"))
                  str2 = str1.substring(5, 6).toUpperCase();
                else {
                  str2 = str1.substring(4, 5).toUpperCase();
                }

                int m = 0;

                int n = World.Rnd().nextInt(0, 150);
                ...
                if (((k == 0) || (k == 1)) && (n > 99) && (chunkDamageVisible("WingLIn") > 1) && (chunkDamageVisible("WingRIn") > 1))
                {
                  this.FM.AS.hitGear(paramShot.initiator, 0, 4);
                  this.FM.AS.hitGear(paramShot.initiator, 1, 4);
                }
                else if (m != 0)
                {
                  if (paramShot.power > 40000.0F)
                  {
                    k += 3;
                  }
                  if (m == 8)
                  {
                    if (World.Rnd().nextFloat() < 0.5D)
                    {
                      this.FM.AS.hitGear(paramShot.initiator, 1, 8);
                      this.FM.AS.hitGear(paramShot.initiator, 0, 8);
                    }
                    else
                    {
                      this.FM.AS.hitGear(paramShot.initiator, k, m);
                    }
                  }
                  else {
                    this.FM.AS.hitGear(paramShot.initiator, k, m);
      ...
      for (int j = 0; j < tmpBonesHit; j++)
      {
        hierMesh(); if (HierMesh.collisionNameMulti(j, 1) != null) { hierMesh(); hierMesh(); if (!HierMesh.collisionNameMulti(j, 1).equals(HierMesh.collisionNameMulti(j, 0))); } else { i = 1; }
...
label0:
    ...
    public static void forceGear(Class class1, HierMesh hiermesh, float f)
    {
        try
        {
            ...
            method.invoke(null, new Object[] {
                hiermesh, new Float(f), new Float(f), new Float(f)
            });
            method = class1.getMethod("moveGear", new Class[] {
    ...
    public void update(float f)
    {
        ...
        {
            float f7 = controls.getGearL();
            float f8 = controls.getGearR();
            float f9 = controls.getGearC();
            if(Math.abs(GearL_ - f7) > 0.003F)
            {
                if(!(this instanceof I_16))
                    if(Math.abs(f7 - controls.GearControl) <= 0.003F)
                        ...
                        sfxGear(true);
                moveGear(GearL_ = f7, GearR_ = f8, GearC_ = f9);
            } else
            if(Math.abs(GearR_ - f8) > 0.003F)
            {
                if(!(this instanceof I_16))
            ...
            if(Math.abs(GearC_ - f9) > 0.003F)
            {
                if(!(this instanceof I_16))
                    if(Math.abs(f9 - controls.GearControl) <= 0.003F)
                        sfxGear(false);
                    else
                        sfxGear(true);
                moveGear(GearL_ = f7, GearR_ = f8, GearC_ = f9);
        ...
        {
            float f6 = controls.getVarWing();
            if(Math.abs(VarWing_ - f6) > 0.003F)
            {
                if(Math.abs(f6 - controls.VarWingControl) <= 0.003F)
                    sfxFlaps(false);
                else
                    sfxFlaps(true);
                moveVarWing(VarWing_ = f6);
            }
        }
        if(!isNet() || !isNetMirror())
        {
...
label0:
    ...
    public static boolean prepareMeshSkin(String paramString1, HierMesh paramHierMesh, String paramString2, String paramString3, Regiment paramRegiment)
    {
      ...
      try
      {
        ...
        if (paramString3 == null)
          return false;
        prepareMeshCamouflage(paramString1, paramHierMesh, paramString3, null, paramRegiment);
        return true;
      }
    ...
    protected static boolean isPlayersWing(Aircraft paramAircraft)
    {
      if ((paramAircraft != World.getPlayerAircraft()) && ((paramAircraft.isNetPlayer()) || (paramAircraft.isNetMaster())))
        return false;
      try
      {
        ...
        for (int i = 0; i < localWing.airc.length; i++)
        {
          Aircraft localAircraft = localWing.airc[i];
          if (localAircraft == World.getPlayerAircraft())
            return true;
          if ((localAircraft.isNetPlayer()) || (localAircraft.isNetMaster()))
            return false;
        }
        return false;
      }
    ...
    public void hideWingWeapons(boolean flag)
    {
        for(int i = 0; i < FM.CT.Weapons.length; i++)
        {
            ...
            for(int j = 0; j < abulletemitter.length; j++)
            {
                if(abulletemitter[j] instanceof BombGun)
                {
                    ((BombGun)abulletemitter[j]).hide(flag);
                    continue;
                }
                if(abulletemitter[j] instanceof RocketGun)
                {
                    ((RocketGun)abulletemitter[j]).hide(flag);
                    continue;
                }
                if(abulletemitter[j] instanceof Pylon)
                    ((Pylon)abulletemitter[j]).drawing(!flag);
            }
        ...
        } else
        {
            ...
            try
            {
                ...
                do
                {
                    if(j >= aclass.length)
                        break;
                    if((Mission.isSingle() || Mission.isDogfight()) && (com.maddox.il2.objects.air.CockpitCopilot.class).isAssignableFrom(aclass[j]))
                    {
                        i--;
                        break;
                    }
                ...
                Main3D.cur3D().cockpits = new Cockpit[i];
                j = 0;
                for(int k = 0; k < aclass.length; k++)
                    if(!Mission.isSingle() && !Mission.isDogfight() || !(com.maddox.il2.objects.air.CockpitCopilot.class).isAssignableFrom(aclass[k]))
                    {
                        Main3D.cur3D().cockpits[j] = (Cockpit)aclass[k].newInstance();
                        j++;
                    }
    ...
    public void onAircraftLoaded()
    {
        ...
        FM.CT.setRocketModeDefaults();
        if(FM instanceof RealFlightModel)
            ((RealFlightModel)FM).calculateBombMomentum();
        loadBallisticTable();
    }
    ...
    public static float round3(float f)
    {
        if(f <= 0.0F)
        {
            return f;
        } else
        {
            int i = 10;
            double d = f * (float)i;
            return (float)(int)(d - (double)(int)d < 0.5D ? d : d + 1.0D) / (float)i;
        }
    }
    ...
    public static float round2(float f)
    {
        int i = 10;
        float f1 = f * (float)i;
        return (float)(int)(f1 - (float)(int)f1 < 0.5F ? f1 : f1 + 1.0F) / (float)i;
    }
    ...
    public String getGearOnSnowSim(int i)
    {
    ...
    public float[] getTurretRestOrient(int i)
    {
    ...
    public boolean hasIntervalometer()
    {
        if(!World.cur().diffCur.Limited_Ammo)
            ...
            return ((FM.actor instanceof TypeBomber) || (FM.actor instanceof TypeFastJet));
    }
    ...
    public double getBombDist(float f, double d)
    {
        if(ballisticTable != null)
            return ballisticTable.getBombDist(f, d);
        else
            return d * Math.sqrt(f * 0.2038736F);
    }
    ...
    private void loadBallisticTable()
    {
        ...
        if(FM.CT.Weapons[3] != null)
        {
            ...
            {
                float f1 = FM.CT.Weapons[3][i].bulletMassa();
                if(f1 > f)
                {
                    class1 = FM.CT.Weapons[3][i].getClass();
                    f = f1;
                }
            }
        ...
        if(class1 != null)
        {
            ...
            if(class2 != null)
            {
                ...
                s = s.substring(27);
                String s1 = "btable_" + s;
                Class class3 = (Class)Property.value(class2, s1, null);
                if(class3 == null)
                {
                    String s2 = "btable";
                    class3 = (Class)Property.value(class2, s2, null);
                }
                if(class3 != null)
                    try
                    {
                        ballisticTable = (BallisticTable)class3.newInstance();
                        System.out.println("BallisticTable " + ballisticTable + " loaded");
                    }
                    catch(Exception exception)
                    {
                        System.out.println("Failed to create new instance of " + class3);
                    }
            }
        }
    }

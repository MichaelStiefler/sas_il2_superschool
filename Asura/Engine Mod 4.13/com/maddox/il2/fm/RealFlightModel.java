...
public class RealFlightModel extends Pilot
{
    ...
    {
        structuralFX = actor.newSound("models.structuralFX", false);
        setSound(false);
    }
    ...
    public void update(float f)
    {
        ...
        if(isTick(44, 0))
        {
            ...
            Sq.computeParasiteDrag((Aircraft)actor, CT, CT.Weapons);
            checkAirborneState();
            putScareShpere();
            dangerEMAces();
        ...
        Cwl.x = -d * (double)(Wing.new_CxM(f7, fullMach) + f14 + 0.5F * GearCX * CT.getGearR() + 0.5F * GearCX * CT.getGearL() + radiatorCX * (EI.getRadiatorPos() + CT.getCockpitDoor() + f15) + Sq.dragAirbrakeCx * CT.getAirBrake()) + (double)(Sq.dragChuteCx * CT.getDragChute());
        ...
        if(f14 < 0.0F)
            f14 *= 0.18F;
        Cwr.x = -d * (double)(Wing.new_CxM(f9, fullMach) + f14 + 0.5F * GearCX * CT.getGearR() + 0.5F * GearCX * CT.getGearL() + radiatorCX * EI.getRadiatorPos() + Sq.dragAirbrakeCx * CT.getAirBrake()) + (double)(Sq.dragChuteCx * CT.getDragChute());
        ...
        } else
        {
            ...
            //
        	if((actor instanceof TypeSupersonic) && getOverload() > 15F && !Gears.onGround() && World.Rnd().nextInt(0, 100) > 98)
                cutWing();
            else
            if(getOverload() > 13.5F && !Gears.onGround() && World.Rnd().nextInt(0, 100) > 98)
                cutWing();	
            //
            if(indSpeed > 112.5F && World.Rnd().nextInt(0, 100) > 98 && CT.getGear() > 0.3F && CT.GearControl == 1.0F)
            {

                if(CT.getGear() >= 0.1F && CT.GearControl != 0.0F && !bGearCut)
                    if(!(actor instanceof F4U) && !(actor instanceof TypeSupersonic))
                    {
                    ...
                    if(indSpeed > 180F && !(actor instanceof TypeSupersonic))
                    {
                ...
                if((double)indSpeed > (double)VmaxFLAPS * 1.5D)
                {
                    if(World.Rnd().nextInt(0, 100) > 76 && gearCutCounter != 1)
                    {
                        ((Aircraft)actor).msgCollision(actor, "GearR2_D0", "GearR2_D0");
            ...
            if(indSpeed > VmaxAllowed && World.Rnd().nextFloat(0.0F, 16F) < indSpeed - VmaxAllowed && World.Rnd().nextInt(0, 99) < 2)
                flutterDamage();
            if(!(super.actor instanceof TypeSupersonic) && indSpeed > 610F)
            {
                if(World.cur().isDebugFM())
        ...
        if(!brakeShoe && (Gears.nOfPoiOnGr == 0 && Gears.nOfGearsOnGr < 3 || f21 == 0.0F || SummM.lengthSquared() > (double)(2.0F * f22) || SummF.lengthSquared() > (double)(80F * f22) || W.lengthSquared() > (double)(0.00014F * f21) || Vwld.lengthSquared() > (double)(0.09F * f21)))
        {
            ...
            Aircraft aircraft = (Aircraft)actor;
            aircraft.rotateGearsL(HM, -Gears.gWheelAngles[0]);
            aircraft.rotateGearsR(HM, -Gears.gWheelAngles[1]);
            aircraft.rotateGearsC(HM, -Gears.gWheelAngles[2]);
        }

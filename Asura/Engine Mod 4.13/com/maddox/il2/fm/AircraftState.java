...
//            RealFlightModel, FlightModel, EnginesInterface, Gear, 
//            Motor, Controls, Mass, Autopilotage, 
//            Pitot
...
public class AircraftState
{
    ...
    public void set(Actor paramActor, boolean paramBoolean)
    {
      ...
      for (int i = 0; i < this.astateNavLightsEffects.length; i++)
      {
        try
        {
          ...
          if (i < 2)
          {
            this.astateNavLightsLights[i].light.setColor(1.0F, 0.0F, 0.0F);
          }
          else if (i < 4)
          {
    ...
    public void setTankState(Actor actor1, int i, int j)
    {
        ...
        if(bIsMaster)
        {
            ...
            if(aircraft.FM.isPlayers() && actor1 != actor && (actor1 instanceof Aircraft) && ((Aircraft)actor1).isNetPlayer() && j > 5 && astateTankStates[0] < 5 && astateTankStates[1] < 5 && astateTankStates[2] < 5 && astateTankStates[3] < 5 && !aircraft.FM.isSentBuryNote())
    ...
    public void setEngineState(Actor actor1, int i, int j)
    {
        ...
        if(bIsMaster)
        {
            ...
            if(aircraft.FM.isPlayers() && actor1 != actor && (actor1 instanceof Aircraft) && ((Aircraft)actor1).isNetPlayer() && j > 3 && astateEngineStates[0] < 3 && astateEngineStates[1] < 3 && astateEngineStates[2] < 3 && astateEngineStates[3] < 3 && !aircraft.FM.isSentBuryNote())
    ...
    private void doExplodeEngine(int i)
    {
        ...
        actor.pos.getCurrent(loc);
        hooknamed.computePos(actor, loc, loc1);
        if(aircraft.FM.EI.engines[i].getType() != 1 && aircraft.FM.EI.engines[i].getType() != 0)
    ...
    private void doSetControlsDamage(int i, Actor actor1)
    {
        ...
        {
        case 3: // '\003'
            if(aircraft.FM.isPlayers() && aircraft.FM.CT.bHasStabilizerControl)
                HUD.log("FailedAutopilot");
            aircraft.FM.CT.bHasStabilizerControl = false;
            aircraft.FM.CT.StabilizerControl = false;
            aircraft.FM.CT.courseAutopilotControl = false;
            break;
    ...
    private void doSetBeacon(Actor actor1, int i, int j)
    {
        ...
        if(i > 0)
        {
            ...
            {
                int k = i - beacon;
                int l = Main.cur().mission.getBeacons(actor.getArmy()).size();
                int i1 = i;
                byte byte0 = 1;
                if(k == l)
                {
                ...
                while(i1 <= l && i1 >= 1) 
                {
                    Actor actor3 = (Actor)Main.cur().mission.getBeacons(actor.getArmy()).get(i1 - 1);
                    if((actor3 instanceof TypeHasYGBeacon) || (actor3 instanceof TypeHasHayRake))
                    {
                        i1 += byte0;
                    } else
                    {
            ...
            {
                Main3D.cur3D().ordersTree.setFrequency(OrdersTree.FREQ_FRIENDLY);
                HUD.log(hudLogBeaconId, "BeaconTACAN", new Object[] {
                    s
                });
                startListeningTACAN();
            } else
    ...
    public void doSetWoundPilot(int i)
    {
        ...
        case 1: // '\001'
            ...
            if(aircraft.FM.isPlayers() && Config.isUSE_RENDER())
    ...
    private void doSetPilotState(int i, int j, Actor actor1)
    {
        ...
        if(j > 99)
        {
            ...
            if(i == 1 && astatePilotFunctions[1] == 2 && World.getPlayerAircraft() != actor)
            {
    ...
    private void doSetJamBullets(int i, int j)
    {
        ...
        {
            if(actor == World.getPlayerAircraft() && aircraft.FM.CT.Weapons[i][j].haveBullets())
    ...
    public void setCockpitDoor(Actor actor1, int i)
    {
        ...
        if(!bIsMaster)
            return;
        if(aircraft.FM.CT.bMoveSideDoor)
        {
    ...
    {
    	aircraft.FM.CT.setActiveDoor(j);
    ...
    public void update(float f)
    {
        if(World.cur().diffCur.RealisticPilotVulnerability)
        {
            for(int i = 0; i < aircraft.FM.crew; i++)
            {
                ...
                {
                    if(astatePilotStates[0] > 20 + 10 * aircraft.FM.courage)
                        aircraft.FM.setCapableOfBMP(false, actor);
                    continue;
                }
                if(astatePilotStates[0] > 20 + 10 * aircraft.FM.courage && astatePilotStates[1] > 20 + 10 * aircraft.FM.courage)
                    aircraft.FM.setCapableOfBMP(false, actor);
            }
        ...
        if(bIsMaster)
        {
            ...
            if(bDumpFuel && (aircraft instanceof TypeFuelDump))
                if(aircraft.FM.M.fuel > ((TypeFuelDump)aircraft).getFuelReserve())
                    aircraft.FM.M.fuel -= ((TypeFuelDump)aircraft).getFlowRate() * f;
                else
            ...
            for(int j1 = 0; j1 < 4; j1++)
                ...
                case 5: // '\005'
                    ...
                    if(!hook.chunkName().toLowerCase().startsWith("cf") || astatePilotStates[0] >= 96)
                        break;
                    hitPilot(actor, 0, 5);
                    if(astatePilotStates[0] < 96)
                        break;
                    hitPilot(actor, 0, 101 - astatePilotStates[0]);
                    if(aircraft.FM.isPlayers() && Mission.isNet() && !aircraft.FM.isSentBuryNote())
                ...
                case 6: // '\006'
                    ...
                    if(!hook1.chunkName().toLowerCase().startsWith("cf") || astatePilotStates[0] >= 96)
                        break;
                    hitPilot(actor, 0, 7);
                    if(astatePilotStates[0] < 96)
                        break;
            ...
            for(int k1 = 0; k1 < aircraft.FM.EI.getNum(); k1++)
            {
                ...
                case 4: // '\004'
                    ...
                    if((actor instanceof Scheme1) && !(actor instanceof P_39) && !(actor instanceof MBR_2AM34) && astatePilotStates[0] < 96)
                    {
                        hitPilot(actor, 0, 4);
                        if(astatePilotStates[0] >= 96)
                        {
        ...
        {
            if((aircraft.FM instanceof RealFlightModel) && ((RealFlightModel)aircraft.FM).isRealMode())
                return;
        ...
        throws IOException
    {
        if(flag1)
        {
            if(flag)
            {
                ...
                case 51: // '3'
                    setRocketHook(actor1, k, true);
                    break;

                case 48: // '0'
                    setBombModeStates(actor1, k, i1, true);
                    break;

                case 49: // '1'
                    setRocketModeStates(actor1, k, true);
                    break;

                case 50: // '2'
                    setCoPilotMode(actor1, k, true);
                    break;
                }
            } else
            {
        ...
        } else
        {
            ...
            case 51: // '3'
                doSetRocketHook(actor2, l);
                break;
                
            case 48: // '0'
                doSetBombModeStates(actor2, l, j1);
                break;

            case 49: // '1'
                doSetRocketModeStates(actor2, l);
                break;

            case 50: // '2'
                doSetCoPilotMode(actor2, l);
                break;
            }
        }
    }
        ...
        throws IOException
    {
        ...
        if(World.getPlayerAircraft() == aircraft)
        {
            ...
            replicateRocketModeStatesToNet();
        }
    }
        ...
        throws IOException
    {
        ...
        doSetRocketHook(aircraft, netmsginput.readUnsignedByte());
    ...
    public void replicateRocketModeStatesToNet()
    {
        setRocketModeStates(actor, aircraft.FM.CT.rocketReleaseMode, false);
    }

    private void setRocketModeStates(Actor actor1, int i, boolean flag)
    {
        if(!Actor.isValid(actor1))
            return;
        if(flag)
            doSetRocketModeStates(actor1, i);
        if(bIsMaster)
            netToMirrors(49, i, 0);
        else
            netToMaster(49, i, 0, actor1);
    }

    private void doSetRocketModeStates(Actor actor1, int i)
    {
        aircraft.FM.CT.rocketReleaseMode = i;
    }

    public void replicateCoPilotModeToNet()
    {
        setCoPilotMode(actor, aircraft.FM.CT.copilotMode, false);
    }

    private void setCoPilotMode(Actor actor1, int i, boolean flag)
    {
        if(!Actor.isValid(actor1))
            return;
        if(flag)
            doSetCoPilotMode(actor1, i);
        if(bIsMaster)
            netToMirrors(50, i, 0);
        else
            netToMaster(50, i, 0, actor1);
    }

    private void doSetCoPilotMode(Actor actor1, int i)
    {
        aircraft.FM.CT.copilotMode = i;
    }
    ...
    private void doSetGearState(int i, int j)
    {
        ...
        if(k > 2)
            return;
        if(i1 > 0)
            gearStates[k] = cvt(i1, 0, 127, 0.0F, 1.0F);
        else
        if(i1 < 0)
            gearStates[k] = cvt(i1, -128, 0, -1F, 0.0F);
        else
        if(i1 == 0)
            gearStates[k] = 0.0F;
        if((k == 0 || k == 1) && (l == 6 || l == 4))
        ...
        if(l == 9)
            aircraft.FM.Gears.setHydroOperable(false);
        if(gearDamType[0] == 8 && gearDamType[1] == 8)
            aircraft.FM.CT.bHasBrakeControl = false;
    }
    ...
    public void hitGear(Actor actor1, int i, int j)
    {
        ...
        if(k > 2)
            k -= 3;
        if(gearStates[k] != 0.0F || k < 0 || k > 2)
            return;
        if(!aircraft.FM.Gears.lgear && k == 0 || !aircraft.FM.Gears.rgear && k == 1)
            return;
        byte byte0 = 0;
        if(j == 12)
        {
        ...
        if(j == 11)
            byte0 = (byte)(int)(Math.abs(gearStates[k]) * 127F);
        int l = i & 0xf;
        l <<= 4;
        l += j & 0xf;
        int i1 = byte0 + 128;
    ...
    public void replicateRocketHookToNet(int i)
    {
        setRocketHook(actor, i, false);
    }

    private void setRocketHook(Actor actor1, int i, boolean flag)
    {
        if(!Actor.isValid(actor1))
            return;
        if(flag)
            doSetRocketHook(actor1, i);
        if(bIsMaster)
            netToMirrors(51, i, 0);
        else
            netToMaster(51, i, 0, actor1);
    }

    private void doSetRocketHook(Actor actor1, int i)
    {
        aircraft.FM.CT.doSetRocketHook(i);
    }
    ...
    public static final int _AS_BOMB_MODE_STATES = 48;
    public static final int _AS_ROCKET_MODE_STATES = 49;
    public static final int _AS_COPILOT_MODE = 50;
    public static final int _AS_COUNT_CODES = 51;
    ...
    public static final int _CONTROLS_AUTOPILOT = 3;
    ...
    public float gearStates[] = {
        0.0F, 0.0F, 0.0F
    };
    public byte gearDamType[] = {
        0, 0, 0
    };
    public byte gearDamRecoveryStates[] = {
        0, 0, 0
    };
    private int beacon;

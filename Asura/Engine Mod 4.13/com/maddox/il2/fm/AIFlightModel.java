...
public class AIFlightModel extends FlightModel
{
    ...
    public void update(float f)
    {
        ...
        if(isTick(44, 0))
        {
            ...
            if(((Pilot)((Aircraft)actor).FM).get_task() == 7 || ((Maneuver)(Maneuver)((Aircraft)actor).FM).get_maneuver() == 43)
                putScareShpere();
        ...
        if(Ss.allParamsSet)
            Density *= Ss.getDragFactorForMach(Mach);
        else
            fullMach = Mach;
        if(Mach > 0.8F)
            Mach = 0.8F;
        ...
        if(!brakeShoe && (Gears.nOfPoiOnGr == 0 && Gears.nOfGearsOnGr < 3 || f12 == 0.0F || SummM.lengthSquared() > (double)(2.0F * f13) || SummF.lengthSquared() > (double)(80F * f13) || W.lengthSquared() > (double)(0.00014F * f12) || Vwld.lengthSquared() > (double)(0.09F * f12)))
        {
            ...
            {
                Gears.gWheelAngles[j1] = (Gears.gWheelAngles[j1] + (float)Math.toDegrees(Math.atan((Gears.gVelocity[j1] * (double)f) / 0.375D))) % 360F;
            ...
            Aircraft aircraft3 = (Aircraft)actor;
            aircraft3.rotateGearsL(HM, -Gears.gWheelAngles[0]);
            aircraft3.rotateGearsR(HM, -Gears.gWheelAngles[1]);
            aircraft3.rotateGearsC(HM, -Gears.gWheelAngles[2]);
        }
        if(CT.dropWithPlayer != null && CT.dropWithPlayer.FM.CT.bDropWithMe && Actor.isAlive(actor))
        {
            CT.bombReleaseMode = CT.dropWithPlayer.FM.CT.bombReleaseMode;
            CT.bombTrainAmount = CT.dropWithPlayer.FM.CT.bombTrainAmount;
            CT.bombTrainDelay = CT.dropWithPlayer.FM.CT.bombTrainDelay;
            Voice.setSyncMode(1);

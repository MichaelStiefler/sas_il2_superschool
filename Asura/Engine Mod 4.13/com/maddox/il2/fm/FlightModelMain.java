// Decompiled by DJ v3.10.10.93 Copyright 2007 Atanas Neshkov  Date: 07.09.2015 13:50:38
...
//            Mass, Squares, Arm, FlightModel, 
//            EnginesInterface, Gear, Polares, Turret, 
//            RealFlightModel, Motor, Autopilotage
...
public class FlightModelMain extends FMMath 
{
    ...
    public Vector3d getMirrorBallAccel()
    {
        TmpV.set(Accel);
    ...
    public void load(String s)
    {
        ...
        j = sectfile.get(s2, "CDifferentialBrakes", 0);
        if(j != 0 && j != 1)
            throw new RuntimeException(s1);
        CT.bHasDiffBrakes = j == 1;
        ...
        CT.bHasStabilizerControl = sectfile.get(s2, "CStabilizer", 0) == 1;
        ...
        {
            CT.dvGear = 1.0F / f;
            CT.dvGearR = 1.0F / f - 1.0F / (f * 20F);
            CT.dvGearL = 1.0F / f + 1.0F / (f * 20F);
            CT.dvGearC = 1.0F / f + 1.0F / (f * 5F);
        }
    ...
    public FlightModelMain(String s)
    {
        ...
        bombAM = new Vector3d(0.0D, 0.0D, 0.0D);
        fmsfxCurrentType = -1;
        fmsfxPrevValue = 0.0F;
        fmsfxTimeDisable = -1L;
        AF = new Vector3d();
    ...
    public boolean tick()
    {
        ...
        float f1 = (float)(((Tuple3d) (Loc)).z - Engine.land().HQ_Air(((Tuple3d) (Loc)).x, ((Tuple3d) (Loc)).y));
    ...
    public Vector3d Vair;
    protected Vector3d Vflow;
    public Vector3d Vwind;

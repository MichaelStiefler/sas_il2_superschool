...
public class AircraftHotKeys
{
    class HotKeyCmdFireMove extends HotKeyCmdMove
    {
        ...
        public void begin()
        {
            ...
            doCmdPilotMove(cmd, Joy.normal(byte0 * move()), false);
        }
    ...
    class HotKeyCmdFire extends HotKeyCmd
    {
        ...
        public HotKeyCmdFire(String s, String s1, int i, int j)
        {
            super(true, s1, s);
            cmd = i;
            copilotCmdMap.put("" + i, this);
            setRecordId(j);
        }
    }


    private void addCopilotMove(int i, float f)
    {
        if(Mission.isCoop() && (Main3D.cur3D().cockpitCur instanceof CockpitCopilot) && !NetMissionTrack.isPlaying())
        {
            if(FM.AS.astatePilotStates[0] < 100)
            {
                if(FM.CT.copilotMode == 3)
                {
                    HUD.logCenter("CopilotControlsBlocked");
                    return;
                }
                if(FM.CT.copilotMode == 2)
                {
                    HUD.logCenter("CopilotAxisControlsBlocked");
                    return;
                }
            }
            int j = i + 300;
            Object aobj[] = {
                new Integer(j), new Float(f)
            };
            copilotControls.add(((Object) (aobj)));
        }
    }

    private void addCopilotAction(int i, boolean flag)
    {
        if(Mission.isCoop() && (Main3D.cur3D().cockpitCur instanceof CockpitCopilot) && !NetMissionTrack.isPlaying())
        {
            if(FM == null)
                FM = World.getPlayerFM();
            if(FM.AS.astatePilotStates[0] < 100 && FM.CT.copilotMode == 3)
            {
                HUD.logCenter("CopilotControlsBlocked");
                return;
            }
    ...
    public void addBombardierAction(int i)
    {
        if(Mission.isCoop() && (Main3D.cur3D().cockpitCur instanceof CockpitBombardier) && !NetMissionTrack.isPlaying())
            bombardierControls.add(new Integer(i));
    }

    public void setCopilotMoveControls(int i, float f)
    {
        doCmdPilotMove(i - 300, f, true);
    }

    public void setCopilotControls(int i, boolean flag)
    {
        HotKeyCmd hotkeycmd = (HotKeyCmd)copilotCmdMap.get("" + i);
        if(Main3D.cur3D().keyRecord.isRecording() && hotkeycmd != null)
            Main3D.cur3D().keyRecord.msgHotKeyCmd(hotkeycmd, flag, false);
        doCmdPilot(i, flag, true);
    }

    public void setBombardierControls(int i)
    {
        HotKeyCmd hotkeycmd = (HotKeyCmd)copilotCmdMap.get("" + i);
        if(Main3D.cur3D().keyRecord.isRecording() && hotkeycmd != null)
        {
            Main3D.cur3D().keyRecord.msgHotKeyCmd(hotkeycmd, true, false);
            Main3D.cur3D().keyRecord.msgHotKeyCmd(hotkeycmd, false, false);
        }
        doCmdPilot(i, true, true);
    }
    ...
    {
        if(FM.EI.isSelectionHasControlBoost())
        {
            if(FM.CT.afterburnerFControl < 1.0F)
                setBoostControl(1.0F);
            else
                setBoostControl(0.0F);
            return;
        }
        if(FM.EI.isSelectionHasControlAfterburner())
        {
            ...
            if(bAfterburner)
            {
                if((FM.actor instanceof Hurricane) || (FM.actor instanceof A6M) && !(FM.actor instanceof A6M7_62) && !(FM.actor instanceof A6M5C) || (FM.actor instanceof P_51) || (FM.actor instanceof SPITFIRE) || (FM.actor instanceof MOSQUITO) || (FM.actor instanceof TEMPEST))
                    HUD.logRightBottom("BoostWepTP0");
                else
                if(FM.actor instanceof FW_190A5165ATA)
    ...
    private boolean setPilot()
    {
        ...
        if(flightmodel instanceof RealFlightModel)
        {
            ...
            if(World.getPlayerAircraft().net.isMirror() && !NetMissionTrack.isPlaying())
                return false;
            return ((RealFlightModel)FM).isRealMode();
        } else
        {
            return false;
        }
    ...
    private void setBoostControl(float f)
    {
        if(f < 0.0F)
            f = 0.0F;
        if(f > 1.0F)
            f = 1.0F;
        lastBoost = f;
        FM.CT.setManualfterburnerControl(f);
        hudBoost(FM.CT.afterburnerFControl);
    }
    ...
    private void setPowerControl(float f)
    {
        ...
        if(f > 1.1F)
            f = 1.1F;
        if((FM.actor instanceof FW_190A4) && f > 0.875F)
            f = 0.875F;
        lastPower = f;
    ...
    private void setPowerControl(float f, int i)
    {
        ...
        if(f > 1.1F)
            f = 1.1F;
        if(i == 1 && (FM.actor instanceof FW_190A4) && f > 0.875F)
            f = 0.875F;
        FM.CT.setPowerControl(f, i - 1);
        if(i <= FM.EI.engines.length)
            hudPower(f);
    }
    ...
    private void setMixControl(float f)
    {
        ...
        {
            if(FM.EI.getFirstSelected().getMixerType() == 3)
                if(f < 0.1F)
                    f = 0.0F;
                else
                if(f < 0.7F)
            ...
            if(FM.EI.getFirstSelected().getMixerType() == 3)
            {
                float f1 = FM.CT.getMixControl();
                if(f1 < 0.1F)
                    HUD.log(hudLogPowerId, "MixIdleCutOff");
                else
                if(f1 < 0.7F)
                    HUD.log(hudLogPowerId, "MixAutoLean");
                else
                if(f1 < 1.1F)
                    HUD.log(hudLogPowerId, "MixAutoRich");
                else
                    HUD.log(hudLogPowerId, "MixFullRich");
            } else
            {
        ...
        if(FM.CT.getRadiatorControlAuto())
        {
            ...
            if(World.cur().diffCur.ComplexEManagement)
            {
                ...
                HUD.log(hudLogPowerId, "RadiatorPercentage", new Object[] {
                    new Integer(Math.round(FM.CT.getRadiatorControl() * 100F))
                });
            } else
            {
    ...
    private void hudBoost(float f)
    {
        HUD.log(hudLogPowerId, "Boost", new Object[] {
            new Integer(Math.round(f * 100F))
        });
    }
    ...
    {
        hudWeapon(flag, i, FM.CT);
    }

    private void hudWeapon(boolean flag, int i, Controls controls)
    {
        boolean flag1 = false;
        boolean flag2 = false;
        boolean flag3 = false;
        boolean flag4 = false;
        BulletEmitter abulletemitter[] = controls.Weapons[i];
    ...
    private boolean isBombardierControlOnly(Aircraft aircraft)
    {
        return (Main3D.cur3D().cockpitCur instanceof CockpitPilot) && aircraft.hasHumanOnlineBomber;
    }

    private void doCmdPilot(int i, boolean flag, boolean flag1)
    {
        if(!setBombAimerAircraft() && !flag1)
            return;
        if(bAAircraft == null)
            if(FM != null)
                bAAircraft = (AircraftLH)FM.actor;
            else
                bAAircraft = (AircraftLH)World.getPlayerFM().actor;
        if(flag)
            switch(i)
            {
            case 166: 
        ...
        if(flag && (Main3D.cur3D().cockpitCur instanceof TypeHasDriftMeter))
            switch(i)
            {
            case 119: // 'w'
            case 120: // 'x'
        ...
        if(flag && (Main3D.cur3D().cockpitCur instanceof TypeTurretK4Sight))
            switch(i)
            {
            case 117: // 'u'
            case 118: // 'v'
            ...
            case 119: // 'w'
                ((TypeTurretK4Sight)Main3D.cur3D().cockpitCur).typeTurretK4AdjSpanPlus();
                toTrackSign(i);
                return;

            case 120: // 'x'
                ((TypeTurretK4Sight)Main3D.cur3D().cockpitCur).typeTurretK4AdjSpanMinus();
                toTrackSign(i);
                return;

            case 125: // '}'
                ((TypeTurretK4Sight)Main3D.cur3D().cockpitCur).typeTurretK4IronSights();
                toTrackSign(i);
                return;
            }
        if(!setPilot() && !(Main3D.cur3D().cockpitCur instanceof CockpitBombardier) && !(Main3D.cur3D().cockpitCur instanceof CockpitCopilot) && !flag1)
            return;
        if(!flag)
            switch(i)
            {
        ...
        if(flag)
            ...
            case 131: 
                if(bAAircraft instanceof TypeBomberAPBombSight)
                    doCmdPilotTick(i);
                else
                if((bAAircraft instanceof TypeBomberPDILights) && bAAircraft.FM.CT.pdiLights != 1 && ((Main3D.cur3D().cockpitCur instanceof CockpitBombardier) || flag1))
                {
                    ((TypeBomberPDILights)bAAircraft).typeBomberTurnLeft();
                    addBombardierAction(131);
                    toTrackSign(i);
                }
                return;
            ...
            case 132: 
                ...
                if((bAAircraft instanceof TypeBomberPDILights) && bAAircraft.FM.CT.pdiLights != 3 && ((Main3D.cur3D().cockpitCur instanceof CockpitBombardier) || flag1))
                {
                    ((TypeBomberPDILights)bAAircraft).typeBomberTurnRight();
                    addBombardierAction(132);
                    toTrackSign(i);
                }
                return;
            ...
            case 133: 
                if(bAAircraft instanceof TypeBomberAPBombSight)
                {
                    ((TypeBomberAPBombSight)bAAircraft).typeBomberTurn(0.0F);
                    addBombardierAction(11);
                } else
                if((bAAircraft instanceof TypeBomberPDILights) && ((Main3D.cur3D().cockpitCur instanceof CockpitBombardier) || flag1))
                {
                    ((TypeBomberPDILights)bAAircraft).typeBomberTurnCenter();
                    addBombardierAction(133);
                    toTrackSign(i);
                }
                return;
            ...
            case 138: 
                if(bAAircraft instanceof TypeBomberAPBombSight)
                {
                    if(isBombardierControlOnly(bAAircraft))
                        return;
                    ((TypeBomberAPBombSight)bAAircraft).typeBomberToggleClutch();
                    addBombardierAction(11);
                    toTrackSign(i);
                }
                return;
            ...
            case 125: // '}'
                if(bAAircraft instanceof TypeBomber)
                {
                    if(isBombardierControlOnly(bAAircraft))
                        return;
                    ((TypeBomber)bAAircraft).typeBomberToggleAutomation();
                    toTrackSign(i);
                    addBombardierAction(10);
                } else
                if(bAAircraft instanceof TypeDiveBomber)
                {
                    ((TypeDiveBomber)bAAircraft).typeDiveBomberToggleAutomation();
                    toTrackSign(i);
                } else
                if(bAAircraft instanceof TypeFighterAceMaker)
                {
                    ((TypeFighterAceMaker)bAAircraft).typeFighterAceMakerToggleAutomation();
                    toTrackSign(i);
                }
                return;

            case 179: 
                if(bAAircraft instanceof TypeBomberOPBBombSight)
                {
                    if(isBombardierControlOnly(bAAircraft))
                        return;
                    ((TypeBomberOPBBombSight)bAAircraft).typeBomberAdjOpbTimer();
                    toTrackSign(i);
                    addBombardierAction(12);
                }
                break;
            ...
            case 157: 
                ...
                addCopilotAction(i, flag);
                addBombardierAction(157);
                return;

            case 158: 
                bAAircraft.auxPressed(2);
                addCopilotAction(i, flag);
                addBombardierAction(158);
                return;
            ...
            case 19: // '\023'
                ...
                {
                    addBombardierAction(19);
                    hudWeapon(flag, 3, bAAircraft.FM.CT);
                }
                addCopilotAction(i, flag);
                return;

            case 65: // 'A'
                if((bAAircraft instanceof TypeHasNorden) || (bAAircraft instanceof TypePatinAutopilot))
                    if((FM instanceof RealFlightModel) && !FM.actor.net.isMirror())
                    {
                        if(((bAAircraft instanceof TypeBomber) || (bAAircraft instanceof TypePatinAutopilot)) && (!bAAircraft.hasHumanOnlineBomber || !World.cur().diffCur.RealisticBombSights) || bAAircraft.FM.CT.bHasStabilizerControl || FM.CT.courseAutopilotControl)
                            toggleCourseAutopilot(bAAircraft);
                    } else
                    if(bAAircraft.FM.CT.bHasStabilizerControl)
                    {
                        toggleCourseAutopilot(bAAircraft);
                        addBombardierAction(i);
                        addCopilotAction(i, flag);
                    }
                return;
            ...
            case 71: // 'G'
                if((FM instanceof RealFlightModel) && !FM.actor.net.isMirror())
                {
                    if(((bAAircraft instanceof TypeBomber) || (bAAircraft instanceof TypePatinAutopilot)) && (!bAAircraft.hasHumanOnlineBomber || !World.cur().diffCur.RealisticBombSights) || bAAircraft.FM.CT.bHasStabilizerControl || FM.CT.StabilizerControl)
                        toggleLevelStabilizer(bAAircraft);
                } else
                if(bAAircraft.FM.CT.bHasStabilizerControl)
                {
                    toggleLevelStabilizer(bAAircraft);
        ...
        if(!setPilot() && !(Main3D.cur3D().cockpitCur instanceof CockpitCopilot) && !flag1)
            return;
        Aircraft aircraft = (Aircraft)bAAircraft.FM.actor;
        if(FM == null)
            FM = bAAircraft.FM;
        addCopilotAction(i, flag);
        switch(i)
...
//            break;
        ...
        case 137:        	
            if(flag)
            {
                FM.CT.toggleRocketReleaseMode();
                return;
            }
            // fall through

        case 168: 
            if(flag && (FM instanceof RealFlightModel) && !FM.actor.net.isMirror() && !(Main3D.cur3D().cockpitCur instanceof CockpitCopilot))
            {
                if(Mission.isSingle() || !hasCoPilotPit())
                    return;
                int j = FM.CT.copilotMode;
                if(++j > 3)
                    j = 0;
                if(j == 0)
                    HUD.log("CopilotModeDual");
                else
                if(j == 1)
                    HUD.log("CopilotModeFull");
                else
                if(j == 2)
                    HUD.log("CopilotModeNoAxis");
                else
                if(j == 3)
                    HUD.log("CopilotModeBlocked");
                FM.CT.copilotMode = j;
                FM.AS.replicateCoPilotModeToNet();
            }
            return;
        ...
        if(!flag)
        {
            ...
            case 0: // '\0'
                ...
                if(FM.CT.bHasDiffBrakes)
                {
                    FM.CT.BrakeControlL = 0.0F;
                    FM.CT.BrakeControlR = 0.0F;
                }
                break;
            ...
            case 55: // '7'
                if(FM.Gears.onGround() || FM.CT.GearControl >= 1.0F || !FM.Gears.isOperable() || FM.Gears.isHydroOperable())
                    break;
                if(!bAAircraft.gearEmergencyDown())
                    return;
                FM.CT.GearControl += 0.02F;
            ...
            case 143: 
                ...
                {
                    FM.CT.DragChuteControl = FM.CT.DragChuteControl > 0.5F ? 0.0F : 1.0F;
                    HUD.log("DragChute" + (FM.CT.DragChuteControl == 0.0F ? "Released" : "Deployed"));
                }
                break;
            ...
            case 163: 
                ...
                {
                    FM.CT.RefuelControl = FM.CT.RefuelControl > 0.5F ? 0.0F : 1.0F;
                    HUD.log("Refuel" + (FM.CT.RefuelControl == 0.0F ? "OFF" : "ON"));
                }
                break;
...
label0:
        ...
        case 7: // '\007'
            ...
            if(FM.CT.getRadiatorControlAuto())
            {
                ...
                {
                    FM.CT.setRadiatorControl(0.0F);
                    HUD.log("RadiatorControl" + (int)(FM.CT.getRadiatorControl() * 10F));
                } else
                {
                    FM.CT.setRadiatorControl(1.0F);
                    HUD.log("RadiatorON");
            ...
            if(World.cur().diffCur.ComplexEManagement)
            {
                if(FM.CT.getRadiatorControl() > 0.8F)
                {
                ...
                {
                    float f = FM.CT.getRadiatorControl();
                    float f3 = f % 0.2F;
                    if(f == f3)
                        FM.CT.setRadiatorControl(0.0F);
                    else
                    if(f3 < 0.1F)
                        FM.CT.setRadiatorControl(f - f3);
                    else
                        FM.CT.setRadiatorControl((f - f3) + 0.2F);
        ...
        case 0: // '\0'
            ...
            if(FM.CT.bHasDiffBrakes)
            {
                FM.CT.BrakeControlL = 1.0F;
                FM.CT.BrakeControlR = 1.0F;
            }
            break;
        ...
        case 56: // '8'
            if(canControl() && FM.CT.RudderControl > -1F)
        ...
        case 58: // ':'
            if(canControl() && FM.CT.RudderControl < 1.0F)
                FM.CT.RudderControl += 0.1F;
            break;
        ...
        case 165: 
            ...
            {
                bDumpFuel = false;
                HUD.log(hudLogWeaponId, "FuelValveClosed");
            } else
            {
                bDumpFuel = true;
                HUD.log(hudLogWeaponId, "FuelValveOpen");
            }
        ...
        case 162: 
            if(FM.CT.toggleRocketHook())                
	            HUD.log(hudLogWeaponId, "RocketSelected", new Object[] {
	            		FM.CT.rocketNameSelected
	            });
        ...
        case 180: 
            if(FM.EI.isSelectionHasControlBoost())
                setBoostControl(FM.CT.afterburnerFControl + 0.05F);
            break;
        ...
        case 86: // 'V'
            if(!FM.EI.isSelectionHasControlMix())
                break;
            if(FM.EI.getFirstSelected() != null && FM.EI.getFirstSelected().getMixerType() == 3)
            {
                float f1 = FM.EI.getFirstSelected().getControlMix();
                if(f1 < 0.6F)
                {
                    setMixControl(0.6F);
                    break;
        ...
        case 87: // 'W'
            ...
            if(FM.EI.getFirstSelected() != null && FM.EI.getFirstSelected().getMixerType() == 3)
            {
                float f2 = FM.EI.getFirstSelected().getControlMix();
                if(f2 >= 1.1F)
                {
                    setMixControl(1.0F);
                    break;
        ...
        case 92: // '\\'
            ...
            for(int i1 = 0; i1 < ai.length; i1++)
                FM.EI.setCurControl(ai[i1], true);
        ...
        case 93: // ']'
            ...
            for(int j1 = 0; j1 < ai1.length; j1++)
                FM.EI.setCurControl(ai1[j1], true);
        ...
        case 102: // 'f'
            ...
            for(int k = 0; k < FM.EI.getNum(); k++)
                FM.EI.setCurControl(k, !FM.EI.getCurControl(k));
        ...
        case 103: // 'g'
            ...
            for(int k1 = 0; k1 < ai2.length; k1++)
                FM.EI.setCurControl(ai2[k1], !FM.EI.getCurControl(ai2[k1]));
        ...
        case 104: // 'h'
            ...
            for(int l1 = 0; l1 < ai3.length; l1++)
                FM.EI.setCurControl(ai3[l1], !FM.EI.getCurControl(ai3[l1]));
        ...
        case 113: // 'q'
            ...
            do
            {
                if(l >= FM.EI.getNum())
                    break label0;
                if(FM.EI.getCurControl(l))
                    FM.EI.engines[l].setExtinguisherFire();
                l++;
            } while(true);
        ...
        case 53: // '5'
            if(!FM.CT.bHasFlapsControl)
                break;
            SetFlapsHotKeys(1, (RealFlightModel)FM);
            if(FM.CT.FlapStageMax != -1F && flapIndex < FM.CT.nFlapStages - 1)
                flapIndex++;
            break;
        ...
        case 52: // '4'
            if(!FM.CT.bHasFlapsControl)
                break;
            SetFlapsHotKeys(0, (RealFlightModel)FM);
            if(FM.CT.FlapStageMax != -1F && flapIndex > 0)
                flapIndex--;
            break;
...
//            break;
        ...
        case 202: 
            if(!FM.CT.bHasVarWingControl)
                break;
            SetVarWingHotKeys(0, (RealFlightModel)FM);
            if(varWingIndex > 0)
                varWingIndex--;
            break;
        ...
        case 203: 
            if(!FM.CT.bHasVarWingControl)
                break;
            SetVarWingHotKeys(1, (RealFlightModel)FM);
        ...
        case 204: 
            ...
            {
                ((FlightModelMain) (FM)).CT.BlownFlapsControl = ((FlightModelMain) (FM)).CT.BlownFlapsControl <= 0.5F ? 1.0F : 0.0F;
                HUD.log(((FlightModelMain) (FM)).CT.BlownFlapsType + (((FlightModelMain) (FM)).CT.BlownFlapsControl != 0.0F ? "ON" : "OFF"));
            }
            break;
        ...
        case 130: 
            if(!FM.canChangeBrakeShoe || (FM.actor instanceof TypeSeaPlane) || (FM.actor instanceof HE_LERCHE3))
                break;
            if(FM.brakeShoe)
            {
                FM.brakeShoe = false;
...
//            break;
    ...
    private void doCmdPilotTick(int i)
    {
        ...
        if(Main3D.cur3D().cockpitCur instanceof TypeHasDriftMeter)
            switch(i)
            {
            case 119: // 'w'
                ((TypeHasDriftMeter)Main3D.cur3D().cockpitCur).typeDriftPlus();
                toTrackSign(i);
                return;

            case 120: // 'x'
                ((TypeHasDriftMeter)Main3D.cur3D().cockpitCur).typeDriftMinus();
                toTrackSign(i);
                return;
            }
        if(Main3D.cur3D().cockpitCur instanceof TypeTurretK4Sight)
            switch(i)
            {
            case 119: // 'w'
            case 120: // 'x'
            ...
            case 117: // 'u'
                ((TypeTurretK4Sight)Main3D.cur3D().cockpitCur).typeTurretK4AdjDistancePlus();
                toTrackSign(i);
                return;

            case 118: // 'v'
                ((TypeTurretK4Sight)Main3D.cur3D().cockpitCur).typeTurretK4AdjDistanceMinus();
                toTrackSign(i);
                return;
            }
        if(!setPilot() && !(Main3D.cur3D().cockpitCur instanceof CockpitBombardier))
            return;
        switch(i)
        {
        case 149: 
            if(bAAircraft instanceof TypePatinAutopilot)
                ((TypePatinAutopilot)bAAircraft).typePatinCourseRight();
            else
        ...
        case 150: 
            if(bAAircraft instanceof TypePatinAutopilot)
                ((TypePatinAutopilot)bAAircraft).typePatinCourseLeft();
            else
        ...
        case 131: 
            if(bAAircraft instanceof TypeBomberAPBombSight)
            {
                ((TypeBomberAPBombSight)bAAircraft).typeBomberTurnLeft();
                addBombardierAction(11);
                toTrackSign(i);
            }
            return;
        ...
        case 132: 
            if(bAAircraft instanceof TypeBomberAPBombSight)
            {
                ((TypeBomberAPBombSight)bAAircraft).typeBomberTurnRight();
                addBombardierAction(11);
                toTrackSign(i);
            }
            return;
        ...
        case 117: // 'u'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjDistancePlus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeFighterAceMaker)
            {
                ((TypeFighterAceMaker)bAAircraft).typeFighterAceMakerAdjDistancePlus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjDiveAnglePlus();
                toTrackSign(i);
            }
            break;
        ...
        case 118: // 'v'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjDistanceMinus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeFighterAceMaker)
            {
                ((TypeFighterAceMaker)bAAircraft).typeFighterAceMakerAdjDistanceMinus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjDiveAngleMinus();
                toTrackSign(i);
            }
            break;
        ...
        case 119: // 'w'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjSideslipPlus();
                toTrackSign(i);
            }
            if(bAAircraft instanceof TypeX4Carrier)
            {
                ((TypeX4Carrier)bAAircraft).typeX4CAdjSidePlus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeFighterAceMaker)
            {
                ((TypeFighterAceMaker)bAAircraft).typeFighterAceMakerAdjSideslipPlus();
                toTrackSign(i);
            }
            break;
        ...
        case 120: // 'x'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjSideslipMinus();
                toTrackSign(i);
            }
            if(bAAircraft instanceof TypeX4Carrier)
            {
                ((TypeX4Carrier)bAAircraft).typeX4CAdjSideMinus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeFighterAceMaker)
            {
                ((TypeFighterAceMaker)bAAircraft).typeFighterAceMakerAdjSideslipMinus();
                toTrackSign(i);
            }
            break;
        ...
        case 121: // 'y'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjAltitudePlus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjAltitudePlus();
                toTrackSign(i);
            }
            if(bAAircraft instanceof TypeX4Carrier)
            {
                ((TypeX4Carrier)bAAircraft).typeX4CAdjAttitudePlus();
                toTrackSign(i);
            }
            break;
        ...
        case 122: // 'z'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjAltitudeMinus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjAltitudeMinus();
                toTrackSign(i);
            }
            if(bAAircraft instanceof TypeX4Carrier)
            {
                ((TypeX4Carrier)bAAircraft).typeX4CAdjAttitudeMinus();
                toTrackSign(i);
            }
            break;
        ...
        case 123: // '{'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjSpeedPlus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjVelocityPlus();
                toTrackSign(i);
            }
            break;
        ...
        case 124: // '|'
            if(bAAircraft instanceof TypeBomber)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomber)bAAircraft).typeBomberAdjSpeedMinus();
                toTrackSign(i);
            } else
            if(bAAircraft instanceof TypeDiveBomber)
            {
                ((TypeDiveBomber)bAAircraft).typeDiveBomberAdjVelocityMinus();
                toTrackSign(i);
            }
            break;
        ...
        case 178: 
            if(bAAircraft instanceof TypeBomberOPBBombSight)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomberOPBBombSight)bAAircraft).typeBomberAdjOpbBack();
                toTrackSign(i);
            }
            break;
        ...
        case 177: 
            if(bAAircraft instanceof TypeBomberOPBBombSight)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomberOPBBombSight)bAAircraft).typeBomberAdjOpbForward();
                toTrackSign(i);
            }
            break;
        ...
        case 176: 
            if(bAAircraft instanceof TypeBomberOPBBombSight)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomberOPBBombSight)bAAircraft).typeBomberAdjOpbLeft();
                toTrackSign(i);
            }
            break;
        ...
        case 175: 
            if(bAAircraft instanceof TypeBomberOPBBombSight)
            {
                if(isBombardierControlOnly(bAAircraft))
                    return;
                ((TypeBomberOPBBombSight)bAAircraft).typeBomberAdjOpbRight();
                toTrackSign(i);
            }
            break;
        }
        ...
        throws IOException
    {
        ...
        if(World.getPlayerAircraft() instanceof TypeBomberAPBombSight)
        {
            TypeBomberAPBombSight typebomberapbombsight = (TypeBomberAPBombSight)World.getPlayerAircraft();
            switch(i)
            {
            case 131: 
                typebomberapbombsight.typeBomberTurnLeft();
                return;

            case 132: 
                typebomberapbombsight.typeBomberTurnRight();
                return;

            case 133: 
                typebomberapbombsight.typeBomberTurn(0.0F);
                return;

            case 138: 
                typebomberapbombsight.typeBomberToggleClutch();
                return;
            }
        }
        if(World.getPlayerAircraft() instanceof TypeBomberPDILights)
        {
            TypeBomberPDILights typebomberpdilights = (TypeBomberPDILights)World.getPlayerAircraft();
            switch(i)
            {
            case 131: 
                typebomberpdilights.typeBomberTurnLeft();
                return;

            case 132: 
                typebomberpdilights.typeBomberTurnRight();
                return;

            case 133: 
                typebomberpdilights.typeBomberTurnCenter();
                return;
            }
        }
        if(World.getPlayerAircraft() instanceof TypeBomberOPBBombSight)
        {
            TypeBomberOPBBombSight typebomberopbbombsight = (TypeBomberOPBBombSight)World.getPlayerAircraft();
            switch(i)
            {
            case 178: 
                typebomberopbbombsight.typeBomberAdjOpbBack();
                return;

            case 177: 
                typebomberopbbombsight.typeBomberAdjOpbForward();
                return;

            case 176: 
                typebomberopbbombsight.typeBomberAdjOpbLeft();
                return;

            case 175: 
                typebomberopbbombsight.typeBomberAdjOpbRight();
                return;

            case 179: 
                typebomberopbbombsight.typeBomberAdjOpbTimer();
                return;
            }
        }
    ...
    private void doCmdPilotMove(int i, float f, boolean flag)
    {
        if(!setBombAimerAircraft() && !flag)
            return;
        ...
        if(!setPilot() && !(Main3D.cur3D().cockpitCur instanceof CockpitCopilot) && !flag)
            return;
        if(FM == null)
        {
            setBombAimerAircraft();
            if(bAAircraft == null)
                bAAircraft = (AircraftLH)World.getPlayerFM().actor;
            FM = bAAircraft.FM;
        }
        if(!flag && FM.CT.copilotMode == 1 && !(Main3D.cur3D().cockpitCur instanceof CockpitCopilot))
        {
            HUD.logCenter("PilotAxisControlsBlocked");
            return;
        }
...
label0:
        ...
        case 7: // '\007'
            ...
            if(Math.abs(f2 - lastProp) >= 0.02F && FM.EI.isSelectionHasControlProp())
            {
                setPropControl(f2);
                addCopilotMove(i, f);
            }
        ...
        case 2: // '\002'
            ...
            if(!FM.CT.bHasFlapsControlRed)
            {
                if(FM.CT.bHasFlapsControlSwitch)
                {
                    ...
                    float f3 = 1.0F / (float)(FM.CT.nFlapStages + 1);
                    int j = 1;
                    do
                    {
                        if(j >= FM.CT.nFlapStages)
                            break label0;
                        if(f > f3 * (float)j && f < f3 * (float)(j + 1))
                        {
                            FM.CT.FlapsControlSwitch = j;
                            break label0;
                        }
                        j++;
                    } while(true);
        ...
        case 6: // '\006'
            ...
            if(FM.CT.bHasDiffBrakes)
            {
                FM.CT.BrakeControlR = FM.CT.BrakeControl;
                FM.CT.BrakeControlL = FM.CT.BrakeControl;
            }
            addCopilotMove(i, f);
            break;
        ...
        case 12: // '\f'
            if(!FM.CT.bHasBrakeControl)
                break;
            FM.CT.BrakeControlR = f * 0.5F + 0.5F;
            if(!FM.CT.bHasDiffBrakes)
                FM.CT.BrakeControl = Math.max(FM.CT.BrakeControlR, FM.CT.BrakeControlL);
            addCopilotMove(i, f);
            break;
        ...
        case 11: // '\013'
            if(!FM.CT.bHasBrakeControl)
                break;
            FM.CT.BrakeControlL = f * 0.5F + 0.5F;
            if(!FM.CT.bHasDiffBrakes)
                FM.CT.BrakeControl = Math.max(FM.CT.BrakeControlR, FM.CT.BrakeControlL);
            addCopilotMove(i, f);
            break;
        ...
        case 25: // '\031'
            if(!FM.EI.isSelectionHasControlBoost())
                break;
            float f11 = f * 0.55F + 0.55F;
            if(Math.abs(f11 - lastBoost) >= 0.01F)
            {
                setBoostControl(f11);
                addCopilotMove(i, f);
            }
        ...
        case 20: // '\024'
            ...
            if(Math.abs(f12 - lastProp1) < 0.02F || 0 >= FM.EI.getNum() || !FM.EI.engines[0].isHasControlProp())
                break;
            if(useSmartAxisForPitch && FM.EI.engines.length == 3)
            {
                setPropControl(f12, 1);
        ...
        case 21: // '\025'
            ...
            if(Math.abs(f13 - lastProp2) < 0.02F || 1 >= FM.EI.getNum() || !FM.EI.engines[1].isHasControlProp())
                break;
            if(useSmartAxisForPitch && FM.EI.engines.length == 3)
            {
                setPropControl(f13, 3);
                float f17 = (lastProp1 + lastProp2) / 2.0F;
                setPropControl(f17, 2);
            } else
            if(useSmartAxisForPitch && FM.EI.engines.length == 4)
            {
                setPropControl(f13, 3);
        ...
        case 22: // '\026'
            ...
            if(Math.abs(f14 - lastProp3) >= 0.02F && 2 < FM.EI.getNum() && FM.EI.engines[2].isHasControlProp())
            {
                setPropControl(f14, 3);
                addCopilotMove(i, f);
            }
            break;
        ...
        case 23: // '\027'
            ...
            if(Math.abs(f15 - lastProp4) >= 0.02F && 3 < FM.EI.getNum() && FM.EI.engines[3].isHasControlProp())
            {
                setPropControl(f15, 4);
                addCopilotMove(i, f);
            }
            break;
    ...
    public void createPilotHotMoves()
    {
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("07", "brakes_left", 11, 21));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("08", "brakes_right", 12, 22));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("09", "pitch", 7, 7));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("10", "trimaileron", 8, 8));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("11", "trimelevator", 9, 9));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("12", "trimrudder", 10, 10));
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove(null, "-brakes_left", 11, 42));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove(null, "-brakes_right", 12, 43));
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("31", "boost", 25, 181));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove(null, "-boost", 25, 182));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("36", "LeanF", 170, 410, true));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove(null, "-LeanF", 170, 413, true));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("37", "LeanS", 171, 411, true));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove(null, "-LeanS", 171, 414, true));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFireMove("38", "Raise", 172, 412, true));
    ...
    private void createCommonHotKeys()
    {
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ65", "Misc_1", 300, 382)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ66", "Misc_2", 301, 383)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ67", "Misc_3", 302, 384)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ68", "Misc_4", 303, 385)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ69", "Misc_5", 304, 386)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ70", "Misc_6", 305, 387)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ71", "Misc_7", 306, 388)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ72", "Misc_8", 307, 389)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ73", "Misc_9", 308, 390)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ74", "Misc_10", 309, 391)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("ZZZ90", "TOGGLE_COPILOT_MODE", 168, 385));
    }
    ...
    public void createPilotHotKeys()
    {
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic08", "CourseAutopilot", 65, 167));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic09", "AIRCRAFT_RUDDER_LEFT_1", 56, 156));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic10", "AIRCRAFT_RUDDER_CENTRE", 57, 157));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic11", "AIRCRAFT_RUDDER_RIGHT_1", 58, 158));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic12", "AIRCRAFT_TRIM_V_PLUS", 44, 144));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic13", "AIRCRAFT_TRIM_V_0", 43, 143));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic14", "AIRCRAFT_TRIM_V_MINUS", 45, 145));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic15", "AIRCRAFT_TRIM_H_MINUS", 48, 148));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic16", "AIRCRAFT_TRIM_H_0", 46, 146));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic17", "AIRCRAFT_TRIM_H_PLUS", 47, 147));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic18", "AIRCRAFT_TRIM_R_MINUS", 51, 151));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic19", "AIRCRAFT_TRIM_R_0", 49, 149));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("1basic20", "AIRCRAFT_TRIM_R_PLUS", 50, 150));
        HotKeyCmdEnv.addCmd(new HotKeyCmd(false, "$$$1", "1basic21") {      });
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("2engine041", "AIRCRAFT_BOOST_PLUS_5", 180, 183));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("2engine042", "AIRCRAFT_BOOST_MINUS_5", 181, 184));
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced021", "ToggleBlownFlaps", 204, 429)); //New
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced11", "DeployDragChute", 143, 412)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced12", "DeployRefuellingDevice", 163, 405)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced121", "DecreaseWingSweep", 202, 154)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced122", "IncreaseWingSweep", 203, 155)); //New
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced19", "SideHatch", 164, 403)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced20", "ToggleFuelDump", 165, 401)); //New
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic6", "DeployFlares", 67, 174)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic7", "DeployChaff", 66, 175)); //New        
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic9", "ROCKET_RELEASE_MODE", 137, 357));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic91", "SelectMissileRocket", 162, 404)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmd(false, "$$+SIGHTCONTROLS", "4basic92") {
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced10", "SIGHT_TURN_LEFT", 131, 351));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced11", "SIGHT_TURN_CENTER", 133, 353));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced12", "SIGHT_TURN_RIGHT", 132, 352));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced13", "SIGHT_TOGGLE_CLUTCH", 138, 358));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced14", "SIGHT_OPB_LEFT", 176, 386));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced15", "SIGHT_OPB_RIGHT", 175, 387));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced16", "SIGHT_OPB_FORWARD", 177, 388));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced17", "SIGHT_OPB_BACK", 178, 389));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced18", "SIGHT_OPB_TIMER", 179, 390));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced19", "BOMB_RELEASE_MODE", 134, 354));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced20", "BOMB_RELEASE_TRAIN_AMOUNT", 135, 355));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced21", "BOMB_RELEASE_TRAIN_DELAY", 136, 356));
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced22", "ToggleRadarMode", 138, 407)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced23", "IncreaseScanRange", 139, 408)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced24", "DecreaseScanRange", 140, 409)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced25", "IncreaseRadarGain", 141, 410)); //New
        HotKeyCmdEnv.addCmd(new HotKeyCmdFire("5advanced26", "DecreaseRadarGain", 142, 411)); //New
    }
    ...
    private CockpitGunner getActiveCockpitGuner()
    {
        ...
        if(Main3D.cur3D().cockpits == null)
            return null;
        if(Main3D.cur3D().cockpitCur instanceof CockpitAuxiliary)
            return null;
        int i = World.getPlayerAircraft().FM.AS.astatePlayerIndex;
        for(int j = 0; j < Main3D.cur3D().cockpits.length; j++)
        {
            if(!(Main3D.cur3D().cockpits[j] instanceof CockpitGunner) || (Main3D.cur3D().cockpits[j] instanceof CockpitAuxiliary))
                continue;
    ...
    public void createGunnerHotKeys()
    {
        ...
        HotKeyCmdEnv.addCmd(s, new HotKeyCmd(true, "GunnerMultiFunction") {
            ...
            public void begin()
            {
                ...
                if(cockpitgunner == null)
                    return;
                if(cockpitgunner.useMultiFunction())
                {
                    Turret turret = cockpitgunner.aiTurret();
                    turret.bMultiFunction = !turret.bMultiFunction;
                    cockpitgunner.doMultiFunction(turret.bMultiFunction);
                }
            }
    ...
    public boolean hasCoPilotPit()
    {
        for(int i = 0; i < Main3D.cur3D().cockpits.length; i++)
            if(Main3D.cur3D().cockpits[i] instanceof CockpitCopilot)
                return true;
    ...
    public static void setCockpitRealMode(int i, boolean flag)
    {
        ...
        if(Main3D.cur3D().cockpits[i] instanceof CockpitGunner)
        {
            ...
            FlightModel flightmodel = World.getPlayerFM();
            AircraftState _tmp = flightmodel.AS;
...
;
    ...
    private boolean isValidCockpit(int i)
    {
        ...
        if(World.getPlayerFM().AS.isPilotParatrooper(j))
            return false;
        if(World.getPlayerFM().AS.isPilotDead(j))
            return false;
        if(Mission.isNet())
        {
            if(Mission.isCoop())
            {
                if(World.isPlayerGunner())
                {
                    if(cockpit instanceof CockpitPilot)
                        return false;
                    if((cockpit instanceof CockpitCopilot) && !World.getPlayerAircraft().isNetPlayer())
                        return false;
                } else
                {
                    if(cockpit instanceof CockpitPilot)
                        return true;
                    if(cockpit instanceof CockpitCopilot)
                        return false;
                }
                if(Time.current() == 0L)
                    return false;
                ...
                return !Actor.isValid(actor) || isCockpitFree(actor) || World.isPlayerDead();
            }
    ...
    private boolean isCockpitFree(Actor actor)
    {
        if(actor == null)
            return true;
        return World.isPlayerGunner() && World.getPlayerGunner() == actor;
    }
...
;
    ...
    private void doSwitchToCockpit(int i)
    {
        ...
        {
            boolean flag = true;
            if((Main3D.cur3D().cockpitCur instanceof CockpitPilot) && ((Main3D.cur3D().cockpits[i] instanceof CockpitPilot) || (Main3D.cur3D().cockpits[i] instanceof CockpitAuxiliary)))
                flag = false;
            if(flag && isAutoAutopilot())
                new MsgAction(true, new Integer(Main3D.cur3D().cockpitCurIndx())) {
                    ...
                    public void doAction(Object obj)
                    {
                        ...
                        HotKeyCmd.exec("misc", "cockpitRealOff" + i1);
                    }
...
;
            new MsgAction(true, new Integer(Main3D.cur3D().cockpitCurIndx())) {
                ...
                {
                    int i1 = ((Integer)obj).intValue();
                    HotKeyCmd.exec("misc", "cockpitLeave" + i1);
                }

            }
;
            if(Main3D.cur3D().cockpits[i] instanceof CockpitAuxiliary)
            {
                int j = Main3D.cur3D().cockpits[i].astatePilotIndx();
                for(int k = 0; k < Main3D.cur3D().cockpits.length; k++)
                {
                    if((Main3D.cur3D().cockpits[k] instanceof CockpitPilot) || (Main3D.cur3D().cockpits[k] instanceof CockpitAuxiliary))
                        continue;
                    int l = Main3D.cur3D().cockpits[k].astatePilotIndx();
                    if(j == l)
                        new MsgAction(true, new Integer(k)) {

                            public void doAction(Object obj)
                            {
                                int i1 = ((Integer)obj).intValue();
                                HotKeyCmd.exec("misc", "cockpitRealOn" + i1);
                            }
...
;
            ...
            new MsgAction(true, new Integer(i)) {
                ...
                {
                    int i1 = ((Integer)obj).intValue();
                    HotKeyCmd.exec("misc", "cockpitEnter" + i1);
                }
...
;
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "CockpitView", "27") {
            ...
            public void begin()
            {
                ...
                Selector.setCurRecordArg0(World.getPlayerAircraft());
                Actor actor = World.getPlayerAircraft().netCockpitGetDriver(Main3D.cur3D().cockpitCurIndx());
                if(!Main3D.cur3D().isDemoPlaying() && isCockpitFree(actor))
                    new MsgAction(true, new Integer(Main3D.cur3D().cockpitCurIndx())) {
...
;
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "PadlockViewGround", "35") {
            ...
            public void begin()
            {
                if(World.cur().diffCur.No_GroundPadlock)
                    return;
                Aircraft aircraft = World.getPlayerAircraft();
                if(!Actor.isValid(aircraft) || World.isPlayerDead() || World.isPlayerParatrooper())
                    return;
        ...
        HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "PadlockViewNext", "37") {
            ...
            public void begin()
            {
                if(World.cur().diffCur.No_Padlock && World.cur().diffCur.No_GroundPadlock)
                    return;
                Aircraft aircraft = World.getPlayerAircraft();
                if(!Actor.isValid(aircraft) || World.isPlayerDead() || World.isPlayerParatrooper())
                    return;
    ...
    public AircraftHotKeys()
    {
        ...
        createTimeHotKeys();
        createCommonHotKeys();
        bombardierControls.clear();
        copilotControls.clear();
    }
    ...
    private Actor myLatestReleasedOrdinance()
    {
        ...
        for(int i = Engine.ordinances().size() - 1; i >= 0; i--)
        {
            ...
            System.out.println("Ordinance owner: " + actor.getOwner() + " myAC: " + aircraft);
    ...
    {
        if(bombSightFovEnabled && (Main3D.cur3D().cockpitCur instanceof CockpitPilot))
            return ((int)((double)unconvertedFoV + 2.5D) / 5) * 5;
        else
            return ((int)((double)Main3D.FOVX + 2.5D) / 5) * 5;
    }

    private float convertToBSightFoV(float f)
    {
        Cockpit cockpit = Main3D.cur3D().cockpitCur;
        float af[] = cockpit.getBombSightFovs();
        float f1 = af[0];
        float f2 = af[1];
        float f3 = (f1 + f2) / 2.0F;
        float f4 = 0.0F;
        if(f >= 60F)
            f4 = cvt(f, 60F, 90F, getFoVInt(f3), getFoVInt(f2, 1.1F));
        else
            f4 = cvt(f, 30F, 60F, f1, getFoVInt(f3));
        return f4;
    }

    private String getFoV(float f)
    {
        if(bombSightFovEnabled && ((Main3D.cur3D().cockpitCur instanceof CockpitPilot) || (Main3D.cur3D().cockpitCur instanceof CockpitAuxiliary)))
        {
            float f1 = convertToBSightFoV(f);
            unconvertedFoV = getFoVInt(f);
            return "fov " + f1;
        }
        if(!RendersMain.isSaveAspect() || !Config.cur.windowsWideScreenFoV)
            return "fov " + f;
        else
            return "fov " + getFoVInt(f);
    }

    private void toggleNavLights()
    {
        if(!isMiscValid())
            return;
        FlightModel flightmodel = World.getPlayerFM();
    ...
    private void toggleLandingLights()
    {
        if(!isMiscValid())
            return;
        FlightModel flightmodel = World.getPlayerFM();
    ...
    {
        return !FM.CT.StabilizerControl && !FM.CT.courseAutopilotControl;
    }

    private void toggleCourseAutopilot(Aircraft aircraft)
    {
        aircraft.FM.CT.courseAutopilotControl = !aircraft.FM.CT.courseAutopilotControl;
        if(aircraft instanceof TypeHasNorden)
            HUD.log("C1Autopilot" + (aircraft.FM.CT.courseAutopilotControl ? "On" : "Off"));
        else
        if(aircraft instanceof TypePatinAutopilot)
            HUD.log("CourseAutopilot" + (aircraft.FM.CT.courseAutopilotControl ? "On" : "Off"));
        if(aircraft.FM.CT.courseAutopilotControl && aircraft.FM.CT.StabilizerControl)
            toggleLevelStabilizer(aircraft);
    }

    private void toggleLevelStabilizer(Aircraft aircraft)
    {
        aircraft.FM.CT.StabilizerControl = !aircraft.FM.CT.StabilizerControl;
        HUD.log("Stabilizer" + (aircraft.FM.CT.StabilizerControl ? "On" : "Off"));
        if(aircraft.FM.CT.courseAutopilotControl && aircraft.FM.CT.StabilizerControl)
            toggleCourseAutopilot(aircraft);
    }

    public boolean areGunsReloading(CockpitGunner cockpitgunner)
    {
        FlightModel flightmodel = World.getPlayerFM();
        BulletEmitter abulletemitter[] = flightmodel.CT.Weapons[cockpitgunner.weaponControlNum()];
        if(abulletemitter == null)
            return false;
        cockpitgunner = getActiveCockpitGuner();
        if(cockpitgunner == null)
            return false;
        for(int i = 0; i < abulletemitter.length; i++)
            if(abulletemitter[i] != null && abulletemitter[i].remainingReloadTime() <= 0L)
            {
                cockpitgunner.hookGunner().gunReload(false);
                return false;
            }

        HUD.log(hudLogWeaponId, "Reloading");
        cockpitgunner.hookGunner().gunFire(false);
        cockpitgunner.hookGunner().gunReload(true);
        return true;
    }

    public boolean SetFlapsHotKeys(int i, RealFlightModel realflightmodel)
    {
        int j = i % 2;
        switch(j)
        {
        ...
        case 0: // '\0'
            if(!realflightmodel.CT.bHasFlapsControlRed)
            {
                if(realflightmodel.CT.bHasFlapsControlSwitch)
                {
                    if(realflightmodel.CT.FlapsControlSwitch > 0)
                    {
                        realflightmodel.CT.FlapsControlSwitch--;
                        if(realflightmodel.CT.FlapStageText != null)
                        	HUD.log("Flaps: " + realflightmodel.CT.FlapStageText[realflightmodel.CT.FlapsControlSwitch]);
                ...
                if(realflightmodel.CT.FlapStage != null && realflightmodel.CT.FlapStageMax != -1F)
                {
                    if(realflightmodel.CT.FlapsControl > realflightmodel.CT.FlapStage[flapIndex])
                    {
                        realflightmodel.CT.FlapsControl = realflightmodel.CT.FlapStage[flapIndex];
                                                
                        String s1 = Double.toString(Math.floor((double)realflightmodel.CT.FlapStage[flapIndex] * (double)realflightmodel.CT.FlapStageMax * 100D) / 100D);
                        HUD.log(hudLogWeaponId, "FlapsDeg", new Object[] {
                                s1
                            });
                        
                        break;
                    }
                    if(flapIndex == 0 && realflightmodel.CT.FlapsControl > 0.0F)
                    {
                        realflightmodel.CT.FlapsControl = 0.0F;
                        HUD.log("FlapsRaised");
                    }
                    break;
                }
                if(realflightmodel.CT.FlapsControl > 0.33F)
                {
                    realflightmodel.CT.FlapsControl = 0.33F;
                    HUD.log("FlapsTakeOff");
                    break;
                }
                if(realflightmodel.CT.FlapsControl > 0.2F)
                {
                    realflightmodel.CT.FlapsControl = 0.2F;
                    HUD.log("FlapsCombat");
                    break;
                }
                if(realflightmodel.CT.FlapsControl > 0.0F)
                {
                    realflightmodel.CT.FlapsControl = 0.0F;
                    HUD.log("FlapsRaised");
                }
            ...
            if(realflightmodel.CT.FlapsControl > 0.5F)
            {
                realflightmodel.CT.FlapsControl = 0.0F;
                HUD.log("FlapsRaised");
            }
        ...
        case 1: // '\001'
            if(!realflightmodel.CT.bHasFlapsControlRed)
            {
                if(realflightmodel.CT.bHasFlapsControlSwitch)
                {
                    if(realflightmodel.CT.FlapsControlSwitch >= realflightmodel.CT.nFlapStages)
                        break;
                    realflightmodel.CT.FlapsControlSwitch++;
                    if(realflightmodel.CT.FlapStageText != null)
                    	 HUD.log("Flaps: " + realflightmodel.CT.FlapStageText[realflightmodel.CT.FlapsControlSwitch]);
                ...
                if(realflightmodel.CT.FlapStage != null && realflightmodel.CT.FlapStageMax != -1F)
                {
                    ...
                    if(flapIndex == realflightmodel.CT.nFlapStages - 1 && realflightmodel.CT.FlapsControl < 1.0F)
                    {
                        realflightmodel.CT.FlapsControl = 1.0F;
                                                
                        String s1 = Double.toString(realflightmodel.CT.FlapStageMax);
                        HUD.log(hudLogWeaponId, "FlapsDeg", new Object[] {
                                s1
                            });
                    }
                ...
                if(realflightmodel.CT.FlapsControl < 1.0F)
                {
                    realflightmodel.CT.FlapsControl = 1.0F;
                    HUD.log("FlapsLanding");
                }
    ...
    public boolean SetVarWingHotKeys(int i, RealFlightModel realflightmodel)
    {
        int j = i % 2;
        switch(j)
        {
        ...
        case 0: // '\0'
            if(realflightmodel.CT.VarWingStage != null && realflightmodel.CT.VarWingStageMax != -1F)
            {
                if(realflightmodel.CT.VarWingControl > realflightmodel.CT.VarWingStage[varWingIndex])
                {
                    realflightmodel.CT.VarWingControl = realflightmodel.CT.VarWingStage[varWingIndex];
                    HUD.log("Wings: " + Math.floor((double)realflightmodel.CT.VarWingStage[varWingIndex] * (double)realflightmodel.CT.VarWingStageMax * 100D) / 100D + " deg.");
                    break;
                }
                if(varWingIndex == 0 && realflightmodel.CT.VarWingControl > 0.0F)
                {
                    realflightmodel.CT.VarWingControl = 0.0F;
        ...
        case 1: // '\001'
            ...
            if(varWingIndex == realflightmodel.CT.nVarWingStages - 1 && realflightmodel.CT.VarWingControl < 1.0F)
            {
                realflightmodel.CT.VarWingControl = 1.0F;
                HUD.log("Wings: " + realflightmodel.CT.VarWingStageMax + " deg.");
            }
            break;
        }
        return true;
    ...
    public static final int WEAPON3 = 19;
    ...
    private static final int AIRCRAFT_COURSE_AUTOPILOT = 65;
    ...
    private static final int SIGHT_TURN_LEFT = 131;
    private static final int SIGHT_TURN_RIGHT = 132;
    private static final int SIGHT_TURN_CENTER = 133;
    private static final int BOMB_RELEASE_MODE = 134;
    private static final int BOMB_RELEASE_TRAIN_AMOUNT = 135;
    private static final int BOMB_RELEASE_TRAIN_DELAY = 136;
    private static final int ROCKET_RELEASE_MODE = 137;
    private static final int SIGHT_TOGGLE_CLUTCH = 138;
    ...
    private static final int COPILOT_NAV_LIGHTS = 166;
    private static final int COPILOT_LANDING_LIGHTS = 167;
    private static final int TOGGLE_COPILOT_MODE = 168;
    ...
    private static final int SIGHT_OPB_RIGHT = 175;
    private static final int SIGHT_OPB_LEFT = 176;
    private static final int SIGHT_OPB_FORWARD = 177;
    private static final int SIGHT_OPB_BACK = 178;
    private static final int SIGHT_OPB_TIMER = 179;
    private static final int BOOST_PLUS_5 = 180;
    private static final int BOOST_MINUS_5 = 181;
    ...
    private static final int MOVE_BRAKE_L = 11;
    private static final int MOVE_BRAKE_R = 12;
    ...
    private static final int MOVE_MIXTURE = 24;
    private static final int MOVE_BOOST = 25;
    private static final int MOVE_ZOOM = 100;
    private int cptdmg;
    private boolean useSmartAxisForPower;
    private boolean useSmartAxisForPitch;
    public static List copilotControls = new ArrayList();
    public static List bombardierControls = new ArrayList();
    public static final int BOMBARDIER_ACTION_BOMBSIGHT_CONTROLS = 10;
    public static final int BOMBARDIER_ACTION_BOMBSIGHT_AUTOPILOT = 11;
    public static final int BOMBARDIER_ACTION_OPB_CONTROLS = 12;
    private boolean changeFovEnabled;
    private boolean bombSightFovEnabled;
    private float unconvertedFoV;
    private HashMapExt copilotCmdMap;

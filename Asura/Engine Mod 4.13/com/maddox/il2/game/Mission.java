// Decompiled by DJ v3.10.10.93 Copyright 2007 Atanas Neshkov  Date: 17.09.2015 11:18:35
...
//            ZutiSupportMethods, ZutiRadarRefresh, DeviceLink, DotRange, 
//            I18N
    ...
    implements Destroy
{
        ...
        throws Exception
    {
        ...
        if(Config.isUSE_RENDER())
        {
            ...
            int j2 = sectfile.get("MAIN", "CloudType", 0, 0, 6);
            ...
            float f2 = 0.25F + (float)(j2 * j2) * 0.12F;
            f2 = sectfile.get("WEATHER", "WindSpeed", f2, 0.0F, 15F);
            float f3 = j2 > 3 ? (float)j2 * 2.0F : 0.0F;
            f3 = sectfile.get("WEATHER", "Gust", f3, 0.0F, 12F);
            float f4 = j2 > 2 ? j2 : 0.0F;
...
label0:
        {
            ...
            for(int k = 0; k < j; k++)
            {
                String s3 = sectfile.line(i, k);
                if(k == 0)
                    arraylist.add(s3);
                if(s3.startsWith("TRIGGERS") && flag)
                    continue;
                if(!s3.startsWith("TAKEOFF"))
                    arraylist.add(s3);
                if(s3.startsWith("NORMFLY"))
                    flag = false;
            }
...
label0:
    ...
    private void populateRunwayLights()
    {
        ...
        for(int i = 0; i < arraylist.size(); i++)
        {
            ...
            for(int k = 0; k < actors.size(); k++)
            {
                if(!(actors.get(k) instanceof TypeRunwayLight))
                    continue;
    ...
    public static int getMissionDate(boolean flag)
    {
        ...
        if(Main.cur().mission == null)
        {
            ...
            int j = sectfile.get("SEASON", "Year", 1940, 1930, 1960);
    ...
    protected void printWithTime(String s)
    {
        if(shortDate == null)

...
public class Way
{
    ...
    public WayPoint autoFastJet(Point3d point3d)
    {
        if(Cur == 0 || isReachedFastJet(point3d))
            return next();
        else
            return curr();
    }
    ...
    public boolean isReached(Point3d point3d)
    {
    	...
    	if((curr().waypointType == 4 || curr().waypointType == 5) && point3d.z > Engine.land().HQ_Air(point3d.x, point3d.y) + 20D)
            return true;
        curr().getP(P);
        V.sub(point3d, P);
    ...
    public void setNearestTaxingPoint(Point3d point3d, Orient orient)
    {
        ...
        {
            WayPoint waypoint2 = (WayPoint)WList.get(j);
        ...
        WayPoint waypoint = (WayPoint)WList.get(i);
        waypoint.getP(tmpP);
        V.sub(tmpP, point3d);
        ...
        {
            WayPoint waypoint1 = (WayPoint)WList.get(i + 1);
            if(waypoint1.waypointType == 4 || waypoint1.waypointType == 5)
                i++;
        }
        Cur = i;
    }

    public boolean isReachedFastJet(Point3d point3d)
    {
        ...
        if(curr().timeout == -1 && !isLast())
        {
            ...
            if(bLogDetail)
                System.out.println("Way: 190 - curDist=" + Integer.toString((int)Math.floor(curDist)) + " , prevdistToNextWP2=" + Integer.toString((int)Math.floor(prevdistToNextWP2)));
            if(curDist < 100000000D && curDist > prevdistToNextWP2)
            {
                ...
                prevdistToNextWP2 = 3.4028234663852886E+038D;
                if(bLogDetail)
                    System.out.println("Way: 196 - return true;");
                bAlreadyTrue = true;
                return true;
            }
        ...
        } else
        {
            ...
            curDist = V.x * V.x + V.y * V.y;
            if(bLogDetail)
                System.out.println("Way: 204 - curDist=" + Integer.toString((int)Math.floor(curDist)) + " , prevdist2=" + Integer.toString((int)Math.floor(prevdist2)));
            if(curDist < 3000000D && curDist > prevdist2)
            {
                prevdist2 = 200000000D;
                if(bLogDetail)
                    System.out.println("Way: 209 - return true;");
                bAlreadyTrue = true;
                return true;
            }
            prevdist2 = curDist;

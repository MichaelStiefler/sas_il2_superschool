...
//            AutopilotAI, Pilot, Point_Any, Point_Null, 
//            AirGroup, AirGroupList, ManString
...
public class Maneuver extends AIFlightModel
{
    ...
    private void resetControls()
    {
        ...
        CT.BrakeControlL = CT.BrakeControlR = 0.0F;
    }
    ...
    public void update(float f)
    {
        ...
        if(oldLeader != Leader)
        {
            ...
            if(aircraft != null && Group != null && Group.realNumInGroup(aircraft) == 0)
                Formation.leaderOffset(aircraft.FM, formationType, aircraft.FM.Offset);
            oldLeader = Leader;
        }
        ...
        case 84: // 'T'
            ...
            if(!flag)
            {
                if((task == 3 || task == 2) && ((Aircraft)actor).getWing().bOnlyAI && Group != null && Group.formationType != 12)
                {
                    Group.setFormationAndScale((byte)12, 2.0F, true);
                    Group.formationType = 12;
                }
        ...
        case 24: // '\030'
            ...
            if(Leader.getSpeed() < 30F || Leader.Loc.z - Engine.land().HQ_Air(Leader.Loc.x, Leader.Loc.y) < 51.5D || Leader.Loc.z - Engine.land().HQ_Air(Leader.Loc.x, Leader.Loc.y) < 51.5D && Leader.getSpeed() < Vmin * 1.3F)
            {
                airClient = Leader;
        ...
        case 65: // 'A'
            ...
            } else
            {
                ...
                if(maneuver7 != null)
                {
                    tmpV3dToDanger.sub(maneuver7.Loc, maneuver2.Loc);
                    float f21 = (float)tmpV3dToDanger.length();
                    if(f21 < 600F || (airClient instanceof TypeBomber))
                        flag3 = true;
                }
                if(flag3 && maneuver2.getDangerAggressiveness() >= 0.8F - (float)Skill * 0.2F && hasCourseWeaponBullets())
                {
                ...
                tailOffset.set(followOffset);
                tailOffset.scale(-1D);
                if((double)dist > 1200D + Ve.x * 400D && get_maneuver() != 27)
                {
                    push();
                    push(53);
                    pop();
        ...
        case 76: // 'L'
            ...
            } else
            {
                ...
                if(maneuver8 != null)
                {
                    tmpV3dToDanger.sub(maneuver8.Loc, maneuver3.Loc);
                    float f22 = (float)tmpV3dToDanger.length();
                    if(f22 < 700F || (airClient instanceof TypeBomber))
                        flag4 = true;
                }
                if(flag4 && maneuver3.getDangerAggressiveness() >= 0.8F - (float)Skill * 0.25F && hasCourseWeaponBullets())
                {
                ...
                tailOffset.set(followOffset);
                tailOffset.scale(-1D);
                if((double)dist > 2000D + Ve.x * 500D && get_maneuver() != 27)
                {
                    push();
                    push(53);
                    pop();
        ...
        case 53: // '5'
            ...
            } else
            {
                ...
                float f23 = (Energy - airClient.Energy) * 0.1019F;
                if((double)f23 < -50D + followOffset.z)
                    setSpeedMode(9);
                else
                    setSpeedMode(10);
                tailForStaying = airClient;
        ...
        case 74: // 'J'
            ...
            } else
            {
                ...
                {
                    boolean flag5 = false;
                    tmpV3dToDanger.sub(maneuver10.Loc, maneuver5.Loc);
                    float f26 = (float)tmpV3dToDanger.length();
                    if(f26 < 700F || (airClient instanceof TypeBomber))
                        flag5 = true;
                    ...
                    if(flag5 && (maneuver5.getDangerAggressiveness() >= 0.5F || dist < 800F && tmpV3dToDanger.x > -0.33000000000000002D) || dist < 300F)
                    {
                        set_task(6);
                        target = maneuver10;
                        push();
            ...
            } else
            {
                ...
                if(dist < 300F)
                {
                    ...
                    if(beNearOffsPhase > 3)
                        beNearOffsPhase = 0;
                    float f27 = (float)Math.sqrt(followOffset.x * followOffset.x + followOffset.y * followOffset.y);
                    followOffset.set(f27 * (float)Math.sin((float)beNearOffsPhase * 1.5708F), f27 * (float)Math.cos((float)beNearOffsPhase * 1.5708F), followOffset.z);
                }
                if(AOA > 12F && Alt > 15F)
                    Or.increment(0.0F, 12F - AOA, 0.0F);
        ...
        case 27: // '\033'
            ...
            if(isTick(77, 0) && !target.isOk() && World.Rnd().nextInt(0, 1000) < 40 * (Skill + 1))
            {
                target = null;
                clear_stack();
                set_maneuver(0);
            ...
            } else
            {
                ...
                if(isTick(64, 0))
                {
                    float f24 = (target.Energy - Energy) * 0.1019F;
                    Ve.sub(target.Loc, Loc);
                    Or.transformInv(Ve);
                    Ve.normalize();
                    float f28 = (470F + (float)Ve.x * 120F) - f24;
                    if(f28 < 0.0F)
                    {
                        clear_stack();
                        set_maneuver(62);
        ...
        case 62: // '>'
            ...
            } else
            {
                ...
                if(isTick(164, 0) && !Group.inCorridor(Loc))
                {
                    target = null;
                    clear_stack();
                    set_maneuver(0);
            ...
            } else
            {
                ...
                case 2: // '\002'
                    ...
                    if(dist < 200F || sub_Man_Count > 100)
                    {
                        ...
                        {
                            float f29 = Math.abs(target.getOverload());
                            float f31 = dist * 0.05F;
                            if(World.Rnd().nextFloat(0.0F, 70F + (float)Skill * 10F) > 10F + f29 * 5F + f31)
                                submaneuver--;
                            else
                                submaneuver++;
                        } else
        ...
        case 25: // '\031'
            ...
            if(AP.way.isLanding())
            {
                ...
                AP.way.curr().getP(Po);
                int i1 = AP.way.Cur();
                float f32 = (float)Loc.z - AP.way.last().z();
                AP.way.setCur(i1);
                Alt = Math.min(Alt, f32);
                Po.add(0.0D, 0.0D, -3D);
                Ve.sub(Po, Loc);
                float f33 = (float)Ve.length();
                boolean flag7 = Alt > 4.5F + Gears.H && AP.way.Cur() < 8;
                if(AP.way.isLandingOnShip())
                    flag7 = Alt > 4.5F + Gears.H && AP.way.Cur() < 8;
                if(flag7)
                {
                    AP.way.prev();
                    AP.way.curr().getP(Pc);
                    ...
                    tmpV3f.sub(Po, Pc);
                    tmpV3f.normalize();
                    if(tmpV3f.dot(Ve) < 0.0D && f33 > 1000F && !TaxiMode)
                    {
                        AP.way.first();
                        push(10);
                        pop();
                    ...
                    tmpV3f.sub(Loc);
                    float f40 = 0.0005F * (3000F - f33);
                    if(f40 > 1.0F)
                        f40 = 1.0F;
                    if(f40 < 0.1F)
                        f40 = 0.1F;
                    float f44 = (float)tmpV3f.length();
                    if(f44 > 40F * f40)
                    {
                        f44 = 40F * f40;
                        tmpV3f.normalize();
                    ...
                    if(AP.way.Cur() >= 6)
                    {
                        if(AP.way.isLandingOnShip())
                        {
                            ...
                            {
                                float f50 = (float)((AirportCarrier)AP.way.landingAirport).speedLen();
                                if(VminFLAPS < f50 + 10F)
                                    f47 = f50 + 10F;
                            }
                        } else
                        {
                    ...
                    Ve.add(tmpV3f);
                    Ve.sub(Vwld);
                    float f54 = (50F * f40 - f44) * f;
                    if(Ve.length() > (double)f54)
                    {
                        Ve.normalize();
                        Ve.scale(f54);
                    }
                    ...
                    if(f55 < 0.0F)
                        f55 = 0.0F;
                    tmpOr.increment(0.0F, 4F + f55, (float)(tmpV3f.y * 0.5D));
                    Or.interpolate(tmpOr, 0.5F * f);
                    callSuperUpdate = false;
                    W.set(0.0D, 0.0D, 0.0D);
                    CT.ElevatorControl = 0.05F + 0.3F * f55;
            ...
            } else
            {
                ...
                if(getSpeed() < VmaxFLAPS * 0.21F && !CT.bHasFlapsControlSwitch)
                    CT.FlapsControl = 0.0F;
                if(Vrel.length() < (double)((super.actor instanceof TypeFastJet) ? Math.min(35F, VminFLAPS * 0.5F) : VmaxFLAPS * 0.25F) && (wayCurPos == null && !AP.way.isLandingOnShip() && isEnableToTaxi()))
                {
                    TaxiMode = true;
                    AP.way.setCur(0);
                    return;
                ...
                float f30 = Gears.Pitch - 2.0F;
                if(f30 < 5F)
                    f30 = 5F;
                if(Alt < 7F && Or.getTangage() < f30 || AP.way.isLandingOnShip())
                    CT.ElevatorControl += 1.5F * f;
        ...
        case 64: // '@'
            ...
            if(actor instanceof HE_LERCHE3)
            {
                boolean flag6 = Actor.isAlive(AP.way.takeoffAirport) && (AP.way.takeoffAirport instanceof AirportCarrier);
                if(!flag6)
                    callSuperUpdate = false;
            }
        ...
        case 102: // 'f'
            ...
            if(wayCurPos == null)
            {
                ...
                do
                {
                    ...
                    taxiToTakeOffWay.add(new Point_Any(waypoint2.x() + f45, waypoint2.y() + f48));
                    k1++;
                } while(true);
            ...
            if(wayCurPos != null)
            {
                ...
                V_to.normalize();
                float f35 = 3F + 0.1F * (distToTaxiPoint - actor.collisionR() * 2.0F);
                if(f35 > 7F && (Leader == null || !Leader.isCapableOfTaxiing()))
                    f35 = 7F;
                else
                if(Leader != null && f35 > 9F && Leader.isCapableOfTaxiing())
                {
                    f35 = 9F;
                    Ve.sub(Leader.Loc, Loc);
                ...
                {
                    boolean flag9 = isTaxingCollisionDanger();
                    if(flag9)
                        f35 = 0.01F;
                }
                V_to.sub(V_taxiLeg);
                float f41 = RAD2DEG((float)Math.atan2(V_to.y, V_to.x));
                for(f41 += Or.azimut(); f41 > 180F; f41 -= 360F);
                for(; f41 < -180F; f41 += 360F);
                Wo += (2.0F * f41 * f) / actor.collisionR();
                if((double)f35 < 0.10000000000000001D)
                    Wo *= 0.95F;
                f41 = (f41 > 20F ? 20F : f41) < -20F ? -20F : f41;
                CT.RudderControl = -f41 * 0.05F;
                if(f41 > 0.0F)
                {
                    if(Wo > f41)
                    {
                ...
                Vwld.x += 2.0F * f * (float)((double)f35 > Vwld.x ? 1 : -1);
        ...
        case 26: // '\032'
            ...
            if(Actor.isAlive(AP.way.takeoffAirport) && (AP.way.takeoffAirport instanceof AirportCarrier))
            {
                ...
                {
                    f38 -= ((AirportCarrier)AP.way.takeoffAirport).height();
            ...
            if(first)
            {
                ...
                {
                    World.getPlayerAircraft();
                    if(Aircraft.isPlayerTaxing() && EI.engines[0].getStage() == 6 && (AP.way.first().waypointType == 4 || AP.way.first().waypointType == 5) && lookAtNextTaxiToTakeOffPoint() != null)
                    {
                        push();
                        push(102);
                        submaneuver = 0;
                        maneuver = 0;
                        safe_pop();
                        AP.way.setNearestTaxingPoint(actor.pos.getAbsPoint(), actor.pos.getAbsOrient());
                        curAirdromePoi = AP.way.Cur() + 1;
                        break;
                    }
                }
            }
            if(Gears.onGround())
            {
                if(EI.engines[0].getStage() == 0)
                {
                    CT.setPowerControl(0.0F);
                    setSpeedMode(8);
                    float f34 = World.Rnd().nextFloat(7F + (float)l1 * 0.25F, 10F);
                    if(World.Rnd().nextFloat() < 0.05F && mn_time > 1.0F || mn_time > f34)
                    {
                        push();
                ...
                } else
                {
                    ...
                    if(!bAlreadyCheckedStage7)
                    {
                        if(AP.way.takeoffAirport instanceof AirportCarrier)
                        {
                            ...
                            BigshipGeneric bigshipgeneric = airportcarrier1.ship();
                        ...
                        if(!bStage7)
                        {
                            ...
                            if(bStage3)
                            {
                                ...
                                if(Gears.getCatapultAlreadySetNum() == -1)
                                {
                                    ...
                                    if(bCatapultAI)
                                    {
                                        d5 = -((Tuple3d) (cellairfield.leftUpperCorner())).x - Gears.getCatapultOffsetX();
                                        d8 = ((Tuple3d) (cellairfield.leftUpperCorner())).y - Gears.getCatapultOffsetY();
                                    } else
                                    {
                                        d5 = -((Tuple3d) (cellairfield.leftUpperCorner())).x - (double)(cellairfield.getWidth() / 2 - 3);
                                        d8 = brakeShoeLoc.getX() + (double)aircraft.getCellAirPlane().getHeight() + 4D;
                                    }
                                    loc.set(d8, d5, brakeShoeLoc.getZ(), brakeShoeLoc.getAzimut(), brakeShoeLoc.getTangage(), brakeShoeLoc.getKren());
                    ...
                    //
                    {
                        ...
                        if(Actor.isAlive(AP.way.takeoffAirport) && flag12 && AP.way.takeoffAirport.takeoffRequest > 0)
                            flag10 = false;
                        if(flag10)
                        {
                            canTakeoff = true;
            ...
            if(EI.engines[0].getStage() == 6 && CT.getPower() > f42)
            {
                CT.BrakeControl = 0.0F;
                brakeShoe = false;
                float f46 = (float)((double)((Vmin + VminFLAPS) * 0.5F) * Math.sqrt(M.getFullMass() / M.referenceWeight));
                float f49 = (AOA_Crit - 2.0F) * (getSpeed() / f46);
                if(Gears.bIsSail)
                    f49 *= 2.0F;
                ...
                if(Or.getTangage() < f49)
                    dA = -0.7F * (Or.getTangage() - f49) + f51 * (float)getW().y + 0.5F * (float)getAW().y;
                else
                    dA = -0.1F * (Or.getTangage() - f49) + f51 * (float)getW().y + 0.5F * (float)getAW().y;
            ...
            if(EI.engines[0].getStage() == 6)
            {
                ...
                if(d2 > -1D && d2 < 1.0D)
                {
                    if(flag11 && Actor.isAlive(AP.way.takeoffAirport) && CT.getPower() > 0.3F)
                    {
            ...
            if(f38 > 5F && !Gears.isUnderDeck())
                CT.GearControl = 0.0F;
            float f52 = 1.0F;
            if(hasBombs() || !flag11)
                f52 *= 1.7F;
            if(f38 > 120F * f52 || getSpeed() > Vmin * 1.8F * f52 || f38 > 80F * f52 && getSpeed() > Vmin * 1.6F * f52 || f38 > 40F * f52 && getSpeed() > Vmin * 1.3F * f52 && mn_time > 60F + (float)((Aircraft)actor).aircIndex() * 3F)
            {
            	if(!CT.bHasFlapsControlSwitch)
                    CT.FlapsControl = 0.0F;
                CT.GearControl = 0.0F;
        ...
        case 69: // 'E'
            ...
            CT.BrakeControl = 1.0F;
            W.scale(0.0D);
            boolean flag13 = Actor.isAlive(AP.way.takeoffAirport) && (AP.way.takeoffAirport instanceof AirportCarrier);
            if(flag13)
            {
                f39 -= ((AirportCarrier)AP.way.takeoffAirport).height();
                f43 = 0.8F;
                if(Alt < 9F && Vwld.z < 0.0D)
                    Vwld.z *= 0.84999999999999998D;
                if(CT.bHasCockpitDoorControl)
            ...
            if(Loc.z != 0.0D)
                if(EI.getPowerOutput() < f43 && !flag13)
                {
                    callSuperUpdate = false;
        ...
        case 29: // '\035'
            ...
            if(danger != null)
            {
                ...
                float f25 = (Energy - danger.Energy) * 0.1019F;
                if(Loc.distanceSquared(danger.Loc) < 5000D || f25 < -200F)
                {
                    setSpeedMode(8);
                    CT.setPowerControl(0.0F);
                } else
        ...
        strikeEmer = false;
        setSpeedControl(f);
        CT.AileronControl = clampA(CT.AileronControl, CT.Sensitivity);
        CT.ElevatorControl = clampA(CT.ElevatorControl, CT.Sensitivity);
        CT.RudderControl = clampA(CT.RudderControl, CT.Sensitivity);
        first = false;
        mn_time += f;
        if(frequentControl)
            AP.update(f);
    ...
    private void groundAttackGuns(Actor actor, float f)
    {
        ...
        case 2: // '\002'
            ...
            float f7 = (float)Ve.length();
            setRSDgrd(shootingDeviation, f7 * bullTime);
            Ve.add(wanderVector);
            actor.getSpeed(tmpV3d);
            ...
            tmpV3f.sub(Vwld);
            tmpV3f.scale(f7 * bullTime * 0.3333F * (float)Skill);
            Ve.add(tmpV3f);
            float f8 = 0.3F * (f7 - 1000F);
            if(f8 < 0.0F)
                f8 = 0.0F;
            if(f8 > 300F)
                f8 = 300F;
            ...
            Or.transformInv(Ve);
            float f9 = getSpeedKMH() / 260F;
            if(f9 < 1.0F)
                f9 = 1.0F;
            if(f7 < 800F * f9 && (shotAtFriend <= 0 || distToFriend > f7))
            {
                shootingDeviation -= f;
                ...
                {
                    if(f7 < 800F * f9 && Group.getAaaNum() > 10F || f7 < 650F * f9)
                        CT.WeaponControl[0] = true;
                    if(f7 < 700F * f9 && Group.getAaaNum() > 10F || f7 < 550F * f9)
                        CT.WeaponControl[1] = true;
                    if(CT.Weapons[2] != null && CT.Weapons[2][0] != null && CT.Weapons[2][CT.Weapons[2].length - 1].countBullets() != 0 && rocketsDelay < 1 && f7 < 570F)
                    {
                        CT.WeaponControl[2] = true;
            ...
            if(sub_Man_Count > 200 && Ve.x < (double)(getSpeed() * 2.4F) || (this.actor instanceof TypeStormovik) && Alt < 80F * f9 || Alt < 40F * f9)
            {
            ...
            Ve.normalize();
            attackTurnToDirection(f7, f, 10F + (float)Skill * 2.0F);
            setSpeedMode(4);
            smConstSpeed = 100F;
            break;
    ...
    private void groundAttack(Actor actor, float f)
    {
        ...
        if(!(this.actor instanceof TypeStormovik) && !(this.actor instanceof TypeFighter) && !(this.actor instanceof TypeDiveBomber))
        {
            set_maneuver(0);
            return;
        }
        ...
        if(Math.abs(Ve.y) > 0.0010000000474974513D)
            CT.RudderControl = -98F * (float)Math.atan2(Ve.y, Ve.x);
        else
            CT.RudderControl = 0.0F;
    ...
    private void groundAttackShallowDive(Actor actor, float f)
    {
        ...
        Ve.y += (float)tmpV3d.y;
        Ve.z += (float)tmpV3d.z;
        if((CT.Weapons[3][0].getClass().getName().endsWith("SnakeEye") ? (double)f5 >= (double)f2 * 1.175D : f5 >= f2))
        {
            bombsOut = true;            
            Voice.speakAttackByBombs((Aircraft)this.actor);
            setSpeedMode(6);
    ...
    protected void turnOffTheWeapon()
    {
        ...
        {
            bombsOutCounter++;
            if(bombsOutCounter > 128 && !CT.isTrainStillDropping())
            {
                bombsOutCounter = 0;
                bombsOut = false;
            }
    ...
    protected void setSpeedControl(float f)
    {
        ...
        CT.setManualfterburnerControl(0.0F);
        switch(speedMode)
        {
        case 1: // '\001'
        ...
        case 3: // '\003'
            ...
            float f6 = AP.way.curr().Speed / VmaxH;
            f1 = 0.2F + 0.8F * (float)Math.pow(f6, 1.5D);
    ...
    {
        if(EI.isSelectionHasControlBoost())
        {
            float f = CT.getManualAfterburnerControl();
            float f1 = EI.getManifoldPressure();
            if(f1 < EI.engines[0].compressorRPMtoWMaxATA)
                CT.setManualfterburnerControl(f + 0.1F);
            else
            if(f1 > EI.engines[0].compressorRPMtoWMaxATA)
                CT.setManualfterburnerControl(f - 0.1F);
        } else
    ...
    private Point_Any lookAtNextTaxiToTakeOffPoint()
    {
        if(taxiToTakeOffWay == null)
            return new Point_Any(AP.way.first().x(), AP.way.first().y());
        if(taxiToTakeOffWay.size() == 0)
            return null;
        if(curAirdromePoi >= taxiToTakeOffWay.size())
            return null;
        else
            return (Point_Any)taxiToTakeOffWay.get(curAirdromePoi + 1);
    }
...
label0:
    ...
    {
        if(maneuver == 21 && Group.nOfAirc == 1 || maneuver == 24 && Group.airc[Group.nOfAirc - 1] == actor)
        {
            if(formationType == 4)
                return;
            if((actor instanceof TypeFighter) && !isBusy() && crew == 1)
            {
        ...
        if(j != 0)
        {
            ...
            do
            {
                if(!iterator1.hasNext())
                    break;
                i++;
                Point_Any point_any4 = (Point_Any)iterator1.next();
    ...
    private boolean isTaxingCollisionDanger()
    {
        ...
        for(int i = 1; i <= byte0; i++)
        {
            ...
            if(actor != null && this.actor != actor && actor != ignoredActor)
            {
                ...
                if(!actor.isAlive() || actor.isDestroyed() || actor.getSpeed(Vpl) < 0.10000000149011612D)
                {

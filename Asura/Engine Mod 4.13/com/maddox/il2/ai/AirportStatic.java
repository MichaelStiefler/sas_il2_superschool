...
public abstract class AirportStatic extends Airport
{
        ...
        while(arraylist.size() > 0) 
        {
            ...
            } else
            {
            	...
            	if (Engine.cur.land.isWater(loc.getPoint().x, loc.getPoint().y))
                  ...
                  ((AirportStatic)localObject).pos = new ActorPosStatic((Actor)localObject, loc);
                  ((AirportStatic)localObject).runway.add(new Runway(loc));
                  arraylist1.add(localObject);
            }
        }
    }
    ...
    public boolean landWay(FlightModel flightmodel)
    {
        ...
        for(int i = x.length - 1; i >= 0; i--)
        {
            ...
            if(((Interpolate) (flightmodel)).actor instanceof TypeFastJet)
            {
                float f3;
                if(i < 3)
                    f3 = Math.max(vFJ[i] * 0.278F, flightmodel.VminFLAPS * 1.2F);
                else
                    f3 = Math.max(vFJ[i] * 0.278F, flightmodel.Vmin * 1.2F);
                waypoint.set(Math.min(f3, 120F));
            } else
            {
    ...
    private boolean setStationaryPlaneTakeoff(Aircraft aircraft, Runway runway1)
    {
        ...
        if(actor != null && (!Mission.isCoop() || aircraft.getWing().bCoopSpawnStationary))
            actor.destroy();
    ...
    public void setTakeoff(Point3d point3d, Aircraft aaircraft[])
    {
        ...
        for(int i = 0; i < aaircraft.length; i++)
        {
            ...
            if(aaircraft[i].spawnLocSingleCoop != null)
            {
                ...
                if(!flag || !Mission.isCoop() || aaircraft[i].getWing().bCoopSpawnStationary)
                    continue;
            }
    ...
    private static float x[] = {
        -500F, 0.0F, 220F, 2000F, 4000F, 5000F, 4000F, 0.0F, -2000F
    };
    private static float y[] = {
        0.0F, 0.0F, 0.0F, 0.0F, -500F, -2000F, -4000F, -4000F, -4000F
    };
    private static float xFJ[] = {
        -1000F, -0F, 220F, 4000F, 8000F, 10000F, 8000F, 0.0F, -4000F
    };

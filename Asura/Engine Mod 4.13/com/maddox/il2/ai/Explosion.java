...
//            World, DifficultySettings, RangeRandom
...
public class Explosion
{
    ...
    public float receivedPower(ActorMesh actormesh)
    {
        if(World.cur().diffCur.RealisticBombSights)
        {
            ...
            if(f < 1.0F)
                ...
                return power / (float)Math.sqrt(f * f * f);
        }
        float f1 = actormesh.collisionR();
        float f2 = (float)actormesh.pos.getAbsPoint().distance(p);
        f2 -= f1;
        if(f2 >= radius)
            return 0.0F;
        float f3 = 1.0F - f2 / radius;
        f3 *= f3;
        if(f3 >= 1.0F)
            return power;
        else
    ...
    public float receivedTNT_1meterWater(float f, float f1)
    {
        ...
        if(f < 1.0F)
            ...
            return Math.min((power * f3) / (float)Math.sqrt(f * f * f), power);
    }
    ...
    public float receivedTNTpower(ActorMesh actormesh)
    {
        ...
        if(World.cur().diffCur.RealisticBombSights)
            if(f1 <= 1.828F)
                ...
                return (power * 4F) / ((f1 + 1.0F) * (f1 + 1.0F));
        if(f1 <= 0.0F)
            return 0.5F * power;

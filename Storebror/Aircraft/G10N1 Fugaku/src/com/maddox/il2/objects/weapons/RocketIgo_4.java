    ...
    public boolean interpolateStep() {
        if (this.tEStart > 0L) {
            if (Time.current() > this.tEStart) {
                ...
                v.set(1F, 0F, 0F);
                this.or3.transform(v);
                v.scale(this.getSpeed(null));
                Vector3d ownerSpeed = ((Aircraft) this.getOwner()).FM.Vwld;
                double zFactor = (double) CommonTools.smoothCvt(this.tEStart - Time.current(), 100F, 1000F, 1F, 0F);
                v.x = 0.99D * ownerSpeed.x + 0.01D * v.x;
                v.y = 0.99D * ownerSpeed.y + 0.01D * v.y;
                v.z = (1D - zFactor) * ownerSpeed.z + zFactor * v.z;
                setSpeed(v);
            }
        }

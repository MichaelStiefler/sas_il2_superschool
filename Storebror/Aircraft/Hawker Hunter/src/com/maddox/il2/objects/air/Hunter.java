...
public class Hunter extends Scheme1 implements TypeSupersonic, TypeFighter, TypeBNZFighter, TypeFighterAceMaker, TypeGSuit {
    public Hunter() {
        ...
        this.shockwave = new Eff3DActor[NUM_SHOCKWAVES];
        for (int i = 0; i < NUM_SHOCKWAVES; i++) {
            this.shockwave[i] = null;
        }
    }
    ...
    public void onAircraftLoaded() {
        if (this.thisWeaponsName.indexOf("in_") == -1) {
            if (this.hierMesh().chunkFindCheck("PylonInL") >= 0) {
                this.hierMesh().chunkVisible("PylonInL", false);
            }
            if (this.hierMesh().chunkFindCheck("PylonInR") >= 0) {
                this.hierMesh().chunkVisible("PylonInR", false);
            }
        }
        if (this.thisWeaponsName.indexOf("out_") == -1) {
            if (this.hierMesh().chunkFindCheck("PylonOutL") >= 0) {
                this.hierMesh().chunkVisible("PylonOutL", false);
            }
            if (this.hierMesh().chunkFindCheck("PylonOutR") >= 0) {
                this.hierMesh().chunkVisible("PylonOutR", false);
            }
        }
        if (this.thisWeaponsName.indexOf("in_100gal") != -1) {
            if (this.hierMesh().chunkFindCheck("Tank100InL") >= 0) {
                this.hierMesh().chunkVisible("Tank100InL", true);
            }
            if (this.hierMesh().chunkFindCheck("Tank100InR") >= 0) {
                this.hierMesh().chunkVisible("Tank100InR", true);
            }
            this.dropTanks[DT_IN_100] = true;
        }
        if (this.thisWeaponsName.indexOf("in_230gal") != -1) {
            if (this.hierMesh().chunkFindCheck("Tank230L") >= 0) {
                this.hierMesh().chunkVisible("Tank230L", true);
            }
            if (this.hierMesh().chunkFindCheck("Tank230R") >= 0) {
                this.hierMesh().chunkVisible("Tank230R", true);
            }
            this.dropTanks[DT_IN_230] = true;
        }
        if (this.thisWeaponsName.indexOf("out_100gal") != -1) {
            if (this.hierMesh().chunkFindCheck("Tank100OutL") >= 0) {
                this.hierMesh().chunkVisible("Tank100OutL", true);
            }
            if (this.hierMesh().chunkFindCheck("Tank100OutR") >= 0) {
                this.hierMesh().chunkVisible("Tank100OutR", true);
            }
            this.dropTanks[DT_OUT_100] = true;
        }
    ...
    public void rareAction(float f, boolean flag) {
        ...
        if (((this != World.getPlayerAircraft()) || !((RealFlightModel) this.FM).isRealMode()) && (this.FM instanceof Pilot)) {
            Pilot pilot = (Pilot) this.FM;
            if (pilot.AP.way.isLanding() && (pilot.getSpeed() > (pilot.VmaxFLAPS * 2F)) && (pilot.CT.AirBrakeControl < 0.01F)) {
                HUD.log("DivebrakeON"); // Deploy divebrake automatically to assist AI landing!
                pilot.CT.AirBrakeControl = 1.0F;
            } else if (this.FM.AP.way.isLanding() && (super.FM.getSpeed() < (pilot.VmaxFLAPS * 1.5F)) && (pilot.CT.AirBrakeControl > 0.99F)) {
                HUD.log("DivebrakeOFF"); // Retract divebrake automatically to assist AI landing!
                pilot.CT.AirBrakeControl = 0.0F;
            }
        }
    }
    ...
    public float getAirPressureFactor(float theAltitude) {
        return this.getAirPressure(theAltitude) / p0;
    }

    public float getAirDensity(float theAltitude) {
        return (this.getAirPressure(theAltitude) * M) / (R * (T0 - (L * theAltitude)));
    }

    public float getAirDensityFactor(float theAltitude) {
        return this.getAirDensity(theAltitude) / Rho0;
    }

    public float getMachForAlt() {
        if (this.lastMachTickCount == Time.tickCounter()) {
            return this.lastMachForAlt;
        }
        this.lastMachTickCount = Time.tickCounter();
        return this.lastMachForAlt = this.getMachForAlt(this.FM.getAltitude());
    }
    ...
    public float getMachForAlt(float theAltValue) {
        ...
        if (i == 0) {
            return fMachAltY[0];
        } else if (i >= fMachAltX.length) {
            i = fMachAltX.length - 1;
        }
    ...
    public float calculateMach() {
        return (this.FM.getSpeedKMH() / this.getMachForAlt());

    }

    public void soundbarier() {

        float f = this.getMachForAlt() - this.FM.getSpeedKMH();
        if (f < 0.5F) {
            f = 0.5F;
        }
        float f_0_ = this.FM.getSpeedKMH() - this.getMachForAlt();
        if (f_0_ < 0.5F) {
            f_0_ = 0.5F;
        }
        if (this.calculateMach() <= 1.0) {
        ...
        if (this.FM.VmaxAllowed > 1500.0F) {
            this.FM.VmaxAllowed = 1500.0F;
        }

        if (this.isSonic && (this.SonicBoom < 1)) {
            ...
            if (Config.isUSE_RENDER() && (World.Rnd().nextFloat() < this.getAirDensityFactor(this.FM.getAltitude()))) {
                for (int i = 0; i < NUM_SHOCKWAVES; i++) {
                    this.shockwave[i] = Eff3DActor.New(this, this.findHook("_Shockwave" + (i + 1)), null, this.shockwavescale[i], "3DO/Effects/Aircraft/Condensation.eff", -1F);
                }
            }
            this.SonicBoom = 1F;
        }
        if ((this.calculateMach() > 1.01) || (this.calculateMach() < 1.0)) {
            for (int i = 0; i < NUM_SHOCKWAVES; i++) {
                Eff3DActor.finish(this.shockwave[i]);
            }
        }
    }
    ...
    public void update(float f) {
        if (((this != World.getPlayerAircraft()) || !((RealFlightModel) this.FM).isRealMode()) && (this.FM instanceof Pilot)) {
            Pilot pilot = (Pilot) this.FM;
            if ((pilot.get_maneuver() == 25) && (pilot.Alt < 60.0F)) {
                if (pilot.Or.getTangage() > 4F) { // Limit nose up attitude to 4 degrees on touchdown to avoid tail strike (only for AI)!
                    pilot.Or.increment(0.0F, -(pilot.Or.getTangage() - 4F), 0.0F);
                }
            }
        }
        this.checkBailout(f);
    ...
    private float            shockwavescale[]  = { 0.1F, 1.0F, 0.3F };
    ...
    private int              lastMachTickCount = 0;
    private float            lastMachForAlt    = 0F;

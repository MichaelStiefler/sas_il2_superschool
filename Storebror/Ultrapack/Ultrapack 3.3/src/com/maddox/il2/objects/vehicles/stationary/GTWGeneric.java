...
public abstract class GTWGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (GTWGeneric.this.engineSFX != null) if (GTWGeneric.this.engineSTimer >= 0) {
                if (--GTWGeneric.this.engineSTimer <= 0) {
                    GTWGeneric.this.engineSTimer = (int) GTWGeneric.SecsToTicks(GTWGeneric.Rnd(10F, 10F));
                    if (!GTWGeneric.this.danger()) GTWGeneric.this.engineSTimer = -GTWGeneric.this.engineSTimer;
                }
            } else if (++GTWGeneric.this.engineSTimer >= 0) {
                GTWGeneric.this.engineSTimer = -(int) GTWGeneric.SecsToTicks(GTWGeneric.Rnd(10F, 10F));
                if (GTWGeneric.this.danger()) GTWGeneric.this.engineSTimer = -GTWGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            GTWGeneric gtwgeneric = null;
            try {
                GTWGeneric.constr_arg2 = actorspawnarg;
                gtwgeneric = (GTWGeneric) this.cls.newInstance();
                GTWGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                GTWGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create GTWGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return gtwgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", GTWGeneric.mesh_name);
            this.cls = class1;
    ...
    protected GTWGeneric() {
        this(constr_arg2);
    }

    private GTWGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) GTWGeneric.SecsToTicks(GTWGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D) {
                this.pos.getAbs(point3d);
                ...
                if (k1 <= 4000D && k1 >= 300D && i2 >= 255 && i2 <= 285 && Math.sqrt(j1 * j1) >= 120D) HUD.logCenter("                                                                   (" + j2 + s + ")");
            }
        }

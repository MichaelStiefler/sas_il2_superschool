...
public abstract class AImkIVGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (AImkIVGeneric.this.engineSFX != null) if (AImkIVGeneric.this.engineSTimer >= 0) {
                if (--AImkIVGeneric.this.engineSTimer <= 0) {
                    AImkIVGeneric.this.engineSTimer = (int) AImkIVGeneric.SecsToTicks(AImkIVGeneric.Rnd(5F, 5F));
                    if (!AImkIVGeneric.this.danger()) AImkIVGeneric.this.engineSTimer = -AImkIVGeneric.this.engineSTimer;
                }
            } else if (++AImkIVGeneric.this.engineSTimer >= 0) {
                AImkIVGeneric.this.engineSTimer = -(int) AImkIVGeneric.SecsToTicks(AImkIVGeneric.Rnd(5F, 5F));
                if (AImkIVGeneric.this.danger()) AImkIVGeneric.this.engineSTimer = -AImkIVGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            AImkIVGeneric aimkivgeneric = null;
            try {
                AImkIVGeneric.constr_arg2 = actorspawnarg;
                aimkivgeneric = (AImkIVGeneric) this.cls.newInstance();
                AImkIVGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                AImkIVGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create AImkIVGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return aimkivgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", AImkIVGeneric.mesh_name);
            this.cls = class1;
    ...
    protected AImkIVGeneric() {
        this(constr_arg2);
    }

    private AImkIVGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) AImkIVGeneric.SecsToTicks(AImkIVGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        do {
            ...
            for (int l = 0; l < k; l++) {
                ...
                if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getArmy() != this.myArmy && actor.getSpeed(vector3d) > 20D) {
                    ...
                    if (l1 < 0) l1 = 360 + l1;
                    int i2 = l1 - j;
                    if (k1 <= f && k1 >= 250D && i2 >= 235 && i2 <= 305 && Math.sqrt(j1 * j1) <= 35D) {

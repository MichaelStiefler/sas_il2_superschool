...
public abstract class ASVmkIIIGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (ASVmkIIIGeneric.this.engineSFX != null) if (ASVmkIIIGeneric.this.engineSTimer >= 0) {
                if (--ASVmkIIIGeneric.this.engineSTimer <= 0) {
                    ASVmkIIIGeneric.this.engineSTimer = (int) ASVmkIIIGeneric.SecsToTicks(ASVmkIIIGeneric.Rnd(5F, 5F));
                    if (!ASVmkIIIGeneric.this.danger()) ASVmkIIIGeneric.this.engineSTimer = -ASVmkIIIGeneric.this.engineSTimer;
                }
            } else if (++ASVmkIIIGeneric.this.engineSTimer >= 0) {
                ASVmkIIIGeneric.this.engineSTimer = -(int) ASVmkIIIGeneric.SecsToTicks(ASVmkIIIGeneric.Rnd(5F, 5F));
                if (ASVmkIIIGeneric.this.danger()) ASVmkIIIGeneric.this.engineSTimer = -ASVmkIIIGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            ASVmkIIIGeneric asvmkiiigeneric = null;
            try {
                ASVmkIIIGeneric.constr_arg2 = actorspawnarg;
                asvmkiiigeneric = (ASVmkIIIGeneric) this.cls.newInstance();
                ASVmkIIIGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                ASVmkIIIGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create ASVmkIIIGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return asvmkiiigeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", ASVmkIIIGeneric.mesh_name);
            this.cls = class1;
    ...
    protected ASVmkIIIGeneric() {
        this(constr_arg2);
    }

    private ASVmkIIIGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) ASVmkIIIGeneric.SecsToTicks(ASVmkIIIGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if ((actor instanceof BigshipGeneric || actor instanceof ShipGeneric) && actor.getArmy() != this.myArmy && actor != World.getPlayerAircraft()) {
                ...
                byte byte0 = 9;
                if (actor instanceof ShipGeneric) byte0 = 50;
                if (actor instanceof BigshipGeneric) byte0 = 80;
                if (actor instanceof Ship.SubTypeVIIC_Sub || actor instanceof Ship.USSGatoSS212_Sub || actor instanceof Ship.USSGreenlingSS213_Sub) byte0 = 15;

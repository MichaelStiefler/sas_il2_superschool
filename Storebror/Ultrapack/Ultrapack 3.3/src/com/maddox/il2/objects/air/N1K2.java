...
public abstract class N1K2 extends Scheme1 implements TypeFighter {
    ...
    public void update(float f) {
        ...
        if (this.FM.CT.FlapsControl <= 0.29F && this.FM.CT.FlapsControl >= 0.05F) {
            ...
            if (0.6334F < this.kuusen_flap) this.kuusen_flap = 0.6334F;
            else if (0.0F > this.kuusen_flap) this.kuusen_flap = 0.0F;
            if (this.FM.CT.getFlap() != this.kuusen_flap) if (this.kuusen_flap == 0.0F && this.FM.CT.getFlap() <= 0.05D) {
                this.sfxFlaps(false);
                this.FM.CT.forceFlaps(0.0F);
            } else this.FM.CT.forceFlaps(0.05F * this.kuusen_flap + 0.95F * this.FM.CT.getFlap());
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        if (s.startsWith("xx")) {
            if (s.startsWith("xxarmor")) {
                ...
                } else if (s.endsWith("p2")) this.getEnergyPastArmor(13.13D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);

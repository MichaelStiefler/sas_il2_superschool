...
public abstract class WindowGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (WindowGeneric.this.engineSFX != null) if (WindowGeneric.this.engineSTimer >= 0) {
                if (--WindowGeneric.this.engineSTimer <= 0) {
                    WindowGeneric.this.engineSTimer = (int) WindowGeneric.SecsToTicks(WindowGeneric.Rnd(2.0F, 2.0F));
                    if (!WindowGeneric.this.danger()) WindowGeneric.this.engineSTimer = -WindowGeneric.this.engineSTimer;
                }
            } else if (++WindowGeneric.this.engineSTimer >= 0) {
                WindowGeneric.this.engineSTimer = -(int) WindowGeneric.SecsToTicks(WindowGeneric.Rnd(2.0F, 2.0F));
                if (WindowGeneric.this.danger()) WindowGeneric.this.engineSTimer = -WindowGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            WindowGeneric windowgeneric = null;
            try {
                WindowGeneric.constr_arg2 = actorspawnarg;
                windowgeneric = (WindowGeneric) this.cls.newInstance();
                WindowGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                WindowGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create WindowGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", WindowGeneric.mesh_name);
            this.cls = class1;
    ...
    private WindowGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) WindowGeneric.SecsToTicks(WindowGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof TypeBomber && actor.getArmy() == this.myArmy && actor.getSpeed(vector3d) > 20D) {
                ...
                if (i < 0) i = 360 + i;
                Aircraft aircraft = War.GetNearestEnemyAircraft(actor, 1000F, 9);
                if (aircraft instanceof TypeFighter && aircraft.getSpeed(vector3d) > 20D) {
                    if (aircraft == World.getPlayerAircraft()) flag = true;
                    ...
                    if (i1 <= 1000D && Math.sqrt(d6 * d6) <= 500D && flag2 && flag3 && !flag) ((Pilot) aircraft.FM).set_maneuver(14);
                    if (i1 <= 1000D && Math.sqrt(d6 * d6) <= 500D && flag2 && flag4 && flag) HUD.logCenter("                                          RO: We're being jammed!");
                }
            }
        }

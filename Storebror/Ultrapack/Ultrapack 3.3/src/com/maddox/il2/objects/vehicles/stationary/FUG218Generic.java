...
public abstract class FUG218Generic extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (FUG218Generic.this.engineSFX != null) if (FUG218Generic.this.engineSTimer >= 0) {
                if (--FUG218Generic.this.engineSTimer <= 0) {
                    FUG218Generic.this.engineSTimer = (int) FUG218Generic.SecsToTicks(FUG218Generic.Rnd(5F, 5F));
                    if (!FUG218Generic.this.danger()) FUG218Generic.this.engineSTimer = -FUG218Generic.this.engineSTimer;
                }
            } else if (++FUG218Generic.this.engineSTimer >= 0) {
                FUG218Generic.this.engineSTimer = -(int) FUG218Generic.SecsToTicks(FUG218Generic.Rnd(5F, 5F));
                if (FUG218Generic.this.danger()) FUG218Generic.this.engineSTimer = -FUG218Generic.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            FUG218Generic fug218generic = null;
            try {
                FUG218Generic.constr_arg2 = actorspawnarg;
                fug218generic = (FUG218Generic) this.cls.newInstance();
                FUG218Generic.constr_arg2 = null;
            } catch (Exception exception) {
                FUG218Generic.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create FUG218Generic object [class:" + this.cls.getName() + "]");
                return null;
            }
            return fug218generic;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", FUG218Generic.mesh_name);
            this.cls = class1;
    ...
    protected FUG218Generic() {
        this(constr_arg2);
    }

    private FUG218Generic(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) FUG218Generic.SecsToTicks(FUG218Generic.Rnd(5F, 5F));
        }
    }
        ...
        do {
            ...
            for (int l = 0; l < k; l++) {
                ...
                if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getArmy() != this.myArmy) {
                    ...
                    if (l1 < 0) l1 = 360 + l1;
                    int i2 = l1 - j;
                    if (k1 <= f && k1 >= 120D && i2 >= 235 && i2 <= 305 && Math.sqrt(j1 * j1) <= 60D && actor.getSpeed(vector3d) > 20D) {

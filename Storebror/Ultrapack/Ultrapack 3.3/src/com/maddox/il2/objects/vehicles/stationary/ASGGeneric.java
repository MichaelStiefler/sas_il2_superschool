...
public abstract class ASGGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (ASGGeneric.this.engineSFX != null) if (ASGGeneric.this.engineSTimer >= 0) {
                if (--ASGGeneric.this.engineSTimer <= 0) {
                    ASGGeneric.this.engineSTimer = (int) ASGGeneric.SecsToTicks(ASGGeneric.Rnd(5F, 5F));
                    if (!ASGGeneric.this.danger()) ASGGeneric.this.engineSTimer = -ASGGeneric.this.engineSTimer;
                }
            } else if (++ASGGeneric.this.engineSTimer >= 0) {
                ASGGeneric.this.engineSTimer = -(int) ASGGeneric.SecsToTicks(ASGGeneric.Rnd(5F, 5F));
                if (ASGGeneric.this.danger()) ASGGeneric.this.engineSTimer = -ASGGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            ASGGeneric asggeneric = null;
            try {
                ASGGeneric.constr_arg2 = actorspawnarg;
                asggeneric = (ASGGeneric) this.cls.newInstance();
                ASGGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                ASGGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create ASGGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return asggeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", ASGGeneric.mesh_name);
            this.cls = class1;
    ...
    protected ASGGeneric() {
        this(constr_arg2);
    }

    private ASGGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) ASGGeneric.SecsToTicks(ASGGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if ((actor instanceof BigshipGeneric || actor instanceof ShipGeneric) && actor.getArmy() != this.myArmy && actor != World.getPlayerAircraft()) {
                ...
                if (actor instanceof BigshipGeneric) byte0 = 64;
                if (actor instanceof Ship.SubTypeVIIC_Sub || actor instanceof Ship.USSGatoSS212_Sub || actor instanceof Ship.USSGreenlingSS213_Sub) byte0 = 10;
                if (actor instanceof Ship.SubTypeVIIC_Srf || actor instanceof Ship.USSGatoSS212_Srf || actor instanceof Ship.USSGreenlingSS213_Srf) byte0 = 29;

...
public abstract class P_47 extends Scheme1 implements TypeFighter, TypeBNZFighter, TypeStormovik {
    ...
    public void moveWheelSink() {
        ...
        this.hierMesh().chunkSetLocate("GearL25_D0", xyz, ypr);
        xyz[1] = cvt(this.FM.Gears.gWheelSinking[0], 0.0F, 0.2F, 0.0F, 0.2F);
        this.hierMesh().chunkSetLocate("GearR25_D0", xyz, ypr);
    }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        if (s.startsWith("xx")) {
            if (s.startsWith("xxarmor")) {
                ...
                if (s.endsWith("p1")) {
                    this.getEnergyPastArmor(World.Rnd().nextFloat(20F, 40F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                    this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 2);
                    if (shot.power <= 0.0F) this.doRicochetBack(shot);
                } else if (s.endsWith("p2")) this.getEnergyPastArmor(12.52D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else if (s.endsWith("p3")) this.getEnergyPastArmor(12.56D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else if (s.endsWith("p4")) this.getEnergyPastArmor(12.56D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else if (s.endsWith("f1")) {
                    this.getEnergyPastArmor(2.1D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                    if (shot.powerType == 3) shot.powerType = 2;
                }
                return;
            }
        ...
        if (s.startsWith("xcf") || s.startsWith("xcockpit")) {
            ...
            if (s.startsWith("xcockpit")) {
                if (point3d.z > 0.716D && World.Rnd().nextFloat() < 0.25F) this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 1);
                if (point3d.z > 0.716D && World.Rnd().nextFloat() < 0.25F) this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 8);
    ...
    public void update(float f) {
        float f1 = cvt(this.FM.EI.engines[0].getControlRadiator(), 0.0F, 1.0F, 0.0F, -5F);
        for (int i = 1; i < 13; i++)
            this.hierMesh().chunkSetAngles("Cowflap" + i + "_D0", 0.0F, f1, 0.0F);

        f1 = cvt(this.FM.EI.engines[0].getControlRadiator(), 0.0F, 1.0F, 25F, 0.0F);
        this.hierMesh().chunkSetAngles("Oilrflap2l_D0", 0.0F, f1, 0.0F);
        this.hierMesh().chunkSetAngles("Oilrflap2r_D0", 0.0F, -f1, 0.0F);
        super.update(f);
    }
    ...
    public void onAircraftLoaded() {
        ...
        for (int i = 0; i < aobj.length; i++)
            if (aobj[i] instanceof FuelTank) {
                this.hierMesh().chunkVisible("ETank_D0", true);
                this.hierMesh().chunkVisible("Rack_D0", true);
            }

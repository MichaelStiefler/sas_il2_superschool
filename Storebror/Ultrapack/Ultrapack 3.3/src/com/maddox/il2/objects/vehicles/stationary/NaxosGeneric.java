...
public abstract class NaxosGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (NaxosGeneric.this.engineSFX != null) if (NaxosGeneric.this.engineSTimer >= 0) {
                if (--NaxosGeneric.this.engineSTimer <= 0) {
                    NaxosGeneric.this.engineSTimer = (int) NaxosGeneric.SecsToTicks(NaxosGeneric.Rnd(15F, 15F));
                    if (!NaxosGeneric.this.danger()) NaxosGeneric.this.engineSTimer = -NaxosGeneric.this.engineSTimer;
                }
            } else if (++NaxosGeneric.this.engineSTimer >= 0) {
                NaxosGeneric.this.engineSTimer = -(int) NaxosGeneric.SecsToTicks(NaxosGeneric.Rnd(15F, 15F));
                if (NaxosGeneric.this.danger()) NaxosGeneric.this.engineSTimer = -NaxosGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            NaxosGeneric naxosgeneric = null;
            try {
                NaxosGeneric.constr_arg2 = actorspawnarg;
                naxosgeneric = (NaxosGeneric) this.cls.newInstance();
                NaxosGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                NaxosGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create NaxosGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return naxosgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", NaxosGeneric.mesh_name);
            this.cls = class1;
    ...
    protected NaxosGeneric() {
        this(constr_arg2);
    }

    private NaxosGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) NaxosGeneric.SecsToTicks(NaxosGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof TypeBomber && actor.getArmy() != this.myArmy && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D) {
                this.pos.getAbs(point3d);
                ...
                if (l1 < 0) l1 = 360 + l1;
                if (k1 <= 50000D && k1 >= 100D && d6 < 0.0D) HUD.logCenter("                                          Naxos: Target bearing " + i1 + "\260");
            }
        }

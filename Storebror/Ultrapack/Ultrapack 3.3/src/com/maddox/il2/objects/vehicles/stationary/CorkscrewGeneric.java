...
public abstract class CorkscrewGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (CorkscrewGeneric.this.engineSFX != null) if (CorkscrewGeneric.this.engineSTimer >= 0) {
                if (--CorkscrewGeneric.this.engineSTimer <= 0) {
                    CorkscrewGeneric.this.engineSTimer = (int) CorkscrewGeneric.SecsToTicks(CorkscrewGeneric.Rnd(5F, 5F));
                    if (!CorkscrewGeneric.this.danger()) CorkscrewGeneric.this.engineSTimer = -CorkscrewGeneric.this.engineSTimer;
                }
            } else if (++CorkscrewGeneric.this.engineSTimer >= 0) {
                CorkscrewGeneric.this.engineSTimer = -(int) CorkscrewGeneric.SecsToTicks(CorkscrewGeneric.Rnd(5F, 5F));
                if (CorkscrewGeneric.this.danger()) CorkscrewGeneric.this.engineSTimer = -CorkscrewGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            CorkscrewGeneric corkscrewgeneric = null;
            try {
                CorkscrewGeneric.constr_arg2 = actorspawnarg;
                corkscrewgeneric = (CorkscrewGeneric) this.cls.newInstance();
                CorkscrewGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                CorkscrewGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create CorkscrewGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return corkscrewgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", CorkscrewGeneric.mesh_name);
            this.cls = class1;
    ...
    protected CorkscrewGeneric() {
        this(constr_arg2);
    }

    private CorkscrewGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) CorkscrewGeneric.SecsToTicks(CorkscrewGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof TypeBomber && actor.getArmy() == this.myArmy && actor.getSpeed(vector3d) > 20D) {
                if (actor == World.getPlayerAircraft()) flag = true;
                this.pos.getAbs(point3d);
                ...
                if (aircraft instanceof TypeFighter && aircraft.getSpeed(vector3d) > 20D) {
                    ...
                    if (k < 0) k = 360 + k;
                    boolean flag2 = false;
                    if (k >= 90 && k <= 270) flag2 = true;
                    String s = "left";
                    if (k <= 180 && k >= 90) s = "right";
                    double d9 = d - d3;
                    double d10 = d1 - d4;
                    java.util.Random random = new Random();
                    int l = random.nextInt(100);
                    int i1 = (int) Math.ceil(Math.sqrt(d10 * d10 + d9 * d9));
                    if (i1 <= 500D && Math.sqrt(d6 * d6) <= 500D && flag2 && !flag && l <= 10) ((Pilot) ((Aircraft) actor).FM).set_maneuver(54);
                    if (i1 <= 500D && Math.sqrt(d6 * d6) <= 500D && flag2 && flag && l <= 10) HUD.logCenter("                                          Corkscrew " + s + ", now!");
                }
            }
        }

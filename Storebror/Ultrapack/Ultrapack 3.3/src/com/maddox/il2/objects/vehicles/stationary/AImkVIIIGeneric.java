...
public abstract class AImkVIIIGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (AImkVIIIGeneric.this.engineSFX != null) if (AImkVIIIGeneric.this.engineSTimer >= 0) {
                if (--AImkVIIIGeneric.this.engineSTimer <= 0) {
                    AImkVIIIGeneric.this.engineSTimer = (int) AImkVIIIGeneric.SecsToTicks(AImkVIIIGeneric.Rnd(5F, 5F));
                    if (!AImkVIIIGeneric.this.danger()) AImkVIIIGeneric.this.engineSTimer = -AImkVIIIGeneric.this.engineSTimer;
                }
            } else if (++AImkVIIIGeneric.this.engineSTimer >= 0) {
                AImkVIIIGeneric.this.engineSTimer = -(int) AImkVIIIGeneric.SecsToTicks(AImkVIIIGeneric.Rnd(5F, 5F));
                if (AImkVIIIGeneric.this.danger()) AImkVIIIGeneric.this.engineSTimer = -AImkVIIIGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            AImkVIIIGeneric aimkviiigeneric = null;
            try {
                AImkVIIIGeneric.constr_arg2 = actorspawnarg;
                aimkviiigeneric = (AImkVIIIGeneric) this.cls.newInstance();
                AImkVIIIGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                AImkVIIIGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create AImkVIIIGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return aimkviiigeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", AImkVIIIGeneric.mesh_name);
            this.cls = class1;
    ...
    protected AImkVIIIGeneric() {
        this(constr_arg2);
    }

    private AImkVIIIGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) AImkVIIIGeneric.SecsToTicks(AImkVIIIGeneric.Rnd(5F, 5F));
        }
    }
        ...
        do {
            ...
            for (int l = 0; l < k; l++) {
                ...
                if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getArmy() != this.myArmy) {
                    ...
                    if (l1 < 0) l1 = 360 + l1;
                    int i2 = l1 - j;
                    if (k1 <= f && k1 >= 250D && i2 >= 240 && i2 <= 300 && Math.sqrt(j1 * j1) <= 30D && actor.getSpeed(vector3d) > 20D) {

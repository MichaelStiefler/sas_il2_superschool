...
public class Front {
    ...
    public static void checkAircraftCaptured(Aircraft aircraft) {
        ...
        // -----------------------------------------------
        if (aircraft == null || isUsingRenderer && Main3D.cur3D().cockpitCur == null) return;
        // -----------------------------------------------

        if (Actor.isAlive(aircraft)) if (!aircraft.FM.isOk() && isCaptured(aircraft)) for (int i = 0; i < aircraft.FM.crew; i++)
    ...
    public static boolean isCaptured(int i, double d, double d_1_) {
        ...
        if (i_2_ == 0) return false;
        if (i == i_2_) return false;
        List list = markers();
        int i_3_ = list.size();
        Marker marker = null;
        double d_4_ = 9.0E10;
        ...
        for (int i_16_ = 0; i_16_ < i_3_; i_16_++) {
            ...
            if (!(actor instanceof Aircraft) && i != actor.getArmy()) {
                ...
                if (d_17_ < 1000000.0) return true;
            }
        }
        double d_18_ = Math.sqrt(d_4_) / 50000.0;
        double d_19_ = d + d_1_;
        if (d_19_ < 0.0) d_19_ = -d_19_;
        d_19_ -= (int) d_19_;
    ...
    public int _army(double d, double d_21_) {
        ...
        for (int i_24_ = 0; i_24_ < i; i_24_++) {
            Marker marker = (Marker) this.allMarkers.get(i_24_);
    ...
    public void resetGameClear() {
        this.zutiFrontLineAlreadyLoaded = false;
    ...
    public void _loadMission(SectFile sectfile) {
        // TODO: Added by |ZUTI|
        // --------------------------------
        if (this.zutiFrontLineAlreadyLoaded) return;
        // --------------------------------
        ...
        // ----------------------------------------
        this.zutiFrontLineAlreadyLoaded = true;
        // ----------------------------------------
    }
    ...
    private void tilesUpdate() {
        ...
        // TODO: Added by |ZUTI|
        // --------------------------------
        if (cameraortho2d == null) return;
        // --------------------------------

        int i = (int) (cameraortho2d.right - cameraortho2d.left) / 16 + 5;
        int i_33_ = (int) (cameraortho2d.top - cameraortho2d.bottom) / 16 + 5;
        if (i != this.tNx || i_33_ != this.tNy) {
            this.tNx = i;
            this.tNy = i_33_;
            this.mask = new byte[i * i_33_];
            this.mask2 = new byte[i * i_33_];
        }
        int i_34_ = this.allMarkers.size();
        if (i_34_ < 2) this.bExistFront = false;
        else {
            int i_35_ = 0;
            for (int i_36_ = 0; i_36_ < i_34_; i_36_++) {
                Marker marker = (Marker) this.allMarkers.get(i_36_);
                i_35_ |= marker._armyMask;
            }
            if (this.isOneArmy(i_35_)) this.bExistFront = false;
            else {
                double d = 16.0 / cameraortho2d.worldScale;
                long l = (long) (cameraortho2d.worldXOffset / d) - 1L;
                this.camWorldXOffset = l * d;
                this.camLeft = (float) (cameraortho2d.left - (cameraortho2d.worldXOffset - this.camWorldXOffset) * cameraortho2d.worldScale);
                l = (long) (cameraortho2d.worldYOffset / d) - 1L;
                this.camWorldYOffset = l * d;
                this.camBottom = (float) (cameraortho2d.bottom - (cameraortho2d.worldYOffset - this.camWorldYOffset) * cameraortho2d.worldScale);
                double d_37_ = cameraortho2d.worldYOffset + (cameraortho2d.right - cameraortho2d.left) / cameraortho2d.worldScale * 0.5;
                double d_38_ = cameraortho2d.worldXOffset + (cameraortho2d.top - cameraortho2d.bottom) / cameraortho2d.worldScale * 0.5;
                double d_39_ = 9.0E10;
                for (int i_40_ = 0; i_40_ < i_34_; i_40_++) {
                    Marker marker = (Marker) this.allMarkers.get(i_40_);
                ...
                d_39_ += (cameraortho2d.right - cameraortho2d.left + (cameraortho2d.top - cameraortho2d.bottom)) / cameraortho2d.worldScale;
                d_39_ *= d_39_;
                i_35_ = 0;
                for (int i_43_ = 0; i_43_ < i_34_; i_43_++) {
                    Marker marker = (Marker) this.allMarkers.get(i_43_);
                    if (d_39_ > marker._d2) {
                        this.markers.add(marker);
                        i_35_ |= marker._armyMask;
                    }
                }
                if (this.markers.size() < 2 || this.isOneArmy(i_35_)) {
                    this.markers.clear();
                    this.bExistFront = false;
                } else {
                    i_34_ = this.markers.size();
                    double d_44_ = this.camWorldYOffset - 0.5 * d;
                    double d_45_ = this.camWorldXOffset - 0.5 * d;
                    int i_46_ = 0;
                    int i_47_ = 0;
                    i_35_ = 0;
                    for (int i_48_ = 0; i_48_ < i_33_; i_48_++) {
                        double d_49_ = d_44_ + i_48_ * d;
                        for (int i_50_ = 0; i_50_ < i; i_50_++) {
                            double d_51_ = d_45_ + i_50_ * d;
                            double d_52_ = d_39_;
                            for (int i_53_ = 0; i_53_ < i_34_; i_53_++) {
                                Marker marker = (Marker) this.markers.get(i_53_);
                            ...
                            this.mask[i * i_48_ + i_50_] = (byte) i_46_;
                            i_35_ |= i_47_;
                        }
                    }
                    this.markers.clear();
                    if (this.isOneArmy(i_35_)) this.bExistFront = false;
                        ...
                        for (int i_55_ = 1; i_55_ < i_33_ - 1; i_55_++)
                            for (int i_56_ = 1; i_56_ < i - 1; i_56_++) {
                                ...
                                byte i_58_ = this.mask[i_57_];
                                if (i_58_ == this.mask[i_57_ - 1] && i_58_ == this.mask[i_57_ + 1] && i_58_ == this.mask[i_57_ - i] && i_58_ == this.mask[i_57_ + i]) this.mask2[i_57_] = (byte) 0;
                                else this.mask2[i_57_] = i_58_;
                            }
                        for (int i_59_ = 1; i_59_ < i_33_ - 1; i_59_++)
                            for (int i_60_ = 1; i_60_ < i - 1; i_60_++) {
                                int i_61_ = i_59_ * i + i_60_;
                                byte i_62_ = this.mask2[i_61_];
                                if (i_62_ == 0) this.mask[i_61_] = (byte) 0;
                                else {
                                    int i_63_ = 0;
                                    if (this.mask2[i_61_ + 1] == i_62_) {
                                        if (this.mask2[i_61_ + i] != this.mask2[i_61_ - i + 1] && this.mask2[i_61_ - i] != this.mask2[i_61_ + i + 1]) i_63_ |= 0x1;
                                    } else if (this.mask2[i_61_ + i + 1] == i_62_ && this.mask2[i_61_ + i] != i_62_ && this.mask2[i_61_ + i] != this.mask2[i_61_ + 1]) i_63_ |= 0x2;
                                    if (this.mask2[i_61_ + i] == i_62_) {
                                        if (this.mask2[i_61_ - 1] != this.mask2[i_61_ + i + 1] && this.mask2[i_61_ + i - 1] != this.mask2[i_61_ + 1]) i_63_ |= 0x4;
                                    } else if (this.mask2[i_61_ + i - 1] == i_62_ && this.mask2[i_61_ - 1] != i_62_ && this.mask2[i_61_ - 1] != this.mask2[i_61_ + i]) i_63_ |= 0x8;
                                    this.mask[i_61_] = (byte) i_63_;
                                }
                            }
    ...
    public void _preRender(boolean bool) {
        ...
        if (!this.tilesChanged) this.tilesChanged = this.prevNCountMarkers != this.allMarkers.size() || this.prevCamLeft != cameraortho2d.left || this.prevCamBottom != cameraortho2d.bottom || this.prevCamWorldScale != cameraortho2d.worldScale
                || this.prevCamWorldXOffset != cameraortho2d.worldXOffset || this.prevCamWorldYOffset != cameraortho2d.worldYOffset;
        if (!this.tilesChanged && bool) for (int i = 0; i < this.prevNCountMarkers; i++) {
            Marker marker = (Marker) this.allMarkers.get(i);
        ...
        if (this.tilesChanged) {
            ...
            this.prevCamLeft = cameraortho2d.left;
            this.prevCamBottom = cameraortho2d.bottom;
            this.prevCamWorldScale = cameraortho2d.worldScale;
            this.prevCamWorldXOffset = cameraortho2d.worldXOffset;
            this.prevCamWorldYOffset = cameraortho2d.worldYOffset;
    ...
    private void drawTile3D(float f, float f_64_, Mat mat, int i) {
        double d = this.camWorldXOffset + f * this.tStep;
        double d_65_ = this.camWorldYOffset + f_64_ * this.tStep;
        double d_66_ = Landscape.HQ((float) d, (float) d_65_);
        this.main.project2d(d, d_65_, d_66_, this.pointLand0);
        d += 32.0 * this.tStep;
        d_65_ += 32.0 * this.tStep;
        d_66_ = Landscape.HQ((float) d, (float) d_65_);
        this.main.project2d(d, d_65_, d_66_, this.pointLand1);
        Render.drawTile((float) this.pointLand0.x, (float) this.pointLand0.y, (float) (this.pointLand1.x - this.pointLand0.x), (float) (this.pointLand1.y - this.pointLand0.y), 0.0F, mat, i, 0.0F, 0.0F, 1.0F, 1.0F);
    }
    ...
    public void _render(boolean bool) {
        if (this.bExistFront) {
            ...
            if (bool) {
                ...
                this.tStep = 1.0 / this.prevCamWorldScale;
                for (int i_68_ = 1; i_68_ < i_67_ - 1; i_68_++) {
                    int i_69_ = 16 * (i_68_ - 1);
                    for (int i_70_ = 1; i_70_ < i - 1; i_70_++) {
                        int i_71_ = i_68_ * i + i_70_;
                        byte i_72_ = this.mask[i_71_];
                        if (i_72_ != 0) {
                            int i_73_ = Army.color(this.mask2[i_71_]) | ~0xffffff;
                            int i_74_ = 16 * (i_70_ - 1);
                            if ((i_72_ & 0x1) != 0) this.drawTile3D(i_74_, i_69_, this.frontMat[0], i_73_);
                            else if ((i_72_ & 0x2) != 0) this.drawTile3D(i_74_, i_69_, this.frontMat[1], i_73_);
                            if ((i_72_ & 0x4) != 0) this.drawTile3D(i_74_, i_69_, this.frontMat[2], i_73_);
                            else if ((i_72_ & 0x8) != 0) this.drawTile3D(i_74_ - 16, i_69_, this.frontMat[3], i_73_);
                        }
                    }
                }
            } else for (int i_75_ = 1; i_75_ < i_67_ - 1; i_75_++) {
                int i_76_ = 16 * (i_75_ - 1);
                for (int i_77_ = 1; i_77_ < i - 1; i_77_++) {
                    int i_78_ = i_75_ * i + i_77_;
                    byte i_79_ = this.mask[i_78_];
                    if (i_79_ != 0) {
                        int i_80_ = Army.color(this.mask2[i_78_]) | ~0xffffff;
                        int i_81_ = 16 * (i_77_ - 1);
                        if ((i_79_ & 0x1) != 0) Render.drawTile(this.camLeft + i_81_, this.camBottom + i_76_, 32.0F, 32.0F, 0.0F, this.frontMat[0], i_80_, 0.0F, 0.0F, 1.0F, 1.0F);
                        else if ((i_79_ & 0x2) != 0) Render.drawTile(this.camLeft + i_81_, this.camBottom + i_76_, 32.0F, 32.0F, 0.0F, this.frontMat[1], i_80_, 0.0F, 0.0F, 1.0F, 1.0F);
                        if ((i_79_ & 0x4) != 0) Render.drawTile(this.camLeft + i_81_, this.camBottom + i_76_, 32.0F, 32.0F, 0.0F, this.frontMat[2], i_80_, 0.0F, 0.0F, 1.0F, 1.0F);
                        else if ((i_79_ & 0x8) != 0) Render.drawTile(this.camLeft + i_81_ - 16.0F, this.camBottom + i_76_, 32.0F, 32.0F, 0.0F, this.frontMat[3], i_80_, 0.0F, 0.0F, 1.0F, 1.0F);
                    }
                }
            }
        }

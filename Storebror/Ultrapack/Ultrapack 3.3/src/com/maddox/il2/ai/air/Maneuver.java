...
// private static final long HOSTCHECK_INTERVAL = 1000L;
    ...
    public void checkCheater() {
        ...
        if (this.originalSkill == -1) {
            ...
            System.out.println(this.actor.name() + " (" + this.actor.getClass().getName() + ") Init Skill = " + this.skillToString(this.Skill));
        }
        ...
        System.out.println(this.actor.name() + " (" + this.actor.getClass().getName() + ") is "
                + (fightAgainstCheater?"":"not ") + "fighting Cheater " + cheaterNames + ", Skill changed from "
                + this.skillToString(oldSkill) + " to " + this.skillToString(this.Skill) + ".");
    }
...
//        if (this.maneuver != i) {
//            System.out.println("" + this.hashCode() + " set_maneuver(" + i + ")");
//          Exception test = new Exception("" + this.hashCode() + " TEST1");
...
//        }
        ...
        // System.out.println("" + this.hashCode() + " set_maneuver(" + i + ") skipped, visibility insufficient, staying on " + this.maneuver);
//            Exception test = new Exception("" + this.hashCode() + " TEST2");
//            test.printStackTrace();
            return;
...
// ZutiSupportMethods_Net.addAircraftToBornPlace(zutiAirdromeBornPlace, currentAcName, true, false);
            ...
            case 58:
                ...
                } else this.CT.ElevatorControl = -0.04F * (this.Or.getTangage() + 5F);
            ...
            case 84:
                ...
                if (!flag) {
                    ...
                    if (this.Vwld.z < 0.0D || this.CT.ElevatorControl <= 0.05F || this.mn_time > 0.75F) {
                        if (this.mn_time > 3F) this.CT.AileronControl = -0.04F * this.Or.getKren();
            ...
            case 67:
                ...
                if (this.danger != null) {
                    ...
                    if (f3 > -90.0F && f3 < 90.0F) this.CT.ElevatorControl = -0.04F * (this.Or.getTangage() + 5.0F);
                    else this.CT.ElevatorControl = 0.05F * (this.Or.getTangage() + 5.0F);
...
//                                        bool = false;
                ...
                if (f151 > 120.0F * f28 || this.getSpeed() > this.Vmin * 1.8F * f28 || f151 > 80.0F * f28 && this.getSpeed() > this.Vmin * 1.6F * f28
                    ...
                    this.pop();
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.Group.numInGroup((Aircraft) this.actor) == 0 && this.Group.grTask == 7) this.Group.setGroupTask(1);
            ...
            case 33:
                ...
                switch (this.submaneuver) {
                    case SUB_MAN0:
                        if (Math.abs(this.Or.getKren()) < 45.0F) this.CT.ElevatorControl = 0.005555556F * Math.abs(this.Or.getKren());
...
//                if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null && this.CT.Weapons[3][0].countBullets() != 0 && !(this.CT.Weapons[3][0] instanceof BombGunPara))
        ...
        if (Math.abs(Ve.y) > 0.1D) this.CT.AileronControl = -(float) Math.atan2(Ve.y, Ve.z) - 0.016F * this.Or.getKren();
        else this.CT.AileronControl = -(float) Math.atan2(Ve.y, Ve.x) - 0.016F * this.Or.getKren();
        if (Math.abs(Ve.y) > 0.001D) this.CT.RudderControl = -98F * (float) Math.atan2(Ve.y, Ve.x);
        else this.CT.RudderControl = 0.0F;
...
//                if (f3 < 100F && this.CT.Weapons[2] != null && this.CT.Weapons[2][0] != null && this.CT.Weapons[2][this.CT.Weapons[2].length - 1].countBullets() != 0 && this.rocketsDelay < 1) {
    ...
    private void turnToDirection(float f) {
        if (Math.abs(Ve.y) > 0.1) this.CT.AileronControl = -(float) Math.atan2(Ve.y, Ve.z) - 0.016F * this.Or.getKren();
        else this.CT.AileronControl = -(float) Math.atan2(Ve.y, Ve.x) - 0.016F * this.Or.getKren();
    ...
    private void turnBaby(float f, float f1, float f2) {
        ...
        if (Math.abs(this.CT.ElevatorControl - f3) < f2 * f1) this.CT.ElevatorControl = f3;
    ...
    private void attackTurnToDirection(float f, float f1, float f2) {
        ...
        if (Ve.x > 0.95) {
            ...
            if (f4 < 0.0F) f4 = 0.0F;
            if (Math.abs(this.CT.ElevatorControl - f4) < f2 * f1) this.CT.ElevatorControl = f4;
...
// TODO: --- TD AI code backport from 4.13 ---
    ...
    private void emergencyLanding(float f) {
        if (this.first) {
            ...
            this.dist = this.Alt;
            if (this.actor instanceof TypeSailPlane) this.EI.setEngineStops();
        }
        this.setSpeedMode(4);
        ...
        if (this.Alt < 40.0F) {
            ...
            if (this.submaneuver == SUB_MAN69) {
                ...
                } else this.CT.AileronControl = -0.04F * this.Or.getKren();
            } else this.CT.AileronControl = -0.04F * this.Or.getKren();

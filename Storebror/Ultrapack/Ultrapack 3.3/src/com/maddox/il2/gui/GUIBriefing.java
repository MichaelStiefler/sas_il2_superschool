...
public class GUIBriefing extends GUIBriefingGeneric {
    ...
    private void fillBeacons() {
        ...
        int i = -1;
        if (com.maddox.il2.game.Mission.isDogfight()) i = ((com.maddox.il2.net.NetUser) com.maddox.rts.NetEnv.host()).getArmy();
        else i = sectfile.get("MAIN", "army", 0);
        this.beacons.clear();
        ...
        for (int l = 0; l < k; l++) {
            ...
            BeaconPoint beaconpoint = this.loadbeacon(i, numbertokenizer.next(""), numbertokenizer.next(""), numbertokenizer.next(0), numbertokenizer.next(0.0D), numbertokenizer.next(0.0D));
    ...
    private BeaconPoint loadbeacon(int i, java.lang.String s, java.lang.String s1, int j, double d, double d1) {
        ...
        if (com.maddox.il2.objects.vehicles.radios.TypeHasBeacon.class.isAssignableFrom(class1)) {
            ...
            beaconpoint.army = j;
            if (RadioBeacon.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/beacon.mat");
            else if (RadioBeaconLowVis.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/beacon.mat");
            else if (YGBeacon.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/beaconYG.mat");
            else if (LorenzBLBeacon.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/ILS.mat");
            else if (LorenzBLBeacon_LongRunway.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/ILS.mat");
            else if (LorenzBLBeacon_AAIAS.class.isAssignableFrom(class1)) beaconpoint.icon = com.maddox.il2.engine.IconDraw.get("icons/ILS.mat");
            else return null;
            java.lang.String s2 = com.maddox.il2.objects.vehicles.radios.Beacon.getBeaconID(this.beacons.size());
            beaconpoint.id = s2;
            return beaconpoint;
        } else return null;
    }
    ...
    private void drawBeacons() {
        if (com.maddox.il2.ai.World.cur().diffCur.RealisticNavigationInstruments) this.drawBeacons(this.renders, this.gridFont, this.emptyMat, this.cameraMap2D, this.beacons);
    }
    ...
    private void drawPlayerPath() {
        ...
        com.maddox.il2.engine.Render.drawLines(this.lineNXYZ, this.lineNCounter, 2.0F, 0xff000000, com.maddox.il2.engine.Mat.NOWRITEZ | com.maddox.il2.engine.Mat.MODULATE | com.maddox.il2.engine.Mat.NOTEXTURE | com.maddox.il2.engine.Mat.BLEND, 3);
        ...
        for (int k = 0; k < i; k++) {
            ...
            com.maddox.il2.engine.IconDraw.render(this.getIconAir(pathpoint1.type), f1, f2);
            if (k == i - 1) this.gridFont.output(0xff000000, (int) f1 + com.maddox.il2.engine.IconDraw.scrSizeX() / 2 + 2, (int) f2 - com.maddox.il2.engine.IconDraw.scrSizeY() / 2 - 2, 0.0F, "" + (k + 1));
            ...
            this.gridFont.output(0xff000000, f1 + f5, f2 + f6 - 44F, 0.0F, s1 + s);
        }
    ...
    private void checkPlayerPath() {
        ...
        if (Mission.isCoop()) {
            ...
            if (string == null) string = sectfile.get("MAIN", "player", (String) null);
        } else string = sectfile.get("MAIN", "player", (String) null);
        if (string == null) {
            if (this.playerName != null) {
                this.playerPath.clear();
                this.playerName = null;
            }
        } else if (!string.equals(this.playerName)) {
            ...
            if (this.playerName != null) {
                ...
                if (i >= 0) {
                    ...
                    for (int i_41_ = 0; i_41_ < i_40_; i_41_++) {
                        ...
                        if ("NORMFLY".equals(string_42_)) pathpoint.type = 0;
                        else if ("TAKEOFF".equals(string_42_)) pathpoint.type = 1;
                        else if ("LANDING".equals(string_42_)) pathpoint.type = 2;
                        else if ("GATTACK".equals(string_42_)) pathpoint.type = 3;
                        else pathpoint.type = 0;
                        String string_43_ = sectfile.value(i, i_41_);
                        if (string_43_ == null || string_43_.length() <= 0) pathpoint.x = pathpoint.y = 0.0F;
                        else {
    ...
    protected boolean isBornPlace(float f, float f_47_) {
        return this.findBornPlace(f, f_47_) >= 0;
    }
    ...
    protected void setBornPlace(float f, float f1) {
        ...
        if (i >= 0) {
             ...
             * com.maddox.il2.ai.Regiment.getCountryFromBranch ((branch.intern())); //System.out.println("GUIBriefing - returned country: " + countryReturned); usercfg.netRegiment = branch; usercfg.netSquadron = 0; }
             */
            NetUser netuser = (NetUser) NetEnv.host();
        ...
        if (i_51_ == 0) {
            ...
            if (this.bSelectBorn) {
                if (this.bLPressed) {
                    ...
                    float f_53_ = (float) (this.cameraMap2D.worldYOffset + (this.renders.win.dy - f_50_ - 1.0F) / this.cameraMap2D.worldScale);
    ...
    protected void doMouseMove(float f, float f_54_) {
        if (this.bLPressed && !this.bSelectBorn) super.doMouseMove(f, f_54_);
        else {
            float f_55_ = (float) (this.cameraMap2D.worldXOffset + f / this.cameraMap2D.worldScale);
            float f_56_ = (float) (this.cameraMap2D.worldYOffset + (this.renders.win.dy - f_54_ - 1.0F) / this.cameraMap2D.worldScale);
            this.bSelectBorn = this.isBornPlace(f_55_, f_56_);

...
public class GUINetServerDBrief extends GUIBriefing {
    public void enter(GameState gamestate) {
        ...
        if (gamestate != null && (gamestate.id() == 42 || gamestate.id() == 38) && this.briefSound != null) {
    ...
    public void leave(GameState gamestate) {
        if (gamestate != null && gamestate.id() == 42 && this.briefSound != null) {
    ...
    protected String textDescription() {
        ...
        if (i < 0) return this.textArmyDescription[0];
    ...
    private boolean isValidBornPlace() {
        ...
        if (i < 0 || i >= World.cur().bornPlaces.size()) {
            new GWindowMessageBox(Main3D.cur3D().guiManager.root, 20.0F, true, this.i18n("brief.BornPlace"), this.i18n("brief.BornPlaceSelect"), 3, 0.0F);
            return false;
        }
        ...
        if (airdromeStay < 0) {
            ...
            new GWindowMessageBox(Main3D.cur3D().guiManager.root, 20.0F, true, this.i18n("brief.StayPlace"), this.i18n("brief.StayPlaceWait"), 3, 0.0F);
            return false;
        }
        return true;
    }
    ...
    protected void doNext_410() {
        if (this.isValidBornPlace()) if (!this.isCarrierDeckFree((NetUser) NetEnv.host())) new GWindowMessageBox(Main3D.cur3D().guiManager.root, 20.0F, true, this.i18n("brief.CarrierDeckFull"), this.i18n("brief.CarrierDeckFullWait"), 3, 0.0F);
        else {
    ...
    protected void clientRender() {
        ...
        dialogclient_2_.draw(f, f_3_, f_4_, f_5_, 1, USGS.isUsed() || Main.cur().netGameSpy != null ? this.i18n("main.Quit") : this.i18n("brief.MainMenu"));
        ...
        dialogclient_6_.draw(f_7_, f_8_, f_9_, f_10_, 1, this.i18n("brief.Difficulty"));
        ...
        dialogclient_11_.draw(f_12_, f_13_, f_14_, f_15_, 1, this.i18n("brief.Arming"));

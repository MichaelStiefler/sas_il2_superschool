...
// TODO: Implement 4.10.1 Codechanges ---
    ...
    public Aircraft loadAir(SectFile sectfile, String string, String string_56_, String string_57_, int i) throws Exception {
        ...
        if (bool) {
            ...
            if (i_58_ == 0) aircraft.load(sectfile, string_56_, i, null, 0);
            else aircraft.load(sectfile, string_56_, i, this.net.masterChannel(), i_58_);
        } else aircraft.load(sectfile, string_56_, i, null, 0);
        if (aircraft.isSpawnFromMission()) if (this.net.isMirror()) {
            if (i_58_ == 0) this.actors.set(this.curActor++, null);
    ...
    private void doResvMission(NetMsgInput netmsginput) {
        try {
            while (netmsginput.available() > 0) {
                ...
                } else this.sectFile.lineAdd(i, netmsginput.read255(), netmsginput.read255());
            }
        } catch (Exception exception) {
            exception.printStackTrace();

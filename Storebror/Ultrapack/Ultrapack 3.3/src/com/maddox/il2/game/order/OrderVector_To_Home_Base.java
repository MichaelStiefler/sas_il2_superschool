...
public class OrderVector_To_Home_Base extends Order {
    ...
    public void run() {
        try {
            ...
            double d2 = BeaconGeneric.lineOfSightDelta(this.Player().FM.getAltitude(), V2.z, d1);
            if ((d2 < 0.0D || d1 > 90000.0D) && World.cur().diffCur.RealisticNavigationInstruments) return;
            if (this.isEnableVoice()) if (World.cur().diffCur.RealisticNavigationInstruments) new DelayedOrder(this.Player(), d1);

...
public class OrderVector_To_Target extends Order {
    ...
    public void run() {
        ...
        if (World.cur().diffCur.RealisticNavigationInstruments && !this.isLosToHomeBase()) return;
        if (Main.cur().netServerParams.isDogfight()) return;

        Way localWay = this.Player().FM.AP.way;
        ...
        for (int i = localWay.Cur(); i < localWay.size(); i++) {
            ...
            if (localWayPoint2.Action == 3 || localWayPoint2.getTarget() != null && localWayPoint2.Action != 2) localWayPoint2.getP(V2);
        }

        this.Player().FM.actor.pos.getAbs(V1);
        V1.sub(V2);

        if (this.isEnableVoice()) if (World.cur().diffCur.RealisticNavigationInstruments) new DelayedOrder(this.Player());
        else Voice.speakHeadingToTarget(this.Player(), V1);
    }

    private boolean isLosToHomeBase() {
        this.Player().FM.moveCarrier();
        Point3d localPoint3d = this.Player().FM.actor.pos.getAbsPoint();
        Object localObject;
        if (Main.cur().netServerParams != null && Main.cur().netServerParams.isDogfight()) {
            localObject = ZutiSupportMethods.getPlayerBornPlace(localPoint3d, this.Player().getArmy());
            if (localObject != null) V2 = new Point3d(((BornPlace) localObject).place.x, ((BornPlace) localObject).place.y, 0.0D);
            else return false;
        } else {
            localObject = this.Player().FM.AP.way;

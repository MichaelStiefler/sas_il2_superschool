...
public class Gear {
    ...
    // TODO: +++ TD AI code backport from 4.13 +++
    public void computePlaneLandPoseOld(FlightModel flightmodel) {
        this.FM = flightmodel;
        if (this.H == 0.0F || this.Pitch == 0.0F) {
            ...
            this.Pitch = -Geom.RAD2DEG((float) Math.atan2(d_4_, d_3_));
            if (d_3_ < 0.0) this.Pitch += 180.0F;
            ...
            this.H -= (this.FM.M.massEmpty + this.FM.M.maxFuel + this.FM.M.maxNitro) * Atmosphere.g() / 2700000.0F;
        }
    }
    ...
    public void set(Gear gear_5_) {
        if (gear_5_.pnti != null) {
            this.pnti = new int[gear_5_.pnti.length];
            if (gear_5_.waterList != null) {
                this.waterList = new int[gear_5_.waterList.length];
                for (int i = 0; i < this.waterList.length; i++)
                    this.waterList[i] = gear_5_.waterList[i];
            }
            for (int i = 0; i < this.pnti.length; i++)
                this.pnti[i] = gear_5_.pnti[i];
            this.bIsSail = gear_5_.bIsSail;
            this.sinkFactor = gear_5_.sinkFactor;
            this.springsStiffness = gear_5_.springsStiffness;
            this.H = gear_5_.H;
            this.Pitch = gear_5_.Pitch;
            this.bFrontWheel = gear_5_.bFrontWheel;
        }
    }
...
//                    gVelocity[k1] = 0.0D;
    ...
    private boolean testGearCollision(int i) {
        ...
        RightVPrj = Right.dot(Vs);
        if (NormalVPrj > 0.0) NormalVPrj = 0.0;
        double d_17_ = this.FM.Vrel.x * this.FM.Vrel.x + this.FM.Vrel.y * this.FM.Vrel.y - 2.0;
        if (d_17_ < 0.0) d_17_ = 0.0;
        double d_18_ = 0.01 * d_17_;
        if (d_18_ < 0.0) d_18_ = 0.0;
        if (d_18_ > 4.5) d_18_ = 4.5;
        double d_19_ = 0.4 * Math.max(this.roughness * this.roughness, this.roughness);
        if (d_18_ > d_19_) d_18_ = d_19_;
        if (this.roughness > d_18_) this.roughness = d_18_;
        if (this.roughness < 0.2) this.roughness = 0.2;
        if (i < 2) {
            this.d += World.Rnd().nextFloat(-2.0F, 1.0F) * 0.04 * d_18_ * this.MassCoeff;
            d = Math.max(-9500.0 * (this.d - 0.1), -950000.0 * this.d);
            d *= this.springsStiffness;
        } else {
            this.d += World.Rnd().nextFloat(-2.0F, 1.0F) * 0.04F * d_18_ * this.MassCoeff;
            d = Math.max(-9500.0 * (this.d - 0.1), -950000.0 * this.d);
            if (Pnt[i].x > 0.0F && Fd.dot(Normal) >= 0.0) d *= 0.45;
            else d *= this.tailStiffness;
        }
        d -= 40000.0 * NormalVPrj;
        Tn.scale(d, Normal);
        double d_20_ = 1.0E-4 * d;
        double d_21_ = this.FM.CT.getBrake();
        double d_22_ = this.FM.CT.getRudder();
        // TAK++
        double d29_Right = this.FM.CT.getBrakeRight();
        double d29_Left = this.FM.CT.getBrakeLeft();
        double d30_2 = d29_Right - d29_Left;
        // TAK--
//		System.out.println("testGearCollision(" + i+ ")");
        switch (i) {
            ...
            case 1: {
                ...
                if (d_21_ > 0.1) {
                    if (d_22_ > 0.1) d_21_ += d_23_ * d_21_ * (d_22_ - 0.1);
                    if (d_22_ < -0.1) d_21_ += d_23_ * d_21_ * (d_22_ + 0.1);
                    // TAK
                    if (d30_2 > 0.1) d_21_ += d_23_ * 4.0F * (d30_2 - 0.1);
                    if (d30_2 < -0.1) d_21_ += d_23_ * 4.0F * (d30_2 + 0.1);
                    // TAK--
                    if (d_21_ > 1.0) d_21_ = 1.0;
                    if (d_21_ < 0.0) d_21_ = 0.0;
                }
                double d_24_ = Math.sqrt(ForwardVPrj * ForwardVPrj + RightVPrj * RightVPrj);
                if (d_24_ < 0.01) d_24_ = 0.01;
                double d_25_ = 1.0 / d_24_;
                double d_26_ = ForwardVPrj * d_25_;
                if (d_26_ < 0.0) d_26_ *= -1.0;
                double d_27_ = RightVPrj * d_25_;
                if (d_27_ < 0.0) d_27_ *= -1.0;
                double d_28_ = 5.0;
                if (PnT.y * RightVPrj > 0.0) {
                    if (PnT.y > 0.0) d_28_ += 7.0 * RightVPrj;
                    else d_28_ -= 7.0 * RightVPrj;
                    if (d_28_ > 20.0) d_28_ = 20.0;
                }
                ...
                this.fricR = -d_28_ * 100000.0 * RightVPrj * d_20_;
                this.maxFric = d_29_ * d_20_ * d_27_;
                if (this.fricR > this.maxFric) this.fricR = this.maxFric;
                if (this.fricR < -this.maxFric) this.fricR = -this.maxFric;
                this.fricF = -d_28_ * 600.0 * ForwardVPrj * d_20_;
                this.maxFric = d_28_ * Math.max(200.0 * (1.0 - 0.04 * d_24_), 5.0) * d_20_ * d_26_;
                if (this.fricF > this.maxFric) this.fricF = this.maxFric;
                if (this.fricF < -this.maxFric) this.fricF = -this.maxFric;
                double d_31_ = 0.03;
                if (Pnt[2].x > 0.0F) d_31_ = 0.06;
                double d_32_ = Math.abs(ForwardVPrj);
                if (d_32_ < 1.0) d_31_ += 3.0 * (1.0 - d_32_);
                d_31_ *= 0.03 * d_21_;
                this.fricF += -300000.0 * d_31_ * ForwardVPrj * d_20_;
                this.maxFric = d_29_ * d_20_ * d_26_;
                if (this.fricF > this.maxFric) this.fricF = this.maxFric;
                if (this.fricF < -this.maxFric) this.fricF = -this.maxFric;
                this.fric = Math.sqrt(this.fricF * this.fricF + this.fricR * this.fricR);
                ...
                if (this.bIsMaster && NormalVPrj < 0.0) {
                    ...
                    if (this.FM.CT.bHasArrestorControl) d_33_ += NormalVPrj * NormalVPrj * 0.025;
                    else d_33_ += NormalVPrj * NormalVPrj * 0.07;
                    if (d_33_ > 1.0) {
                        this.fatigue[i] += 10;
                        double d_34_ = 38000.0 + this.FM.M.massEmpty * 6.0;
                        double d_35_ = (Tn.x * Tn.x * 0.15 + Tn.y * Tn.y * 0.15 + Tn.z * Tn.z * 0.08) / (d_34_ * d_34_);
                        if (this.fatigue[i] > 100 || d_35_ > 1.0) {
                            this.landHit(i, (float) d_33_);
            ...
            case 2:
                ...
                if (this.bTailwheelLocked && this.steerAngle > -5.0F && this.steerAngle < 5.0F) {
                    ...
                    // TAK++
                    this.tmpV.set(1.0, -0.5 * (d29_Right - d29_Left + this.FM.CT.getRudder()), 0.0);
                    // TAK--
                ...
                if (this.bIsMaster && NormalVPrj < 0.0) {
                    ...
                    if (this.FM.CT.bHasArrestorControl) d_38_ += NormalVPrj * NormalVPrj * 0.0040;
                    else d_38_ += NormalVPrj * NormalVPrj * 0.02;
                    if (d_38_ > 1.0) {
                        this.landHit(i, d_38_);
                        Aircraft aircraft = (Aircraft) this.FM.actor;

...
public class ZutiSupportMethods_NetSend_ToClients {
        ...
        try {
            ...
            if (acName.endsWith("_0")) {
                ...
                if (ac.FM instanceof RealFlightModel) {
                    ...
                    if (netuser != null) NetEnv.host().postTo(netuser.masterChannel(), netmsgguaranted);
    ...
    public static void bombBayDoorsStatus_ToClient(String acName, int status) {
        try {
            ...
            if (acName.endsWith("_0")) {
                // Aircraft is controlled by user. Must forward bomb release data to aircraft owner.
                if (ac.FM instanceof RealFlightModel) // Server is live plane owner and can change bomb bay doors!
    ...
    public static void bombardierInstrumentsChanged_ToClient(NetUser sender, String acName, int distance, int sideslip, int altitude, int speed, int gunsightAutomation) {
        try {
            if (sender == null || acName == null) return;
    ...
    /**
     * This method is executed by server to inform remaining crew members that their comrade scored some victory. They all should share that benefit (or loss, if that member hit friendly unit).
     *
     * @param sender
     * @param netmsginput
     */
    public static void syncCrewScore_ToClient(NetUser sender, String acName, String actorName, boolean damagedOnly, double explosionPower) {
        try {
            ...
            if (sender == null || acName == null || actorName == null) return;
    ...
    public static void transferControls_ToClient(NetUser sender, String targetUser) {
        try {
            if (sender == null || targetUser == null || sender.uniqueName().equals(targetUser)) return;
    ...
    /**
     * Server received information that acting pilot/instructor changed aircraft related control (flaps,...). If host is also plane owner, nothing is dorwarded.
     *
     * @param sender
     * @param netmsginput
     */
    ...
    /**
     * Calculate available fuel amount for born place that player parked on and forward that info back to the player.
     *
     * @param fuel
     * @param user
     */
    public static void refuelAircraft_ToClient(float fuel, String user) {
        try {
            ...
            Point3d pos = userAircraft.pos.getAbsPoint();
            fuel = ZutiSupportMethods_ResourcesManagement.getFuelForPlayer(fuel, pos.x, pos.y);
            if (((NetUser) NetEnv.host()).uniqueName().equals(user)) // We are server and target user. Start Refueling process
                ZutiSupportMethods_FM.startRefueling(userAircraft, fuel);
    ...
    /**
     * Calculate available bullets amount for born place that player parked on and forward that info back to the player.
     *
     * @param bullets
     * @param user
     */
    ...
    /**
     * Calculate available bombs amount for born place that player parked on and forward that info back to the player.
     *
     * @param bullets
     * @param user
     */
    ...
    /**
     * Calculate available rockets amount for born place that player parked on and forward that info back to the player.
     *
     * @param bullets
     * @param user
     */
    ...
    /**
     * Check if there are any engines left and report it back to the client. that info back to the player.
     *
     * @param engines
     * @param engineId
     * @param user
    ...
    public static void repairKit_ToClient(long requestedKits, String user) {
        try {
            if (user == null || Mission.isSingle()) {
                ...
                if (requestedKits <= 0) ZutiSupportMethods_FM.startRepairing(userAircraft);
    ...
    public static void aircraftAvailability_ToClient(String acName, int bornPlaceId, NetUser client) {
        ...
        try {
            ...
            if (ZutiSupportMethods_Air.getAircraft(acName, bornPlaceId) != null) netmsgguaranted.writeInt(1);
            else netmsgguaranted.writeInt(-1);
    ...
    public static void carrierStayPlace_ToClient(NetUser user) {
        try {
            ...
            if (((NetUser) NetEnv.host()).equals(user)) // We are server and target user.
    ...
    public static void updateNetPlace_ToClients(NetUser sender) {
        try {
            ...
            for (int i = 0; i < hosts.size(); i++) {
                ...
                if (netuser.uniqueName().equals(sender.uniqueName())) continue;

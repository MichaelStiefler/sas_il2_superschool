...
//        return "Server uses a different version of the game (4.10.1m + Ultrapack 3 RC4).";
        return "Server uses a different version of the game (" + Config.VERSION_STRING + " " + Config.VERSION_NUMBER + ").";
    }
...
//        // ---
        ...
        if (!Connect.VERSION.equals(clientVersion) && !Arrays.asList(Connect.VERSION2).contains(clientVersion) && !"il2_r01_0f".equals(clientVersion)) {
        ...
        if (netchannel == null) {
            ...
            if (!Connect.VERSION.equals(clientVersion) && !Arrays.asList(Connect.VERSION2).contains(clientVersion)) this.kickChannel(netchannel);
        }
    ...
    private void joinReceiveConnected(StringTokenizer stringtokenizer, NetSocket netsocket, NetAddress netaddress, int i) {
        if (this.bJoin && netsocket.equals(this.joinSocket) && netaddress.equals(this.joinAddr) && i == this.joinPort && stringtokenizer.hasMoreTokens()) {
            ...
            if ((Connect.VERSION.equals(string) || Arrays.asList(Connect.VERSION2).contains(string)) && stringtokenizer.hasMoreTokens()) {

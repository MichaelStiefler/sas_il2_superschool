...
//	          BufferedReader bufferedreader = new BufferedReader(new InputStreamReader(new KryptoInputFilter(new SFSInputStream(Finger.LongFN(0L, "cod/" + Finger.incInt(i, "adt"))), getSwTbl(i))));
    ...
    protected static void weaponsLoad(Aircraft aircraft, int i, HashMapInt hashmapint) throws Exception {
        ...
        for (int hookIndex = 0; hookIndex < strings.length; hookIndex++) {
            try {
                if (hookIndex < 0 || hookIndex >= var__WeaponSlots.length)
                    continue;
                if (var__WeaponSlots[hookIndex] != null) {
                    if (aircraft.mesh().hookFind(strings[hookIndex]) != -1) {
                        ...
                        if (aircraft.isNet() && aircraft.isNetMirror()) {
                            if (!World.cur().diffCur.Limited_Ammo)
                                ...
                                if (var__WeaponSlots[hookIndex].bullets == -12345)
                                    bulletemitter.loadBullets();
                                else
                                    bulletemitter._loadBullets(var__WeaponSlots[hookIndex].bullets);
                            } else
                                bulletemitter.loadBullets(-1);
                        } else if (var__WeaponSlots[hookIndex].bullets == -12345)
                            bulletemitter.loadBullets();
                        else
                            bulletemitter.loadBullets(var__WeaponSlots[hookIndex].bullets);
                ...
                System.out.println("WeaponHooksRegistered length = " + strings.length + ", WeaponSlots length = " + var__WeaponSlots.length);
                e.printStackTrace();
            }
        }
    }

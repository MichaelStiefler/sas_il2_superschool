...
public class PlMisTrigger extends Plugin {
    ...
    public boolean save(SectFile sectfile) {
        ...
        for (int k = 0; k < i; k++) {
            ...
            String s = "";
            String s2 = "";
            if (actortrigger.getType() != ActorTrigger.TRIGGER_TYPE_MESSAGE) {
                if (actortrigger.getType() != ActorTrigger.TRIGGER_TYPE_MESSAGE && !Actor.isValid(actortrigger.getTriggerActor())) continue;
            ...
            if (Actor.isValid(actortrigger.getLinkActor())) {
                if (actortrigger.getLinkActor() instanceof PPoint) s2 = actortrigger.getLinkActor().getOwner().name();
                else s2 = actortrigger.getLinkActor().name();
                bLink = true;
            }
            ...
            sectfile.lineAdd(j, actortrigger.name(),
                    actortrigger.getType() + " " + actortrigger.getTriggeredByArmy() + " " + (actortrigger.isHasTimeout() ? "1 " : "0 ") + actortrigger.getTimeout() + " " + (int) actortrigger.pos.getAbsPoint().x + " "
                            + (int) actortrigger.pos.getAbsPoint().y + " " + actortrigger.getRadius() + " " + actortrigger.getAltitudeMin() + " " + actortrigger.getAltitudeMax()
                            + (actortrigger.getType() == ActorTrigger.TRIGGER_TYPE_MESSAGE ? "" : " " + s) + " " + actortrigger.getTriggeredBy() + " " + (actortrigger.isHasTriggerActor() ? "1" : "0") + " " + actortrigger.getNoObjectsMin() + " "
                            + actortrigger.getProbability() + (actortrigger.getType() != ActorTrigger.TRIGGER_TYPE_AIRSPAWN_RELATIVE_ALTITUDE ? "" : " " + actortrigger.getDeltaAltitude()) + (bLink ? " 1 " + s2 : " 0")
    ...
    public void load(SectFile sectfile) {
        ...
        if (i >= 0) {
            ...
            for (; k < j; k++) {
                ...
                int zAltiDiff = 0;
                if (type == 2) zAltiDiff = numbertokenizer.next(0, -10000, 10000);
                boolean bLink = numbertokenizer.next(0) == 1;
                String s2 = null;
                if (bLink) s2 = numbertokenizer.next(null);
                ...
                while (numbertokenizer.hasMoreElements()) {
                    ...
                    if (numbertokenizer.hasMoreElements()) sTextDisplay = sTextDisplay + " ";
                }
                ActorTrigger actorTrigger = this.insert(name, point3d, type, false, army, btimeout, timeout, l1, zmin, zmax, zAppliesFor, bSortie, zAvionMin, zProba, zAltiDiff, sTextDisplay, zTextDuree);
                if (actorTrigger != null) {
                    actorTrigger.addActor(s);
                    if (bLink) actorTrigger.addLinkActor(s2);
                }
            }
    ...
    private ActorTrigger insert(String name, Point3d point3d, int i, boolean flag, int army, boolean btimeout, int timeout, int r, int zmin, int zmax, int ziaHumans, boolean bSortie, int zAvionMin, int zProba, int zAltiDiff, String sTextDisplay,
            int zTextDuree) {
        try {
            if (name != null) {
    ...
    public void syncSelector() {
        ...
        if (actortrigger.getType() == ActorTrigger.TRIGGER_TYPE_AIRSPAWN_RELATIVE_ALTITUDE) {
        ...
        this.wTriggerTarget.setChecked(actortrigger.isHasTriggerActor(), false);
    ...
    public void createGUI() {
        ...
        f += 2.0F;
        gwindowdialogclient.addControl(this.wRadius = new GWindowHSliderInt(gwindowdialogclient, 0, 151, 15, 1.0F, f, 20F) {
            ...
            public boolean notify(int j, int k) {
                if (j != 2) return false;
                ActorTrigger actortrigger = (ActorTrigger) Plugin.builder.selectedActor();
                actortrigger.setRadius(this.pos() * 500);
        ...
        f += 2.0F;
        gwindowdialogclient.addControl(this.wTriggerTarget = new GWindowCheckBox(gwindowdialogclient, 1.0F, f, null) {

            public boolean notify(int j, int k) {
                if (j != 2) return false;
                else {
                    ActorTrigger actortrigger = (ActorTrigger) Plugin.builder.selectedActor();
                    actortrigger.setHasTriggerActor(this.isChecked());
                    PlMission.setChanged();
                    return false;
                }
            }
...
//                    return true;
        ...
        gwindowdialogclient.addLabel(new GWindowLabel(gwindowdialogclient, 23F, 6.5F, 14F, 1.3F, Plugin.i18n("TTextToShow"), null));
        gwindowdialogclient.addControl(this.wDisplayMessage = new GWindowEditTextControl(gwindowdialogclient, 23F, 8F, 14F, 12F, "") {
        ...
        gwindowdialogclient.addLabel(new GWindowLabel(gwindowdialogclient, 23F, 21F, 8F, 1.3F, Plugin.i18n("TTextDuree"), null));
        gwindowdialogclient.addControl(this.wDisplayTime = new GWindowEditControl(gwindowdialogclient, 32F, 21F, 5F, 1.3F, "5") {
    ...
    Item                               item[]       = { new Item("NewAircraftAir", ActorTrigger.TRIGGER_TYPE_SPAWN), new Item("NewAircraftSol", ActorTrigger.TRIGGER_TYPE_ACTIVATE),
            new Item("NewAircraftAirLevel", ActorTrigger.TRIGGER_TYPE_AIRSPAWN_RELATIVE_ALTITUDE), new Item("NewMessage", ActorTrigger.TRIGGER_TYPE_MESSAGE) };
    ...
    private GWindowCheckBox            wTriggerTarget;

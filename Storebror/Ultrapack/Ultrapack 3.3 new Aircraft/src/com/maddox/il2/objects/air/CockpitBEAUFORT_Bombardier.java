...
public class CockpitBEAUFORT_Bombardier extends CockpitPilot {
	class Interpolater extends InterpolateRef {

		public boolean tick() {
			float f = ((Beaufort) CockpitBEAUFORT_Bombardier.this.aircraft()).fSightCurForwardAngle;
			float f1 = ((Beaufort) CockpitBEAUFORT_Bombardier.this.aircraft()).fSightCurSideslip;
			CockpitBEAUFORT_Bombardier.calibrAngle = 360F - CockpitBEAUFORT_Bombardier.this.fm.Or.getPitch();
			CockpitBEAUFORT_Bombardier.this.mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitBEAUFORT_Bombardier.calibrAngle + f);
			if (CockpitBEAUFORT_Bombardier.this.bEntered) {
				HookPilot hookpilot = HookPilot.current;
				hookpilot.setInstantOrient(CockpitBEAUFORT_Bombardier.this.aAim + 10F * f1, CockpitBEAUFORT_Bombardier.this.tAim + CockpitBEAUFORT_Bombardier.calibrAngle + f, 0.0F);
			}
			return true;
		}
	...
	public CockpitBEAUFORT_Bombardier() {
		super("3DO/Cockpit/Beaufort-Bombardier/hier.him", "he111");
		this.w = new Vector3f();
		this.bEntered = false;
		try {
			Loc loc = new Loc();
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (this.bEntered) {
			this.mesh.chunkSetAngles("zAngleMark", -this.floatindex(this.cvt(((Beaufort) this.aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
			boolean flag = ((Beaufort) this.aircraft()).fSightCurReadyness > 0.93F;
	...
	static {
		Property.set(CockpitBEAUFORT_Bombardier.class, "astatePilotIndx", 0);
	}

...
public class HE_111H3 extends HE_111 {
	...
	public void onAircraftLoaded() {
		...
		this.gun0 = this.getGunByHookName("_MGUN01");
		this.gun6 = this.getGunByHookName("_MGUN07");
	...
	public void update(float f) {
		...
		if (this.FM.AS.astatePlayerIndex == 1) {
			...
			else if (Main3D.cur3D().cockpitCur == Main3D.cur3D().cockpits[3]) this.SturmanBusy(3);
		} else if (Time.current() > this.tme3) {
			this.tme3 = Time.current() + 1495L;
			if (this.FM.turret.length != 0) {
				Actor actor = null;
				actor = this.FM.turret[0].target;
				if (actor == null) actor = this.FM.turret[6].target;
				if (actor != null) if (Actor.isValid(actor)) {
					...
					if (Aircraft.tmpLoc3.getPoint().z < 0.0D) this.SturmanBusy(2);
					else this.SturmanBusy(3);
				} else if (!this.pilot2kill) {
					this.FM.turret[0].bIsOperable = true;
					this.FM.turret[6].bIsOperable = true;
				}
			}
		}
		if (this.FM.AS.astatePlayerIndex == 3) {
			if (Main3D.cur3D().cockpitCur == Main3D.cur3D().cockpits[5]) this.SturmanBusy(5);
			...
			if (this.FM.turret.length != 0) {
				...
				if (actor1 == null) actor1 = this.FM.turret[5].target;
				if (actor1 != null) if (Actor.isValid(actor1)) {
					this.pos.getAbs(Aircraft.tmpLoc2);
					actor1.pos.getAbs(Aircraft.tmpLoc3);
					Aircraft.tmpLoc2.transformInv(Aircraft.tmpLoc3.getPoint());
					if (Aircraft.tmpLoc3.getPoint().x > 0.0D) this.SturmanBusy(6);
		...
		if (this.FM.AS.astatePlayerIndex == 4) {
			if (Main3D.cur3D().cockpitCur == Main3D.cur3D().cockpits[8]) this.SturmanBusy(8);
			else if (Main3D.cur3D().cockpitCur == Main3D.cur3D().cockpits[7]) this.SturmanBusy(7);
		} else if (Time.current() > this.tme) {
			this.tme = Time.current() + 1500L;
			if (this.FM.turret.length != 0) {
				Actor actor2 = null;
				actor2 = this.FM.turret[3].target;
				if (actor2 == null) actor2 = this.FM.turret[4].target;
				if (actor2 != null) if (Actor.isValid(actor2)) {
					this.pos.getAbs(Aircraft.tmpLoc2);
					actor2.pos.getAbs(Aircraft.tmpLoc3);
					Aircraft.tmpLoc2.transformInv(Aircraft.tmpLoc3.getPoint());
					if (Aircraft.tmpLoc3.getPoint().y < 0.0D) this.SturmanBusy(8);
					else this.SturmanBusy(7);
	...
	private void SturmanBusy(int i) {
		switch (i) {
			...
			case 2:
				if (this.pilot2kill) break;
				this.FM.turret[0].bIsOperable = true;
				this.FM.turret[6].bIsOperable = false;
				if (this.FM.turret.length != 0 && World.cur().diffCur.Limited_Ammo) {
					this.gunmax = Math.max(this.gun0.countBullets(), this.gun6.countBullets());
					this.gunmin = 0;
					this.gun0.loadBullets(this.gunmax);
					this.gun6.loadBullets(this.gunmin);
				}
				break;
			...
			case 3:
				...
				if (this.FM.turret.length != 0 && World.cur().diffCur.Limited_Ammo) {
					this.gunmax = Math.max(this.gun0.countBullets(), this.gun6.countBullets());
					this.gunmin = 0;
					this.gun6.loadBullets(this.gunmax);
					this.gun0.loadBullets(this.gunmin);
				}
				break;
	...
	public boolean turretAngles(int i, float af[]) {
		...
		switch (i) {
			...
			case 5:
				...
				if (f1 > -7.5F && f < 0.5959F * f1 - 35.5278F) {
					f = 0.5959F * f1 - 35.5278F;
					flag = false;
					break;
				}
				if (f1 > -7.5F && f > -0.5959F * f1 + 35.5278F) {
	...
	static {
		Class class1 = HE_111H3.class;
		new NetAircraft.SPAWN(class1);
		Property.set(class1, "iconFar_shortClassName", "He-111");
		Property.set(class1, "meshName", "3do/plane/He-111H-3/hier.him");
		Property.set(class1, "PaintScheme", new PaintSchemeBMPar02());
		Property.set(class1, "yearService", 1940.5F);
		Property.set(class1, "yearExpired", 1948F);
		Property.set(class1, "FlightModel", "FlightModels/He-111H-3.fmd");
		Property.set(class1, "cockpitClass", new Class[] { CockpitHE_111P4.class, CockpitHE_111P4_Bombardier.class, CockpitHE_111P4_NGunner.class, CockpitHE_111P4_NBGunner.class, CockpitHE_111P4_TGunner.class, CockpitHE_111P4_BGunner.class,
				CockpitHE_111P4_FGunner.class, CockpitHE_111P4_LGunner.class, CockpitHE_111P4_RGunner.class });
		Aircraft.weaponTriggersRegister(class1, new int[] { 10, 11, 12, 13, 14, 15, 16, 3, 3, 3, 3, 3, 3, 3, 3 });
		Aircraft.weaponHooksRegister(class1,
				new String[] { "_MGUN01", "_MGUN02", "_MGUN03", "_MGUN04", "_MGUN05", "_MGUN06", "_MGUN07", "_BombSpawn01", "_BombSpawn02", "_BombSpawn03", "_BombSpawn04", "_BombSpawn05", "_BombSpawn06", "_BombSpawn07", "_BombSpawn08" });
	}
}

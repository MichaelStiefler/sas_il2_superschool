...
public class CockpitB26B_Bombardier extends CockpitPilot {
	class Interpolater extends InterpolateRef {

		public boolean tick() {
			CockpitB26B_Bombardier.this.setTmp = CockpitB26B_Bombardier.this.setOld;
			CockpitB26B_Bombardier.this.setOld = CockpitB26B_Bombardier.this.setNew;
			CockpitB26B_Bombardier.this.setNew = CockpitB26B_Bombardier.this.setTmp;
			float f = ((B_26B) CockpitB26B_Bombardier.this.aircraft()).fSightCurForwardAngle;
			float f1 = ((B_26B) CockpitB26B_Bombardier.this.aircraft()).fSightCurSideslip;
			CockpitB26B_Bombardier.this.mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, f);
			if (CockpitB26B_Bombardier.this.bEntered) {
				HookPilot hookpilot = HookPilot.current;
				hookpilot.setInstantOrient(CockpitB26B_Bombardier.this.aAim + 10F * f1, CockpitB26B_Bombardier.this.tAim + f, 0.0F);
			}
			CockpitB26B_Bombardier.this.mesh.chunkSetAngles("TurretA", 0.0F, CockpitB26B_Bombardier.this.aircraft().FM.turret[0].tu[0], 0.0F);
			CockpitB26B_Bombardier.this.mesh.chunkSetAngles("TurretB", 0.0F, CockpitB26B_Bombardier.this.aircraft().FM.turret[0].tu[1], 0.0F);
			float f2 = CockpitB26B_Bombardier.this.waypointAzimuth();
			CockpitB26B_Bombardier.this.setNew.azimuth.setDeg(CockpitB26B_Bombardier.this.setOld.azimuth.getDeg(1.0F), CockpitB26B_Bombardier.this.fm.Or.azimut());
			if (CockpitB26B_Bombardier.this.useRealisticNavigationInstruments()) {
				CockpitB26B_Bombardier.this.setNew.waypointAzimuth.setDeg(f2 - 90F);
				CockpitB26B_Bombardier.this.setOld.waypointAzimuth.setDeg(f2 - 90F);
			} else CockpitB26B_Bombardier.this.setNew.waypointAzimuth.setDeg(CockpitB26B_Bombardier.this.setOld.waypointAzimuth.getDeg(0.1F), f2);
			return true;
		}
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (this.bEntered) {
			this.mesh.chunkSetAngles("zAngleMark", -this.floatindex(this.cvt(((B_26B) this.aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
			boolean flag = ((B_26B) this.aircraft()).fSightCurReadyness > 0.93F;
	...
	static {
		Property.set(CockpitB26B_Bombardier.class, "astatePilotIndx", 0);
	}
}

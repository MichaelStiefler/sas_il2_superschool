...
public class B_24D extends B_24 implements TypeBomber {
	...
	protected void nextDMGLevel(String s, int i, Actor actor) {
		...
		if (this.FM.isPlayers()) B_24D.bChangedPit = true;
	}
	...
	public void typeBomberAdjDistancePlus() {
		...
		this.fSightCurDistance = B_24D.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
	...
	public void typeBomberUpdate(float f) {
		...
		if (this.fSightCurReadyness < 1.0F) this.fSightCurReadyness += 0.0333333F * f;
			...
			this.fSightCurDistance -= B_24D.toMetersPerSecond(this.fSightCurSpeed) * f;
			if (this.fSightCurDistance < 0.0F) {
				this.fSightCurDistance = 0.0F;
				this.typeBomberToggleAutomation();
			}
			this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / B_24D.toMeters(this.fSightCurAltitude)));
			if (this.fSightCurDistance < B_24D.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(B_24D.toMeters(this.fSightCurAltitude) * (2F / 9.81F))) this.bSightBombDump = true;
	...
	static {
		Class class1 = B_24D.class;
		new NetAircraft.SPAWN(class1);
		Property.set(class1, "iconFar_shortClassName", "B-24");
		Property.set(class1, "meshName", "3DO/Plane/B-24D(Multi1)/hier.him");
		Property.set(class1, "PaintScheme", new PaintSchemeBMPar05());
		Property.set(class1, "meshName_us", "3DO/Plane/B-24D(USA)/hier.him");
		...
		Property.set(class1, "cockpitClass",
				new Class[] { CockpitB_24D.class, CockpitB_24D_Bombardier.class, CockpitB_24D_FGunner.class, CockpitB_24D_TGunner.class, CockpitB_24D_BGunner.class, CockpitB_24D_RGunner.class, CockpitB_24D_LGunner.class, CockpitB_24D_AGunner.class });
		Aircraft.weaponTriggersRegister(class1, new int[] { 10, 11, 11, 12, 12, 13, 14, 15, 15, 3, 3, 3, 3, 3, 3, 3, 3 });
		Aircraft.weaponHooksRegister(class1, new String[] { "_MGUN01", "_MGUN03", "_MGUN04", "_MGUN05", "_MGUN06", "_MGUN07", "_MGUN08", "_MGUN09", "_MGUN10", "_BombSpawn01", "_BombSpawn02", "_BombSpawn03", "_BombSpawn04", "_BombSpawn05", "_BombSpawn06",
				"_BombSpawn07", "_BombSpawn08" });
	}
}

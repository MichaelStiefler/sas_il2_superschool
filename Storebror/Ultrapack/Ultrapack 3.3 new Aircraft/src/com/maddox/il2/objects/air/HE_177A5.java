...
public class HE_177A5 extends HE_177_MOD implements TypeBomber, TypeTransport, TypeX4Carrier, TypeGuidedBombCarrier, TypeHasToKG {
	...
	public void onAircraftLoaded() {
		...
		if (this.thisWeaponsName.startsWith("A51") || this.thisWeaponsName.startsWith("A53") || this.thisWeaponsName.startsWith("A54")) if (this.thisWeaponsName.indexOf("293") != -1) this.hierMesh().chunkVisible("PylonHs293C", true);
		else if (this.thisWeaponsName.indexOf("FritzX") != -1) this.hierMesh().chunkVisible("PylonFritzXC", true);
			...
			this.hasToKG = true;
		} else if (this.thisWeaponsName.indexOf("SC2500") != -1) this.hierMesh().chunkVisible("PylonSC2500C", true);
		else this.hierMesh().chunkVisible("PylonC", true);
		if (this.thisWeaponsName.startsWith("A52") || this.thisWeaponsName.startsWith("A53")) if (this.thisWeaponsName.indexOf("293") != -1) {
	...
	public void rareAction(float f, boolean flag) {
		...
		if (Main3D.cur3D().cockpitCurIndx() != 3 && Main3D.cur3D().cockpitCurIndx() != 4) if (Time.current() > this.ventralGunnerTargetCheckTime) { // frequently (every 5-20 seconds) check if ventral gunner should change his position
			this.ventralGunnerTargetCheckTime = Time.current() + World.Rnd().nextLong(5000L, 20000L);
			if (this.FM.turret.length != 0) if (this.FM.turret[2].target == null) // If aft ventral gun has no target in sight, let gunner face forward
				this.setVentralGunnerDirection(0);
			else this.setVentralGunnerDirection(1);
		}
	}
	...
	static {
		...
		Property.set(class1, "meshName", "3do/plane/He-177A-5/hier.him");
		Property.set(class1, "PaintScheme", new PaintSchemeBMPar02());
		Property.set(class1, "yearService", 1939.5F);
		Property.set(class1, "yearExpired", 1948F);
		Property.set(class1, "FlightModel", "FlightModels/He-177A-5.fmd");

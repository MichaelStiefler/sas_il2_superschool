...
public class RadarLiSN2 {
	public class Contact {
		...
		public void transform() {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (this.size[i] > 0.0F) this.acc[i] += this.size[i] / f;
			}
	...
	public class SpikeHolder {
		...
		public int toArray(Vector2f avector2f[]) {
			for (int i = 0; i < this.spikes.size(); i++) {
				avector2f[i].x = ((Tuple4f) (Vector4f) this.spikes.get(i)).x;
				avector2f[i].y = ((Tuple4f) (Vector4f) this.spikes.get(i)).y;
			}
		...
		public boolean addSpike(float f, float f1) {
			...
			if (f1 < 0.0F) return false;
			for (int i = 0; i < this.spikes.size(); i++) {
				Vector4f vector4f = (Vector4f) this.spikes.get(i);
				if (((Tuple4f) vector4f).z < f2 && ((Tuple4f) vector4f).w > f3) return false;
				if (((Tuple4f) vector4f).z > f2 && ((Tuple4f) vector4f).w < f3) {
	...
	public RadarLiSN2() {
		this.BEAM_WIDTH_NORMAL = 15F;
		this.BEAM_WIDTH_SHORT = 5F;
		this.DETECTION_WIDTH_NORMAL = 10F;
		this.DETECTION_WIDTH_SHORT = 3.3F;
		this.AGL_SAMPLES = 5;
	...
	public void rareAction() {
		...
		com.maddox.JGP.Point3d point3d2 = this.me.pos.getAbsPoint();
		for (int j = 0; j < i; j++)
			if (!Actor.isValid(((Contact) this.contacts.get(j)).actor)) {
				...
				com.maddox.JGP.Point3d point3d = ((Contact) this.contacts.get(j)).actor.pos.getAbsPoint();
			...
			if (actor instanceof Aircraft && !actor.equals(this.me)) {
				...
				if (Math.abs(point3d2.x - point3d1.x) < this.MAX_DIST_FASTREJECT && Math.abs(point3d2.y - point3d1.y) < this.MAX_DIST_FASTREJECT) {
					...
					for (l = 0; l < this.contacts.size(); l++)
						if (((Contact) this.contacts.get(l)).actor == actor) break;

					if (l == this.contacts.size()) this.contacts.add(new Contact(actor));
				}
			}
		}
	...
	protected void updateAGL() {
		com.maddox.JGP.Point3d point3d = this.me.pos.getAbsPoint();
	...
	public float getGroundClutterDist() {
		float f = this.agl;
		if (this.shortMode) return f * 2.5F;
		else return f;
	}
	...
	protected float dB2size(float f) {
		...
		if (f1 > this.spikeSize05) f1 = this.spikeSize05 + this.spikeSize05 * (f1 - this.spikeSize05) / f1;
		return f1;
	}
	...
	public int[] getContacts(Vector2f avector2f[][]) {
		...
		for (int i = 0; i < this.contacts.size(); i++) {
			...
			if (Actor.isValid(contact.actor)) {
				...
				if (f > 10500F) contact.setInvis();
				else if (this.shortMode && f > 4200F) contact.setInvis();
				else {
		...
		for (int j = 0; j < 4; j++) {
			...
			for (int k = 0; k < MAX_CONTACTS; k++) {
				...
				contact1.acc[j] = 0.0F;
				if (contact1.size[j] > 0.0F && !this.spikeHolder[j].addSpike(contact1.dist, contact1.size[j])) k--;
			}

			float f3;
			if (this.shortMode) f3 = 4000F;
			else f3 = 10800F;
	...
	private float           BEAM_WIDTH_NORMAL;
	private float           BEAM_WIDTH_SHORT;
	private float           DETECTION_WIDTH_NORMAL;
	private float           DETECTION_WIDTH_SHORT;

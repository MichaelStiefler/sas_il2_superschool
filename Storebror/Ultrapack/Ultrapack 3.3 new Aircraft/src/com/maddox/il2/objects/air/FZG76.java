...
public class FZG76 extends Scheme1 implements TypeDockable, MsgCollisionRequestListener {
	...
	public void onAircraftLoaded() {
		...
		if (this.thisWeaponsName.endsWith("_50km")) this.distance = 49F;
		else if (this.thisWeaponsName.endsWith("_55km")) this.distance = 54F;
		else if (this.thisWeaponsName.endsWith("_60km")) this.distance = 59F;
		else if (this.thisWeaponsName.endsWith("_65km")) this.distance = 64F;
		else if (this.thisWeaponsName.endsWith("_70km")) this.distance = 69F;
		else if (this.thisWeaponsName.endsWith("_75km")) this.distance = 74F;
		else if (this.thisWeaponsName.endsWith("_80km")) this.distance = 79F;
		else if (this.thisWeaponsName.endsWith("_85km")) this.distance = 84F;
		else if (this.thisWeaponsName.endsWith("_90km")) this.distance = 89F;
		else if (this.thisWeaponsName.endsWith("_95km")) this.distance = 94F;
		else if (this.thisWeaponsName.endsWith("_100km")) this.distance = 99F;
		else if (this.thisWeaponsName.endsWith("_125km")) this.distance = 124F;
		else if (this.thisWeaponsName.endsWith("_150km")) this.distance = 149F;
		else if (this.thisWeaponsName.endsWith("_175km")) this.distance = 174F;
		else if (this.thisWeaponsName.endsWith("_200km")) this.distance = 199F;
		else if (this.thisWeaponsName.endsWith("_225km")) this.distance = 224F;
		else if (this.thisWeaponsName.endsWith("_250km")) this.distance = 249F;
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (!this.typeDockableIsDocked()) {
			...
			} else if (s.startsWith("xkeel")) this.hitChunk("Engine1", shot);
			else if (s.startsWith("xstabl")) this.hitChunk("WingLIn", shot);
			else if (s.startsWith("xstabr")) this.hitChunk("WingRIn", shot);
			else if (s.startsWith("xwinglin")) {
				if (this.chunkDamageVisible("WingLIn") < 1) this.hitChunk("WingLIn", shot);
			} else if (s.startsWith("xwingrin")) {
				if (this.chunkDamageVisible("WingRIn") < 1) this.hitChunk("WingRIn", shot);
			} else if (s.startsWith("xengine") && this.chunkDamageVisible("Engine1") < 1) this.hitChunk("Engine1", shot);
	...
	protected void doExplosion() {
		...
		if (this.FM.Loc.z - 10D < World.land().HQ_Air(this.FM.Loc.x, this.FM.Loc.y)) {
			if (Engine.land().isWater(this.FM.Loc.x, this.FM.Loc.y)) Explosions.BOMB1000a_Water(this.FM.Loc, 1.0F, 1.0F);
			else Explosions.BOMB1000a_Land(this.FM.Loc, 1.0F, 1.0F, false);
		} else Explosions.BOMB1000a_Object(this.FM.Loc, 1.0F, 1.0F);
	}
	...
	public void update(float f) {
		...
		if (this.FM instanceof Maneuver) if (this.typeDockableIsDocked()) {
			...
			if (this.FM.EI.engines[0].getStage() == 0) {
				if (this.bStalled && (this.FM.getSpeedKMH() <= 350F || (float) this.FM.Loc.z >= 4000F)) {
					...
					if (this.bHasEngine && this.FM.M.fuel > 0.0F) this.FM.EI.setEngineRunning();
				...
				} else if (!this.bStalled && (float) this.FM.Loc.z < 4000F) this.FM.EI.setEngineRunning();
		...
		if (this.FM.AS.isMaster()) {
			...
			if (this.FM.isTick(44, 0) && !this.bBombtargets && this.bBombdrop) {
				...
				if (this.distance < f3) this.bBombtargets = true;
			}
			...
			} else if (this.FM.M.fuel == 0.0F && !this.typeDockableIsDocked()) this.FM.EI.setEngineStops();
		}
		if (!this.isNetMirror() && !this.typeDockableIsDocked()) {
			if (this.FM.CT.bHasAileronControl) if (this.FM.Or.getKren() < -15F || this.FM.Or.getKren() > 15F) {
				this.FM.CT.bHasAileronControl = false;
				this.FM.CT.bHasRudderControl = false;
				this.FM.CT.bHasElevatorControl = false;
			} else this.FM.SensRoll = (90F - Math.abs(this.FM.Or.getKren())) / 2880F;
			if (this.FM.getSpeedKMH() > 200F && !this.bBombtargets) if (this.FM.getVertSpeed() > 0.01F) {
				if (this.trimmer > -0.98F) this.trimmer = this.FM.CT.getTrimElevatorControl() - 0.01F;
		...
		if (Config.isUSE_RENDER()) if (this.FM.EI.getThrustOutput() > 0.0F) {
			if (this.soundfx != null) this.soundfx.setVolume(1.0F);
			...
			if (this.soundfx != null) this.soundfx.setVolume(0.0F);
			Eff3DActor.setIntesity(this.sprite, 0.0F);
			Eff3DActor.setIntesity(this.flame, 0.0F);
			Eff3DActor.setIntesity(this.trail, 0.0F);
			Eff3DActor.setIntesity(this.dust, 0.0F);
	...
	public void typeDockableAttemptAttach() {
		if (this.FM.AS.isMaster() && !this.typeDockableIsDocked()) {
			...
			if (aircraft instanceof HE_111H22) ((TypeDockable) aircraft).typeDockableRequestAttach(this);
		}
	}

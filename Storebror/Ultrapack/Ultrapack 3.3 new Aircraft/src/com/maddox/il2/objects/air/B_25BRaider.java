...
public class B_25BRaider extends B_25 implements TypeBomber {
	...
	protected void nextDMGLevel(String s, int i, Actor actor) {
		...
		if (this.FM.isPlayers()) B_25BRaider.bChangedPit = true;
	}
	...
	public void typeBomberAdjDistancePlus() {
		...
		this.fSightCurDistance = B_25BRaider.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
	...
	public void typeBomberUpdate(float f) {
		...
		if (this.fSightCurReadyness < 1.0F) this.fSightCurReadyness += 0.0333333F * f;
			...
			this.fSightCurDistance -= B_25BRaider.toMetersPerSecond(this.fSightCurSpeed) * f;
			if (this.fSightCurDistance < 0.0F) {
				this.fSightCurDistance = 0.0F;
				this.typeBomberToggleAutomation();
			}
			this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / B_25BRaider.toMeters(this.fSightCurAltitude)));
			if (this.fSightCurDistance < B_25BRaider.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(B_25BRaider.toMeters(this.fSightCurAltitude) * (2F / 9.81F))) this.bSightBombDump = true;
	...
	static {
		Class class1 = B_25BRaider.class;
		new NetAircraft.SPAWN(class1);
		Property.set(class1, "iconFar_shortClassName", "B-25");
		Property.set(class1, "meshName", "3DO/Plane/B-25BRaider(Multi1)/hier.him");
		Property.set(class1, "PaintScheme", new PaintSchemeBMPar02());
		Property.set(class1, "meshName_us", "3DO/Plane/B-25BRaider(USA)/hier.him");
		Property.set(class1, "PaintScheme_us", new PaintSchemeBMPar02());
		Property.set(class1, "yearService", 1943F);
		Property.set(class1, "yearExpired", 1956.6F);
		Property.set(class1, "FlightModel", "FlightModels/B-25Doolittle.fmd");
		Property.set(class1, "cockpitClass", new Class[] { CockpitB25BRaider.class, CockpitB25BRaider_Bombardier.class, CockpitB25Braider_FGunner.class, CockpitB25BRaider_TGunner.class });
		Property.set(class1, "LOSElevation", 0.73425F);
		Aircraft.weaponTriggersRegister(class1, new int[] { 10, 11, 11, 3, 3 });
		Aircraft.weaponHooksRegister(class1, new String[] { "_MGUN02", "_MGUN03", "_MGUN04", "_BombSpawn01", "_BombSpawn02" });
	}
}

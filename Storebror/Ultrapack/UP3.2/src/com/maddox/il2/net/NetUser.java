...
public class NetUser extends NetHost implements NetFileClient, NetUpdate {
	...
	public void setPatchLevel(String patchLevel) {
		if (this.patchLevel != patchLevel) { System.out.println("Player " + this.uniqueName() + " uses Patch Level " + patchLevel + "!"); }
	...
	public void setSelectorVersion(String selectorVersion) {
		if (this.selectorVersion != selectorVersion) { System.out.println("Player " + this.uniqueName() + " uses Selector Version " + selectorVersion + "!"); }
	...
	public void setUltrapackVersion(String ultrapackVersion) {
		if (this.ultrapackVersion != ultrapackVersion) { System.out.println("Player " + this.uniqueName() + " uses Ultrapack Version " + ultrapackVersion + "!"); }
...
// e.printStackTrace();
	...
	public void netUpdate() {
		// TODO: +++ New "slap" command implementation by SAS~Storebror +++
		if (!this.isMaster() && this.remainingSlaps > 0) { this.doRemainingSlaps(); }
		// TODO: --- New "slap" command implementation by SAS~Storebror ---
		if (!this.isMaster()) { return; }
...
// System.out.println("Replicating Selector Version " + theNetUser.getSelectorVersion() + " for user " + theNetUser.uniqueName() + " to " + (netchannel == null ? "all" : "Channel " + netchannel.id()));
	...
	private void checkUpdateChatMessages() {
		...
		for (int j = 0; j < list.size(); j++) {
			NetUser netuser = (NetUser) list.get(j);
			if (netuser.getUltrapackVersion().equalsIgnoreCase(UP_VERSION) && netuser.getPatchLevel().equalsIgnoreCase(PATCH_LEVEL)) { continue; }
...
// System.out.println(this.uniqueName() + " sends Patch Update Message to " + netuser.uniqueName() + " (using Patch Level " + netuser.getPatchLevel() + ")");
			if (netuser.getUltrapackVersion().equalsIgnoreCase("unknown") || netuser.getPatchLevel().equalsIgnoreCase("none")) {
				...
				Main.cur().chat.send(NetEnv.host(), "!! " + netuser.uniqueName() + " !! You've got Ultrapack " + netuser.getUltrapackVersion() + " Patch " + netuser.getPatchLevel() + " installed !!", arraylist, (byte) 0, false);
			}
			Main.cur().chat.send(NetEnv.host(), "!! " + netuser.uniqueName() + " !! New Version/Patch " + UP_VERSION + "/" + PATCH_LEVEL + " is available at <<< https://j.mp/sas-up3 >>> !!", arraylist, (byte) 0, false);
		}
	}
...
// ---
	...
	public NetUser(String s) {
		...
		if (this.isMaster()) {
			...
			this.ultrapackVersion = UP_VERSION;
		}
// if (Config.isUSE_RENDER())
...
// System.out.println("MSG_ULTRAPACK_VERSION received (Master = " + isMasterChannel + ") for netuser null, Ultrapack Version is " + theUltrapackVersion);
	...
	public void slap(NetUser netuser, int numSlaps) {
		...
		if (!Actor.isValid(aircraft) || !Actor.isAlive(aircraft)) { return; }
	...
	private void doRemainingSlaps() {
		do {
			...
			Aircraft aircraft = this.findAircraft();
			if (!Actor.isValid(aircraft) || !Actor.isAlive(aircraft)) { break; }
			...
			System.out.println("Slapping " + this.uniqueName() + " (aircraft:" + aircraft.netName() + "), slap point is " + twoDigits.format(Math.abs(t.x)) + "m " + (t.x > 0 ? "in front of" : "behind     ") + ", " + twoDigits.format(Math.abs(t.y)) + "m "
					+ (t.y > 0 ? "right" : " left") + " of and " + twoDigits.format(Math.abs(t.z)) + "m " + (t.z > 0 ? "above" : "below") + " the aircraft. Remaining slaps: " + --this.remainingSlaps);
			return;
		} while (false);

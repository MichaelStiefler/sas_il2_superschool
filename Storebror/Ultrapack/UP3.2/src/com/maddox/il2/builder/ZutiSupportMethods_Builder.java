...
//                    System.out.println("zutiHomeBaseCapturedBlueCountries add default 2");
	...
	public static void loadBornPlaceCountries_oldMDS(ActorBorn actorborn, SectFile sectfile)
	{
		...
		if (count >= 0)
		{
            ...
            // TODO: +++ Add Default Country "None" Mod by SAS~Storebror +++
            if (Config.cur.bAddDefaultCountryNone) {
                if( !actorborn.zutiHomeBaseCountries.contains("None") ) {
//                    System.out.println("zutiHomeBaseCountries old add default");
		...
		if (count >= 0)
		{
            ...
            // TODO: +++ Add Default Country "None" Mod by SAS~Storebror +++
            if (Config.cur.bAddDefaultCountryNone) {
                if( !actorborn.zutiHomeBaseCapturedRedCountries.contains("None") ) {
//                    System.out.println("zutiHomeBaseCapturedRedCountries old add default");
		...
		if (count >= 0)
		{
            ...
            // TODO: +++ Add Default Country "None" Mod by SAS~Storebror +++
            if (Config.cur.bAddDefaultCountryNone) {
                if( !actorborn.zutiHomeBaseCapturedBlueCountries.contains("None") ) {
	...
	public static GWindowTabDialogClient.Tab getPlMisBornSpawnTab()
	{
		...
		gwindowdialogclient_Spawn.addControl(new GWindowComboControl(gwindowdialogclient_Spawn, 30.0F, 15.0F, 5.0F)
		{
			...
			public void preRender()
			{
				...
				if( actorborn.zutiCarrierSpawnPoints < 1 )
					return;
				
				for( int i=0; i<actorborn.zutiCarrierSpawnPoints; i++ )
					this.add(new Integer(i+1).toString());
				
				if( actorborn.zutiCarrierSelectedSpawnPointsIndex > this.size() || actorborn.zutiCarrierSelectedSpawnPointsIndex-1 < 0 )
					setSelected(this.size()-1, true, false);
				else
					setSelected(actorborn.zutiCarrierSelectedSpawnPointsIndex-1, true, false);
			}

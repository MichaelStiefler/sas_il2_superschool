...
//                indx = chunkFind(s);
            ...
            getChunkLocObj(HierMesh._chunkLoc);
            x = (float) HierMesh._chunkLoc.getX();
            y = (float) HierMesh._chunkLoc.getY();
            z = (float) HierMesh._chunkLoc.getZ();
            yaw = HierMesh._chunkLoc.getOrient().getYaw();
            pitch = HierMesh._chunkLoc.getOrient().getPitch();
            roll = HierMesh._chunkLoc.getOrient().getRoll();
        }
    }
    ...
    public int detectCollision(Loc loc, HierMesh hiermesh, Loc loc1) {
        loc.get(tmp);
        loc1.get(tmp2);
        return DetectCollision(cppObj, tmp, hiermesh.cppObj, tmp2);
    }

    public int detectCollision(Loc loc, Mesh mesh, Loc loc1) {
        loc.get(tmp);
        loc1.get(tmp2);
        return DetectCollisionMesh(cppObj, tmp, mesh.cppObj, tmp2);
    }
    ...
    public float detectCollisionLine(Loc loc, Point3d point3d, Point3d point3d1) {
        ...
        point3d.get(ad);
        point3d1.get(tmp2);
        return DetectCollisionLine(cppObj, tmp, ad, tmp2);
    }
    ...
    public int detectCollisionLineMulti(Loc loc, Point3d point3d, Point3d point3d1) {
        ...
        point3d.get(ad);
        point3d1.get(tmp2);
        return DetectCollisionLineMulti(cppObj, tmp, ad, tmp2);
    }
    ...
    public int detectCollision_Quad_Multi(Loc loc, Point3d point3d, Point3d point3d1, Point3d point3d2, Point3d point3d3) {
        ...
        tmp2[10] = point3d3.y;
        tmp2[11] = point3d3.z;
        return DetectCollisionQuadMultiH(cppObj, tmp, tmp2);
    }
    ...
    public boolean isChunkVisible(String s) {
        ChunkState chunkstate = chunkState(s);
        if (chunkstate == null)
            return false;
        else
    ...
    private ChunkState chunkState(String s) {
        ...
        ChunkState chunkstate = (ChunkState) chunkMap.get(s);
        if (chunkstate == null) {
...
//        System.out.println("chunkFind(" + s + ") = " + retVal);
    ...
    public int hookFaceCollisFind(Matrix4d matrix4d, int ai[], Matrix4d matrix4d1) {
        ...
        tmp[11] = matrix4d.m32;
        tmp[15] = matrix4d.m33;
        int i = HookFaceCollisFind(cppObj, tmp, ai, tmp2);
        if (i != -1)
            matrix4d1.set(tmp2);
        return i;
    }
    ...
    public int hookChunkFaceFind(Matrix4d matrix4d, Matrix4d matrix4d1) {
        ...
        tmp[11] = matrix4d.m32;
        tmp[15] = matrix4d.m33;
        int i = HookChunkFaceFind(cppObj, tmp, tmp2);
        if (i != -1)
            matrix4d1.set(tmp2);
        return i;
    }
    ...
    public int hookFaceFind(Matrix4d matrix4d, int ai[], Matrix4d matrix4d1) {
        ...
        tmp[11] = matrix4d.m32;
        tmp[15] = matrix4d.m33;
        int i = HookFaceFind(cppObj, tmp, ai, tmp2);
        if (i != -1)
            matrix4d1.set(tmp2);
        return i;
    }
    ...
    public int materialFind(String s, int i, int j) {
        int retVal = MaterialFind(cppObj, s, i, j);
        if (retVal < 0) {
            printDebugMessage(DEBUG_DETAILED, "HierMesh Debugger Error in materialFind(" + s + ", " + i + ", " + j + "): Material '" + s + "' does not exist.");
        }
        return retVal;
    }

    public int materialFind(String s, int i) {
        int retVal = MaterialFind(cppObj, s, i, -1);
        if (retVal < 0) {
            printDebugMessage(DEBUG_DETAILED, "HierMesh Debugger Error in materialFind(" + s + ", " + i + "): Material '" + s + "' does not exist.");
        }
        return retVal;
    }

    public int materialFind(String s) {
        int retVal = MaterialFind(cppObj, s, 0, -1);
        if (retVal < 0) {
            printDebugMessage(DEBUG_DETAILED, "HierMesh Debugger Error in materialFind(" + s + "): Material '" + s + "' does not exist.");
        }
        return retVal;
    }

    public int materialFindInChunk(String s, int i) {
        int retVal = MaterialFindInChunk(cppObj, s, i);
        if (retVal < 0) {
            printDebugMessage(DEBUG_DETAILED, "HierMesh Debugger Error in materialFindInChunk(" + s + ", " + i + "): Material '" + s + "' does not exist.");
        }
        return retVal;
    }

    public Mat material(int i) {
        return (Mat) Material(cppObj, i);
    }
    ...
    public boolean isVisualRayHit(Point3d point3d, Vector3d vector3d, double d, double d1, Matrix4d matrix4d) {
        ...
        return isVisualRayHit(cppObj, arrDouble6, d, d1, tmp) != 0;
    }
    ...
    public int ApplyDecal_test(float f, float f1, boolean flag, Loc loc, int i) {
        ...
        int k = ApplyDecal(cppObj, rnd.nextInt(0, 2), f, f1, flag ? 1 : 0, tmp, af, i, ai, ai.length);
    ...
    public HierMesh(HierMesh hiermesh, int i, Loc loc) {
        ...
        tmp[11] = m4.m32;
        tmp[15] = m4.m33;
        cppObj = LoadSubtreeLoc(hiermesh.cppObj, tmp);
        if (cppObj == 0) {

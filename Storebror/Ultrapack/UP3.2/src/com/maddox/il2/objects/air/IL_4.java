...
public abstract class IL_4 extends Scheme2 {
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				if (s.endsWith("p1")) {
					this.getEnergyPastArmor(World.Rnd().nextFloat(12.5F, 16F), shot);
				} else if (s.endsWith("p2")) {
					this.getEnergyPastArmor(1.0099999904632568D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
			...
			if (s.startsWith("xxcontrols")) {
				...
				switch (i) {
					...
					case 3:
						if (this.getEnergyPastArmor(2.2F, shot) <= 0.0F) { break; }
					...
					case 8:
						if (this.getEnergyPastArmor(0.12F, shot) <= 0.0F) { break; }
			...
			if (s.startsWith("xxspar")) {
				if (s.startsWith("xxsparli") && this.chunkDamageVisible("WingLIn") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
					this.debuggunnery("*** WingLIn Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingLIn_D3", shot.initiator);
				}
				if (s.startsWith("xxsparri") && this.chunkDamageVisible("WingRIn") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
					this.debuggunnery("*** WingRIn Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingRIn_D3", shot.initiator);
				}
				if (s.startsWith("xxsparlm") && this.chunkDamageVisible("WingLMid") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
					this.debuggunnery("*** WingLMid Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingLMid_D3", shot.initiator);
				}
				if (s.startsWith("xxsparrm") && this.chunkDamageVisible("WingRMid") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
					this.debuggunnery("*** WingRMid Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingRMid_D3", shot.initiator);
				}
				if (s.startsWith("xxsparlo") && this.chunkDamageVisible("WingLOut") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
					this.debuggunnery("*** WingLOut Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingLOut_D3", shot.initiator);
				}
				if (s.startsWith("xxsparro") && this.chunkDamageVisible("WingROut") > 2 && this.getEnergyPastArmor(26.799999237060547D / (1.0001000165939331D - Math.abs(Aircraft.v1.y)), shot) > 0.0F) {
			...
			if (s.startsWith("xxeng")) {
				...
				if (s.endsWith("eqpt")) {
					if (this.getEnergyPastArmor(0.1F, shot) > 0.0F) {
						if (Aircraft.Pd.y > 0.0D && Aircraft.Pd.z < 0.18899999558925629D && World.Rnd().nextFloat(0.0F, 16000F) < shot.power) { this.FM.EI.engines[j].setMagnetoKnockOut(shot.initiator, 0); }
						if (Aircraft.Pd.y < 0.0D && Aircraft.Pd.z < 0.18899999558925629D && World.Rnd().nextFloat(0.0F, 16000F) < shot.power) { this.FM.EI.engines[j].setMagnetoKnockOut(shot.initiator, 1); }
	...
	public void msgExplosion(Explosion explosion) {
		...
		if (explosion.chunkName != null && explosion.power > 0.0F) {
			...
			if (explosion.chunkName.equals("Engine1_D2")) { return; }
			if (explosion.chunkName.equals("Engine2_D2")) { return; }
		}
			...
			if (this.FM.turret.length != 0) {
				if (this.FM.turret[0].bIsOperable != (this.radist[1] == 0)) {
					...
					if (actor != null) {
						...
						Actor actor3 = this.FM.turret[1].target;
						if (actor3 == null) { actor3 = this.FM.turret[2].target; }
						if (actor3 != null) {
							this.setRadist(1, 1);
							this.FM.turret[0].target = actor3;
						} else {
							this.setRadist(1, 0);
						}
					}
				...
				if (this.FM.turret[1].bIsOperable) {
					...
					Actor actor2 = this.FM.turret[2].target;
					if (actor2 != null && Actor.isValid(actor2)) {
						this.pos.getAbs(Aircraft.tmpLoc2);
						actor2.pos.getAbs(Aircraft.tmpLoc3);
						Aircraft.tmpLoc2.transformInv(Aircraft.tmpLoc3.getPoint());
						if (Aircraft.tmpLoc3.getPoint().z > 0.0D) { this.setRadist(2, 0); }
					}
				}
			}
		}
				...
				case 1:
					...
					switch (j) {
						case 0:
							...
							this.hierMesh().chunkVisible("HMask2_D0", this.FM.Loc.z > 3000D);
							break;

						case 1:
							this.hierMesh().chunkVisible("Pilot2a_D0", true);
							this.hierMesh().chunkVisible("HMask2a_D0", this.FM.Loc.z > 3000D);
							this.FM.turret[0].bIsOperable = true;
							break;
					}
					break;
				...
				case 2:
					...
					switch (j) {
						case 0:
							...
							this.hierMesh().chunkVisible("HMask3_D0", this.FM.Loc.z > 3000D);
							this.FM.turret[1].bIsOperable = true;
							break;

						case 1:
							this.hierMesh().chunkVisible("Pilot3a_D0", true);
							this.hierMesh().chunkVisible("HMask3a_D0", this.FM.Loc.z > 3000D);
							this.FM.turret[2].bIsOperable = true;
							break;
					}
					break;

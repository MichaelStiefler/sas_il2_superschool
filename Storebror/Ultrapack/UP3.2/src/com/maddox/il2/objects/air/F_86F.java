	...
	private class _cls0 {
		...
		private void $1() {
			if (F_86F.this.ts) {
				...
				if (f2 > 0.6F) {
					F_86F.this.ictl = true;
				} else {
					F_86F.this.ictl = false;
				}
				if (this.ftl > 0.0F) {
					if (World.Rnd().nextFloat() > 0.6F) {
						if (F_86F.this.FM.CT.RudderControl > 0.0F) {
							F_86F.this.FM.CT.RudderControl -= this.ftl * f2;
						} else if (F_86F.this.FM.CT.RudderControl < 0.0F) {
							F_86F.this.FM.CT.RudderControl += this.ftl * f2;
						} else {

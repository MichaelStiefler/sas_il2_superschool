...
public abstract class ASDGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(ASDGeneric.this.engineSFX != null)
                if(ASDGeneric.this.engineSTimer >= 0)
                {
                    if(--ASDGeneric.this.engineSTimer <= 0)
                    {
                        ASDGeneric.this.engineSTimer = (int)ASDGeneric.SecsToTicks(ASDGeneric.Rnd(5F, 5F));
                        if(!ASDGeneric.this.danger())
                            ASDGeneric.this.engineSTimer = -ASDGeneric.this.engineSTimer;
                    }
                } else
                if(++ASDGeneric.this.engineSTimer >= 0)
                {

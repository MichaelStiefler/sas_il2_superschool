...
public abstract class NAVGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(NAVGeneric.this.engineSFX != null)
                if(NAVGeneric.this.engineSTimer >= 0)
                {
                    if(--NAVGeneric.this.engineSTimer <= 0)
                    {
                        NAVGeneric.this.engineSTimer = (int)NAVGeneric.SecsToTicks(NAVGeneric.Rnd(180F, 180F));
                        if(!NAVGeneric.this.danger())
                            NAVGeneric.this.engineSTimer = -NAVGeneric.this.engineSTimer;
                    }
                } else
                if(++NAVGeneric.this.engineSTimer >= 0)
                {

...
public abstract class RCGCIGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(RCGCIGeneric.this.engineSFX != null)
                if(RCGCIGeneric.this.engineSTimer >= 0)
                {
                    if(--RCGCIGeneric.this.engineSTimer <= 0)
                    {
                        RCGCIGeneric.this.engineSTimer = (int)RCGCIGeneric.SecsToTicks(RCGCIGeneric.Rnd(30F, 30F));
                        if(!RCGCIGeneric.this.danger())
                            RCGCIGeneric.this.engineSTimer = -RCGCIGeneric.this.engineSTimer;
                    }
                } else
                if(++RCGCIGeneric.this.engineSTimer >= 0)
                {
    ...
    public boolean danger()
    {
        ...
        int j = 0;
        if(this.mainforcefound)
            while(j < 99 && ((Maneuver)aircraft.FM).Skill != 2) 
            {
                aircraft = War.GetNearestEnemyAircraft(World.getPlayerAircraft(), 500000F, 9);
                if(++j == 98)
                    this.destroy();
        ...
        if(((Maneuver)aircraft.FM).Skill >= 2 && (aircraft instanceof TypeBomber) && aircraft.pos.getAbsPoint().distance(point3d) < 200000D && aircraft.getSpeed(vector3d) > 20D && aircraft.pos.getAbsPoint().z >= 150D)
        {
            ...
            if(l < 0)
                l = 360 + l;
            if(this.mainforcefound)
                HUD.logCenter("                                          Confirmed Mainforce at " + s + "-" + d5 + ", heading " + l + "\260");
            else
            if(!this.mainforcefound)
                HUD.logCenter("                                          Incoming Raid at " + s + "-" + d5 + ", heading " + l + "\260");
            if(!this.mainforcefound && !((Maneuver)aircraft.FM).hasBombs() && ((Maneuver)aircraft.FM).Skill == 2)

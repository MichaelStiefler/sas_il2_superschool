...
public abstract class JU_52 extends Scheme6 implements TypeTransport {
	...
	public void msgShot(Shot shot) {
		...
		if (shot.chunkName.startsWith("Tail1") && Pd.z > 0.5D && Pd.x > -6D && Pd.x < -4.9499998092651367D && World.Rnd().nextFloat() < 0.5F) { this.FM.AS.hitPilot(shot.initiator, 2, (int) (shot.mass * 1000F * 0.5F)); }
		if (shot.chunkName.startsWith("CF") && v1.x < -0.2D && Pd.x > 2.5999999046325684D && Pd.z > 0.735D && World.Rnd().nextFloat() < 0.178F) { this.FM.AS.hitPilot(shot.initiator, Pd.y <= 0.0D ? 1 : 0, (int) (shot.mass * 900F)); }
		if (shot.chunkName.startsWith("WingLIn") && Math.abs(Pd.y) < 2.0999999046325684D) { this.FM.AS.hitTank(shot.initiator, 0, World.Rnd().nextInt(0, (int) (shot.mass * 30F))); }
		if (shot.chunkName.startsWith("WingRIn") && Math.abs(Pd.y) < 2.0999999046325684D) { this.FM.AS.hitTank(shot.initiator, 0, World.Rnd().nextInt(1, (int) (shot.mass * 30F))); }
		super.msgShot(shot);
	}

...
//        mesh.chunkSetAngles("Z_Need_compass", f1, 0.0F, 0.0F);
	...
	private void initCompass() {
		...
		this.compassSc = 0.10193679899999999D / Time.tickLenFs() / Time.tickLenFs();
	}
	...
	private void updateCompass() {
		...
		if (this.accel.length() > -compassZ * 0.69999999999999996D) { this.accel.scale(-compassZ * 0.69999999999999996D / this.accel.length()); }
		for (int i = 0; i < 4; i++) {
		...
		this.mesh.chunkSetAngles("Z_Need_compassBase", -(float) (d2 * 180D / 3.1415926000000001D), -(float) (d3 * 180D / 3.1415926000000001D), 0.0F);
		this.mesh.chunkSetAngles("Z_Need_compass", 0.0F, (float) (90D - d4 * 180D / 3.1415926000000001D), 0.0F);
		this.compassFirst++;
	}

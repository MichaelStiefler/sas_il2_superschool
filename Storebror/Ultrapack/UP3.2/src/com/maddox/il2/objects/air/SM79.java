...
public abstract class SM79 extends Scheme6 {
	...
	public void onAircraftLoaded() {
		...
		if (this.thisWeaponsName.startsWith("12") || this.thisWeaponsName.startsWith("6")) {
			for (int i = 1; i <= 12; i++) {
				this.hierMesh().chunkVisible("BombRack" + i + "_D0", true);
				this.numBombsOld = 12;
		...
		if (this.thisWeaponsName.startsWith("5")) {
			for (int j = 1; j <= 5; j++) {
				this.hierMesh().chunkVisible("BombRack250_" + j + "_D0", true);
				this.numBombsOld = 5;
			}

		}
		if (this.thisWeaponsName.startsWith("2")) {
			this.hierMesh().chunkVisible("BombRack500_1_D0", true);
			this.hierMesh().chunkVisible("BombRack500_2_D0", true);
			this.numBombsOld = 2;
		}
	...
	protected void drawBombs() {
		...
		if (this.thisWeaponsName.endsWith("drop")) {
			...
			if (this.FM.CT.Weapons[3] != null) {
				...
				if (this.thisWeaponsName.startsWith("2")) {
					if (i == 2) {
						this.hierMesh().chunkVisible("Bomb500Kg2_D0", true);
					} else {
						this.hierMesh().chunkVisible("Bomb500Kg2_D0", false);
					}
				}
			}
		}
	...
	public void update(float f) {
		...
		if (this.bPitUnfocused && Config.isUSE_RENDER()) {
			com.maddox.il2.engine.Mat mat = this.hierMesh().material(this.hierMesh().materialFind("InteriorFake1"));
			this.hierMesh().materialReplace("InteriorFake", mat);
		}
		float f1 = 0.0F;

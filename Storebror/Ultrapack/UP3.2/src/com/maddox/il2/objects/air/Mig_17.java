...
public class Mig_17 extends Scheme1 implements TypeSupersonic, TypeFighter, TypeBNZFighter, TypeFighterAceMaker {
			...
			if (Mig_17.this.transsonic) {
				...
				if (this.flutterLevel > 0.0F) {
					if (World.Rnd().nextFloat() > 0.6F) {
						if (Mig_17.this.FM.CT.RudderControl > 0.0F) {
							Mig_17.this.FM.CT.RudderControl -= this.flutterLevel * f2;
						} else if (Mig_17.this.FM.CT.RudderControl < 0.0F) {
							Mig_17.this.FM.CT.RudderControl += this.flutterLevel * f2;
						} else {
	...
	private final void updateMachNumber() {
		...
		if (this.machNumber >= lowerTranssonicEffectBoundary) {
			this.transsonic = true;
		} else {
			this.transsonic = false;
		}
	}

...
public abstract class ASGGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(ASGGeneric.this.engineSFX != null)
                if(ASGGeneric.this.engineSTimer >= 0)
                {
                    if(--ASGGeneric.this.engineSTimer <= 0)
                    {
                        ASGGeneric.this.engineSTimer = (int)ASGGeneric.SecsToTicks(ASGGeneric.Rnd(5F, 5F));
                        if(!ASGGeneric.this.danger())
                            ASGGeneric.this.engineSTimer = -ASGGeneric.this.engineSTimer;
                    }
                } else
                if(++ASGGeneric.this.engineSTimer >= 0)
                {

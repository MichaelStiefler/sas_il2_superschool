	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				if (s.endsWith("p1")) {
					if (v1.z > 0.5D) {
						this.getEnergyPastArmor(5D / v1.z, shot);
					...
					this.getEnergyPastArmor(5D / Math.abs(v1.z), shot);
				} else if (s.endsWith("p3a") || s.endsWith("p3b")) {
					this.getEnergyPastArmor(8D / Math.abs(v1.x) * World.Rnd().nextFloat(1.0F, 1.2F), shot);
				} else if (s.endsWith("p4")) {
					if (v1.x > 0.70710676908493042D) {
						this.getEnergyPastArmor(8D / v1.x * World.Rnd().nextFloat(1.0F, 1.2F), shot);

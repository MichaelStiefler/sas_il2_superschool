...
public abstract class OBOEGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(OBOEGeneric.this.engineSFX != null)
                if(OBOEGeneric.this.engineSTimer >= 0)
                {
                    if(--OBOEGeneric.this.engineSTimer <= 0)
                    {
                        OBOEGeneric.this.engineSTimer = (int)OBOEGeneric.SecsToTicks(OBOEGeneric.Rnd(1.0F, 1.0F));
                        if(!OBOEGeneric.this.danger())
                            OBOEGeneric.this.engineSTimer = -OBOEGeneric.this.engineSTimer;
                    }
                } else
                if(++OBOEGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        {
            this.pos.getAbs(point3d);
            double d = Main3D.cur3D().land2D.worldOfsX() + ((Tuple3d) (point3d)).x;
            double d1 = Main3D.cur3D().land2D.worldOfsY() + ((Tuple3d) (point3d)).y;
            ...
            double d15 = d7 - World.land().HQ(((Tuple3d) (point3d)).x, ((Tuple3d) (point3d)).y);
            if(d15 < 0.0D)
                d15 = 0.0D;
            double d16 = d10 - d11;
            String s = "<<<";

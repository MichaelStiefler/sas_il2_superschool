...
public abstract class Hurricane extends Scheme1 implements TypeFighter, TypeTNBFighter {
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				...
				} else if (s.endsWith("g1") || s.endsWith("g2")) { this.getEnergyPastArmor(8F / (1E-005F + (float) Math.abs(Aircraft.v1.x)), shot); }
			}
			...
			if (s.startsWith("xxoil")) { this.FM.AS.hitOil(shot.initiator, 0); }
			...
			if (s.startsWith("xxmgunl4")) { this.FM.AS.setJamBullets(0, World.Rnd().nextInt(0, 3) * 2 + 1 - 1); }
			if (s.startsWith("xxmgunr4")) { this.FM.AS.setJamBullets(0, World.Rnd().nextInt(1, 4) * 2 - 1); }
			if (s.startsWith("xxmgunl2")) { this.FM.AS.setJamBullets(0, World.Rnd().nextInt(0, 1) * 2 + 9 - 1); }
			if (s.startsWith("xxmgunr2")) { this.FM.AS.setJamBullets(0, World.Rnd().nextInt(5, 6) * 2 - 1); }
			...
			if (s.startsWith("xxhispa1")) { this.FM.AS.setJamBullets(0, 0); }
			if (s.startsWith("xxhispa2")) { this.FM.AS.setJamBullets(0, 1); }
			if (s.startsWith("xxhispa3")) { this.FM.AS.setJamBullets(0, 2); }
			if (s.startsWith("xxhispa4")) { this.FM.AS.setJamBullets(0, 3); }
			if (s.startsWith("xxshvak1")) { this.FM.AS.setJamBullets(1, 0); }
			if (s.startsWith("xxshvak2")) { this.FM.AS.setJamBullets(1, 1); }
			if (s.startsWith("xxshkas1")) { this.FM.AS.setJamBullets(0, 0); }
			if (s.startsWith("xxshkas2")) { this.FM.AS.setJamBullets(0, 1); }
		} else if (s.startsWith("xcf") || s.startsWith("xoil")) {
			...
			if (point3d.x > -2.2D) {
				...
				if (Aircraft.v1.x < -0.89999997615814209D && World.Rnd().nextFloat() < 0.2F) { this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 1); }

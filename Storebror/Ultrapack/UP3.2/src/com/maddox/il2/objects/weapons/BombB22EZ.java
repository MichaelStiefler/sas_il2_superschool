...
//        System.out.println("BombSD2A doExplosion(" + actor.getClass().getName() + ", " + s + ")");
		...
		if (actor instanceof ActorLand) {
			if (World.Rnd().nextBoolean()) {
				MsgExplosion.send(actor, s, point3d, this.getOwner(), this.M * 100F, f * 10F, i, f1 * 25F, 0);
			} else {
				MsgExplosion.send(actor, s, point3d, this.getOwner(), this.M * 100F, f * 10F, 1, f1 * 25F, 0);
			}
		}

...
public abstract class RESCAPGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(RESCAPGeneric.this.engineSFX != null)
                if(RESCAPGeneric.this.engineSTimer >= 0)
                {
                    if(--RESCAPGeneric.this.engineSTimer <= 0)
                    {
                        ...
                        if(!RESCAPGeneric.this.danger())
                            RESCAPGeneric.this.engineSTimer = -RESCAPGeneric.this.engineSTimer;
                    }
                } else
                if(++RESCAPGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if(!this.pickup)
            {
                ...
                if(aircraft.pos.getAbsPoint().distance(point3d) < 3000D)
                    flag = true;
                this.pos.getAbs(point3d);
                double d = (Main3D.cur3D().land2D.worldOfsX() + ((Tuple3d) (point3d)).x) / 10000D;
                double d1 = (Main3D.cur3D().land2D.worldOfsY() + ((Tuple3d) (point3d)).y) / 10000D;
                ...
                if(flag)
                    ...
                    HUD.logCenter("                                                                             RESCAP required at map grid " + s1 + "-" + d7);
            }
        }

	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				...
				if (s.endsWith("f1")) {
					this.getEnergyPastArmor(World.Rnd().nextFloat(8F, 12F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
		...
		if (s.startsWith("xcf") || s.startsWith("xblister")) {
			...
			if (s.startsWith("xcf3") && point3d.x > -0.645D && point3d.x < 0.40600000000000003D && point3d.z > 0.3D) { this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 0x40); }
			return;
		}

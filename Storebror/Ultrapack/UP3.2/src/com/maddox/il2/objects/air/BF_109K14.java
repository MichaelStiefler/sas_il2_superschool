...
public class BF_109K14 extends BF_109 implements TypeFighter, TypeBNZFighter, TypeStormovik {
	...
	public void update(float f) {
		...
		if (this.FM.CT.getCockpitDoor() > 0.2D && this.bHasBlister && this.FM.getSpeedKMH() > this.fMaxKMHSpeedForOpenCanopy && this.hierMesh().chunkFindCheck("Blister1_D0") != -1) {
			try {
				if (this == World.getPlayerAircraft()) { ((CockpitBF_109K4) Main3D.cur3D().cockpitCur).removeCanopy(); }
		...
		if (this.FM.isPlayers()) {
			...
			if (point3d.z - World.land().HQ(point3d.x, point3d.y) < 0.0099999997764825821D) { this.hierMesh().chunkVisible("CF_D0", true); }
	...
	public void moveSteering(float f) {
		if (this.FM.CT.getGear() >= 0.98F) { this.hierMesh().chunkSetAngles("GearC2_D0", 90F, -f, 0.0F); }
	}
	...
	public void onAircraftLoaded() {
		...
		if (this.getGunByHookName("_MGUN01") instanceof GunEmpty) {
			this.hierMesh().chunkVisible("GunL_D0", false);
			this.hierMesh().chunkVisible("GunR_D0", false);
		}
		if (this.getGunByHookName("_CANNON02") instanceof GunEmpty) { this.hierMesh().chunkVisible("GunL_D0", false); }
		if (this.getGunByHookName("_CANNON03") instanceof GunEmpty) { this.hierMesh().chunkVisible("GunR_D0", false); }
	}
	...
	static {
		...
		Aircraft.weaponTriggersRegister(class1, new int[] { 0, 0, 1, 1, 1, 9, 3, 3, 3, 3, 3 });
		Aircraft.weaponHooksRegister(class1, new String[] { "_MGUN01", "_MGUN02", "_CANNON01", "_CANNON02", "_CANNON03", "_ExternalDev01", "_ExternalBomb01", "_ExternalBomb02", "_ExternalBomb03", "_ExternalBomb04", "_ExternalBomb05" });
	}
}

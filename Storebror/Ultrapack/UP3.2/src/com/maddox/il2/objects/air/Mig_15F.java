	...
	private class _cls0 {
		...
		private void $1() {
			if (Mig_15F.this.ts) {
				...
				if (f2 > 0.6F) {
					Mig_15F.this.ictl = true;
				} else {
					Mig_15F.this.ictl = false;
				}
				if (this.ftl > 0.0F) {
					if (World.Rnd().nextFloat() > 0.6F) {
						if (Mig_15F.this.FM.CT.RudderControl > 0.0F) {
							Mig_15F.this.FM.CT.RudderControl -= this.ftl * f2;
						} else if (Mig_15F.this.FM.CT.RudderControl < 0.0F) {
							Mig_15F.this.FM.CT.RudderControl += this.ftl * f2;
						} else {

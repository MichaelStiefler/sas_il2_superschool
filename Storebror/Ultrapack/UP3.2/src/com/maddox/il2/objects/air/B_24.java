			...
			if (string.startsWith("xxcontrols")) {
				...
				if (string.endsWith("case")) {
					if (this.getEnergyPastArmor(0.2F, shot) > 0.0F) {
						...
						this.FM.EI.engines[i_3_].setCyliderKnockOut(shot.initiator, 1);
					} else {
				...
				this.getEnergyPastArmor(World.Rnd().nextFloat(5.0F, 25.0F), shot);
				...
				if (this.getEnergyPastArmor(0.06F, shot) > 0.0F) {
					...
					if (shot.powerType == 3) {
						if (shot.power < 16100.0F) {
							...
							this.FM.AS.hitTank(shot.initiator, i_5_, World.Rnd().nextInt(1, 1 + (int) (shot.power / 16100.0F)));
						}

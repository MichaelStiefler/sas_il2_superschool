...
public abstract class F4U extends Scheme1 implements TypeFighter, TypeDiveBomber {
	...
	protected void moveGear(float f) {
		...
		if (bGearExtending) {
			...
			this.cGearPos = cvt(f, 8.5F, 10F, 0.0F, 1.0F);
		}
	}
		...
		if (this.FM.Gears.arrestorVAngle != 0.0F) {
			...
			if (this.arrestorVel >= -0.1F) {
				if (Engine.cur.land.isWater(this.FM.Loc.x, this.FM.Loc.y)) {
					...
					f4 = cvt(this.FM.getSpeedKMH(), 0.0F, 250F, 0.0F, 0.75F);
				}
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				...
				if (s.endsWith("f1")) {
					this.getEnergyPastArmor(World.Rnd().nextFloat(8F, 12F) / (Math.abs(v1.z) + 9.9999997473787516E-005D), shot);
					...
					this.getEnergyPastArmor(11D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);

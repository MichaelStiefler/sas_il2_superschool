...
public abstract class SN2Generic extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(SN2Generic.this.engineSFX != null)
                if(SN2Generic.this.engineSTimer >= 0)
                {
                    if(--SN2Generic.this.engineSTimer <= 0)
                    {
                        SN2Generic.this.engineSTimer = (int)SN2Generic.SecsToTicks(SN2Generic.Rnd(5F, 5F));
                        if(!SN2Generic.this.danger())
                            SN2Generic.this.engineSTimer = -SN2Generic.this.engineSTimer;
                    }
                } else
                if(++SN2Generic.this.engineSTimer >= 0)
                {

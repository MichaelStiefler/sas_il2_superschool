...
public abstract class CantZ1007 extends Scheme6 {
	...
	public void onAircraftLoaded() {
		...
		if (this.thisWeaponsName.endsWith("wing")) {
			this.hierMesh().chunkVisible("250kgWingRackL_D0", true);
			this.hierMesh().chunkVisible("250kgWingRackR_D0", true);
		}
	}
	...
	public void rareAction(float f, boolean flag) {
		if (this.hierMesh().isChunkVisible("Prop2_D1") && this.hierMesh().isChunkVisible("Prop3_D1") && (this.hierMesh().isChunkVisible("Prop1_D0") || this.hierMesh().isChunkVisible("PropRot1_D0"))) { this.hitProp(0, 0, Engine.actorLand()); }
		if (this.hierMesh().isChunkVisible("Prop2_D1") && this.hierMesh().isChunkVisible("Prop1_D1") && (this.hierMesh().isChunkVisible("Prop3_D0") || this.hierMesh().isChunkVisible("PropRot3_D0"))) { this.hitProp(2, 0, Engine.actorLand()); }
	...
	protected void setControlDamage(Shot shot, int i) {
		if (World.Rnd().nextFloat() < 0.002F && this.getEnergyPastArmor(4.2F, shot) > 0.0F) { this.FM.AS.setControlsDamage(shot.initiator, i); }
	}
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				...
				} else if (s.endsWith("a1") || s.endsWith("a3") || s.endsWith("a4")) { this.getEnergyPastArmor(0.6F, shot); }
			}
			if (s.startsWith("xxspar")) {
				...
				if ((s.endsWith("li1") || s.endsWith("li2")) && World.Rnd().nextFloat() < 1.0D - 0.92D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingLIn") > 1
						&& this.getEnergyPastArmor(13.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					this.nextDMGLevels(1, 2, "WingLIn_D2", shot.initiator);
				}
				if ((s.endsWith("ri1") || s.endsWith("ri2")) && World.Rnd().nextFloat() < 1.0D - 0.92D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingRIn") > 1
						&& this.getEnergyPastArmor(10.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					this.nextDMGLevels(1, 2, "WingRIn_D2", shot.initiator);
				}
				if ((s.endsWith("lm1") || s.endsWith("lm2")) && World.Rnd().nextFloat() < 1.0D - 0.86D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingLMid") > 1
						&& this.getEnergyPastArmor(10.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					this.nextDMGLevels(1, 2, "WingLMid_D2", shot.initiator);
				}
				if ((s.endsWith("rm1") || s.endsWith("rm2")) && World.Rnd().nextFloat() < 1.0D - 0.86D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingRMid") > 1
						&& this.getEnergyPastArmor(13.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					this.nextDMGLevels(1, 2, "WingRMid_D2", shot.initiator);
				}
				if ((s.endsWith("lo1") || s.endsWith("lo2")) && World.Rnd().nextFloat() < 1.0D - 0.79D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingLOut") > 1
						&& this.getEnergyPastArmor(8.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					this.nextDMGLevels(1, 2, "WingLOut_D2", shot.initiator);
				}
				if ((s.endsWith("ro1") || s.endsWith("ro2")) && World.Rnd().nextFloat() < 1.0D - 0.79D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingROut") > 1
						&& this.getEnergyPastArmor(8.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
				...
				if ((s.endsWith("sl1") || s.endsWith("sl2")) && World.Rnd().nextFloat() < 1.0D - 0.79D * Math.abs(Aircraft.v1.x) && this.chunkDamageVisible("WingROut") > 1
						&& this.getEnergyPastArmor(8.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
			...
			if (s.startsWith("xxprop")) {
				...
				if (this.getEnergyPastArmor(2.0F, shot) > 0.0F && World.Rnd().nextFloat() < 0.35F) { this.FM.AS.setEngineSpecificDamage(shot.initiator, byte0, 3); }
			}
			if (s.startsWith("xxeng")) {
				...
				if (s.endsWith("case")) {
					if (this.getEnergyPastArmor(0.1F, shot) > 0.0F) {
						if (World.Rnd().nextFloat() < shot.power / 220000F) { this.FM.AS.setEngineStuck(shot.initiator, byte1); }
						if (World.Rnd().nextFloat() < shot.power / 54000F) { this.FM.AS.hitEngine(shot.initiator, byte1, 2); }
					}
					...
					if (this.getEnergyPastArmor(1.5F, shot) > 0.0F && World.Rnd().nextFloat() < this.FM.EI.engines[byte1].getCylindersRatio() * 0.6F) {
						...
						if (World.Rnd().nextFloat() < shot.power / 990000F) { this.FM.AS.hitEngine(shot.initiator, byte1, 3); }
						this.getEnergyPastArmor(25F, shot);
					}
				} else if (s.endsWith("supc") && this.getEnergyPastArmor(0.1F, shot) > 0.0F && World.Rnd().nextFloat() < 0.8F) { this.FM.AS.setEngineSpecificDamage(shot.initiator, byte1, 0); }
				if (s.endsWith("oil1") || s.endsWith("oil2") || s.endsWith("oil3")) {
					if (this.getEnergyPastArmor(0.63F, shot) > 0.0F) { this.FM.AS.hitOil(shot.initiator, byte1); }
			...
			if (s.startsWith("xxtank")) {
				...
				if (this.getEnergyPastArmor(1.1F, shot) > 0.0F) {
					if (shot.power < 14100F) {
						if (this.FM.AS.astateTankStates[i] < 1) { this.FM.AS.hitTank(shot.initiator, i, 1); }
						if (this.FM.AS.astateTankStates[i] < 4 && World.Rnd().nextFloat() < 0.1F) { this.FM.AS.hitTank(shot.initiator, i, 1); }
						if (shot.powerType == 3 && this.FM.AS.astateTankStates[i] > 0 && World.Rnd().nextFloat() < 0.05F) { this.FM.AS.hitTank(shot.initiator, i, 10); }
					} else {

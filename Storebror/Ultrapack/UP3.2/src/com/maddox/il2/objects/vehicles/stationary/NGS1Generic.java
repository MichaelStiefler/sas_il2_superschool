...
public abstract class NGS1Generic extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(NGS1Generic.this.engineSFX != null)
                if(NGS1Generic.this.engineSTimer >= 0)
                {
                    if(--NGS1Generic.this.engineSTimer <= 0)
                    {
                        NGS1Generic.this.engineSTimer = (int)NGS1Generic.SecsToTicks(NGS1Generic.Rnd(1.0F, 1.0F));
                        if(!NGS1Generic.this.danger())
                            NGS1Generic.this.engineSTimer = -NGS1Generic.this.engineSTimer;
                    }
                } else
                if(++NGS1Generic.this.engineSTimer >= 0)
                {

...
//            int i_8_ = -1073741569;
        ...
        if (this.order != null && this.bDrawAllMessages) {
            ...
            for (int i_21_ = 0; i_21_ < this.order.length; i_21_++) {
                if (this.orderStr[i_21_] != null) {
                    if (this.order[i_21_] instanceof OrderAnyone_Help_Me)
                        bool = true;
                    if (Main3D.cur3D().ordersTree.frequency() == null)
                        grayedOut = true;
                    if (string != null)
                        this.drawOrder(string, i, i_17_, i_19_ == 0 ? -1 : i_19_, i_18_, grayedOut);
                    i_19_ = i_21_;
            ...
            if (Main3D.cur3D().ordersTree.frequency() == null)
                grayedOut = true;
            if (string != null)
                this.drawOrder(string, i, i_17_, 0, i_18_, grayedOut);
            if (bool) {
    ...
    private void drawOrder(String string, int i, int i_25_, int i_26_, int i_27_, boolean grayedOut) {
        ...
        if (i_26_ >= 0)
            ttfont.output(color, i, i_25_, 0.0F, "" + i_26_ + ". " + string);
        else
            ttfont.output(color, i, i_25_, 0.0F, string);
    }
            ...
            for (int i_31_ = 0; i_31_ < is.length && is[i_31_] != 0; i_31_++) {
                ...
                while (stringtokenizer.hasMoreTokens()) {
                    ...
                    if (this.msgLines.size() == 0)
                        this.msgLines.add(new MsgLine(string_33_, i_34_, i, i_30_, Time.current()));
                    else {
                        ...
                        if (msgline.iActor == i && msgline.army == i_30_ && !bool) {
                            ...
                            if (i_35_ < this.msgDX) {
                                ...
                                this.msgLines.add(new MsgLine(string_33_, i_34_, i, i_30_, 0L));
                        } else
                            this.msgLines.add(new MsgLine(string_33_, i_34_, i, i_30_, 0L));
                    }
                    bool = false;
    ...
    public void resetMsgSizes() {
        ...
        if (this.msgDY > (int) (this.viewDY * 0.9))
            this.msgDY = (int) (this.viewDY * 0.9);
        ...
        if (string != null) {
            ...
            while (stringtokenizer.hasMoreTokens()) {
                ...
                if (this.trainingLines.size() == 0)
                    ...
                    if (i_43_ < this.trainingDX)
                        this.trainingLines.set(this.trainingLines.size() - 1, string_41_ + " " + string_40_);
                    else {
    ...
    private void renderLog() {
        ...
        if (this.logLen != 0) {
            for (/**/; this.logLen > 0; this.logLen--) {
                if (l < this.logTime[this.logPtr] + logTimeLife)
                    break;
            ...
            if (this.logLen != 0) {
                ...
                for (int i_55_ = 0; i_55_ < this.logLen; i_55_++) {
                    ...
                    if (this.bDrawAllMessages)
                        ttfont.output(i_57_, i - f, i_54_, 0.0F, this.logBufStr[i_56_]);
                    i_54_ -= i_53_;
                }
            }
        }
    ...
    public void renderNetStat() {
        ...
        if (this.bDrawNetStat && Mission.isPlaying() && !Mission.isSingle() && !Mission.MDS_VARIABLES().zutiHud_DisableHudStatistics) {
            ...
            for (int statUserIndex = this.pageSizeNetStat * this.pageNetStat; (statUserIndex < this.pageSizeNetStat * (this.pageNetStat + 1) && statUserIndex < this.statUsers.size()); statUserIndex++) {
                ...
                if (Mission.MDS_VARIABLES().zutiHud_ShowPilotNumber)
                    ttfont.output(statColor, pilotNumX - ttfont.width(statuser.sNum), statRowY, 0.0F, statuser.sNum);
                if (Mission.MDS_VARIABLES().zutiHud_ShowPilotPing)
                    ttfont_64_.output(-1, (pingX - ttfont_64_.width(statuser.sPing) - this.x1024(4.0F)), statRowY, 0.0F, statuser.sPing);
                if (Mission.MDS_VARIABLES().zutiHud_ShowPilotName)
                    ttfont.output(statColor, pingX, statRowY, 0.0F, statuser.user.uniqueName());
                if (!Mission.isCoop() && Mission.MDS_VARIABLES().zutiHud_ShowPilotScore)
                    ttfont.output(statColor, uniqueNameX, statRowY, 0.0F, statuser.sScore);
                if (Mission.MDS_VARIABLES().zutiHud_ShowPilotArmy)
                    ttfont.output(statColor, scoreX, statRowY, 0.0F, statuser.sArmy);
                if (Mission.MDS_VARIABLES().zutiHud_ShowPilotACDesignation)
                    ttfont.output(statColor, armyX, statRowY, 0.0F, statuser.sAircraft);
...
//	                }
                ...
                String selectorVersion = statuser.user.getSelectorVersion();
                if (!selectorVersion.equalsIgnoreCase("unknown")) {
                    versionInfo += " / Selector " + selectorVersion;
                }
//                System.out.println("Rendering Patch Level " + statuser.user.getPatchLevel() + " for user " + statuser.user.uniqueName() + " with color " + patchLevelColor);
    ...
    private void _addPointer(float f, float f_95_, int i, float f_96_, float f_97_) {
        if (this.nPointers == this.pointers.size())
            this.pointers.add(new Ptr(f, f_95_, i, f_96_, f_97_));
        else {
    ...
    private void preRenderDashBoard() {
        if (Actor.isValid(World.getPlayerAircraft()) && Actor.isValid(this.main3d.cockpitCur)) {
            if (this.main3d.isViewOutside()) {
                if (this.main3d.viewActor() != World.getPlayerAircraft() || !this.main3d.cockpitCur.isNullShow())
                    return;
    ...
    private void renderDashBoard() {
        if (Actor.isValid(World.getPlayerAircraft()) && Actor.isValid(this.main3d.cockpitCur)) {
            ...
            if (this.bDrawDashBoard) {
                ...
                if (f_104_ > 90.0F)
                    this.meshNeedle5.setScale(90.0F * f_102_);
                else
                    this.meshNeedle5.setScaleXYZ(90.0F * f_102_, 90.0F * f_102_, this.cvt(f_104_, 0.0F, 90.0F, 13.0F, 90.0F) * f_102_);
                ...
                if (World.getPlayerFM().getLoadDiff() < World.getPlayerFM().getLimitLoad() * 0.25F)
                    this.fntLcd.output(-16776961, 215.0F * f_102_ - f_109_, 182.0F * f_103_, 0.0F, string_108_);
                else if (i < 0)
                    this.fntLcd.output(-16777216, 215.0F * f_102_ - f_109_, 182.0F * f_103_, 0.0F, string_108_);
                else
                    this.fntLcd.output(-1, 215.0F * f_102_ - f_109_, 182.0F * f_103_, 0.0F, string_108_);
            }
        }
    }

...
public class CmdUser extends Cmd {
    ...
    private boolean isDServer(NetUser netuser) {
        ...
        if (!Main.cur().netServerParams.isDedicated())
            ...
            return Main.cur().netServerParams.host() == netuser;
    }
    ...
    public Object exec(CmdEnv cmdenv, Map map) {
        ...
        if (arraylist.size() == 0 && !exist(map, "STAT"))
            return null;
        bEventLog = exist(map, "EVENTLOG");
        if (exist(map, "STAT")) {
    ...
    private void fillUsers(Map map, List list, boolean flag) {
        ...
        if (!map.containsKey("_$$") && !map.containsKey("#") && !map.containsKey("ARMY"))
            flag1 = true;
        if (map.containsKey("_$$") && nargs(map, "_$$") == 1 && "*".equals(arg(map, "_$$", 0)))
            flag1 = true;
        if (flag1) {
            if (flag) {
                list.addAll(NetUserLeft.all);
            } else {
        ...
        if (map.containsKey("#")) {
            ...
            for (int j1 = 0; j1 < k; j1++) {
                ...
                if (flag) {
                    ...
                    if (l2 < 0 || l2 >= NetUserLeft.all.size())
                        continue;

...
public class Squares {
	...
	public void load(SectFile sectfile) {
		...
		if (f1 < 0.9F || f1 > 1.1F) {
			...
			if (f1 > 1.0F) squareWing = 2.0F * (liftWingLIn + liftWingLMid + liftWingLOut);
			else liftWingLIn = liftWingLMid = liftWingLOut = liftWingRIn = liftWingRMid = liftWingROut = 0.166667F * squareWing;
		}
	}
		...
		for (int i = 0; i < abulletemitter.length; i++) {
			...
			for (int j = 0; j < abulletemitter[i].length; j++) {
				...
				if (((abulletemitter[i][j] instanceof BombGun) || (abulletemitter[i][j] instanceof RocketBombGun)) && abulletemitter[i][j].haveBullets() && abulletemitter[i][j].getHookName().startsWith("_External") && dragParasiteCx < 0.704F)
					if ((abulletemitter[i][j] instanceof BombGunSC50) || (abulletemitter[i][j] instanceof BombGunSC70) || (abulletemitter[i][j] instanceof FuelTankGun)) dragParasiteCx += 0.02F;
					else dragParasiteCx += 0.06F;
				if ((abulletemitter[i][j] instanceof RocketGun) && abulletemitter[i][j].haveBullets() && !(abulletemitter[i][j] instanceof RocketGunR4M)) dragParasiteCx += 0.02F;
...
//				if (abulletemitter[i][j] instanceof PylonRO_WfrGr21Dual) dragParasiteCx += 0.02F;
				if ((abulletemitter[i][j] instanceof PylonHS129BK75) || (abulletemitter[i][j] instanceof PylonHS129BK37)) dragParasiteCx += 0.45F;
				else if (abulletemitter[i][j] instanceof PylonRO_WfrGr21) dragParasiteCx += 0.05F;
				else if (abulletemitter[i][j] instanceof PylonRO_WfrGr21Dual) dragParasiteCx += 0.055F;
				else dragParasiteCx += Property.floatValue(abulletemitter[i][j].getClass(), "drag", 0.01F);
				// --- TODO: By SAS~Storebror: Parasite Pylon Drag from Pylon Properties ---

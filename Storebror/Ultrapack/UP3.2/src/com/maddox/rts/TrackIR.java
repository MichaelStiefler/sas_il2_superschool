...
public class TrackIR implements MsgAddListenerListener, MsgRemoveListenerListener {
    ...
    public void msgAddListener(Object listener, Object dummy) {
        if (Message.current().isRealTime())
            this.realListeners.insListener(listener);
        else
            this.listeners.insListener(listener);
    }

    public void msgRemoveListener(Object listener, Object dummy) {
        if (Message.current().isRealTime())
            this.realListeners.removeListener(listener);
        else
    ...
    protected void setAngles(float yaw, float pitch, float roll, float headX, float headY, float headZ) {
        if (this.yaw == yaw && this.pitch == pitch && this.roll == roll && this.headX == headX && this.headY == headY && this.headZ == headZ)
            return;
        this.yaw = yaw;
        this.pitch = pitch;
        this.roll = roll;

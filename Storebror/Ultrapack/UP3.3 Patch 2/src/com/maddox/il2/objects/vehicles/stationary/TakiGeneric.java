...
public abstract class TakiGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (TakiGeneric.this.engineSFX != null) if (TakiGeneric.this.engineSTimer >= 0) {
                if (--TakiGeneric.this.engineSTimer <= 0) {
                    TakiGeneric.this.engineSTimer = (int) TakiGeneric.SecsToTicks(TakiGeneric.Rnd(5F, 5F));
                    if (!TakiGeneric.this.danger()) TakiGeneric.this.engineSTimer = -TakiGeneric.this.engineSTimer;
                }
            } else if (++TakiGeneric.this.engineSTimer >= 0) {
                TakiGeneric.this.engineSTimer = -(int) TakiGeneric.SecsToTicks(TakiGeneric.Rnd(5F, 5F));
                if (TakiGeneric.this.danger()) TakiGeneric.this.engineSTimer = -TakiGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            TakiGeneric takigeneric = null;
            try {
                TakiGeneric.constr_arg2 = actorspawnarg;
                takigeneric = (TakiGeneric) this.cls.newInstance();
                TakiGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                TakiGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create TakiGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return takigeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", TakiGeneric.mesh_name);
            this.cls = class1;
    ...
    private TakiGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) TakiGeneric.SecsToTicks(TakiGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if ((actor instanceof BigshipGeneric || actor instanceof ShipGeneric) && actor.getArmy() != this.myArmy && actor != World.getPlayerAircraft()) {
                ...
                int i2 = l1 - j;
                k1 = (int) (k1 / 1000D);
//                int j2 = (int) Math.ceil(k1); // TODO: Removed by SAS~Storebror
                String s = "Surface Contact";
                byte byte0 = 9;
                if (actor instanceof ShipGeneric) byte0 = 40;
                if (actor instanceof BigshipGeneric) byte0 = 60;
...
//                if (k1 <= byte0 && i2 >= 210 && i2 <= 270 && Math.sqrt(j1 * j1) <= 20D) HUD.logCenter("                                              " + s + " bearing " + i1 + "\260" + ", range " + j2 + " km");
                if (k1 <= byte0 && i2 >= 210 && i2 <= 270 && Math.sqrt(j1 * j1) <= 20D) HUD.logCenter("                                              " + s + " bearing " + i1 + "\260" + ", range " + k1 + " km"); // TODO: Changed by SAS~Storebror for j2 removal
            }
        }

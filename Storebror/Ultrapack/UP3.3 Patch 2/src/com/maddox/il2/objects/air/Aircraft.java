...
//			l <<= 1;
    ...
    protected void moveFan(float f) {
        ...
        for (int i_93_ = 0; i_93_ < this.FM.EI.getNum(); i_93_++) {
            ...
            if (i == 0) this.propPos[i_93_] = (this.propPos[i_93_] + 57.3F * this.FM.EI.engines[i_93_].getw() * f) % 360.0F;
            else {
                float f_94_ = 57.3F * this.FM.EI.engines[i_93_].getw();
                f_94_ %= 2880.0F;
                f_94_ /= 2880.0F;
                if (f_94_ <= 0.5F) f_94_ *= 2.0F;
                else f_94_ = f_94_ * 2.0F - 2.0F;
                f_94_ *= 1200.0F;
...
//	          BufferedReader bufferedreader = new BufferedReader(new InputStreamReader(new KryptoInputFilter(new SFSInputStream(Finger.LongFN(0L, "cod/" + Finger.incInt(i, "adt"))), getSwTbl(i))));
    ...
    protected static void weaponsLoad(Aircraft aircraft, String string) throws Exception {
        ...
        if (!hashmapint.containsKey(i)) {
            ...
            for (int loadoutsIndex = 0; loadoutsIndex < loadouts.size(); loadoutsIndex++) {
                ...
                int distance = calculateDistance(loadout.toLowerCase(), string.toLowerCase());
    ...
    protected static void weaponsLoad(Aircraft aircraft, int i, HashMapInt hashmapint) throws Exception {
        ...
        for (int i_163_ = 0; i_163_ < strings.length; i_163_++)
            if (var__WeaponSlots[i_163_] != null) if (aircraft.mesh().hookFind(strings[i_163_]) != -1) {
                ...
                if (aircraft.isNet() && aircraft.isNetMirror()) {
                    if (!World.cur().diffCur.Limited_Ammo) bulletemitter.loadBullets(-1);
                        ...
                        if (var__WeaponSlots[i_163_].bullets == -12345) bulletemitter.loadBullets();
                        else bulletemitter._loadBullets(var__WeaponSlots[i_163_].bullets);
                    } else bulletemitter.loadBullets(-1);
                } else if (var__WeaponSlots[i_163_].bullets == -12345) bulletemitter.loadBullets();
                else bulletemitter.loadBullets(var__WeaponSlots[i_163_].bullets);
            ...
            } else System.err.println("Hook '" + strings[i_163_] + "' NOT found in mesh of " + aircraft.getClass());
    }

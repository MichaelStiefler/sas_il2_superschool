...
//                  af[i]*=360F;
    ...
    public void computePos(Actor actor, Loc loc, Loc loc_24_) {
        if (this.bUse) {
            if (Time.isPaused() && !this.bPadlock) {
                ...
                loc_24_.add(this.le, loc);
                if (this.bPadlock) loc_24_.set(this.op);
            }
        } else loc_24_.set(loc);
    }
    ...
    public void mouseMove(int i, int i_33_, int i_34_) {
        if (this.bUse && !this.bPadlock && !this.bSimpleUse) if (this.bUseMouse && Time.real() > this.timeViewSet + 1000L) {
            ...
            if (f < -this.maxAzimut) {
                if (i <= 0) f = -this.maxAzimut;
                else f = this.maxAzimut;
            } else if (f > this.maxAzimut) if (i >= 0) f = this.maxAzimut;
            else f = -this.maxAzimut;
            float f_35_ = (this.o.tangage() + i_33_ * HookView.koofTangage) % 360.0F;
            if (f_35_ > 180.0F) f_35_ -= 360.0F;
            else if (f_35_ < -180.0F) f_35_ += 360.0F;
            if (f_35_ < this.minTangage) {
                if (i_33_ <= 0) f_35_ = this.minTangage;
                else f_35_ = this.maxTangage;
            } else if (f_35_ > this.maxTangage) if (i_33_ >= 0) f_35_ = this.maxTangage;
            else f_35_ = this.minTangage;

...
//			cPilot.set1024PosSize(628F, 416F, 364F, 32F);
    ...
    private void fillSkins() {
        ...
        try {
            ...
            String s1 = validateFileName((String) this.airNames.get(i));
            File file = new File(HomePath.toFileSystemName(s + "/" + s1, 0));
            File afile[] = file.listFiles();
            if (afile != null) for (int j = 0; j < afile.length; j++) {
                File file1 = afile[j];
                if (file1.isFile()) {
                    ...
                    if (s3.endsWith(".bmp") && s3.length() + s1.length() <= 122) {
                        ...
                        if (k == 512 || k == 1024) this.cSkin.add(s2);
                        else System.out.println("Skin " + s + "/" + s1 + "/" + s2 + " NOT loaded");
                    }
                }
            }
        } catch (Exception exception) {

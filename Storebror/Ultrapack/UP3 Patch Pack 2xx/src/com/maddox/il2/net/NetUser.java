...
public class NetUser extends NetHost implements NetFileClient, NetUpdate {
    ...
    public static final String MIN_PATCH_LEVEL        = "216";
    public static final String PATCH_LEVEL            = "216";
    public static final String PATCH_LEVEL_TEST       = "102b1";
    public static String[]     PATCHLEVEL_G           = { "216" };
    public static String[]     PATCHLEVEL_Y           = { "214", "212", "211", "210" };
...
//        System.out.println("uniqueName = \"" + this.uniqueName + "\"");
    ...
    public void setShortName(String s) {
        ...
        if (this.isMaster() && !this.isMirrored())
            this.makeUniqueName();
    }
...
// e.printStackTrace();
    ...
    private void sendCurStatInc() {
        ...
        __st.set(this.stat);
        __st.inc(_st);
        if (__st.isEqualsCurrent(this.curstat)) {
            return;
        } else {
    ...
    public void netUpdate() {
        ...
        // ---
        long l = Time.real();
        if (this.lastTimeUpdate + 20000L > l)
            return;
    ...
    public void setBornPlace(int i) {
        ...
        if (this.bornPlace >= 0 && World.cur().bornPlaces != null && this.bornPlace < World.cur().bornPlaces.size() && Main.cur().netServerParams != null) {
            ...
            if (Main.cur().netServerParams.isMaster()) {
                ...
                if (this.isMirror())
                    this.sendAirdromeStay(this.bornPlace, this.airdromeStay);
            }
        }
    }
    ...
    public int getPlace() {
        if (this.isMaster() && this.localRequestPlace != this.place)
            return -1;
        else
            return this.place;
    }
...
// }
    ...
    private void replicateCameraBaseChanged(NetObj netobj) {
        ...
        if (netserverparams.isMaster()) {
            ...
            this.doReplicateCameraBaseChanged(netobj);
        }
    }
    ...
    public void orderCmd(int i) {
        ...
        if (netserverparams.isMaster()) {
            if (i == -1)
                this.ordersTree.activate();
            else if (i == -2)
                this.ordersTree.unactivate();
            else
...
//                return false;
    ...
    public void netFileAnswer(NetFileRequest netfilerequest) {
        if (this.netUserRegiment.netFileRequest == netfilerequest) {
            ...
            if (this.localPilotBmp.length() == 0) {
                ...
                if (netgunner == null)
                    this.tryPreparePilot(aircraft1);
                else if (Actor.isValid(aircraft1))
                    this.tryPreparePilot(aircraft1, aircraft1.netCockpitAstatePilotIndx(netgunner.getCockpitNum()));
    ...
    public void setUserRegiment(String s, String s1, char ac[], int i) {
        if (this.netUserRegiment.equals(s, s1, ac, i))
            return;
    ...
    public void tryPrepareSkin(NetAircraft netaircraft) {
        ...
        if (this.skinDir == null) {
            ...
            if (this.ownerSkinBmp.equals(this.localSkinBmp)) {
                ...
                if (j >= 0)
                    this.skinDir = this.localSkinBmp.substring(0, j);
                else
            ...
            if (!BmpUtils.bmp8PalTo4TGA4(s3, s2, this.skinDir))
                this.skinDir = null;
        }
        if (this.skinDir == null) {
            return;
    ...
    public void tryPreparePilotSkin(Actor actor, int i) {
        ...
        if (this.localPilotTga == null) {
            ...
            if (this.ownerPilotBmp.equals(this.localPilotBmp))
                s1 = netfileserverpilot.primaryPath() + "/" + this.localPilotBmp;
            else
                s1 = netfileserverpilot.alternativePath() + "/" + this.localPilotBmp;
        ...
        if (actor instanceof NetAircraft)
            Aircraft.prepareMeshPilot(((NetAircraft) actor).hierMesh(), i, s, s2, this.cachePilotMat);
        else if (actor instanceof Paratrooper)
            ((Paratrooper) actor).prepareSkin(s, s2, this.cachePilotMat);
    }
    ...
    public void setPilot(String s) {
        if (s == null)
            s = "";
        if (s.equals(this.ownerPilotBmp))
            return;
...
// replicatePatchLevel(null);
        ...
        for (int j = 0; j < list.size(); j++) {
            NetChannel netchannel = (NetChannel) list.get(j);
            if (netchannel.isMirrored(this) || netchannel == this.masterChannel())
                this.replicatePatchLevel(netchannel, theNetUser);
        }
...
// System.out.println(this.uniqueName() + " sends Patch Update Message to " + netuser.uniqueName() + " (using Patch Level " + netuser.getPatchLevel() + ")");
    ...
    public void tryPrepareNoseart(NetAircraft netaircraft) {
        ...
        if (this.localNoseartTga == null) {
            ...
            if (this.ownerNoseartBmp.equals(this.localNoseartBmp))
                s = netfileservernoseart.primaryPath() + "/" + this.localNoseartBmp;
            else
                s = netfileservernoseart.alternativePath() + "/" + this.localNoseartBmp;
    ...
    public void setNoseart(String s) {
        if (s == null)
            s = "";
        if (s.equals(this.ownerNoseartBmp))
            return;
    ...
    public boolean isRadioArmy() {
        if (this.radio == null)
            return false;
        if (this.radio.length() < 2)
            return false;
        if (this.radio.charAt(0) != ' ')
            return false;
    ...
    public void radio_onCreated(String s) {
        if (!Chat.USE_NET_PHONE)
            return;
        if (this.radio != null && this.radio.equals(s))
    ...
    private void replicateRadio(String s, int i) {
        ...
        if (Main.cur().netServerParams == null)
            return;
        if (Main.cur().netServerParams.isMaster() && this.radio != null && !Chat.radioSpawn.isExistChannel(this.radio))
    ...
    public void destroy() {
        super.destroy();
        if (this.isMirror() && NetEnv.isServer() && !((Connect) NetEnv.cur().connect).banned.isExist(this.shortName))
            Chat.sendLog(0, "user_leaves", this.shortName(), null);
        ...
        if (this.airNetFilter != null) {
            ...
            if (netchannel != null && !netchannel.isDestroying())
                netchannel.filterRemove(this.airNetFilter);
            this.airNetFilter = null;
        }
        if (Mission.isPlaying()) {
            if (Mission.isCoop() && Time.current() > 1L)
                new NetUserLeft(this.uniqueName(), this.army, this.curstat);
    ...
    public void msgNetNewChannel(NetChannel netchannel) {
        try {
            ...
            if ((netchannel instanceof NetChannelOutStream) && (Main.cur() instanceof Main3D) && Main3D.cur3D().isDemoPlaying() && this.bTrackWriter)
            ...
            if (!"".equals(this.netUserRegiment.ownerFileNameBmp))
                this.replicateNetUserRegiment(netchannel);
            if (!"".equals(this.ownerSkinBmp))
                this.replicateSkin(netchannel);
            if (!"".equals(this.ownerPilotBmp))
                this.replicatePilot(netchannel);
            if (!"".equals(this.ownerNoseartBmp))
                this.replicateNoseart(netchannel);
            if (this.radio != null) {
                NetMsgGuaranted netmsgguaranted = new NetMsgGuaranted();
                netmsgguaranted.writeByte(21);
                netmsgguaranted.write255(this.radio);
...
// System.out.println("NetUser - Checking if user >" + s + " [" + IP + "]< has any refly/KIA time penalties...");
            ...
            if (((Connect) NetEnv.cur().connect).banned.isExist(s)) {
                ...
                Chat.sendLog(0, "user_joins", this.shortName(), null);
            }
            ...
            if (theNetUser == null) {
                ...
                for (int nui = 0; nui < list.size(); nui++) {
                    if (((NetUser) list.get(nui)).masterChannel() == netmsginput.channel()) {
                        ...
                        if (theNetUser.getPatchLevel().equalsIgnoreCase("none")) {
                            break;
                        }
                        theNetUser = null;
                    }

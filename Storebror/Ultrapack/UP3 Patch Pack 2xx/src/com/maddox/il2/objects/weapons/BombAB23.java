...
public class BombAB23 extends Bomb
{
    ...
    {
        super.start();
        t1 = Time.current() + (long)Math.max(this.armingTime, 750L) + World.Rnd().nextLong(-100L, 100L);
        t2 = t1 + this.fireTime;
    }
    ...
    private void doFireContaineds(int numToFire)
    {
        ...
        while (numToFire-- > 0) {
            ...
            if(bombletsLeft-- % 2 == 0) Eff3DActor.New(bombsd2a, null, null, 3F, "effects/Smokes/SmokeBlack_BuletteTrail.eff", 30F);
        }
    }

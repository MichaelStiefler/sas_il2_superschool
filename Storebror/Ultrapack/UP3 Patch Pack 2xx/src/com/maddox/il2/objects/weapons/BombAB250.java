...
public class BombAB250 extends Bomb
{
    ...
    {
        super.start();
        t1 = Time.current() + (long)Math.max(this.armingTime, 1000L) + World.Rnd().nextLong(-250L, 250L);
        t2 = t1 + this.fireTime;
    }
    ...
    private void doFireContaineds(int numToFire)
    {
        ...
        while (numToFire-- > 0) {
            ...
            if(bombletsLeft-- % 4 == 0) Eff3DActor.New(bombsd2a, null, null, 3F, "effects/Smokes/SmokeBlack_BuletteTrail.eff", 30F);
        }
    }

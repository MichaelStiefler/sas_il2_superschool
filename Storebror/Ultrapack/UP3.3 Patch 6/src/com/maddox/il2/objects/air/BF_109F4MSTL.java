...
public class BF_109F4MSTL extends BF_109 implements TypeDockable, Mistel {
    ...
    public BF_109F4MSTL() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL ()");
    ...
    public Aircraft getDrone() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL getDrone()");
        return this;
    }
    ...
    public void mistelExplosion() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL mistelExplosion()");
        if (this.mistelQueen == null) {
            if (DEBUG >= 1) System.out.println("BF_109F4MSTL mistelExplosion() mistelQueen=null");
            return;
        }
        if (this.mistelQueen.pos.getAbsPoint().z - Engine.cur.land.HQ(this.mistelQueen.pos.getAbsPoint().x, this.mistelQueen.pos.getAbsPoint().y) > 50D) {
            if (DEBUG >= 1) System.out.println("BF_109F4MSTL mistelExplosion() Altitude above land > 50m");
            MsgExplosion.send(this.mistelQueen, null, this.mistelQueen.pos.getAbsPoint(), this, 0.0F, 6550.0F, 0, 890.0F);
            return;
        }
        if (DEBUG >= 1) System.out.println("BF_109F4MSTL mistelExplosion() spawning BombFAB5000");
    ...
    protected void finalize() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL finalize()");
        super.finalize();
        NetMistel.removeNetMistelFromList(this);
    }

    protected void moveFan(float f) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL moveFan(" + f + ")");
        int i = 0;
        for (int j = 0; j < 1; j++) {
            if (this.oldProp[j] < 2) {
    ...
    protected void moveGear(float f) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL moveGear(" + f + ")");
        if (this.typeDockableIsDocked()) moveGear(this.hierMesh(), 0.0F);
        else moveGear(this.hierMesh(), f);
    }

    public void moveSteering(float f) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL moveSteering(" + f + ")");
        if (this.FM.CT.getGear() >= 0.98F) this.hierMesh().chunkSetAngles("GearC2_D0", 0.0F, -f, 0.0F);
    }

    public void update(float f) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL update(" + f + ")");
        ...
        if (this.FM.CT.getCockpitDoor() > 0.2D && this.bHasBlister && this.FM.getSpeedKMH() > this.fMaxKMHSpeedForOpenCanopy && this.hierMesh().chunkFindCheck("Blister1_D0") != -1) {
            try {
                if (this == World.getPlayerAircraft()) ((CockpitBF_109F2) Main3D.cur3D().cockpitCur).removeCanopy();
    ...
    public void onAircraftLoaded() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL onAircraftLoaded()");
    ...
    private void onCoopMasterSpawned() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL onCoopMasterSpawned()");
        Actor actor = null;
    ...
    public void missionStarting() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL missionStarting()");
        this.checkAsDrone();
    }

    private void checkAsDrone() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL checkAsDrone()");
    ...
    public int typeDockableGetDockport() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableGetDockport()");
        if (this.typeDockableIsDocked()) return this.dockport_;
        else return -1;
    }

    public Actor typeDockableGetQueen() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableGetQueen()");
        return this.queen_;
    }

    public boolean typeDockableIsDocked() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableIsDocked()");
        return Actor.isValid(this.queen_);
    }

    public void typeDockableAttemptAttach() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableAttemptAttach()");
    ...
    public void typeDockableAttemptDetach() {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableAttemptDetach()");
    ...
    public void typeDockableDoAttachToQueen(Actor actor, int i) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableDoAttachToQueen(" + actor.hashCode() + ", " + i + ")");
        this.queen_ = actor;
    ...
    public void typeDockableDoDetachFromQueen(int i) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableDoDetachFromQueen(" + i + ")");
        if (this.dockport_ == i) {
            this.queen_ = null;
            this.dockport_ = 0;
    ...
    public void typeDockableReplicateToNet(NetMsgGuaranted netmsgguaranted) throws IOException {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableReplicateToNet(" + netmsgguaranted.hashCode() + ")");
        if (this.typeDockableIsDocked()) {
    ...
    public void typeDockableReplicateFromNet(NetMsgInput netmsginput) throws IOException {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL typeDockableReplicateFromNet(" + netmsginput.hashCode() + ")");
    ...
    public void moveCockpitDoor(float f) {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL moveCockpitDoor(" + f + ")");
        if (this.bHasBlister) {
    ...
    public boolean netGetGMsg(NetMsgInput netmsginput, boolean bool) throws IOException {
        if (this.Debug_Methods()) System.out.println("BF_109F4MSTL netGetGMsg(" + netmsginput.hashCode() + ", " + bool + ")");

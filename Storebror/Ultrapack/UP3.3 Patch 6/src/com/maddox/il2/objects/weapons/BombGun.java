...
//        armingTime = 2000L;
        ...
        if (!Config.isUSE_RENDER()) this.bombDelay = 0.0F;
        // FIXME: Storebror: --- TEST Bomb/Rocket Fuze/Delay Replication
    ...
    public BulletEmitter detach(HierMesh hiermesh, int i) {
        if (!this.ready) return GunEmpty.get();
    ...
    protected void bullets(int i) {
        if (this.actor != null) this.bulletss = i + this.actor.hashCode();
        else this.bulletss = 0;
    }

    public void hide(boolean flag) {
        this.bHide = flag;
        if (this.bHide) {
            if (Actor.isValid(this.bomb) && this.bExternal) this.bomb.drawing(false);
        } else if (Actor.isValid(this.bomb) && this.bExternal) this.bomb.drawing(true);
    }
    ...
    public void loadBullets(int i) {
        this.bullets(i);
        if (this.bullets() != 0) {
            if (!Actor.isValid(this.bomb)) this.newBomb();
    ...
    public void shots(int i) {
        ...
        if (this.isCassette() && i != 0) i = this.bullets();
        if (this.bullets() == -1 && i == -1) i = 25;
        if (!this.bExecuted && i != 0) {
            if (this.bullets() == 0) return;
            if (this.bomb instanceof FuelTank) this.bullets(1);
            this.curShotStep = 0;
            this.curShots = i;
            this.bExecuted = true;
        } else if (this.bExecuted && i != 0) this.curShots = i;
        else if (this.bExecuted && i == 0) this.bExecuted = false;
    }
    ...
    protected void interpolateStep() {
        ...
        if (this.curShotStep == 0) {
            ...
            if (this.actor instanceof Aircraft) {
                ...
                if (flightmodel.getOverload() < 0.0F) return;
            }
            this.bTickShot = true;
            if (this.bomb != null) {
            ...
            if (this.curShots > 0) this.curShots--;
            if (this.bullets() > 0) this.bullets(this.bullets() - 1);
            if (this.bullets() != 0) this.newBomb();
    ...
    {
        String s = getCleanACName(class1);
    ...
    private void newBomb() {
        try {
            ...
            float f = getBombVShift(actor.getClass(), getClass());
            if(f != 0.0F)
            {
                Point3d point3d = bomb.pos.getRelPoint();
                point3d.z += f;
                bomb.pos.setRel(point3d);
            }
            f = getBombHShift(actor.getClass(), getClass());
            if(f != 0.0F)
            {
                Point3d point3d1 = bomb.pos.getRelPoint();
                point3d1.x += f;
                bomb.pos.setRel(point3d1);
            }
            this.bomb.pos.changeHookToRel();
            this.bomb.pos.resetAsBase();
            if (!this.bExternal) this.bomb.drawing(false);
            else this.bomb.visibilityAsBase(true);
            // TODO: Storebror: Bomb Spread Replication
            // ------------------------------------
            this.bomb.setSeed(this.armingRnd.nextInt());
            // ------------------------------------
    ...
    public void setArmingTime(long l) {
        ...
        // TODO: Storebror: --- Bomb/Rocket Fuze/Delay Replication
        this.armingTime = l;
        if (this.bomb != null) this.bomb.armingTime = l;
    }
    ...
    public void setRnd(int paramInt) {
        ...
        if (this.bomb != null) this.bomb.setSeed(this.armingRnd.nextInt());
    }

...
public class Bomb extends ActorMesh implements MsgCollisionRequestListener, MsgCollisionListener {
    ...
    private static class PlateFilter implements ActorFilter {
        ...
        public boolean isUse(Actor actor, double d) {
            ...
            if (Bomb.plateBox[0] - 2.5F < Bomb.corn1.x && Bomb.corn1.x < Bomb.plateBox[3] + 2.5F && Bomb.plateBox[1] - 2.5F < Bomb.corn1.y && Bomb.corn1.y < Bomb.plateBox[4] + 2.5F) Bomb.bPlateExist = true;
            return true;
        }
    ...
    public void msgCollisionRequest(Actor actor, boolean aflag[]) {
        if (actor == this.getOwner()) aflag[0] = false;
    }

    public void msgCollision(Actor actor, String s, String s1) {
        this.pos.getTime(Time.current(), p);
        this.impact = Time.current() - this.started;
        if (this.impact < this.armingTime && this.isArmed) // System.out.println("Bomb NOT armed!");
//            System.out.println("================================");
            this.isArmed = false;
        else if (this.impact >= this.armingTime && !this.isArmed) if (!(actor instanceof ActorLand)) // System.out.println("Bomb impacted >" + actor.name() + "< in more than 2s...");
//                System.out.println("================================");
            this.isArmed = true;

        if (actor != null && actor instanceof ActorLand && this.isPointApplicableForJump()) {
            if (this.speed.z >= 0.0D) return;
            ...
            if (f >= 30F) {
                ...
                if (-dir.z < 0.31F) {
                    ...
                    if (dir.dot(dirN) >= 0.91F) {
                        ...
                        this.pos.setAbs(p);
                        if (this.M >= 200F) f1 = 1.0F;
                        else if (this.M <= 5F) f1 = 0.0F;
                        else f1 = (this.M - 5F) / 195F;
                        float f2 = 3.5F + f1 * 12F;
                        if (Engine.land().isWater(p.x, p.y)) Explosions.SomethingDrop_Water(p, f2);
                        else Explosions.Explode10Kg_Land(p, 2.0F, 2.0F);
                        return;
                    }
                }
            }
        ...
        if (this.getOwner() == World.getPlayerAircraft() && !(actor instanceof ActorLand)) {
            ...
            if (Mission.isNet() && actor instanceof Aircraft && ((Aircraft) actor).isNetPlayer()) Chat.sendLogRnd(3, "gore_bombed", (Aircraft) this.getOwner(), (Aircraft) actor);
        }
        if (this.delayExplosion > 0.0F) {
            ...
            if (p.z < Engine.land().HQ(p.x, p.y) + 5D) if (Engine.land().isWater(p.x, p.y)) Explosions.Explode10Kg_Water(p, 2.0F, 2.0F);
            else Explosions.Explode10Kg_Land(p, 2.0F, 2.0F);
            this.interpEndAll();
            new MsgInvokeMethod_Object("doDelayExplosion", delayparam).post(this, this.delayExplosion);
            if (this.sound != null) this.sound.cancel();
        } else this.doExplosion(actor, s1);
    }
    ...
    public void doDelayExplosion(Object obj) {
        ...
        } else this.doExplosion(Engine.actorLand(), "Body", delayparam.p);
    }
    ...
    protected void doExplosion(Actor actor, String s) {
        ...
        // TODO: Storebror: --- Bomb/Rocket Fuze/Delay Replication
        this.pos.getTime(Time.current(), p);
        if (this.isArmed()) this.doExplosion(actor, s, p);
        else {
            if (p.z < Engine.land().HQ(p.x, p.y) + 5D) if (Engine.land().isWater(p.x, p.y)) Explosions.Explode10Kg_Water(p, 2.0F, 2.0F);
            else Explosions.Explode10Kg_Land(p, 2.0F, 2.0F);
            this.destroy();
    ...
    private void randomizeStart(Orient orient, Vector3d vector3d, float f, int i) {
        // ------------------------------------
        if (i != 0) {
            dir.set(this.RndFloatSign(0.1F, 1.0F), this.RndFloatSign(0.1F, 1.0F), this.RndFloatSign(0.1F, 1.0F));
            dir.normalize();
        } else {
            ...
            // TODO: Storebror: Bomb Spread Replication
            // ------------------------------------
            dir.add(this.getRnd().nextFloat(-f1, f1), this.getRnd().nextFloat(-f1, f1), this.getRnd().nextFloat(-f1, f1));
            // ------------------------------------
            dir.normalize();
        }
        orient.setAT0(dir);
    ...
    public double getSpeed(Vector3d vector3d) {
        if (vector3d != null) vector3d.set(this.speed);
    ...
    protected void init(float f, float f1) {
        if (Actor.isValid(this.getOwner()) && World.getPlayerAircraft() == this.getOwner()) this.setName("_bomb_");
    ...
    protected void updateSound() {
        if (this.sound != null) {
            ...
            if (this.curTm < 5F) this.sound.setControl(201, this.curTm);
            else if (this.curTm < 5F + 2 * Time.tickConstLen()) this.sound.setControl(201, 5F);
        }
    }
    ...
    public void start() {
        ...
        this.pos.getAbs(or);
        this.randomizeStart(or, this.rotAxis, this.M, Property.intValue(class1, "randomOrient", 0));
        this.pos.setAbs(or);
        this.getSpeed(spd);
        ...
        if (this.haveSound()) {
            ...
            if (s != null) this.sound = this.newSound(s, true);
        }
    }
...
//        System.out.println("Bomb setSeed(" + seedValue + ")");
    ...
    private static int curDebugLevel() {
        if (debugLevel == Integer.MIN_VALUE) debugLevel = Config.cur.ini.get("Mods", "DEBUG_BOMB", DEBUG_DEFAULT);
        return debugLevel;
    }
    ...
    public static void printDebug(Actor actor, String theMessage) {
        ...
        if (netAircraft.netUser() != null) System.out.println("[DEBUG_BOMB] " + netAircraft.netUser().uniqueName() + " (" + simpleClassName(actor) + ") " + theMessage);
        else System.out.println("[DEBUG_BOMB] " + netAircraft.name() + " (" + simpleClassName(actor) + ") " + theMessage);

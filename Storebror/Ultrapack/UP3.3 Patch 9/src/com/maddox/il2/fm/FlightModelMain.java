...
public class FlightModelMain extends FMMath {
    ...
    public void load(String s) {
        ...
        if (!this.formationOffsetValid) {
            try {
                ...
                for (int wingSquadIndex = 0; wingSquadIndex < 4; wingSquadIndex++) {
                    float leaderOffsetX = sectfile.get(s2, "FormationOffset" + (wingSquadIndex + 1) + "X", Float.NEGATIVE_INFINITY);
                    if (leaderOffsetX == Float.NEGATIVE_INFINITY)
                        break;
                    float leaderOffsetY = sectfile.get(s2, "FormationOffset" + (wingSquadIndex + 1) + "Y", Float.NEGATIVE_INFINITY);
                    if (leaderOffsetY == Float.NEGATIVE_INFINITY)
                        break;
                    float leaderOffsetZ = sectfile.get(s2, "FormationOffset" + (wingSquadIndex + 1) + "Z", Float.NEGATIVE_INFINITY);
        ...
        this.formationScaleCoeff = sectfile.get(s2, "FormationScaleCoeff", Float.NEGATIVE_INFINITY);
        // TODO: By SAS~Storebror: New Formation Parameters ---

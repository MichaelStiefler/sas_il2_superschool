...
public class Formation
{
    ...
    private static final float scaleCoeff(Aircraft aircraft)
    {
        ...
        if (newScaleCoeff == Float.NEGATIVE_INFINITY) newScaleCoeff = Property.floatValue(aircraft.getClass(), "FormationScaleCoeff", Float.NEGATIVE_INFINITY);
        if (newScaleCoeff != Float.NEGATIVE_INFINITY)
...
//        return (aircraft instanceof Scheme1) ? 1.2F : 2.2F;
    ...
    public static final void gather(FlightModel flightmodel, byte formationType, Vector3d vector3d, int year)
    {
        ...
        case F_DEFAULT:
            ...
            if((aircraft instanceof PE_8xyz) || (aircraft instanceof PE_8) || (aircraft instanceof B_17) || (aircraft instanceof B_25) || (aircraft instanceof B_29))
            {
                gather(flightmodel, F_VIC, vector3d);
                return;
            } else
    ...
    public static final void leaderOffset(FlightModel flightmodel, byte formationType, Vector3d vector3d)
    {
        ...
        if (aircraft.FM.isLeaderOffsetValid()) {
            ...
            do {
                try {
                    float leaderOffsetX = Property.floatValue(aircraft.getClass(), "FormationOffset" + (wingSquadIndex + 1) + "X", Float.NEGATIVE_INFINITY);
                    if (leaderOffsetX == Float.NEGATIVE_INFINITY)
                        break;
                    float leaderOffsetY = Property.floatValue(aircraft.getClass(), "FormationOffset" + (wingSquadIndex + 1) + "Y", Float.NEGATIVE_INFINITY);
                    if (leaderOffsetY == Float.NEGATIVE_INFINITY)
                        break;
                    float leaderOffsetZ = Property.floatValue(aircraft.getClass(), "FormationOffset" + (wingSquadIndex + 1) + "Z", Float.NEGATIVE_INFINITY);
                    if (leaderOffsetZ == Float.NEGATIVE_INFINITY)
                        break;

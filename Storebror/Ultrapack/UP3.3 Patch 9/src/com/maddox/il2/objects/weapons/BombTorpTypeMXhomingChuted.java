...
public class BombTorpTypeMXhomingChuted extends BombParaTorp {
    ...
    public void msgCollision(Actor actor, String s, String s1) {
        if ((actor instanceof ActorLand) && ((this.chute1 != null) || (this.chute2 != null))) {
            this.bOnChute1 = false;
        }
...
//        if (!this.flow) Ballistics.update(this, this.M, this.S);
        if (this.flow) {
            ...
            if (Actor.isValid(this.victim)) {
                this.victim.pos.getAbs(BombTorpTypeMXhomingChuted.pT);
                BombTorpTypeMXhomingChuted.pT.sub(Torpedo.P);
                Torpedo.Or.transformInv(BombTorpTypeMXhomingChuted.pT);
                if (BombTorpTypeMXhomingChuted.pT.y > 1.0D) {
                    this.deltaAzimuth = -1F;
                }
                if (BombTorpTypeMXhomingChuted.pT.y < -1D) {
            ...
            if (this.bOnChute2) {
                this.getSpeed(BombTorpTypeMXhomingChuted.v3d);
                BombTorpTypeMXhomingChuted.v3d.scale(0.99D);
                if (BombTorpTypeMXhomingChuted.v3d.z < -40D) {
                    BombTorpTypeMXhomingChuted.v3d.z += 8F * Time.tickConstLenFs();
                }
                this.setSpeed(BombTorpTypeMXhomingChuted.v3d);
                this.pos.getAbs(Torpedo.P, Torpedo.Or);
            } else if (this.bOnChute1) {
                this.getSpeed(BombTorpTypeMXhomingChuted.v3d);
                BombTorpTypeMXhomingChuted.v3d.scale(0.99D);
                if (BombTorpTypeMXhomingChuted.v3d.z < -90D) {
                    BombTorpTypeMXhomingChuted.v3d.z += 1.1F * Time.tickConstLenFs();
                }
                this.setSpeed(BombTorpTypeMXhomingChuted.v3d);
                this.pos.getAbs(Torpedo.P, Torpedo.Or);
                if (Torpedo.P.z < this.openHeight) {
                    this.bOnChute2 = true;
                    this.bOnChute1 = false;

...
class TDefenceGround extends Target {
    ...
    protected boolean checkActorDied(Actor actor) {
        ...
        if (!Target.isStaticActor(actor) && (!this.zutiCountMapObjects || this.zutiCountMapObjects && !(actor instanceof House))) // System.out.println("TDefenceGround returning false while checking if actor is dead...");
            return false;
    ...
    public TDefenceGround(int i, int j, int k, int l, int i1, int j1) {
        ...
        if (this.countActors == 0) this.setTaskCompleteFlag(true);
        else {
    ...
    public TDefenceGround(int i, int j, int k, int l, int i1, int j1, int zutiCountHouses) {
        ...
        if (this.countActors == 0) {
            ...
            System.out.println("TDefenceGround " + this.pos.getAbsPoint().toString() + " - Objects to defend " + this.countActors);

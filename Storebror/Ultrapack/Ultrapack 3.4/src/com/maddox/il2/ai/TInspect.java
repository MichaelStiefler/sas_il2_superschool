...
public class TInspect extends com.maddox.il2.ai.Target {
    ...
    boolean checkActor() {
        if (com.maddox.il2.engine.Actor.isAlive(this.actor)) {
            this.actor.pos.getAbs(this.point);
            return true;
        }
        if (this.actor != null) return false;
        this.actor = com.maddox.il2.engine.Actor.getByName(this.nameTarget);
        if (com.maddox.il2.engine.Actor.isAlive(this.actor)) {
            this.actor.pos.getAbs(this.point);
            return true;
        } else return false;
    ...
    protected boolean checkPeriodic() {
        ...
        if (com.maddox.il2.game.Mission.isCoop() || com.maddox.il2.game.Mission.isDogfight()) {
            ...
            for (int k = 0; k < i; k++) {
                ...
                if (actor1 instanceof com.maddox.il2.objects.air.Aircraft && com.maddox.il2.engine.Actor.isAlive(actor1) && actor1.getArmy() == j && this.checkPeriodic(actor1)) return true;
            }

            return false;
        }
        com.maddox.il2.objects.air.Aircraft aircraft = com.maddox.il2.ai.World.getPlayerAircraft();
        if (!com.maddox.il2.engine.Actor.isValid(aircraft)) return false;
        else return this.checkPeriodic(aircraft);
    }
    ...
    protected boolean checkPeriodic(com.maddox.il2.engine.Actor actor1) {
        ...
        if (p.distance(this.point) <= this.r) {
            ...
            if (scoutsCompleteRecon && !ZutiRadarObject.isPlayerArmyScout(actor1, World.getMissionArmy())) return false;
            // ---------------------------------------------------------------------------------

            com.maddox.il2.objects.air.Aircraft aircraft = (com.maddox.il2.objects.air.Aircraft) actor1;
            if (aircraft.FM.isWasAirborne()) {
                if (this.bLanding) if (com.maddox.il2.engine.Actor.isAlive(this.actor)) {
                    actor1.getSpeed(v0);
                    this.actor.getSpeed(v1);
                    v0.sub(v1);
                    if (v0.length() > 5D) return false;
                } else if (d - com.maddox.il2.ai.World.land().HQ(p.x, p.y) > 10D || actor1.getSpeed(null) > 5D) return false;
                ...
                try {
                    ...
                    if (user != null && user.equals(NetEnv.host())) {
                    ...
                    } else if (user != null) ZutiSupportMethods_NetSend.awardedForInspectingArea(user);
    ...
    protected boolean checkActorDied(com.maddox.il2.engine.Actor actor1) {
        ...
        // if(com.maddox.il2.game.Mission.isCoop())
        if (com.maddox.il2.game.Mission.isCoop() || com.maddox.il2.game.Mission.isDogfight()) {
            if (!(actor1 instanceof com.maddox.il2.objects.air.Aircraft)) return false;
            int i = com.maddox.il2.ai.World.getMissionArmy();
            if (actor1.getArmy() != i) return false;
            java.util.List list = com.maddox.il2.engine.Engine.targets();
            int j = list.size();
            for (int k = 0; k < j; k++) {
                actor1 = (com.maddox.il2.engine.Actor) list.get(k);
                if (actor1 instanceof com.maddox.il2.objects.air.Aircraft && com.maddox.il2.engine.Actor.isAlive(actor1) && actor1.getArmy() == i) return false;
            }
        } else if (actor1 != com.maddox.il2.ai.World.getPlayerAircraft()) return false;
    ...
    public TInspect(int i, int j, int k, int l, int i1, boolean flag, java.lang.String s) {
        ...
        this.point = new Point3d(k, l, com.maddox.il2.engine.Landscape.HQ(k, l));
        this.r = i1;
    }
}
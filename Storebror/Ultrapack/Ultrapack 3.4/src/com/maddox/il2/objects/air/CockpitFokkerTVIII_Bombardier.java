...
public class CockpitFokkerTVIII_Bombardier extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            float f = ((FokkerTVIII) CockpitFokkerTVIII_Bombardier.this.aircraft()).fSightCurForwardAngle;
            float f1 = ((FokkerTVIII) CockpitFokkerTVIII_Bombardier.this.aircraft()).fSightCurSideslip;
            CockpitFokkerTVIII_Bombardier.this.mesh.chunkSetAngles("BlackBox", 0.0F, -f1, f);
            if (CockpitFokkerTVIII_Bombardier.this.bEntered) {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setSimpleAimOrient(CockpitFokkerTVIII_Bombardier.this.aAim + f1, CockpitFokkerTVIII_Bombardier.this.tAim + f, 0.0F);
            }
            return true;
        }
    ...
    protected boolean doFocusEnter() {
        if (super.doFocusEnter()) {
            this.aircraft().hierMesh().chunkVisible("Cinner_D0", false);
            HookPilot hookpilot = HookPilot.current;
    ...
    protected void doFocusLeave() {
        if (!this.isFocused()) {
            ...
            this.leave();
            this.aircraft().hierMesh().chunkVisible("Cinner_D0", true);
            super.doFocusLeave();
            return;
        }
    }
    ...
    public CockpitFokkerTVIII_Bombardier() {
        super("3DO/Cockpit/FokkerTVIII-Bombardier/hier.him", "he111");
        this.w = new Vector3f();
        this.bEntered = false;
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("zSpeed", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 804.6721F, 0.0F, 10F), CockpitFokkerTVIII_Bombardier.speedometerScale), 0.0F);
        ...
        if (this.bEntered) {
            this.mesh.chunkSetAngles("zAngleMark", -this.floatindex(this.cvt(((FokkerTVIII) this.aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), CockpitFokkerTVIII_Bombardier.angleScale), 0.0F, 0.0F);
            boolean flag = ((FokkerTVIII) this.aircraft()).fSightCurReadyness > 0.93F;
    ...
    static {
        Property.set(CockpitFokkerTVIII_Bombardier.class, "astatePilotIndx", 0);
    }

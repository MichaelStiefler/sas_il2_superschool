...
public abstract class FUGGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (FUGGeneric.this.engineSFX != null) if (FUGGeneric.this.engineSTimer >= 0) {
                if (--FUGGeneric.this.engineSTimer <= 0) {
                    FUGGeneric.this.engineSTimer = (int) FUGGeneric.SecsToTicks(FUGGeneric.Rnd(5F, 5F));
                    if (!FUGGeneric.this.danger()) FUGGeneric.this.engineSTimer = -FUGGeneric.this.engineSTimer;
                }
            } else if (++FUGGeneric.this.engineSTimer >= 0) {
                FUGGeneric.this.engineSTimer = -(int) FUGGeneric.SecsToTicks(FUGGeneric.Rnd(5F, 5F));
                if (FUGGeneric.this.danger()) FUGGeneric.this.engineSTimer = -FUGGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            FUGGeneric fuggeneric = null;
            try {
                FUGGeneric.constr_arg2 = actorspawnarg;
                fuggeneric = (FUGGeneric) this.cls.newInstance();
                FUGGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                FUGGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create FUGGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return fuggeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", FUGGeneric.mesh_name);
            this.cls = class1;
    ...
    protected FUGGeneric() {
        this(constr_arg2);
    }

    private FUGGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) FUGGeneric.SecsToTicks(FUGGeneric.Rnd(5F, 5F));
        }
    }
        ...
        do {
            ...
            for (int l = 0; l < k; l++) {
                ...
                if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getArmy() != this.myArmy) {
                    ...
                    if (l1 < 0) l1 = 360 + l1;
                    int i2 = l1 - j;
                    if (k1 <= f && k1 >= 200D && i2 >= 235 && i2 <= 305 && Math.sqrt(j1 * j1) <= 35D && actor.getSpeed(vector3d) > 20D) {

...
public class P_43xyz extends Scheme1 implements TypeFighter, TypeBNZFighter, TypeStormovik {
    ...
    public void onAircraftLoaded() {
        ...
        if (this.thisWeaponsName.startsWith("default")) {
            this.hierMesh().chunkVisible("RackC_D0", false);
            this.hierMesh().chunkVisible("RackL_D0", false);
            this.hierMesh().chunkVisible("RackR_D0", false);
            return;
        }
        if (this.thisWeaponsName.startsWith("2x100lbs")) {
            this.hierMesh().chunkVisible("RackC_D0", false);
            this.hierMesh().chunkVisible("RackL_D0", true);
            this.hierMesh().chunkVisible("RackR_D0", true);
            return;
        }
        if (this.thisWeaponsName.startsWith("1x250lbs")) {
            this.hierMesh().chunkVisible("RackC_D0", true);
            this.hierMesh().chunkVisible("RackL_D0", false);
            this.hierMesh().chunkVisible("RackR_D0", false);
            return;
        }
        if (this.thisWeaponsName.startsWith("2xdpt")) {
            this.hierMesh().chunkVisible("RackC_D0", false);
            this.hierMesh().chunkVisible("RackL_D0", true);
            this.hierMesh().chunkVisible("RackR_D0", true);
            return;
        }
        if (this.thisWeaponsName.endsWith("none")) {
            this.hierMesh().chunkVisible("RackC_D0", false);
            this.hierMesh().chunkVisible("RackL_D0", false);
            this.hierMesh().chunkVisible("RackR_D0", false);
            return;

...
public abstract class SN2bGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (SN2bGeneric.this.engineSFX != null) if (SN2bGeneric.this.engineSTimer >= 0) {
                if (--SN2bGeneric.this.engineSTimer <= 0) {
                    SN2bGeneric.this.engineSTimer = (int) SN2bGeneric.SecsToTicks(SN2bGeneric.Rnd(5F, 5F));
                    if (!SN2bGeneric.this.danger()) SN2bGeneric.this.engineSTimer = -SN2bGeneric.this.engineSTimer;
                }
            } else if (++SN2bGeneric.this.engineSTimer >= 0) {
                SN2bGeneric.this.engineSTimer = -(int) SN2bGeneric.SecsToTicks(SN2bGeneric.Rnd(5F, 5F));
                if (SN2bGeneric.this.danger()) SN2bGeneric.this.engineSTimer = -SN2bGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            SN2bGeneric sn2bgeneric = null;
            try {
                SN2bGeneric.constr_arg2 = actorspawnarg;
                sn2bgeneric = (SN2bGeneric) this.cls.newInstance();
                SN2bGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                SN2bGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create SN2bGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return sn2bgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", SN2bGeneric.mesh_name);
            this.cls = class1;
    ...
    protected SN2bGeneric() {
        this(constr_arg2);
    }

    private SN2bGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) SN2bGeneric.SecsToTicks(SN2bGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        if (j < 0) j = 360 + j;
        boolean flag = false;
        for (int k = 100; !flag && k <= 4000; k += 100)
            for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
                actor2 = (Actor) entry.getValue();
                if (actor2 instanceof Aircraft && actor2 != World.getPlayerAircraft() && actor2.getArmy() != this.myArmy && actor2.getSpeed(vector3d) > 20D) {
                    double d3 = Main3D.cur3D().land2D.worldOfsX() + actor2.pos.getAbsPoint().x;
                    double d4 = Main3D.cur3D().land2D.worldOfsY() + actor2.pos.getAbsPoint().y;
                    double d5 = Main3D.cur3D().land2D.worldOfsY() + actor2.pos.getAbsPoint().z;
                    double d6 = d3 - d;
                    double d7 = d4 - d1;
                    ...
                    if (k1 < 0) k1 = 360 + k1;
                    int l1 = k1 - j;
                    if (j1 <= k && j1 >= 200D && l1 >= 220 && l1 <= 320 && Math.sqrt(i1 * i1) <= 60D) {
                        ...
                        String s1 = "m";
                        if (i2 >= 1000) {
                            i2 = (int) Math.floor(i2 / 1000);
                            s1 = "km";
                        }
                        HUD.logCenter("                                          RO: Target bearing " + l + "\260" + ", range " + i2 + s1 + ", " + s);
                        flag = true;
                    }
                }
            }

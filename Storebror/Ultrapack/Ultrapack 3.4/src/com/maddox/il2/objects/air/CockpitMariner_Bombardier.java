...
public class CockpitMariner_Bombardier extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitMariner_Bombardier.this.bEntered) {
                float f = ((PBM) CockpitMariner_Bombardier.this.aircraft()).fSightCurForwardAngle;
                float f1 = -((PBM) CockpitMariner_Bombardier.this.aircraft()).fSightCurSideslip;
                CockpitMariner_Bombardier.this.mesh.chunkSetAngles("BlackBox", f1, 0.0F, -f);
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setSimpleAimOrient(-f1, CockpitMariner_Bombardier.this.tAim + f, 0.0F);
            }
            return true;
        }
    ...
    public CockpitMariner_Bombardier() {
        super("3DO/Cockpit/Mariner-Bombardier/hier.him", "he111");
        this.bEntered = false;
        try {
            Loc loc = new Loc();
    ...
    public void reflectWorldToInstruments(float f) {
        this.mesh.chunkSetAngles("zMark1", ((PBM) this.aircraft()).fSightCurForwardAngle * 3.675F, 0.0F, 0.0F);
        float f1 = this.cvt(((PBM) this.aircraft()).fSightSetForwardAngle, -15F, 75F, -15F, 75F);
    ...
    static {
        Property.set(CockpitMariner_Bombardier.class, "astatePilotIndx", 0);
    }

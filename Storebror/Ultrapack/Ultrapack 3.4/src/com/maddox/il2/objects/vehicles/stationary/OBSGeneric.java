...
public abstract class OBSGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (OBSGeneric.this.engineSFX != null) if (OBSGeneric.this.engineSTimer >= 0) {
                if (--OBSGeneric.this.engineSTimer <= 0) {
                    OBSGeneric.this.engineSTimer = (int) OBSGeneric.SecsToTicks(OBSGeneric.Rnd(30F, 30F));
                    if (!OBSGeneric.this.danger()) OBSGeneric.this.engineSTimer = -OBSGeneric.this.engineSTimer;
                }
            } else if (++OBSGeneric.this.engineSTimer >= 0) {
                OBSGeneric.this.engineSTimer = -(int) OBSGeneric.SecsToTicks(OBSGeneric.Rnd(30F, 30F));
                if (OBSGeneric.this.danger()) OBSGeneric.this.engineSTimer = -OBSGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            OBSGeneric obsgeneric = null;
            try {
                OBSGeneric.constr_arg2 = actorspawnarg;
                obsgeneric = (OBSGeneric) this.cls.newInstance();
                OBSGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                OBSGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create OBSGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return obsgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", OBSGeneric.mesh_name);
            this.cls = class1;
    ...
    protected OBSGeneric() {
        this(constr_arg2);
    }

    private OBSGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) OBSGeneric.SecsToTicks(OBSGeneric.Rnd(30F, 30F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry1 = Engine.name2Actor().nextEntry(null); entry1 != null; entry1 = Engine.name2Actor().nextEntry(entry1)) {
            ...
            if (Actor.isAlive(actor1) && actor1 instanceof Aircraft && actor1.getArmy() != this.myArmy && actor1.pos.getAbsPoint().distance(point3d) < 15000D) {
                this.pos.getAbs(point3d);
                ...
                String s1 = "aircraft";
                if (actor1 instanceof TypeBomber) s1 = "bombers";
                else if (actor1 instanceof TypeFighter) s1 = "fighters";
                if (!flag) HUD.logCenter("                                                                     Enemy " + s1 + " at map grid " + s + "-" + d7 + ", altitude " + d5 + "m");
            }
        }

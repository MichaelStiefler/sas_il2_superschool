...
public abstract class AImkXGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (AImkXGeneric.this.engineSFX != null) if (AImkXGeneric.this.engineSTimer >= 0) {
                if (--AImkXGeneric.this.engineSTimer <= 0) {
                    AImkXGeneric.this.engineSTimer = (int) AImkXGeneric.SecsToTicks(AImkXGeneric.Rnd(5F, 5F));
                    if (!AImkXGeneric.this.danger()) AImkXGeneric.this.engineSTimer = -AImkXGeneric.this.engineSTimer;
                }
            } else if (++AImkXGeneric.this.engineSTimer >= 0) {
                AImkXGeneric.this.engineSTimer = -(int) AImkXGeneric.SecsToTicks(AImkXGeneric.Rnd(5F, 5F));
                if (AImkXGeneric.this.danger()) AImkXGeneric.this.engineSTimer = -AImkXGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            AImkXGeneric aimkxgeneric = null;
            try {
                AImkXGeneric.constr_arg2 = actorspawnarg;
                aimkxgeneric = (AImkXGeneric) this.cls.newInstance();
                AImkXGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                AImkXGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create AImkXGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return aimkxgeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", AImkXGeneric.mesh_name);
            this.cls = class1;
    ...
    protected AImkXGeneric() {
        this(constr_arg2);
    }

    private AImkXGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) AImkXGeneric.SecsToTicks(AImkXGeneric.Rnd(5F, 5F));
        }
    }
        ...
        do {
            ...
            for (int l = 0; l < k; l++) {
                ...
                if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getArmy() != this.myArmy) {
                    ...
                    if (l1 < 0) l1 = 360 + l1;
                    int i2 = l1 - j;
                    if (k1 <= f && k1 >= 250D && i2 >= 220 && i2 <= 320 && Math.sqrt(j1 * j1) <= 50D && actor.getSpeed(vector3d) > 20D) {

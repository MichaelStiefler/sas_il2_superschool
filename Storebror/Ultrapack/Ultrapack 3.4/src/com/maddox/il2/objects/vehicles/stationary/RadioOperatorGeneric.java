...
public abstract class RadioOperatorGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (RadioOperatorGeneric.this.engineSFX != null) if (RadioOperatorGeneric.this.engineSTimer >= 0) {
                if (--RadioOperatorGeneric.this.engineSTimer <= 0) {
                    RadioOperatorGeneric.this.engineSTimer = (int) RadioOperatorGeneric.SecsToTicks(RadioOperatorGeneric.Rnd(5F, 5F));
                    if (!RadioOperatorGeneric.this.danger()) RadioOperatorGeneric.this.engineSTimer = -RadioOperatorGeneric.this.engineSTimer;
                }
            } else if (++RadioOperatorGeneric.this.engineSTimer >= 0) {
                RadioOperatorGeneric.this.engineSTimer = -(int) RadioOperatorGeneric.SecsToTicks(RadioOperatorGeneric.Rnd(5F, 5F));
                if (RadioOperatorGeneric.this.danger()) RadioOperatorGeneric.this.engineSTimer = -RadioOperatorGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            RadioOperatorGeneric radiooperatorgeneric = null;
            try {
                RadioOperatorGeneric.constr_arg2 = actorspawnarg;
                radiooperatorgeneric = (RadioOperatorGeneric) this.cls.newInstance();
                RadioOperatorGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                RadioOperatorGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create RadioOperatorGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", RadioOperatorGeneric.mesh_name);
            this.cls = class1;
    ...
    private RadioOperatorGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) RadioOperatorGeneric.SecsToTicks(RadioOperatorGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof TypeFighter && actor.getArmy() == this.myArmy && actor.getSpeed(vector3d) > 20D) {
                ...
                if (aircraft instanceof TypeFighter && aircraft.getSpeed(vector3d) > 20D) {
                    ...
                    if (i1 <= 650D && Math.sqrt(d6 * d6) <= 650D && flag2 && flag && l <= 50) HUD.logCenter("                                          Nightfighter! Break " + s + "!");
                }
            }
        }

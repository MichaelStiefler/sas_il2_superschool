...
public class CockpitLeo_Bombardier extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitLeo_Bombardier.this.bEntered) {
                float f = ((Leoxyz) CockpitLeo_Bombardier.this.aircraft()).fSightCurForwardAngle;
                float f1 = -((Leoxyz) CockpitLeo_Bombardier.this.aircraft()).fSightCurSideslip;
                CockpitLeo_Bombardier.this.mesh.chunkSetAngles("BlackBox", f1, 0.0F, -f);
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setSimpleAimOrient(-f1, CockpitLeo_Bombardier.this.tAim + f, 0.0F);
            }
            return true;
        }
    ...
    public CockpitLeo_Bombardier() {
        super("3DO/Cockpit/LeO-Bombardier/hier.him", "he111");
        this.bEntered = false;
        try {
            Loc loc = new Loc();
    ...
    public void reflectWorldToInstruments(float f) {
        this.mesh.chunkSetAngles("zMark1", ((Leoxyz) this.aircraft()).fSightCurForwardAngle * 3.675F, 0.0F, 0.0F);
        float f1 = this.cvt(((Leoxyz) this.aircraft()).fSightSetForwardAngle, -15F, 75F, -15F, 75F);
    ...
    static {
        Property.set(CockpitLeo_Bombardier.class, "astatePilotIndx", 0);
    }

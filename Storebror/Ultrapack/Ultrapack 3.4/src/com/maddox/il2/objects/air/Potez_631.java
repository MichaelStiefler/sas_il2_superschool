...
public class Potez_631 extends Potez implements TypeFighter, TypeBNZFighter {
    ...
    public void onAircraftLoaded() {
        ...
        if (this.thisWeaponsName.equals("default") || this.thisWeaponsName.equals("none")) {
            this.hierMesh().chunkVisible("CAN_D0", false);
            this.hierMesh().chunkVisible("CAN1_D0", true);
        } else {
            this.hierMesh().chunkVisible("CAN1_D0", false);
            this.hierMesh().chunkVisible("CAN_D0", true);
        }
        if (this.thisWeaponsName.startsWith("H")) {
            this.hierMesh().chunkVisible("MGL_D0", true);
            this.hierMesh().chunkVisible("MGR_D0", true);
        } else {
            this.hierMesh().chunkVisible("MGL_D0", false);
            this.hierMesh().chunkVisible("MGR_D0", false);
        }
        if (this.thisWeaponsName.endsWith("Flares")) {

...
public abstract class AR_196 extends Scheme1 implements TypeSailPlane, TypeScout, TypeStormovik {
    ...
    protected void moveRudder(float f) {
        ...
        this.hierMesh().chunkSetAngles("FRudderL_D0", 45F * f, 0.0F, 0.0F);
        this.hierMesh().chunkSetAngles("FRudderR_D0", 45F * f, 0.0F, 0.0F);
    }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        if (s.startsWith("xx")) {
            if (s.startsWith("xxarmor")) {
                ...
                else if (s.endsWith("p2")) this.getEnergyPastArmor(5.25D / (Math.abs(v1.y) + 9.9999997473787516E-005D), shot);
            ...
            if (s.startsWith("xxspar")) {
                if ((s.endsWith("t1") || s.endsWith("t2")) && World.Rnd().nextFloat() < 0.1F && this.chunkDamageVisible("Tail1") > 2 && this.getEnergyPastArmor(19.9F / (float) Math.sqrt(v1.y * v1.y + v1.z * v1.z), shot) > 0.0F) {

...
public abstract class MonicaGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (MonicaGeneric.this.engineSFX != null) if (MonicaGeneric.this.engineSTimer >= 0) {
                if (--MonicaGeneric.this.engineSTimer <= 0) {
                    MonicaGeneric.this.engineSTimer = (int) MonicaGeneric.SecsToTicks(MonicaGeneric.Rnd(10F, 10F));
                    if (!MonicaGeneric.this.danger()) MonicaGeneric.this.engineSTimer = -MonicaGeneric.this.engineSTimer;
                }
            } else if (++MonicaGeneric.this.engineSTimer >= 0) {
                MonicaGeneric.this.engineSTimer = -(int) MonicaGeneric.SecsToTicks(MonicaGeneric.Rnd(10F, 10F));
                if (MonicaGeneric.this.danger()) MonicaGeneric.this.engineSTimer = -MonicaGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            ...
            try {
                ...
                monicageneric = (MonicaGeneric) this.cls.newInstance();
                MonicaGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                MonicaGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create MonicaGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) MonicaGeneric.SecsToTicks(MonicaGeneric.Rnd(5F, 5F));
        }
    }
    ...
    private boolean danger() {
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (actor instanceof Aircraft && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D) {
                ...
                if (k1 <= 6400D && k1 >= 308D && i2 >= 255 && i2 <= 285 && Math.sqrt(j1 * j1) >= 120D) HUD.logCenter("                                                                   (" + j2 + s + ")");
            }
        }

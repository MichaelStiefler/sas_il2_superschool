...
public abstract class AGCIGeneric extends ActorHMesh implements com.maddox.il2.objects.ActorAlign {
    class Move extends Interpolate {

        public boolean tick() {
            if (AGCIGeneric.this.engineSFX != null) if (AGCIGeneric.this.engineSTimer >= 0) {
                if (--AGCIGeneric.this.engineSTimer <= 0) {
                    AGCIGeneric.this.engineSTimer = (int) AGCIGeneric.SecsToTicks(AGCIGeneric.Rnd(30F, 50F));
                    if (!AGCIGeneric.this.danger()) AGCIGeneric.this.engineSTimer = -AGCIGeneric.this.engineSTimer;
                }
            } else if (++AGCIGeneric.this.engineSTimer >= 0) {
                AGCIGeneric.this.engineSTimer = -(int) AGCIGeneric.SecsToTicks(AGCIGeneric.Rnd(30F, 50F));
                if (AGCIGeneric.this.danger()) AGCIGeneric.this.engineSTimer = -AGCIGeneric.this.engineSTimer;
            }
            return true;
    ...
    public static class SPAWN implements ActorSpawn {
        ...
        public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
            AGCIGeneric agcigeneric = null;
            try {
                AGCIGeneric.constr_arg2 = actorspawnarg;
                agcigeneric = (AGCIGeneric) this.cls.newInstance();
                AGCIGeneric.constr_arg2 = null;
            } catch (Exception exception) {
                AGCIGeneric.constr_arg2 = null;
                System.out.println(exception.getMessage());
                exception.printStackTrace();
                System.out.println("SPAWN: Can't create AGCIGeneric object [class:" + this.cls.getName() + "]");
                return null;
            }
            return agcigeneric;
        }
        ...
        public SPAWN(Class class1) {
            ...
            Property.set(class1, "meshName", AGCIGeneric.mesh_name);
            this.cls = class1;
    ...
    protected AGCIGeneric() {
        this(constr_arg2);
    }

    private AGCIGeneric(ActorSpawnArg actorspawnarg) {
        super(mesh_name);
        this.engineSFX = null;
    ...
    public void startMove() {
        if (!this.interpEnd("move")) {
            ...
            this.engineSTimer = -(int) AGCIGeneric.SecsToTicks(AGCIGeneric.Rnd(30F, 30F));
        }
    }
    ...
    private boolean danger() {
        ...
        if (aircraft.getSpeed(vector3d) > 20D && aircraft.pos.getAbsPoint().distance(point3d) < 100000D && aircraft.pos.getAbsPoint().z >= 150D && aircraft != World.getPlayerAircraft() && aircraft.getArmy() != this.myArmy) {
            ...
            new String();
            int j = (int) (Math.floor(aircraft.pos.getAbsPoint().z * 3.2808399D * 0.1D) * 10D);
            new String();
            ...
            if (i1 < 0) i1 = 360 + i1;
            int j1 = (int) (Math.ceil(Math.sqrt(d14 * d14 + d13 * d13)) * 0.621371192D);
            if (flag) {
                if (j1 > 4) HUD.logCenter("                                          Target bearing " + l + "\260" + ", range " + j1 + " miles, height " + j + "ft, heading " + i1 + "\260");
                else HUD.logCenter(" ");
            } else HUD.logCenter("                                                                             Target at " + s + "-" + d9 + ", height " + j + "ft, heading " + i1 + "\260");
        }
        return true;
    }

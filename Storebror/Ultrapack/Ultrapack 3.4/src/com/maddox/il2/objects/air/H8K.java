...
public abstract class H8K extends Scheme4 implements TypeSailPlane, TypeTransport {
    ...
    public void update(float f) {
        ...
        for (int k = 0; k < 4; k++) {
            float f1 = this.FM.EI.engines[k].getControlRadiator();
            if (Math.abs(this.flapps[k] - f1) > 0.01F) {
                this.flapps[k] = f1;
                for (int l = 1; l < 9; l++)
                    this.hierMesh().chunkSetAngles("Water" + (k * 8 + l) + "_D0", 0.0F, -40F * f1, 0.0F);
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        ...
        if (s.startsWith("xcf1") || s.startsWith("xcf2")) {
            ...
            if (s.startsWith("xcf2") && World.Rnd().nextFloat() < 0.0575F) if (point3d.y > 0.0D) this.FM.AS.setCockpitState(shot.initiator, this.FM.AS.astateCockpitState | 8);

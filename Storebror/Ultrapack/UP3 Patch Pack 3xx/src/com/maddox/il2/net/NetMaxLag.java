...
public class NetMaxLag {
    ...
    private void checkLag(Aircraft aircraftToCheck, boolean isServerCheck) {
        ...
        if (!isServerCheck)
            cheaterWarningDelay += 3D;
        if (this.lastWarningTime > 0.0D && timeInSeconds - this.lastWarningTime < cheaterWarningDelay)
            return;
        ...
        for (int targetListIterator = 0; targetListIterator < targetListSize; targetListIterator++) {
            ...
            if (targetFromList == aircraftToCheck)
                continue; // target is aircraft to check, ignore!
            if (!(targetFromList instanceof Aircraft))
                continue; // target is no aircraft, ignore!
            if (!Actor.isAlive(targetFromList))
                continue; // target is already dead, ignore!
            if (targetFromList.getArmy() == armyIndex)
                continue; // target is from own army, ignore!
            if (!((Aircraft) targetFromList).isNetPlayer())
                continue; // target is AI, ignore!
            ...
            if (distanceToTargetSquared > far2) {
                if (isServerCheck)
                    maxLagAllowedFromThisTarget = aircraftToCheck.net.masterChannel().getMaxTimeout() / 2;
                else
                    maxLagAllowedFromThisTarget = netserverparams.masterChannel().getMaxTimeout() / 2;
            } else {
            ...
            if (maxLagAllowed < 0L || maxLagAllowed > maxLagAllowedFromThisTarget)
                maxLagAllowed = maxLagAllowedFromThisTarget;
        }
        ...
        while (true) {
            ...
            if (isServerCheck)
                masterNetChannel = aircraftToCheck.net.masterChannel();
            else
                masterNetChannel = netserverparams.masterChannel();
            if (masterNetChannel == null)
                break;
            if (maxLagAllowed > masterNetChannel.getCurTimeoutOk())
                break;
            this.lastWarningTime = timeInSeconds;
            this.warningCounter++;
            boolean warningCounterElapsed = false;
            if (netserverparams.cheaterWarningNum() >= 0)
                warningCounterElapsed = this.warningCounter > netserverparams.cheaterWarningNum();
...
//            }
            } else if (isServerCheck) { // Only check on server side!

...
public class Chat extends NetObj {
            ...
            switch (i) {
                ...
                case 2:
                    if (chatmessage.param0 != null)
                        this.params[0] = ((NetUser) chatmessage.param0).shortName();
                    break;
                ...
                case 3:
                    ...
                    if (chatmessage.param1 != null)
                        this.params[1] = ((NetUser) chatmessage.param1).shortName();
                    break;
    ...
    private void addMsg(ChatMessage chatmessage) {
        ...
        if (chatmessage.msg != null && (chatmessage.msg.startsWith("Morse:") || chatmessage.msg.startsWith("morse:"))) {
            try {
                if (!World.cur().blockMorseChat)
                    World.getPlayerAircraft().playChatMsgAsMorse(chatmessage.msg);
            } catch (Exception exception) {
            ...
            // TODO: --- Auto Login / FBDj Admin Mod ---
                ...
                System.out.println("Chat: --- " + chatmessage.msg);
        }
    }
    ...
    public void checkServerLoginMessage(ChatMessage chatmessage) {
        ...
        if (chatmessage.from.shortName().equals("Server")) {
            if (chatmessage.msg.endsWith("using admin name. Must login or will be kicked!")) {
                List receiverList = new ArrayList();
                receiverList.add(chatmessage.from);
                if (Config.cur.bUseAutoAdminLogin && Config.cur.sAutoAdminPassword.length() > 0)
                    this.send(NetEnv.host(), "!login " + Config.cur.sAutoAdminPassword, receiverList);
                else if (Config.cur.bUseAutoUserLogin && Config.cur.sAutoUserPassword.length() > 0)
                    this.send(NetEnv.host(), ">login " + Config.cur.sAutoUserPassword, receiverList);
            } else if (chatmessage.msg.endsWith("Logged in as Admin")) {
                HUD.setAdminMode(true);
            }
        }
    }
    // TODO: --- Auto Login / FBDj Admin Mod ---
    ...
    public static void sendLog(int i, String s, Aircraft aircraft, Aircraft aircraft1) {
        ...
        if (Main.cur().netServerParams == null)
            return;
        if (i > Main.cur().netServerParams.autoLogDetail())
            return;
    ...
    public void send(NetHost nethost, String s, List list, byte byte0, Object obj, Object obj1, boolean flag) {
        ...
        if (list != null)
            i += list.size() * NetMsgOutput.netObjReferenceLen();
        if (i > 250) {
            i -= 250;
        ...
        if (flag)
            this.addMsg(chatmessage);
        if (this.isMirror() || this.isMirrored())
            try {
    ...
    public boolean netInput(NetMsgInput netmsginput) throws IOException {
        ...
        if (j == 0) {
            ...
            if (netmsginput.channel() instanceof NetChannelInStream) {
                ...
                flag = chatmessage.to.indexOf(NetEnv.host()) >= 0;
            }
        }
        if (flag)
            this.addMsg(chatmessage);
        ...
        if (l > 0)
            this.postExclude(netmsginput.channel(), new NetMsgGuaranted(netmsginput, j + byte0 + 1));
        return true;
    }
    ...
    public Chat(NetChannel netchannel, int i) {
        ...
        this.params = new Object[2];
        Main.cur().chat = this;
        if (USE_NET_PHONE && !NetMissionTrack.isPlaying())
            AudioDevice.beginNetPhone();
    }

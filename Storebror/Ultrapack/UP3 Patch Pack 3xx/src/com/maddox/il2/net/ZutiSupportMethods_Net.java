...
public class ZutiSupportMethods_Net {
    ...
    public static boolean canUserJoinBornPlace(BornPlace bp) {
        ...
        /*
         * System.out.println("tmpForBrief=" + bp.tmpForBrief);
         * System.out.println("zutiNumberOfSpawnPlaces=" + bp.zutiNumberOfSpawnPlaces);
         * System.out.println("zutiMaxBasePilots=" + bp.zutiMaxBasePilots);
         */

        if (bp.tmpForBrief < bp.zutiNumberOfSpawnPlaces || (bp.zutiMaxBasePilots > 0 && bp.tmpForBrief < bp.zutiMaxBasePilots)) {
            return true;
        }
        ...
        for (int i = 0; i < length; i++) {
            try {
                if (point_stays[i] != null) {
                    ...
                    double distance = ((((double) point_stay.x - x) * ((double) point_stay.x - x)) + (((double) point_stay.y - y) * ((double) point_stay.y - y)));
                    if (distance <= d) {
                        if (bp.zutiStayPointStartIndex == -1)
                            bp.zutiStayPointStartIndex = i;
...
//                System.out.println("  " + zac.getAcName());
    ...
    public static int getStayPlace(NetUser user) {
        ...
        if (bp != null) {
            ...
            /*
             * System.out.println("================================");
             * System.out.println("ZSM_N - >" + user.uniqueName() + "< requesting stay place!");
             * for( int i=0; i<bp.zutiStayPointAssignment.length; i++ )
             * {
             * System.out.println("  Stay point id >" + i + "< has status >" + bp.zutiStayPointAssignment[i]);
             * }
             * System.out.println("================================");
             */
            ...
            for (int i = 0; i < bp.zutiStayPointAssignment.length; i++) {
                ...
                if (bp.zutiStayPointAssignment[i] == 1)
                    spawnPlace = i;
            }
    ...
    public static void releaseStayPlace(String userName, int bornPlaceId, int stayPlaceId) {
        ...
        if (bp == null || bp.zutiStayPointAssignment == null)
            return;

...
public class Pilot extends Maneuver {
    ...
    public boolean killed(FlightModel flightmodel) {
        ...
        if (Actor.isValid(flightmodel.actor))
            return this.killed(flightmodel.actor);
        else
    ...
    protected void setPriorities() {
        if (this.killed(this.danger))
            this.danger = null;
        if (this.killed(this.target))
            this.target = null;
        if (this.killed(this.airClient))
            this.airClient = null;
        if (this.killed(this.target_ground))
        ...
        if (this.M.fuel < 0.3F * this.M.maxFuel) {
            ...
            if (this.M.maxFuel < 0.001F)
                this.M.maxFuel = 0.001F;
            float f2 = (1000F * this.Range * this.M.fuel) / this.M.maxFuel;
            if (f2 < 2.0F * f1 && !(this.actor instanceof TypeGlider))
                this.setReadyToReturn(true);
        }
        if (this.M.fuel < 0.01F && !(this.actor instanceof TypeGlider))
            this.setReadyToDie(true);
        ...
        if (this.get_task() == 3 && this.AP.way.isLanding())
            this.setBusy(true);
    }
    ...
    public void setManeuverByTask() {
        ...
        switch (this.get_task()) {
            ...
            case 2:
                if (this.Leader != null && Actor.isValid(this.Leader.actor) && !this.Leader.isReadyToReturn() && !this.Leader.isReadyToDie() && this.Leader.getSpeed() > 35F)
            ...
            case 4:
                ...
                vecDanger.normalize();
                if (this.onBack && this.near && (this.danger instanceof TypeFighter) && ((this.actor instanceof TypeTransport) || this.Wingman == null || this.killed(this.Wingman) || ((Pilot) this.Wingman).target != this.danger))
                    if (this.isLeader()) {
                        if (((this.actor instanceof TypeFighter) || (this.actor instanceof TypeStormovik) && this.Skill > 1 && this.AP.way.curr().Action == 0) && this.Wingman != null && !this.killed(this.Wingman)
                                && !((Pilot) this.Wingman).requestCoverFor(this))
                            if (this.Wingman.Wingman != null && !this.killed(this.Wingman.Wingman))
...
//                this.target = this.danger; // TODO: AI TEST, didn't work.
                ...
                if (this.actor instanceof TypeStormovik)
                    this.stormovikDefence();
                else
                    this.transportDefence();
                break;
            ...
            case 7:
                ...
                if ((this.target_ground instanceof Prey) && (((Prey) this.target_ground).HitbyMask() & 1) == 0) {
                    float f1 = 0.0F;
                    for (int k = 0; k < 4; k++)
                        if (this.CT.Weapons[k] != null && this.CT.Weapons[k][0] != null && this.CT.Weapons[k][0].countBullets() != 0 && this.CT.Weapons[k][0].bulletMassa() > f1)
                ...
                if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null && this.CT.Weapons[3][0].countBullets() != 0) {
                    ...
                    if ((this.actor instanceof TypeDiveBomber) && this.Alt > 1200F)
                        this.set_maneuver(50);
                    else
                        this.set_maneuver(43);
                    break;
                }
    ...
    public boolean requestCoverFor(FlightModel flightmodel) {
        ...
        if (this.danger == null || ((Pilot) this.danger).target != this || this.danger.Loc.distance(this.Loc) > 600D + 200D * this.Skill || (this.danger.actor instanceof TypeStormovik) || (this.danger.actor instanceof TypeBomber)) {
            ...
            if (World.Rnd().nextBoolean())
                Voice.speakCoverProvided((Aircraft) this.actor, (Aircraft) flightmodel.actor);
            else
                Voice.speakHelpFromAir((Aircraft) this.actor, 1);
            return true;
        } else {
    ...
    public void setAsDanger(Actor actor) {
        if (this.get_maneuver() == 44)
            return;
        if (this.get_maneuver() == 26)
            return;
        if (this.isDumb() && !this.isReadyToReturn())
            return;
        ...
        switch (this.Skill) {
            ...
            case 1:
                if (!this.detectable(actor))
                    return;
                if (World.Rnd().nextInt(0, 99) < 97)
                    return;
                break;

            case 2:
                if (this.getMnTime() < 1.0F)
                    return;
                break;
        ...
        if (actor instanceof Aircraft) {
            if (actor instanceof TypeFighter) {
                if (this.turret.length > 0 && this.AS.astatePilotStates[this.turret.length] < 90)
    ...
    public void attackBombers() {
        ...
        if (this.actor instanceof ME_163B1A)
            ...
            //        if((actor instanceof TypeJazzPlayer) && ((TypeJazzPlayer)actor).hasSlantedWeaponBullets())
                ...
                switch (i) {
                    ...
                    case 0:
                        switch (this.flying) {
                            ...
                            case 5:
                                if (World.Rnd().nextBoolean())
                                    this.setWingmanAttackType(8);
                                else
                                    this.setWingmanAttackType(9);
                                this.set_maneuver(81);
                                break;
                        }
                        break label0;
    ...
    public boolean isLeader() {
        if (this.actor instanceof TypeFighter)
            return ((Aircraft) this.actor).aircIndex() % 2 == 0;
        else
    ...
    public boolean isLonely(float f, boolean flag) {
        if (flag) {
            if (this.Leader == null && this.Wingman == null)
                return true;
            double d = 0.0D;
            if (this.Leader != null)
                d = this.Leader.Loc.distance(this.Loc);
            if (this.Wingman != null)
                d = Math.min(this.Wingman.Loc.distance(this.Loc), d);
            return d > f;
        }
        Actor actor = NearestTargets.getEnemy(9, -1, this.Loc, f, 0);
        if (Actor.isValid(actor))
            return actor.pos.getAbsPoint().distance(this.Loc) > f;
        else
            return true;
    }

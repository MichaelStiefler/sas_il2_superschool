...
public class ZutiRadarRefresh
{
	...
	/**
	 * Refresh pad objects positions. Executed always if radar refresh interval = 0, otherwise
	 * this method is executed as specified by radar refresh interval. Precondition is that
	...
	private void updatePadObjectsPositions()
	{
		...
		/*
		System.out.println("ZutiRadarRefresh - intervalSet = " + refreshIntervalSet);
		System.out.println("Current time: " + Time.current());
		System.out.println("Start time: " + startTime);
		System.out.println("Result: " + (Time.current()-startTime) + " > " + mission.zutiRadar_RefreshInterval);
		System.out.println("------------------------------");
		*/
...
//			System.out.println("-------------------------------");
	...
	/**
	 * Set pad objects (actors and targets) visibility flags that either render them
	...
	private void executeVisibilityCheck()
	{
		...
		runIfActive_RefreshPadObjects(worldXOffset, worldYOffset, worldX1ffset, worldY1ffset, playerArmy);
		runIfActive_RefreshTargets(worldXOffset, worldYOffset, worldX1ffset, worldY1ffset, playerArmy);
	}
	...
	private void runIfActive_RefreshPadObjects(double worldXOffset, double worldYOffset, double worldX1ffset, double worldY1ffset, int playerArmy)
	{
		try
		{
			...
			while( iterator.hasNext() )
			{
				...
				if( !(position.x < worldXOffset) && !(position.x > worldX1ffset) && 
					!(position.y < worldYOffset) && !(position.y > worldY1ffset) )
				{
					//Speed optimization
					if( zo.hashCode()%MODUS != Time.current()%MODUS )
						continue;
		...
		{			
			if( ZutiTargetsSupportMethods.ZUTI_TARGETPOINTS != null && Mission.MDS_VARIABLES().zutiIcons_ShowTargets )
			{
				...
				while( iterator.hasNext() )
				{
					...
					if( (!(targetPosition.x < worldXOffset) && !(targetPosition.x > worldX1ffset) && 
						!(targetPosition.y < worldYOffset) && !(targetPosition.y > worldY1ffset)) || GUIBriefing.ZUTI_IS_BRIEFING_ACTIVE )
					{				
						//Speed optimization
						long modusResult = Time.current() % MODUS;
						if( tp.actor != null && tp.hashCode()%MODUS != modusResult )
							continue;
						else if( tp.hashCode()%MODUS != modusResult )
							continue;
	...
	/**
	 * Fill radars list by going through all of the objects that are part of missions.
...
//		System.out.println("ZutiRadarRefresh_New - radars after searching aircrafts: " + ZUTI_RADARS.radars.size());
	...
	{
		/*
		System.out.println("Checking for radars... findRadarsStartTime = " + findRadarsStartTime);
		System.out.println("current time: " + Time.current());

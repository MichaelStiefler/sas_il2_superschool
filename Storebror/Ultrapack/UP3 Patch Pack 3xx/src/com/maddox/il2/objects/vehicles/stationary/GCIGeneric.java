...
public abstract class GCIGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(GCIGeneric.this.engineSFX != null)
                if(GCIGeneric.this.engineSTimer >= 0)
                {
                    if(--GCIGeneric.this.engineSTimer <= 0)
                    {
                        GCIGeneric.this.engineSTimer = (int)GCIGeneric.SecsToTicks(GCIGeneric.Rnd(30F, 50F));
                        if(!GCIGeneric.this.danger())
                            GCIGeneric.this.engineSTimer = -GCIGeneric.this.engineSTimer;
                    }
                } else
                if(++GCIGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        if(aircraft.getSpeed(vector3d) > 20D && aircraft.pos.getAbsPoint().distance(point3d) < 100000D && aircraft.pos.getAbsPoint().z >= 150D && aircraft != World.getPlayerAircraft() && aircraft.getArmy() != this.myArmy)
        {
            ...
            if(d4 > 260D)
                ...
                s = "" + c1;
            new String();
            int j = (int)(Math.floor(aircraft.pos.getAbsPoint().z * 0.10000000000000001D) * 10D);
            new String();
            ...
            {
                if(j1 > 4)
                    HUD.logCenter("                                          Target bearing " + l + "\260" + ", range " + j1 + "km, height " + j + "m, heading " + i1 + "\260");
                else
                    HUD.logCenter(" ");
            } else
            {
                HUD.logCenter("                                                                             Target at " + s + "-" + d9 + ", height " + j + "m, heading " + i1 + "\260");
            }
        }
        return true;
    }

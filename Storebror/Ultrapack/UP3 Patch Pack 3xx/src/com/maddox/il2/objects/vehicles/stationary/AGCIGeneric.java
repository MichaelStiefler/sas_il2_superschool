...
public abstract class AGCIGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(AGCIGeneric.this.engineSFX != null)
                if(AGCIGeneric.this.engineSTimer >= 0)
                {
                    if(--AGCIGeneric.this.engineSTimer <= 0)
                    {
                        AGCIGeneric.this.engineSTimer = (int)AGCIGeneric.SecsToTicks(AGCIGeneric.Rnd(30F, 50F));
                        if(!AGCIGeneric.this.danger())
                            AGCIGeneric.this.engineSTimer = -AGCIGeneric.this.engineSTimer;
                    }
                } else
                if(++AGCIGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        if(aircraft.getSpeed(vector3d) > 20D && aircraft.pos.getAbsPoint().distance(point3d) < 100000D && aircraft.pos.getAbsPoint().z >= 150D && aircraft != World.getPlayerAircraft() && aircraft.getArmy() != this.myArmy)
        {
            ...
            if(aircraft1.pos.getAbsPoint().distance(point3d) < 100000D && aircraft1.pos.getAbsPoint().z >= 150D)
                flag = true;
            ...
            if(d4 > 260D)
                ...
                s = "" + c1;
            new String();
            int j = (int)(Math.floor(aircraft.pos.getAbsPoint().z * 3.2808399000000001D * 0.10000000000000001D) * 10D);
            new String();
            ...
            if(i1 < 0)
                i1 = 360 + i1;
            int j1 = (int)(Math.ceil(Math.sqrt(d14 * d14 + d13 * d13)) * 0.62137119200000002D);
            if(flag)
            {
                if(j1 > 4)
                    HUD.logCenter("                                          Target bearing " + l + "\260" + ", range " + j1 + " miles, height " + j + "ft, heading " + i1 + "\260");
                else
                    HUD.logCenter(" ");
            } else
            {
                HUD.logCenter("                                                                             Target at " + s + "-" + d9 + ", height " + j + "ft, heading " + i1 + "\260");
            }
        }
        return true;
    }

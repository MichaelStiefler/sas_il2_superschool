...
public class FW_189A2 extends Scheme3 implements TypeScout, TypeBomber {
	...
	public void msgShot(Shot var1) {
		...
		if (var1.chunkName.startsWith("WingLIn")) {
			...
			if ((Aircraft.v1.x < 0.25D) && (World.Rnd().nextFloat() < 0.25F)
					&& (World.Rnd().nextFloat(0.01F, 0.121F) < var1.mass)) {
	...
	public void rareAction(float var1, boolean var2) {
		...
		for (int var3 = 1; var3 < 4; ++var3) {
			if (this.FM.getAltitude() < 3000.0F) {
				...
				this.hierMesh().chunkVisible("HMask" + var3 + "_D0",
						this.hierMesh().isChunkVisible("Pilot" + var3 + "_D0"));
			}
		}
	...
	static {
		...
		Property.set(clazz, "cockpitClass",
				new Class[] { CockpitFw189.class, CockpitFw189_RGunner.class, CockpitFW189_BGunner.class });
		Aircraft.weaponTriggersRegister(clazz, new int[] { 0, 0, 3, 3, 3, 3, 10, 10, 11, 11 });
		Aircraft.weaponHooksRegister(clazz, new String[] { "_MGUN01", "_MGUN02", "_ExternalBomb01", "_ExternalBomb02",
				"_ExternalBomb03", "_ExternalBomb04", "_MGUN03", "_MGUN04", "_MGUN05", "_MGUN06" });
	}
}

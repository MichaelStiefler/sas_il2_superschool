...
public abstract class AircraftLH extends Aircraft {
	...
	public void beaconPlus() {
		if (this.bWantBeaconKeys && (Main.cur().mission.getBeacons(this.getArmy()) == null
				|| Main.cur().mission.getBeacons(this.getArmy()).size() != 0))
	...
	public void auxPlus(int i) {
		...
		case 1:
			...
			if (printCompassHeading && World.cur().diffCur.RealisticNavigationInstruments && this.bWantBeaconKeys)
	...
	protected void hitFlesh(int i, Shot shot, int i_0_) {
		...
		if (World.cur().diffCur.RealisticPilotVulnerability) {
			if (i_1_ > 0)
				this.FM.AS.setBleedingPilot(shot.initiator, i, i_1_);
			if (i == 0 && i_0_ > 0)
				this.FM.AS.woundedPilot(shot.initiator, i_0_, i_2_);
		}
		if (this.FM.AS.astatePilotStates[i] > 95 && i_0_ == 0)
	...
	public void update(float f) {
		super.update(f);
		if (World.getPlayerAircraft() == this /* && !NetMissionTrack.isPlaying() */ && !World.isPlayerGunner()
				&& this.FM.AS.astatePilotStates[0] <= 95) {
...
//            }
		...
		} else if (this.FM instanceof Maneuver) {
			this.headTurn(f, (Maneuver) this.FM);
		}

...
public abstract class TakiGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(TakiGeneric.this.engineSFX != null)
                if(TakiGeneric.this.engineSTimer >= 0)
                {
                    if(--TakiGeneric.this.engineSTimer <= 0)
                    {
                        TakiGeneric.this.engineSTimer = (int)TakiGeneric.SecsToTicks(TakiGeneric.Rnd(5F, 5F));
                        if(!TakiGeneric.this.danger())
                            TakiGeneric.this.engineSTimer = -TakiGeneric.this.engineSTimer;
                    }
                } else
                if(++TakiGeneric.this.engineSTimer >= 0)
                {

...
public abstract class FACGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(FACGeneric.this.engineSFX != null)
                if(FACGeneric.this.engineSTimer >= 0)
                {
                    if(--FACGeneric.this.engineSTimer <= 0)
                    {
                        FACGeneric.this.engineSTimer = (int)FACGeneric.SecsToTicks(FACGeneric.Rnd(60F, 60F));
                        if(!FACGeneric.this.danger())
                            FACGeneric.this.engineSTimer = -FACGeneric.this.engineSTimer;
                    }
                } else
                if(++FACGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(java.util.Map.Entry entry1 = Engine.name2Actor().nextEntry(null); entry1 != null; entry1 = Engine.name2Actor().nextEntry(entry1))
        {
            ...
            if(Actor.isAlive(actor1) && ((actor1 instanceof TankGeneric) || (actor1 instanceof ArtilleryGeneric) || (actor1 instanceof CarGeneric)) && actor1.getArmy() != this.myArmy && actor1.pos.getAbsPoint().distance(point3d) < 500D)
            {
                ...
                new String();
                String s2 = "units";
                if(actor1 instanceof TankGeneric)
                    s2 = "armor";
                if(actor1 instanceof ArtilleryGeneric)
                    s2 = "guns";
                if(actor1 instanceof CarGeneric)
                    s2 = "vehicles";
                new String();
                ...
                {
                    HUD.logCenter("                                                                             Request CAS at map grid " + s + "-" + d9);
                }
            }
        }

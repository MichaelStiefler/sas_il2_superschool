    ...
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(MonicaGeneric.this.engineSFX != null)
                if(MonicaGeneric.this.engineSTimer >= 0)
                {
                    if(--MonicaGeneric.this.engineSTimer <= 0)
                    {
                        ...
                        if(!MonicaGeneric.this.danger())
                            MonicaGeneric.this.engineSTimer = -MonicaGeneric.this.engineSTimer;
                    }
                } else
                if(++MonicaGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((actor instanceof Aircraft) && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D)
            {
                ...
                if(k1 <= 6400D && k1 >= 308D && i2 >= 255 && i2 <= 285 && Math.sqrt(j1 * j1) >= 120D)
                    HUD.logCenter("                                                                   (" + j2 + s + ")");
            }
        }

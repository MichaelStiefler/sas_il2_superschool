...
public abstract class HS123xyz extends Scheme1
		implements TypeScout, TypeFighter, TypeTNBFighter, TypeDiveBomber, TypeStormovik {
	...
	public void moveCockpitDoor(float f) {
		...
		this.hierMesh().chunkSetAngles("blister0_D0", 0.0F, -f * 177.7F, 0.0F);
		if ((this.FM.getSpeedKMH() > 314F) && (f < 0.6F) && !this.blisterRemoved) {
	...
	private final void doRemoveBlisters() {
		...
		if (this.hierMesh().chunkFindCheck("blister0_D0") != -1) {
			this.hierMesh().hideSubTrees("blister0_D0");
			Wreckage wreckage = new Wreckage(this, this.hierMesh().chunkFind("blister0_D0"));
	...
	public void moveWheelSink() {
		if (this.FM.Gears.onGround()) {
			this.suspension = this.suspension + 0.008F;
		} else {
			this.suspension = this.suspension - 0.008F;
		}
		...
		Aircraft.xyz[2] = Aircraft.cvt(this.suspL, 0.0F, 0.24F, 0.0F, 0.24F);
		...
		Aircraft.xyz[2] = Aircraft.cvt(this.suspR, 0.0F, 0.24F, 0.0F, 0.24F);
		...
		if (s.startsWith("xx")) {
			...
			if (s.startsWith("xxeng")) {
				...
				if (s.endsWith("prop")) {
					...
					if (this.getEnergyPastArmor(2.1F, shot) > 0.0F) {
						...
						if (World.Rnd().nextFloat() < (shot.power / 50000F)) {
							...
							Aircraft.debugprintln(this, "*** Engine Module: Crank Case Hit, Readyness Reduced to "
									+ this.FM.EI.engines[0].getReadyness() + "..");
						}
						this.FM.EI.engines[0].setReadyness(shot.initiator, this.FM.EI.engines[0].getReadyness()
								- World.Rnd().nextFloat(0.0F, shot.power / 48000F));
						Aircraft.debugprintln(this, "*** Engine Module: Crank Case Hit, Readyness Reduced to "
								+ this.FM.EI.engines[0].getReadyness() + "..");
					}
					this.getEnergyPastArmor(12.7F, shot);
				} else if (s.endsWith("cyls")) {
					if ((this.getEnergyPastArmor(World.Rnd().nextFloat(0.2F, 4.4F), shot) > 0.0F)
							&& (World.Rnd().nextFloat() < (this.FM.EI.engines[0].getCylindersRatio() * 1.12F))) {
						this.FM.EI.engines[0].setCyliderKnockOut(shot.initiator,
								World.Rnd().nextInt(1, (int) (shot.power / 4800F)));
						Aircraft.debugprintln(this,
								"*** Engine Module: Cylinders Hit, " + this.FM.EI.engines[0].getCylindersOperable()
				...
				if (s.startsWith("xxlockr")
						&& (this.getEnergyPastArmor(1.5F * World.Rnd().nextFloat(1.0F, 3F), shot) > 0.0F)) {
				...
				if (s.startsWith("xxspart") && (this.chunkDamageVisible("Tail1") > 2)
						&& (this.getEnergyPastArmor(9.5F
								/ (float) Math.sqrt((Aircraft.v1.y * Aircraft.v1.y) + (Aircraft.v1.z * Aircraft.v1.z)),
								shot) > 0.0F)) {
					Aircraft.debugprintln(this, "*** Tail1 Spars Broken in Half..");
					this.nextDMGLevels(1, 2, "Tail1_D3", shot.initiator);
				} else if (s.startsWith("xxspark")) {
					if ((this.chunkDamageVisible("Keel1") > 1)
							&& (this.getEnergyPastArmor(6.8F * World.Rnd().nextFloat(1.0F, 1.5F), shot) > 0.0F)) {
					...
					if ((this.chunkDamageVisible("StabR") > 1)
						...
						&& (this.getEnergyPastArmor(9.5F * World.Rnd().nextFloat(1.0F, 3F), shot) > 0.0F)) {
				...
				} else if (s.startsWith("xxsparlo") && (World.Rnd().nextFloat() < 0.25F)
						&& (this.getEnergyPastArmor(9.5F * World.Rnd().nextFloat(1.0F, 3F), shot) > 0.0F)) {
					Aircraft.debugprintln(this, "*** WingLOut Spars Damaged..");
					this.nextDMGLevels(1, 2, "WingLOut_D" + this.chunkDamageVisible("WingLOut"), shot.initiator);
				} else if (s.startsWith("xxsparro") && (World.Rnd().nextFloat() < 0.25F)
						&& (this.getEnergyPastArmor(9.5F * World.Rnd().nextFloat(1.0F, 3F), shot) > 0.0F)) {
	...
	static {
		Property.set(HS123xyz.class, "originCountry", PaintScheme.countryGermany);
	}
}

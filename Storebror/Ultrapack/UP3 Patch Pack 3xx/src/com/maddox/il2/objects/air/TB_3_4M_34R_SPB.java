...
public class TB_3_4M_34R_SPB extends TB_3 implements TypeDockable {
	...
	public void rareAction(float f, boolean flag) {
		...
		if (flag) {
			...
			for (int j = 0; j < 4; j++)
				if (FM.AS.astateTankStates[j] > 3 && FM.AS.astatePilotStates[4] < 50F
						&& FM.AS.astatePilotStates[7] < 50F && World.Rnd().nextFloat() < 0.1F)
					FM.AS.repairTank(j);
	...
	public void msgExplosion(Explosion explosion) {
		...
		if (explosion.chunkName != null
				&& (explosion.chunkName.startsWith("Wing") || explosion.chunkName.startsWith("Tail"))
				&& explosion.chunkName.endsWith("D3") && explosion.power < 0.014F) {
			return;
		} else {
...
//        DebugLog("typeDockableIsDocked()", 1);
	...
	public void typeDockableAttemptDetach() {
		...
		DebugLog("FM.AS.isMaster() = " + FM.AS.isMaster(), 1);
		if (FM.AS.isMaster()) {
	...
	public void typeDockableRequestAttach(Actor actor) {
		...
		DebugLog("parasiteAircraft.FM.AS.isMaster()=" + parasiteAircraft.FM.AS.isMaster()
				+ ", parasiteAircraft.FM.Gears.onGround()=" + parasiteAircraft.FM.Gears.onGround()
				+ ", parasiteAircraft.FM.getSpeedKMH()=" + parasiteAircraft.FM.getSpeedKMH()
				+ ", this.FM.Gears.onGround()=" + this.FM.Gears.onGround() + ", this.FM.getSpeedKMH()="
				+ this.FM.getSpeedKMH(), 1);
		if (parasiteAircraft.FM.AS.isMaster() && parasiteAircraft.FM.Gears.onGround()
				&& parasiteAircraft.FM.getSpeedKMH() < 10F && FM.getSpeedKMH() < 10F) {
			attachMaxDistance = 5D;
			attachEnabled = true;
		} else if (parasiteAircraft.FM.AS.isMaster() && !parasiteAircraft.FM.Gears.onGround()
				&& !this.FM.Gears.onGround() && parasiteAircraft.FM.getSpeedKMH() > 50F
				&& this.FM.getSpeedKMH() > 50F) {
			...
			if (Math.abs(angleBetween) < 25F
					&& Math.abs(this.FM.getSpeedKMH() - parasiteAircraft.FM.getSpeedKMH()) < 30F) {
		...
		if (!attachEnabled) {
			...
			if (parasiteAircraft == World.getPlayerAircraft() && (parasiteAircraft instanceof TypeDockableAutoDock)) {
				if (((TypeDockableAutoDock) parasiteAircraft).TypeDockableAutoDockIsDockModeActive()) {
					((TypeDockableAutoDock) parasiteAircraft).showDockDistance(-1);
				}
			}
			// ---
			return;
		...
		if (Actor.isValid(drones[hookIndex])) {
			DebugLog("drones[" + hookIndex + "] = " + drones[hookIndex].getClass().getName(), 1);
			// TODO: +++ Auto Docking Mode
			if (parasiteAircraft == World.getPlayerAircraft() && (parasiteAircraft instanceof TypeDockableAutoDock)) {
				if (((TypeDockableAutoDock) parasiteAircraft).TypeDockableAutoDockIsDockModeActive()) {
					((TypeDockableAutoDock) parasiteAircraft).showDockDistance(-1);
		...
		if (parasiteAircraft == World.getPlayerAircraft() && (parasiteAircraft instanceof TypeDockableAutoDock)) {
			if (((TypeDockableAutoDock) parasiteAircraft).TypeDockableAutoDockIsDockModeActive()) {
				if (hookDistance >= attachMaxDistance * 20D) {
					((TypeDockableAutoDock) parasiteAircraft).showDockDistance(-1);
				} else {
					((TypeDockableAutoDock) parasiteAircraft).showDockDistance((int) Aircraft.cvt((float) hookDistance,
							(float) attachMaxDistance, (float) attachMaxDistance * 20F, 0F, 20F));
				}
			}
		}
		// ---

		if (hookDistance >= attachMaxDistance)
			return;
		DebugLog("FM.AS.isMaster() = " + FM.AS.isMaster(), 1);
		if (FM.AS.isMaster()) {
			typeDockableRequestAttach(actor, hookIndex, true);
			return;
		} else {
			AircraftStateClearSendedDockableItemsToMaster(FM.AS, 32);
			FM.AS.netToMaster(32, hookIndex, 0, actor);
			return;
		}
	...
	public void typeDockableRequestDetach(Actor actor) {
		...
		for (int i = 0; i < DRONES_LENGTH; i++) {
			if (actor != drones[i])
				continue;
			DebugLog("actor == drones[" + i + "] (" + drones[i].getClass().getName() + ")", 1);
			...
			if (FM.AS.isMaster())
				...
				AircraftStateClearSendedDockableItemsToMaster(FM.AS, 33);
	...
	public void typeDockableRequestAttach(Actor actor, int i, boolean flag) {
		...
		if (flag) {
			if (FM.AS.isMaster()) {
				...
				AircraftStateClearSendedDockableItemsToMirrors(FM.AS, 34);
				FM.AS.netToMirrors(34, i, 1, actor);
				typeDockableDoAttachToDrone(actor, i);
			} else {
				DebugLog("FM.AS.isMaster() = false", 1);
				AircraftStateClearSendedDockableItemsToMaster(FM.AS, 34);
	...
	public void typeDockableRequestDetach(Actor actor, int i, boolean flag) {
		...
		if (flag)
			if (FM.AS.isMaster()) {
				DebugLog("FM.AS.isMaster() = true", 1);
				AircraftStateClearSendedDockableItemsToMirrors(FM.AS, 35);
				FM.AS.netToMirrors(35, i, 1, actor);
				typeDockableDoDetachFromDrone(i);
			} else {
				DebugLog("FM.AS.isMaster() = false", 1);
				AircraftStateClearSendedDockableItemsToMaster(FM.AS, 35);
	...
	public void typeDockableDoAttachToDrone(Actor actor, int i) {
		...
		if (!Actor.isValid(drones[i])) {
			...
			drones[i] = actor;
			DebugLog("Attaching drones[" + i + "] (" + drones[i].getClass().getName() + ")", 1);
	...
	public void typeDockableDoDetachFromDrone(int i) {
		...
		if (!Actor.isValid(drones[i])) {
			...
			DebugLog("Detaching drones[" + i + "] (" + drones[i].getClass().getName() + ")", 1);
	...
	private static void DebugLog(String logLine, int minLogLevel) {
		if (DEBUG >= minLogLevel) {
			System.out.println("[TB_3_4M_34R_SPB] " + logLine);
		}
	}
	...
	protected static Aircraft GetNearestFriendlyInstance(Aircraft referenceAircraft, float maxDistance) {
		...
		for (int targetListIndex = 0; targetListIndex < targetListSize; targetListIndex++) {
			...
			if (curTarget instanceof TB_3_4M_34R_SPB && curTarget != referenceAircraft
					&& curTarget.getArmy() == referenceArmy) {
				Point3d curTargetPos = curTarget.pos.getAbsPoint();
				double curTargetDistanceSquared = ((referencePos.x - curTargetPos.x) * (referencePos.x - curTargetPos.x)
						+ (referencePos.y - curTargetPos.y) * (referencePos.y - curTargetPos.y)
	...
	protected static void AircraftStateClearSendedDockableItems(AircraftState as, int asMessageIndex,
			String fieldName) {
		Object[] itemsToMaster = (Object[]) Reflection.getValue(as, fieldName);
		if (itemsToMaster == null)
			return;
		if (itemsToMaster.length <= asMessageIndex) {
			System.out.println("[TB_3_4M_34R_SPB] cannot clear " + fieldName + "[" + asMessageIndex + "], " + fieldName
					+ " length is " + itemsToMaster.length);
			return;
		}
		if (itemsToMaster[asMessageIndex] == null)
			return;
	...
	static {
		...
		Property.set(class1, "cockpitClass", new Class[] { CockpitTB_3.class, CockpitTB_3_Bombardier2.class,
				CockpitTB_3_NGunner.class, CockpitTB_3_TGunner3.class, CockpitTB_3_RGunner.class });
		weaponTriggersRegister(class1,
				new int[] { 10, 10, 11, 11, 12, 12, 9, 9, 9, 9, 3, 3, 3, 3, 3, 3, 3, 3, 9, 9, 9, 9, 3, 3, 3, 3, 3, 3 });
		weaponHooksRegister(class1,
				new String[] { "_MGUN01", "_MGUN02", "_MGUN03", "_MGUN04", "_MGUN05", "_MGUN06", "_ExternalDev01",
						"_ExternalDev02", "_ExternalDev03", "_ExternalDev04", "_ExternalBomb01", "_ExternalBomb02",
						"_ExternalBomb03", "_ExternalBomb04", "_ExternalBomb05", "_ExternalBomb06", "_ExternalBomb07",
						"_ExternalBomb08", "_ExternalDev05", "_ExternalDev06", "_ExternalDev07", "_ExternalDev08",
						"_ExternalBomb09", "_ExternalBomb10", "_ExternalBomb11", "_ExternalBomb12", "_BombSpawn01",
						"_BombSpawn02" });
	}
}

...
public abstract class GTWGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(GTWGeneric.this.engineSFX != null)
                if(GTWGeneric.this.engineSTimer >= 0)
                {
                    if(--GTWGeneric.this.engineSTimer <= 0)
                    {
                        GTWGeneric.this.engineSTimer = (int)GTWGeneric.SecsToTicks(GTWGeneric.Rnd(10F, 10F));
                        if(!GTWGeneric.this.danger())
                            GTWGeneric.this.engineSTimer = -GTWGeneric.this.engineSTimer;
                    }
                } else
                if(++GTWGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((actor instanceof Aircraft) && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D)
            {
                ...
                if(k1 <= 4000D && k1 >= 300D && i2 >= 255 && i2 <= 285 && Math.sqrt(j1 * j1) >= 120D)
                    HUD.logCenter("                                                                   (" + j2 + s + ")");
            }
        }

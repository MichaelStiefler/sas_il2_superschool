...
public abstract class WindowGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(WindowGeneric.this.engineSFX != null)
                if(WindowGeneric.this.engineSTimer >= 0)
                {
                    if(--WindowGeneric.this.engineSTimer <= 0)
                    {
                        ...
                        if(!WindowGeneric.this.danger())
                            WindowGeneric.this.engineSTimer = -WindowGeneric.this.engineSTimer;
                    }
                } else
                if(++WindowGeneric.this.engineSTimer >= 0)
                {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((actor instanceof TypeBomber) && actor.getArmy() == this.myArmy && actor.getSpeed(vector3d) > 20D)
            {
                ...
                if((aircraft instanceof TypeFighter) && aircraft.getSpeed(vector3d) > 20D)
                {
                    ...
                    if(i1 <= 1000D && Math.sqrt(d6 * d6) <= 500D && flag2 && flag3 && !flag)
                        ((Pilot)aircraft.FM).set_maneuver(14);
                    if(i1 <= 1000D && Math.sqrt(d6 * d6) <= 500D && flag2 && flag4 && flag)

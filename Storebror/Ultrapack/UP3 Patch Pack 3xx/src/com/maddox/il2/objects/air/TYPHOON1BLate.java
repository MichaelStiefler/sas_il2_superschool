...
public class TYPHOON1BLate extends TEMPEST {
	...
	protected void moveFan(float f) {
		...
		for (int j = 0; j < this.FM.EI.getNum(); j++) {
			if (this.oldProp[j] < 2) {
				...
				if (i >= 1)
					i = 1;
				if ((i != this.oldProp[j]) && (this.hierMesh()
						.isChunkVisible(isPropB ? Propsb[j][this.oldProp[j]] : Props[j][this.oldProp[j]]))) {
					this.hierMesh().chunkVisible(isPropB ? Propsb[j][this.oldProp[j]] : Props[j][this.oldProp[j]],
							false);
					this.oldProp[j] = i;
			...
			if (i == 0) {
				...
				float f1 = 57.299999F * this.FM.EI.engines[j].getw();
				f1 %= 2880.0F;
				f1 /= 2880.0F;
				if (f1 <= 0.5F)
					f1 *= 2.0F;
	...
	public void moveCockpitDoor(float f) {
		...
		if (f < 0.5F)
			Aircraft.ypr[2] = Aircraft.cvt(f, 0.01F, 0.5F, 0.0F, 3.0F);
		else
			Aircraft.ypr[2] = Aircraft.cvt(f, 0.5F, 0.99F, 3.0F, 1.0F);
	...
	public void rareAction(float f, boolean flag) {
		...
		if (this.FM.getAltitude() < 3000F && this.FM.CT.getCockpitDoor() > 0.8F)
			this.hierMesh().chunkVisible("HMask1_D0", false);
		else
			this.hierMesh().chunkVisible("HMask1_D0", this.hierMesh().isChunkVisible("Pilot1_D0"));
	}

	public static final String Propsb[][] = { { "Prop1b_D0", "PropRot1b_D0", "Prop1b_D1" },
			{ "Prop2b_D0", "PropRot2b_D0", "Prop2b_D1" }, { "Prop3b_D0", "PropRot3b_D0", "Prop3b_D1" },
			{ "Prop4b_D0", "PropRot4b_D0", "Prop4b_D1" }, { "Prop5b_D0", "PropRot5b_D0", "Prop5b_D1" },
			{ "Prop6b_D0", "PropRot6b_D0", "Prop6b_D1" } };
	...
	static {
		...
		Aircraft.weaponTriggersRegister(class1,
				new int[] { 1, 1, 1, 1, 9, 9, 9, 9, 3, 3, 9, 9, 2, 2, 2, 2, 2, 2, 2, 2 });
		Aircraft.weaponHooksRegister(class1,
				new String[] { "_CANNON01", "_CANNON02", "_CANNON03", "_CANNON04", "_ExternalDev01", "_ExternalDev02",
						"_ExternalDev05", "_ExternalDev06", "_ExternalBomb01", "_ExternalBomb02", "_ExternalDev07",
						"_ExternalDev08", "_ExternalRock01", "_ExternalRock02", "_ExternalRock03", "_ExternalRock04",
						"_ExternalRock05", "_ExternalRock06", "_ExternalRock09", "_ExternalRock10" });
	}
}

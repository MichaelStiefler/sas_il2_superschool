...
public abstract class SPRGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(SPRGeneric.this.engineSFX != null)
                if(SPRGeneric.this.engineSTimer >= 0)
                {
                    if(--SPRGeneric.this.engineSTimer <= 0)
                    {
                        SPRGeneric.this.engineSTimer = (int)SPRGeneric.SecsToTicks(SPRGeneric.Rnd(45F, 45F));
                        if(!SPRGeneric.this.danger())
                            SPRGeneric.this.engineSTimer = -SPRGeneric.this.engineSTimer;
                    }
                } else
                if(++SPRGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((pilot.Wingman != null || pilot.crew > 1) && Actor.isAlive(actor) && ((actor instanceof TankGeneric) || (actor instanceof Wagon) || (actor instanceof ArtilleryGeneric) || (actor instanceof BigshipGeneric) || (actor instanceof ShipGeneric) || (actor instanceof CarGeneric)) && actor.getArmy() != this.myArmy)
            {
                this.pos.getAbs(point3d);

...
public abstract class CorkscrewGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(CorkscrewGeneric.this.engineSFX != null)
                if(CorkscrewGeneric.this.engineSTimer >= 0)
                {
                    if(--CorkscrewGeneric.this.engineSTimer <= 0)
                    {
                        ...
                        if(!CorkscrewGeneric.this.danger())
                            CorkscrewGeneric.this.engineSTimer = -CorkscrewGeneric.this.engineSTimer;
                    }
                } else
                if(++CorkscrewGeneric.this.engineSTimer >= 0)
                {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((actor instanceof TypeBomber) && actor.getArmy() == this.myArmy && actor.getSpeed(vector3d) > 20D)
            {
                ...
                if((aircraft instanceof TypeFighter) && aircraft.getSpeed(vector3d) > 20D)
                {
                    ...
                    if(i1 <= 500D && Math.sqrt(d6 * d6) <= 500D && flag2 && !flag && l <= 10)

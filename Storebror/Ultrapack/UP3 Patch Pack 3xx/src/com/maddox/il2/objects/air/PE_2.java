...
public abstract class PE_2 extends Scheme2a {
		...
		if (s.startsWith("xx")) {
			...
			if (s.startsWith("xxeng1")) {
				...
				if (s.endsWith("case")) {
					if (getEnergyPastArmor(World.Rnd().nextFloat(0.0F, 6.8F), shot) > 0.0F) {
						...
						if (World.Rnd().nextFloat() < shot.power / 28000F) {
							...
							debugprintln(this,
									"*** Engine1 Crank Case Hit - Cylinder Feed Out, "
						...
						debugprintln(this, "*** Engine1 Crank Case Hit - Readyness Reduced to "
				...
				if ((s.endsWith("cyl1") || s.endsWith("cyl2"))
						&& getEnergyPastArmor(World.Rnd().nextFloat(0.5F, 2.542F), shot) > 0.0F
						&& World.Rnd().nextFloat() < FM.EI.engines[0].getCylindersRatio() * 1.72F) {
					FM.EI.engines[0].setCyliderKnockOut(shot.initiator,
							World.Rnd().nextInt(1, (int) (shot.power / 4800F)));
					debugprintln(this, "*** Engine1 Cylinders Hit, " + FM.EI.engines[0].getCylindersOperable() + "/"
							+ FM.EI.engines[0].getCylinders() + " Left..");
					if (World.Rnd().nextFloat() < 0.01F) {
						FM.EI.engines[0].setEngineStuck(shot.initiator);
				...
				if (s.endsWith("eqpt") && getEnergyPastArmor(World.Rnd().nextFloat(0.001F, 0.2F), shot) > 0.0F
			...
			if (s.startsWith("xxeng2")) {
				...
				if (s.endsWith("case")) {
					if (getEnergyPastArmor(World.Rnd().nextFloat(0.0F, 6.8F), shot) > 0.0F) {
						...
						if (World.Rnd().nextFloat() < shot.power / 28000F) {
							...
							debugprintln(this,
									"*** Engine2 Crank Case Hit - Cylinder Feed Out, "
											+ FM.EI.engines[1].getCylindersOperable() + "/"
											+ FM.EI.engines[1].getCylinders() + " Left..");
						}
						if (World.Rnd().nextFloat() < 0.08F) {
						...
						FM.EI.engines[1].setReadyness(shot.initiator,
								FM.EI.engines[1].getReadyness() - World.Rnd().nextFloat(0.0F, shot.power / 48000F));
						debugprintln(this, "*** Engine2 Crank Case Hit - Readyness Reduced to "
				...
				if ((s.endsWith("cyl1") || s.endsWith("cyl2"))
						&& getEnergyPastArmor(World.Rnd().nextFloat(0.5F, 2.542F), shot) > 0.0F
						&& World.Rnd().nextFloat() < FM.EI.engines[1].getCylindersRatio() * 1.72F) {
					FM.EI.engines[1].setCyliderKnockOut(shot.initiator,
							World.Rnd().nextInt(1, (int) (shot.power / 4800F)));
					debugprintln(this, "*** Engine2 Cylinders Hit, " + FM.EI.engines[1].getCylindersOperable() + "/"
							+ FM.EI.engines[1].getCylinders() + " Left..");
					if (World.Rnd().nextFloat() < 0.01F) {
						FM.EI.engines[1].setEngineStuck(shot.initiator);
			...
			if (s.startsWith("xxlock")) {
				...
				if (s.startsWith("xxlockr")) {
					if ((s.startsWith("xxlockr1") || s.startsWith("xxlockr2"))
							&& getEnergyPastArmor(5.5F * World.Rnd().nextFloat(1.0F, 1.5F), shot) > 0.0F) {
						debuggunnery("Lock Construction: Rudder1 Lock Shot Off..");
						nextDMGLevels(3, 2, "Rudder1_D" + chunkDamageVisible("Rudder1"), shot.initiator);
					}
					if ((s.startsWith("xxlockr3") || s.startsWith("xxlockr4"))
							&& getEnergyPastArmor(5.5F * World.Rnd().nextFloat(1.0F, 1.5F), shot) > 0.0F) {
			...
			if (s.startsWith("xxspar")) {
				...
				if (s.startsWith("xxspart") && chunkDamageVisible("Tail1") > 2
						&& getEnergyPastArmor(7.5F / (float) Math.sqrt(v1.y * v1.y + v1.z * v1.z), shot) > 0.0F
	...
	private final void doHitMeATank(Shot shot, int i) {
		if (getEnergyPastArmor(0.2F, shot) > 0.0F)
			if (shot.power < 14100F) {
				...
				if (FM.AS.astateTankStates[i] > 0
						&& (World.Rnd().nextFloat() < 0.02F || shot.powerType == 3 && World.Rnd().nextFloat() < 0.25F))
	...
	public void doWoundPilot(int i, float f) {
		...
		case 2: // '\002'
			...
			// ------------------------------------
			if (FM.turret.length < 3)
				break;
			if (FM.turret[2] == null)
				break;
			FM.turret[2].setHealth(f);
			if (FM.turret.length < 4)
				break;
			if (FM.turret[3] == null)
				break;

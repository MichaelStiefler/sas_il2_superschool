...
public abstract class ASVGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(ASVGeneric.this.engineSFX != null)
                if(ASVGeneric.this.engineSTimer >= 0)
                {
                    if(--ASVGeneric.this.engineSTimer <= 0)
                    {
                        ASVGeneric.this.engineSTimer = (int)ASVGeneric.SecsToTicks(ASVGeneric.Rnd(15F, 15F));
                        if(!ASVGeneric.this.danger())
                            ASVGeneric.this.engineSTimer = -ASVGeneric.this.engineSTimer;
                    }
                } else
                if(++ASVGeneric.this.engineSTimer >= 0)
                {

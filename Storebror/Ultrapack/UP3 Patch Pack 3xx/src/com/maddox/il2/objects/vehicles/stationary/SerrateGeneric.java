...
public abstract class SerrateGeneric extends ActorHMesh
    implements com.maddox.il2.objects.ActorAlign
{
    class Move extends Interpolate
    {
        ...
        public boolean tick()
        {
            if(SerrateGeneric.this.engineSFX != null)
                if(SerrateGeneric.this.engineSTimer >= 0)
                {
                    if(--SerrateGeneric.this.engineSTimer <= 0)
                    {
                        ...
                        if(!SerrateGeneric.this.danger())
                            SerrateGeneric.this.engineSTimer = -SerrateGeneric.this.engineSTimer;
                    }
                } else
                if(++SerrateGeneric.this.engineSTimer >= 0)
                {
    ...
    private boolean danger()
    {
        ...
        for(java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry))
        {
            ...
            if((actor instanceof TypeFighter) && actor.getArmy() != this.myArmy && actor != World.getPlayerAircraft() && actor.getSpeed(vector3d) > 20D)
            {
                ...
                int l1 = k1 - j;
                int i2 = 16000;
                if(l1 >= 220 && l1 <= 320 && Math.sqrt(i1 * i1) <= 60D)
                    i2 = 0x13880;
                if(j1 <= i2)

...
//                this.target = this.danger; // TODO: AI TEST, didn't work.
    ...
    private void transportDefence() {
        ...
        if (l == Maneuver.WAYPOINT || l == Maneuver.FOLLOW) {
            if (AircraftState.isCheaterAdvancedTransportDefence(this.danger.actor)) {
                float onMeThreshold = CommonTools.cvt(this.dist, 300F, 1000F, 0.85F, 0.995F);
                if ((float)onMe.x > onMeThreshold && this.dist < 1500F) {
                    switch (TrueRandom.nextInt(5)) {
                        case 0:
                            l = Maneuver.EVADE_UP;
                            break;
                        case 1:

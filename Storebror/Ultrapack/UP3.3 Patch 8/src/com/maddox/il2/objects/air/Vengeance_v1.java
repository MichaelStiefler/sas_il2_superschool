    ...
    implements TypeStormovik, TypeDiveBomber
{
    ...
    public void update(float f)
    {
        ...
        super.update(f);
        FM.EI.engines[0].addVside *= 0.0000001D;
    }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                {
                    if(s.endsWith("1"))
                        getEnergyPastArmor((double)World.Rnd().nextFloat(10F, 60F) / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
                    if(s.endsWith("2"))
                        getEnergyPastArmor(19.1D / (Math.abs(Aircraft.v1.z) + 9.9999997473787516E-005D), shot);
                    if(s.endsWith("3"))
                        getEnergyPastArmor(12.7D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
                    if(s.endsWith("4") || s.endsWith("5"))
                        getEnergyPastArmor(9.5D / (Math.abs(Aircraft.v1.y) + 9.9999997473787516E-005D), shot);
                } else
                if(s.startsWith("xxarmort"))
                {
                    ...
                    if(s.endsWith("3"))
                        getEnergyPastArmor((double)World.Rnd().nextFloat(9.5F, 19.1F) / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
                }
                return;
            }

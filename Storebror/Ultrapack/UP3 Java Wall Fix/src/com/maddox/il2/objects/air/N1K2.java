...
public abstract class N1K2 extends Scheme1
    implements TypeFighter
{
    ...
    public void update(float f)
    {
        InitFlapMgr();
        super.update(f);
        float airSpeedPerSec = Pitot.Indicator((float)FM.Loc.z, FM.getSpeed());
        if(FM.CT.FlapsControl <= 0.29F && FM.CT.FlapsControl >= 0.05F)
        {
            ...
            if(FM.CT.getFlap() != kuusen_flap)
                if(kuusen_flap == 0.0F && FM.CT.getFlap() <= 0.050000000000000003D)
                {
                    sfxFlaps(false);
            ...
            if(Math.abs(oldFlap - FM.CT.getFlap()) <= 0.01D)
                sfxFlaps(false);
        }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("p3"))
                    getEnergyPastArmor(World.Rnd().nextFloat(8.7F, 9.81F) / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
                return;
            }

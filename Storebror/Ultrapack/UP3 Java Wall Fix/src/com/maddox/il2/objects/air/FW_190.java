    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("p6"))
                    getEnergyPastArmor(8D / (Math.abs(v1.x) + 9.9999997473787516E-006D), shot);
                return;
            }
            ...
            if(s.startsWith("xxspar"))
            {
                ...
                if(s.startsWith("xxspart") && chunkDamageVisible("Tail1") > 2 && getEnergyPastArmor(2.4F / (float)Math.sqrt(v1.y * v1.y + v1.z * v1.z), shot) > 0.0F)
                {
            ...
            if(s.startsWith("xxradiat"))
            {
                ...
                debugprintln(this, "*** Engine Module: Radiator Hit, Readyness Reduced to " + FM.EI.engines[0].getReadyness() + "..");
            }
            return;
        }

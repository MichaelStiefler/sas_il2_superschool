    ...
    implements TypeScout, TypeTransport
{
    ...
    protected void moveFan(float f)
    {
        ...
        hierMesh().chunkSetAngles(Props[k][0], 0.0F, -this.propPos[k], 0.0F);
    }

    private void stability()
    {
        double d = 0.0D;
        if(this.FM.getSpeedKMH() > this.FM.VmaxFLAPS)
    ...
    public void update(float f)
    {
        ...
        ((FlightModelMain) (super.FM)).Or.increment(f2 * (((FlightModelMain) (super.FM)).CT.getRudder() + this.FM.CT.getTrimRudderControl()), f2 * (this.FM.CT.getElevator() + this.FM.CT.getTrimElevatorControl()), f2 * (this.FM.CT.getAileron() + this.FM.CT.getTrimAileronControl()));
    }

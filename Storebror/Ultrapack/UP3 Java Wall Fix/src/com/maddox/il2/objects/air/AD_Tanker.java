...
public abstract class AD_Tanker extends Scheme1
    implements TypeTransport, TypeTankerDrogue
{
    ...
    public void update(float f)
    {
        ...
        if(bNeedSetup)
        {
            cGear = this.FM.CT.GearControl;
            bNeedSetup = false;
        }
        cGear = filter(f, this.FM.CT.GearControl, cGear, 999.9F, this.FM.CT.dvGear);
        if(prevGear2 > cGear)

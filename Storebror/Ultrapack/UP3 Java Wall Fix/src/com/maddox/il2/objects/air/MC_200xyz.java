    ...
    implements TypeFighter
{
    ...
    {
        resetYPRmodifier();
        xyz[1] = cvt(FM.Gears.gWheelSinking[0], 0.0F, 0.2085F, 0.0F, 0.2085F);
        hierMesh().chunkSetLocate("GearL3_D0", xyz, ypr);
        xyz[1] = cvt(FM.Gears.gWheelSinking[1], 0.0F, 0.2085F, 0.0F, 0.2085F);
    ...
    {
        float f1 = cvt(FM.EI.engines[0].getControlRadiator(), 0.0F, 1.0F, 0.0F, -32.2F);
        ...
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.startsWith("xxarmorp"))
                {
                    ...
                    case 1:
                        getEnergyPastArmor(22.76D / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                        if(shot.power <= 0.0F)
                            doRicochetBack(shot);
                        break;

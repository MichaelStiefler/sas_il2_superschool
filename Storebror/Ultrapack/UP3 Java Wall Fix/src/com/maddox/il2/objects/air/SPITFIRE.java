    ...
    implements TypeFighter
{
    ...
    {
        resetYPRmodifier();
        xyz[1] = cvt(FM.Gears.gWheelSinking[0], 0.0F, 0.247F, 0.0F, 0.247F);
        hierMesh().chunkSetLocate("GearL3_D0", xyz, ypr);
        xyz[1] = cvt(FM.Gears.gWheelSinking[1], 0.0F, 0.247F, 0.0F, 0.247F);
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("g1"))
                    getEnergyPastArmor(9.96F / (1E-005F + (float)Math.abs(v1.x)), shot);
                else
                if(s.endsWith("g2"))
                    getEnergyPastArmor(World.Rnd().nextFloat(30F, 50F) / (1E-005F + (float)Math.abs(v1.x)), shot);
                return;
            }

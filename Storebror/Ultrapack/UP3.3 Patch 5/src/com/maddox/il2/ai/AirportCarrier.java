...
public class AirportCarrier extends Airport {
    ...
    public AirportCarrier(BigshipGeneric bigshipgeneric, Loc[] locs) {
        this.ship = bigshipgeneric;
        this.pos = new ActorPosMove(this, new Loc());
        this.pos.setBase(bigshipgeneric, null, false);
        this.pos.reset();
        this.runway = locs;
        if (Mission.isDogfight()) {
            Point_Stay[][] point_stays = this.getStayPlaces();
    ...
    public int getArmy() {
        if (Actor.isAlive(this.ship)) return this.ship.getArmy();
    ...
    public void rebuildLandWay(FlightModel flightmodel) {
        if (!this.ship.isAlive()) flightmodel.AP.way.setLanding(false);
        else {
            this.tmpLoc.set(this.runway[1]);
            this.tmpLoc.add(this.ship.initLoc);
            float f = flightmodel.M.massEmpty * 3.333E-4F;
    ...
    public void rebuildLastPoint(FlightModel flightmodel) {
        if (Actor.isAlive(this.ship)) {
            ...
            if (flightmodel.AP.way.curr().Action == 2) {
                ...
                flightmodel.AP.way.curr().set(this.tmpP3d);
            }
    ...
    public double ShiftFromLine(FlightModel flightmodel) {
        this.tmpLoc.set(flightmodel.Loc);
    ...
    public boolean nearestRunway(Point3d point3d, Loc loc) {
        loc.add(this.runway[1], this.pos.getAbs());
        return true;
    }
    ...
    public int landingFeedback(Point3d point3d, Aircraft aircraft) {
        this.tmpLoc.set(this.runway[1]);
        this.tmpLoc.add(this.ship.initLoc);
        Aircraft aircraft_4_ = War.getNearestFriendAtPoint(this.tmpLoc.getPoint(), aircraft, 50.0F);
        if (aircraft_4_ != null && aircraft_4_ != aircraft) return 1;
    ...
    public void setTakeoffOld(Point3d point3d, Aircraft aircraft) {
        if (Actor.isValid(aircraft)) {
            ...
            this.tmpLoc.set(-(this.curPlaneShift * 200.0F), -(this.curPlaneShift * 100.0F), 300.0, 0.0F, 0.0F, 0.0F);
            this.tmpLoc.add(this.r);
            aircraft.pos.setAbs(this.tmpLoc);
            aircraft.pos.getAbs(this.tmpP3d, this.tmpOr);
    ...
    public Loc setClientTakeOff(Point3d point3d, Aircraft aircraft) {
        Loc loc = null;
        if (this.clientLoc != null) loc = new Loc(this.clientLoc);
        this.clientLoc = null;
        if (loc == null || !this.isLocValid(loc)) {
            this.setTakeoffOld(point3d, aircraft);
        ...
        aircraft.FM.brakeShoeLoc.sub(this.ship.pos.getAbs());
        aircraft.FM.brakeShoeLastCarrier = this.ship;
    ...
    private Point3d reservePlaceForPlane(CellAirPlane cellairplane, Aircraft aircraft) {
        CellAirField cellairfield = this.ship.getCellTO();
        if (cellairfield.findPlaceForAirPlane(cellairplane)) {
            ...
            double d = -cellairfield.leftUpperCorner().x - cellairfield.resX() * cellairfield.getCellSize() - cellairplane.ofsX;
            double d_10_ = cellairfield.leftUpperCorner().y - cellairfield.resY() * cellairfield.getCellSize() - cellairplane.ofsY;
            double d_11_ = this.runway[0].getZ();
            this.curPlaneShift++;
    ...
    public void setTakeoff(Point3d point3d, Aircraft[] aircrafts) {
        CellAirField cellairfield = this.ship.getCellTO();
        if (cellairfield == null) for (int i = 0; i < aircrafts.length; i++)
            this.setTakeoffOld(point3d, aircrafts[i]);
        else {
            this.ship.getSpeed(shipSpeed);

            // TODO: CTO Mod
            // ----------------------------------------
            int j = this.setAircraftDimensions(aircrafts[0]);
            // ----------------------------------------
            ...
            for (int k = 0; k < aircrafts.length; k++)
                if (Actor.isValid(aircrafts[k])) {
                    ...
                    if (cellairfield.findPlaceForAirPlaneCarrier(cellairplane)) {
                        ...
                        double d = -((com.maddox.JGP.Tuple3d) cellairfield.leftUpperCorner()).x - cellairfield.resX() * cellairfield.getCellSize() - cellairplane.getWidth() / 2;
                        double d1 = ((com.maddox.JGP.Tuple3d) cellairfield.leftUpperCorner()).y - cellairfield.resY() * cellairfield.getCellSize() - cellairplane.ofsY;
                        double d2 = this.runway[0].getZ();
                        this.tmpLoc.set(d1, d, d2 + ((FlightModelMain) ((SndAircraft) aircrafts[k]).FM).Gears.H, this.runway[0].getAzimut(), this.runway[0].getTangage(), this.runway[0].getKren());
                        this.tmpLoc.add(((Actor) this.ship).pos.getAbs());
                        Point3d point3d1 = this.tmpLoc.getPoint();
                        Orient orient = this.tmpLoc.getOrient();
                        ...
                        if (((SndAircraft) aircrafts[k]).FM instanceof com.maddox.il2.ai.air.Maneuver) ((com.maddox.il2.ai.air.Maneuver) ((SndAircraft) aircrafts[k]).FM).direction = ((Actor) aircrafts[k]).pos.getAbsOrient().getAzimut();
                        ...
                        ((SndAircraft) aircrafts[k]).FM.brakeShoeLoc.sub(((Actor) this.ship).pos.getAbs());
                        ((SndAircraft) aircrafts[k]).FM.brakeShoeLastCarrier = this.ship;
                    ...
                    // ----------------------------------------
                    else this.setTakeoffOld(point3d, aircrafts[k]);
                }
    ...
    public void getTakeoff(Aircraft aircraft, Loc loc) {
        this.tmpLoc.sub(loc, this.ship.initLoc);
        this.tmpLoc.set(this.tmpLoc.getPoint().x, this.tmpLoc.getPoint().y, this.runway[0].getZ() + aircraft.FM.Gears.H, this.runway[0].getAzimut(), this.runway[0].getTangage(), this.runway[0].getKren());
    ...
    private Point_Stay[][] getStayPlaces() {
        ...
        Class var_class = this.ship.getClass();
        ...
        if (point_stays == null) {
            ...
            for (;;) {
                ...
                cellairfield.placeAirPlane(cellairplane, cellairfield.resX(), cellairfield.resY());
                double d = -cellairfield.leftUpperCorner().x - cellairfield.resX() * cellairfield.getCellSize() - cellairplane.ofsX;
                double d_26_ = cellairfield.leftUpperCorner().y - cellairfield.resY() * cellairfield.getCellSize() - cellairplane.ofsY;
        ...
        for (int i = 0; i < point_stays.length; i++) {
            ...
            double d_29_ = point_stay.y;
            double d_30_ = this.runway[0].getZ();
            this.tmpLoc.set(d, d_29_, d_30_, this.runway[0].getAzimut(), this.runway[0].getTangage(), this.runway[0].getKren());
            this.tmpLoc.add(this.ship.pos.getAbs());
            Point3d point3d = this.tmpLoc.getPoint();
    ...
    public Loc requestCell(Aircraft aircraft) {
        if (!this.ship().isAlive()) return invalidLoc;
        CellAirPlane cellairplane = aircraft.getCellAirPlane();
        Point3d point3d = this.reservePlaceForPlane(cellairplane, null);
        if (point3d != null) {
            double d = point3d.x;
            double d_31_ = point3d.y;
            double d_32_ = point3d.z;
            Loc loc = new Loc();
            loc.set(d, d_31_, d_32_ + aircraft.FM.Gears.H, this.runway[0].getAzimut(), this.runway[0].getTangage(), this.runway[0].getKren());
            return loc;
        }
        return invalidLoc;
    }
    ...
    public void setGuiCallback(GUINetClientDBrief guinetclientdbrief) {
        this.ui = guinetclientdbrief;
    }

    public void setClientLoc(Loc loc) {
        this.clientLoc = loc;
        boolean bool = this.isLocValid(loc);
        if (this.ui != null) this.ui.flyFromCarrier(bool);
    }
    ...
    private int setAircraftDimensions(com.maddox.il2.objects.air.Aircraft aircraft) {
        byte byte0 = -1;
        if (aircraft instanceof com.maddox.il2.objects.air.F4F && !(aircraft instanceof com.maddox.il2.objects.air.F4F3)) byte0 = 5;
        else if (aircraft instanceof com.maddox.il2.objects.air.F6F || aircraft instanceof com.maddox.il2.objects.air.F4U) byte0 = 6;
        else if (aircraft instanceof com.maddox.il2.objects.air.TBF) byte0 = 7;
        else if (aircraft instanceof com.maddox.il2.objects.air.B5N || aircraft instanceof com.maddox.il2.objects.air.B6N) byte0 = 8;
        else if (aircraft instanceof com.maddox.il2.objects.air.D3A) byte0 = 12;
        else if (aircraft instanceof com.maddox.il2.objects.air.A6M2_21) byte0 = 11;
        else if (aircraft instanceof com.maddox.il2.objects.air.SEAFIRE3 || aircraft instanceof com.maddox.il2.objects.air.SEAFIRE3F) byte0 = 5;
        if (byte0 == -1) try {
            // TODO: Disabled for 410 compatibility
            /*
             * if (aircraft instanceof com.maddox.il2.objects.air.F9F) byte0 = 6;
             */
        } catch (java.lang.Throwable throwable) {}
        if (byte0 == -1) try {
            // TODO: Disabled for 410 compatibility
            /*
             * if (aircraft instanceof com.maddox.il2.objects.air.SeaFury) byte0 = 6;
             */
        } catch (java.lang.Throwable throwable1) {}
        if (byte0 == -1) try {
            if (aircraft instanceof com.maddox.il2.objects.air.Swordfish) byte0 = 6;

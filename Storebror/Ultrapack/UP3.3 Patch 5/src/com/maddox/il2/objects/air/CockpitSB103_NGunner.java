...
public class CockpitSB103_NGunner extends CockpitGunner
{
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        mesh.chunkSetAngles("esbr_2_switch_1", 0.0F, 0.0F, (float)(aircraft().FM.CT.getBombDropMode() - 1) * -8.3F);
        mesh.chunkSetAngles("esbr_2_switch_2", 0.0F, 0.0F, 0.0F);
        if(aircraft().FM.CT.getSelectedBombReleaseDelayIndex() < bombTrainSwitchPos.length)
            mesh.chunkSetAngles("esbr_2_switch_3", 0.0F, 0.0F, bombTrainSwitchPos[aircraft().FM.CT.getSelectedBombReleaseDelayIndex()]);
        ...
        if(RPK)
            if(useRealisticNavigationInstruments())
            {
                ...
                mesh.chunkSetAngles("Z_ND_TInd", cvt(setNew.beaconRange, 0.0F, 30F, -25F, 25F), 0.0F, 0.0F);
            } else
            {
                mesh.chunkSetAngles("Z_ND_RPK", -cvt(setNew.waypointAzimuth.getDeg(f), -30F, 30F, -45F, 45F), 0.0F, 0.0F);
            }
    ...
    protected void interpTick()
    {
        ...
        if(RPK)
            ...
            {
                setNew.waypointAzimuth.setDeg(setOld.waypointAzimuth.getDeg(1.0F), waypointAzimuth(1.0F) - fm.Or.azimut() - 90F);
            }
        if(bGunFire)
        {
            if(hook1 == null)
    ...
    public CockpitSB103_NGunner()
    {
        super("3DO/Cockpit/SB_103-NGunner/hier.him", "he111_gunner");
        patron = 1.0F;
        patronMat = null;
        bNeedSetUp = true;
        RPK = false;
    ...
    public CockpitSB103_NGunner(String s)
    {
        ...
        RPK = true;
        cockpitNightMats = (new String[] {
            "ACHO_arrow", "Dprib_one", "Dprib_six", "equip_AN4_sh", "prib_one", "Prib_six", "Tune_Indicator", "RPK", "RPK_dmg"
        });
        setNightMats(false);
        int i = -1;
        i = mesh.materialFind("patron");
    ...
    private float bombTrainSwitchPos[] = {
        0.0F, 55F, 94F, 150F, 199F, 247F
    };
    ...
    static 
    {
        Property.set(CockpitSB103_NGunner.class, "aiTuretNum", 0);
        Property.set(CockpitSB103_NGunner.class, "weaponControlNum", 10);
        Property.set(CockpitSB103_NGunner.class, "astatePilotIndx", 1);
        Property.set(CockpitSB103_NGunner.class, "normZN", 1.1F);
        Property.set(CockpitSB103_NGunner.class, "gsZN", 1.1F);
    }
}

...
public class BulletAircraftGeneric extends Bullet {
    ...
    public void move(float f) {
        if (this.gun == null) return;
            ...
            this.p1 = this.p0;
            this.p0 = point3d;
            dspeed.scale(this.gun.bulletKV[this.indx()] * f * 1.0F * fv(this.speed.length()) / this.speed.length(), this.speed);
            dspeed.z += this.gun.bulletAG[this.indx()] * f;
    ...
    public void timeOut() {
        if (this.gun() != null) Explosions.generateExplosion(null, this.p1, this.gun().prop.bullet[this.indx()].power, this.gun().prop.bullet[this.indx()].powerType, this.gun().prop.bullet[this.indx()].powerRadius, 0.0D);
    }
    ...
    public void destroy() {
        if (Mission.isPlaying() && !NetMissionTrack.isPlaying() && Actor.isValid(this.owner()) && Actor.isValid(this.gun()) && this.gun() instanceof MGunAircraftGeneric && this.owner() == World.getPlayerAircraft() && World.cur().diffCur.Limited_Ammo) {
            ...
            if (i != 0 && i <= this.gun().countBullets() && (i != -1 || !World.isPlayerGunner())) this.postRemove(this.owner());
        }
        super.destroy();
    }
        ...
        new MsgAction(false, actor) {
            ...
            public void doAction(Object obj) {
                if (obj instanceof Aircraft) {
                    ...
                    if (Actor.isValid(aircraft) && Mission.isPlaying()) aircraft.detachGun(-1);
                }
            }
    ...
    public BulletAircraftGeneric(Vector3d vector3d, int i, GunGeneric gungeneric, Loc loc, Vector3d vector3d1, long l) {
        ...
        if (Mission.isPlaying() && !NetMissionTrack.isPlaying() && Actor.isValid(this.owner()) && Actor.isValid(this.gun()) && this.gun() instanceof MGunAircraftGeneric && this.owner() == World.getPlayerAircraft() && World.cur().diffCur.Limited_Ammo) {
            ...
            if (mgunaircraftgeneric.guardBullet != null && j >= mgunaircraftgeneric.guardBullet.bulletss - mgunaircraftgeneric.guardBullet.hashCode() && (j != -1 || !World.isPlayerGunner())) this.postRemove(this.owner());
            mgunaircraftgeneric.guardBullet = this;
        }
    }

    static float fv(double d) {
        return d <= 1090D ? (fv[(int) d / 100] + fv[(int) d / 100 + 1]) / 2.0F : 333F;
    }
...
//            System.out.println();
        ...
        } else bRet = super.collided(actor, s, paramDouble);
        return bRet;
    }
    // ---

...
public class P_11CPL extends P_11 {
    ...
    private void setAnimFrame(long l) {
        ...
        float f = 2200F / this.numberFrames;
        if (((float) d1 >= (this.stepFrame * f)) && (this.stepFrame < this.numberFrames)) {
            this.stepFrame++;
            this.hierMesh().setFrame(this.reverse ? this.currentFrame-- : this.currentFrame++);
            System.out.println("*** Animated frame set - " + this.currentFrame + "  Interval " + f + "  advanve " + (this.stepFrame * f) + "  time " + (float) d1);
        }
    ...
    private boolean startAnimation() {
        ...
        if (i != -1) {
            ...
            this.numberFrames = this.hierMesh().frames();

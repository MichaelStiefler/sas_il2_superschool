...
public class SOPWITH_SWALLOW extends SOPWITH {

    public SOPWITH_SWALLOW() {
    ...
    protected void moveFan(float f) {
        ...
        if (this.oldProp[j] < 2) {
            ...
            if ((i != this.oldProp[j]) && this.hierMesh().isChunkVisible(SOPWITH_SWALLOW.Props[j][this.oldProp[j]])) {
                this.hierMesh().chunkVisible(SOPWITH_SWALLOW.Props[0][this.oldProp[j]], false);
                this.oldProp[j] = i;
                this.hierMesh().chunkVisible(SOPWITH_SWALLOW.Props[j][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(SOPWITH_SWALLOW.Props[j][0], 0.0F, 0.0F, this.propPos[j]);
        if (this.oldProp[k] < 2) {
            i = Math.abs((int) (this.FM.EI.engines[0].getw() * 0.06F));
            if (i >= 1) {
                i = 1;
            }
            if ((i != this.oldProp[k]) && this.hierMesh().isChunkVisible(SOPWITH_SWALLOW.Props[k][this.oldProp[k]])) {
                this.hierMesh().chunkVisible(SOPWITH_SWALLOW.Props[k][this.oldProp[k]], false);
                this.oldProp[k] = i;
                this.hierMesh().chunkVisible(SOPWITH_SWALLOW.Props[k][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(SOPWITH_SWALLOW.Props[k][0], 0.0F, 0.0F, this.propPos[k]);
    }
    ...
    static {
        Class class1 = SOPWITH_SWALLOW.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "SOPWITH_SWALLOW");
        Property.set(class1, "meshName", "3DO/Plane/SOPWITH_SWALLOW/hier.him");
        ...
        Property.set(class1, "cockpitClass", new Class[] { CockpitSOPWITH_SWALLOW.class });

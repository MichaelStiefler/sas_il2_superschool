...
public class Nieuport_17B extends Scheme1 implements TypeFighter, TypeTNBFighter {

    public Nieuport_17B() {
        this.dynamoOrient = 0.0F;
        this.bDynamoRotary = false;
        Nieuport_17B.bChangedPit = true;
    }
    ...
    protected void moveFan(float f) {
        ...
        if (this.oldProp[j] < 2) {
            ...
            if ((i != this.oldProp[j]) && this.hierMesh().isChunkVisible(Nieuport_17B.Props[j][this.oldProp[j]])) {
                this.hierMesh().chunkVisible(Nieuport_17B.Props[0][this.oldProp[j]], false);
                this.oldProp[j] = i;
                this.hierMesh().chunkVisible(Nieuport_17B.Props[j][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(Nieuport_17B.Props[j][0], 0.0F, -this.propPos[j], 0.0F);
        if (this.oldProp[k] < 2) {
            i = Math.abs((int) (this.FM.EI.engines[0].getw() * 0.06F));
            if (i >= 1) {
                i = 1;
            }
            if ((i != this.oldProp[k]) && this.hierMesh().isChunkVisible(Nieuport_17B.Props[k][this.oldProp[k]])) {
                this.hierMesh().chunkVisible(Nieuport_17B.Props[k][this.oldProp[k]], false);
                this.oldProp[k] = i;
                this.hierMesh().chunkVisible(Nieuport_17B.Props[k][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(Nieuport_17B.Props[k][0], 0.0F, -this.propPos[k], 0.0F);
    }
    ...
    static {
        Class class1 = Nieuport_17B.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "Nieup");
        Property.set(class1, "meshName", "3DO/Plane/Nieuport-17b/hier.him");
        ...
        Property.set(class1, "cockpitClass", new Class[] { CockpitNieu17B.class });
        Property.set(class1, "LOSElevation", 0.7956F);
        Aircraft.weaponTriggersRegister(class1, new int[] { 0, 1, 3 });
        Aircraft.weaponHooksRegister(class1, new String[] { "_MGUN01", "_MGUN02", "_BombSpawn01" });
    }
}

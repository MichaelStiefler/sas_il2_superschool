...
public class FOKKER_DVI extends Scheme1 implements TypeScout, TypeFighter, TypeTNBFighter {
    ...
    protected void moveFan(float f) {
        ...
        if (this.oldProp[j] < 2) {
            ...
            if ((i != this.oldProp[j]) && this.hierMesh().isChunkVisible(FOKKER_DVI.Props[j][this.oldProp[j]])) {
                this.hierMesh().chunkVisible(FOKKER_DVI.Props[0][this.oldProp[j]], false);
                this.oldProp[j] = i;
                this.hierMesh().chunkVisible(FOKKER_DVI.Props[j][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(FOKKER_DVI.Props[j][0], 0.0F, 0.0F, -this.propPos[j]);
        if (this.oldProp[k] < 2) {
            i = Math.abs((int) (this.FM.EI.engines[0].getw() * 0.06F));
            if (i >= 1) {
                i = 1;
            }
            if ((i != this.oldProp[k]) && this.hierMesh().isChunkVisible(FOKKER_DVI.Props[k][this.oldProp[k]])) {
                this.hierMesh().chunkVisible(FOKKER_DVI.Props[k][this.oldProp[k]], false);
                this.oldProp[k] = i;
                this.hierMesh().chunkVisible(FOKKER_DVI.Props[k][i], true);
            }
        }
        if (i == 0) {
        ...
        this.hierMesh().chunkSetAngles(FOKKER_DVI.Props[k][0], 0.0F, 0.0F, -this.propPos[k]);
    }
    ...
    public void update(float f) {
        ...
        if ((this.FM.EI.engines[0].getStage() == 6) && this.FM.isPlayers()) {
            if (this.FM.CT.getMagnetoControl() == 1.0F) {
                if (this.FM.EI.engines[0].getControlThrottle() > 0.33F) {
                    this.FM.EI.engines[0].setControlThrottle(0.33F);
                }
            } else if (this.FM.CT.getMagnetoControl() == 2.0F) {
                if (this.FM.EI.engines[0].getControlThrottle() > 0.66F) {
                    this.FM.EI.engines[0].setControlThrottle(0.66F);
                } else if (this.FM.EI.engines[0].getControlThrottle() <= 0.33F) {
                    this.FM.EI.engines[0].setControlThrottle(0.33F);
                }
            } else if ((this.FM.CT.getMagnetoControl() == 3F) && (this.FM.EI.engines[0].getControlThrottle() <= 0.66F)) {
                this.FM.EI.engines[0].setControlThrottle(0.66F);
            }
        }
        this.gunSync();
        ...
        if (this.hierMesh().isChunkVisible("WingLIn_CAP") && !this.hierMesh().isChunkVisible("WingRIn_CAP")) {
            this.hierMesh().chunkVisible("WingLMid_CAP", true);
        }
        if (this.hierMesh().isChunkVisible("WingRIn_CAP") && !this.hierMesh().isChunkVisible("WingLIn_CAP")) {
            this.hierMesh().chunkVisible("WingRMid_CAP", true);
        }
        if (this.hierMesh().isChunkVisible("WingRIn_CAP") && this.hierMesh().isChunkVisible("WingLIn_CAP")) {
            this.hierMesh().chunkVisible("WingLMid_CAP", false);
            this.hierMesh().chunkVisible("WingRMid_CAP", false);
        }
    }
    ...
    static {
        Class class1 = FOKKER_DVI.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "FokkerDVI");
        Property.set(class1, "meshName", "3DO/Plane/FokkerDVI/hier.him");
        Property.set(class1, "PaintScheme", new PaintSchemeFMPar00());
        Property.set(class1, "yearService", 1916F);
        Property.set(class1, "yearExpired", 1918F);
        Property.set(class1, "FlightModel", "FlightModels/FokkerDVI.fmd:FokkerDVI");
        Property.set(class1, "originCountry", PaintScheme.countryGermany);
        Property.set(class1, "LOSElevation", 0.66F);
        Property.set(class1, "cockpitClass", new Class[] { CockpitFOKKERDVI.class });

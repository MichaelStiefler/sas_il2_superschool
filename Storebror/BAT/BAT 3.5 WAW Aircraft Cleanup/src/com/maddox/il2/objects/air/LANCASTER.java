...
public class LANCASTER extends Scheme4 implements TypeTransport, TypeBomber {
    ...
    protected void moveGear(float f) {
        LANCASTER.moveGear(this.hierMesh(), f);
    }
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = LANCASTER.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= LANCASTER.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / LANCASTER.toMeters(this.fSightCurAltitude)));
            if (this.fSightCurDistance < (LANCASTER.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(LANCASTER.toMeters(this.fSightCurAltitude) * 0.2038736F))) {

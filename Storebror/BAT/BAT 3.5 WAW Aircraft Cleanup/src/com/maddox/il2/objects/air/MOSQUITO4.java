...
public class MOSQUITO4 extends MOSQUITO implements TypeBomber {
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = MOSQUITO4.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= MOSQUITO4.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / MOSQUITO4.toMeters(this.fSightCurAltitude)));
            this.calibDistance = MOSQUITO4.toMetersPerSecond(this.fSightCurSpeed) * AircraftLH.floatindex(Aircraft.cvt(MOSQUITO4.toMeters(this.fSightCurAltitude), 0.0F, 7000F, 0.0F, 7F), MOSQUITO4.calibrationScale);
            if (this.fSightCurDistance < (this.calibDistance + (MOSQUITO4.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(MOSQUITO4.toMeters(this.fSightCurAltitude) * 0.2038736F)))) {

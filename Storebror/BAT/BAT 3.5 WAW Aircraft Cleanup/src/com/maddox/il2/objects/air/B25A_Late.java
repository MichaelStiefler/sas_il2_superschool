...
public class B25A_Late extends B_25 implements TypeBomber {
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = B25A_Late.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= B25A_Late.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / B25A_Late.toMeters(this.fSightCurAltitude)));
            if (this.fSightCurDistance < (B25A_Late.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(B25A_Late.toMeters(this.fSightCurAltitude) * 0.2038736F))) {
    ...
    static {
        Class var_class = B25A_Late.class;
        new NetAircraft.SPAWN(var_class);
        Property.set(var_class, "iconFar_shortClassName", "B-25");
        Property.set(var_class, "meshName", "3DO/Plane/B-25ALate(Multi1)/hier.him");
        Property.set(var_class, "PaintScheme", new PaintSchemeBMPar02());
        Property.set(var_class, "meshName_us", "3DO/Plane/B-25ALate(USA)/hier.him");
        Property.set(var_class, "PaintScheme_us", new PaintSchemeBMPar02());
        Property.set(var_class, "yearService", 1943F);
        Property.set(var_class, "yearExpired", 1956.6F);
        Property.set(var_class, "FlightModel", "FlightModels/B-25C.fmd");
        Property.set(var_class, "cockpitClass", new Class[] { CockpitB25A_Late.class, CockpitB25A_Late_Bombardier.class, CockpitB25A_Late_FGunner.class });
        Property.set(var_class, "LOSElevation", 0.73425F);
        Aircraft.weaponTriggersRegister(var_class, new int[] { 10, 3, 3, 3 });
        Aircraft.weaponHooksRegister(var_class, new String[] { "_MGUN02", "_BombSpawn01", "_BombSpawn02", "_BombSpawn03" });
    }
}

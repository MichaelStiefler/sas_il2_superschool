...
public class BLENHEIM1 extends BLENHEIM {
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = BLENHEIM1.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= BLENHEIM1.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / BLENHEIM1.toMeters(this.fSightCurAltitude)));
            this.calibDistance = BLENHEIM1.toMetersPerSecond(this.fSightCurSpeed) * AircraftLH.floatindex(Aircraft.cvt(BLENHEIM1.toMeters(this.fSightCurAltitude), 0.0F, 7000F, 0.0F, 7F), BLENHEIM1.calibrationScale);
            if (this.fSightCurDistance < (this.calibDistance + (BLENHEIM1.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(BLENHEIM1.toMeters(this.fSightCurAltitude) * 0.2038736F)))) {
    ...
    static {
        ...
        Property.set(class1, "cockpitClass", new Class[] { CockpitBLENHEIM1.class, CockpitBLENHEIM1_Bombardier.class, CockpitBLENHEIM_TGunner.class });

...
public class PE_8_M40F extends PE_8xyz {

    public PE_8_M40F() {
    }

    protected void moveGear(float f, float f1, float f2) {
        PE_8_M40F.moveGear(this.hierMesh(), f, f1, f2);
    }
    ...
    public void update(float f) {
        float f1 = this.FM.EI.engines[0].getControlRadiator() * 30F;
        if (Math.abs(this.flapps[0] - f1) > 0.01F) {
            this.flapps[0] = f1;
            this.hierMesh().chunkSetAngles("Radiator1_D0", 0.0F, f1, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator4_D0", 0.0F, -f1, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator2_D0", -f1 * 1.3F, 0.0F, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator3_D0", f1 * 1.3F, 0.0F, 0.0F);
        }
        float f2 = this.FM.EI.engines[1].getControlRadiator() * 30F;
        if (Math.abs(this.flapps[1] - f2) > 0.01F) {
            this.flapps[1] = f2;
            this.hierMesh().chunkSetAngles("Radiator5_D0", 0.0F, f2, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator8_D0", 0.0F, -f2, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator6_D0", -f2 * 1.3F, 0.0F, 0.0F);
            this.hierMesh().chunkSetAngles("Radiator7_D0", f2 * 1.3F, 0.0F, 0.0F);
        }
        super.update(f);
    }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        ...
        if (s.startsWith("xxtank")) {
            ...
            if (this.getEnergyPastArmor(1.0F, shot) > 0.0F) {
                if ((this.FM.AS.astateTankStates[i] == 0) && (World.Rnd().nextFloat() < 0.1F)) {
                    this.FM.AS.hitTank(shot.initiator, i, 1);
                    this.FM.AS.doSetTankState(shot.initiator, i, 1);
                }
                if (shot.powerType == 3) {
                    if (shot.power < 18000F) {
                        if ((this.FM.AS.astateTankStates[i] < 4) && (World.Rnd().nextFloat() < 0.21F)) {
                            this.FM.AS.hitTank(shot.initiator, i, 1);
                        }
                    } else {
                        this.FM.AS.hitTank(shot.initiator, i, World.Rnd().nextInt(1, 1 + (int) (shot.power / 18000F)));
                    }
                } else if (shot.power > 18000F) {
                    this.FM.AS.hitTank(shot.initiator, i, World.Rnd().nextInt(1, 1 + (int) (shot.power / 18000F)));
                }
            }
            return;

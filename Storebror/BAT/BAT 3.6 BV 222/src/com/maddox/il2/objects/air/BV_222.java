...
public abstract class BV_222 extends Scheme7 implements TypeSeaPlane, TypeTransport {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 2; j++) {
                if (this.FM.Gears.clpGearEff[i][j] != null) {
                    BV_222.tmpp.set(this.FM.Gears.clpGearEff[i][j].pos.getAbsPoint());
                    BV_222.tmpp.z = 0.01D;
                    this.FM.Gears.clpGearEff[i][j].pos.setAbs(BV_222.tmpp);
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        ...
        if (s.startsWith("xengine5")) {
            if (this.chunkDamageVisible("Engine5") < 2) {
                this.hitChunk("Engine5", shot);
            }
            return;
        }
        if (s.startsWith("xengine6")) {
            if (this.chunkDamageVisible("Engine6") < 2) {
                this.hitChunk("Engine6", shot);
            }
            return;
        }
    ...
    protected boolean cutFM(int i, int j, Actor actor) {
        switch (i) {
            ...
            case 13:
                ...
                //TODO: Modified by SAS~Storebror: Engines are attached to inner wing only
            case 33:
    ...
    protected void moveGear(float f, float f1, float f2) {
        BV_222.moveGear(this.hierMesh(), f, f1, f2);
    }

    private static Point3d tmpp = new Point3d();

    static {
        Class class1 = BV_222.class;

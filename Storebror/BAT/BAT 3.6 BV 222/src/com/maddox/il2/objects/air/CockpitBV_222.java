...
//        mesh.chunkSetAngles("zMix2", 0.0F, -45.8F * fm.EI.engines[1].getControlMix(), 0.0F);
        ...
        for (int i = 0; i < 6; i++) {
            CockpitBV_222.tmpstr = "zThrottle" + (i + 1);
            this.mesh.chunkSetAngles(CockpitBV_222.tmpstr, 0.0F, -49F * this.interp(this.setNew.throttle[i], this.setOld.throttle[i], f), 0.0F);
        }
...
//        }
        ...
        if ((this.fm.AS.astateCockpitState & 0x40) == 0) {
            for (int i = 0; i < 6; i++) {
                CockpitBV_222.tmpstr = "zRPM" + (i + 1);
                this.mesh.chunkSetAngles(CockpitBV_222.tmpstr, 0.0F, this.cvt(this.fm.EI.engines[i].getRPM(), 0.0F, 4500F, 0.0F, 323F), 0.0F);
                CockpitBV_222.tmpstr = "Z_Pres" + (i + 1);
                this.mesh.chunkSetAngles(CockpitBV_222.tmpstr, 0.0F, this.pictManf[i] = (0.9F * this.pictManf[i]) + (0.1F * this.cvt(this.fm.EI.engines[i].getManifoldPressure(), 0.3386378F, 2.539784F, 0.0F, 346.5F)), 0.0F);
            }
        }

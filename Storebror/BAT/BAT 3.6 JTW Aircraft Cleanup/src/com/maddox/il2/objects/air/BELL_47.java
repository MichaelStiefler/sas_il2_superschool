...
public class BELL_47 extends Scheme1 implements TypeScout, TypeTransport, TypeStormovik {
    ...
    protected void moveGear(float f) {
        BELL_47.moveGear(this.hierMesh(), f);
    }
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        if (s.startsWith("xx")) {
            if (s.startsWith("xxarmor")) {
                ...
                if (s.endsWith("p1")) {
                    this.getEnergyPastArmor(8.1D / (Math.abs(Aircraft.v1.x) + 1E-005D), shot);
                }
            } else {

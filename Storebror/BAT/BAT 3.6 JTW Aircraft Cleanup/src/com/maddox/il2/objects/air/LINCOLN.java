...
public class LINCOLN extends Scheme4 implements TypeTransport, TypeBomber {
    ...
    protected void moveGear(float f) {
        LINCOLN.moveGear(this.hierMesh(), f);
    }
    ...
    public void msgShot(Shot shot) {
        ...
        if (shot.chunkName.startsWith("CF")) {
            if ((((Tuple3d) (Aircraft.Pd)).x > 4.55D) && (Aircraft.Pd.x < 7.15D) && (Aircraft.Pd.z > 0.57999998331069946D)) {
            ...
            if ((((Tuple3d) (Aircraft.Pd)).x > 9.5299997329711914D) && (Aircraft.Pd.z < 0.14D) && (Aircraft.Pd.z > -0.62999999523162842D)) {
                this.FM.AS.hitPilot(shot.initiator, 2, (int) (shot.power * 0.002F));
            }
            if ((((Tuple3d) (Aircraft.Pd)).x > 2.4749999046325684D) && (((Tuple3d) (Aircraft.Pd)).x < 4.4899997711181641D) && (((Tuple3d) (Aircraft.Pd)).z > 0.61D) && ((shot.power * Math.sqrt((((Tuple3d) (Aircraft.v1)).y * ((Tuple3d) (Aircraft.v1)).y) + (Aircraft.v1.z * Aircraft.v1.z))) > 11900D) && (World.Rnd().nextFloat() < 0.45F)) {
                for (int i = 0; i < 4; i++) {
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = LINCOLN.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= LINCOLN.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / LINCOLN.toMeters(this.fSightCurAltitude)));
            if (this.fSightCurDistance < (LINCOLN.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(LINCOLN.toMeters(this.fSightCurAltitude) * (2F / 9.81F)))) {
    ...
    public float getLlpos() {
        return this.llpos;
    }

    public void setLlpos(float llpos) {
        this.llpos = llpos;
    }
    ...
    static {
        Class class1 = LINCOLN.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "LINCOLN");
        Property.set(class1, "meshName", "3DO/Plane/LINCOLN/hier.him");
        Property.set(class1, "PaintScheme", new PaintSchemeBMPar00());
        Property.set(class1, "originCountry", PaintScheme.countryRussia);
        Property.set(class1, "yearService", 1940F);
        Property.set(class1, "yearExpired", 1945.5F);
        Property.set(class1, "FlightModel", "FlightModels/Lancaster.fmd");
        Property.set(class1, "cockpitClass", new Class[] { CockpitLincoln.class, CockpitLincoln_Bombardier.class, CockpitLincoln_NGunner.class, CockpitLincoln_TGunner.class, CockpitLincoln_AGunner.class });

...
public class HE_LERCHE3 extends Scheme2 implements TypeFighter, TypeX4Carrier {
    ...
    protected void moveGear(float f) {
        HE_LERCHE3.moveGear(this.hierMesh(), f);
    }
    ...
    public void update(float f) {
        ...
        if (flag) {
            for (int j = 0; j < 9; j++) {
                float f2 = -60F * this.pictVBrake * ((HE_LERCHE3.fcA[j] * this.pictAileron) + (HE_LERCHE3.fcE[j] * this.pictVator) + (HE_LERCHE3.fcR[j] * this.pictRudder));
        ...
        float f4 = this.FM.EI.getPowerOutput() * Aircraft.cvt(this.FM.getSpeedKMH(), 0.0F, 600F, 2.0F, 0.0F);
    ...
    static {
        ...
        Aircraft.weaponTriggersRegister(class1, new int[] { 1, 1, 2, 2, 2, 2, 2, 2 });
        Aircraft.weaponHooksRegister(class1, new String[] { "_CANNON01", "_CANNON02", "_ExternalRock01", "_ExternalRock01", "_ExternalRock02", "_ExternalRock02", "_ExternalRock03", "_ExternalRock03" });
    }
}

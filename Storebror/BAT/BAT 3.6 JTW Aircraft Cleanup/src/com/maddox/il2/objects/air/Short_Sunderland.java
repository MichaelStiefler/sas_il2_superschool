...
public class Short_Sunderland extends Sunderlandxyz implements TypeSailPlane, TypeSeaPlane, TypeTransport, TypeBomber {
    ...
    public void typeBomberUpdate(float f) {
        if (Math.abs(this.FM.Or.getKren()) > 4.5D) {
            this.fSightCurReadyness -= 0.06666667F * f;
            if (this.fSightCurReadyness < 0.0F) {
                this.fSightCurReadyness = 0.0F;
            }
        }
        if (this.fSightCurReadyness < 1.0F) {
            this.fSightCurReadyness += 0.03333334F * f;
            ...
            if (this.fSightCurDistance < ((this.fSightCurSpeed / 3.6F) * Math.sqrt((this.fSightCurAltitude * 2.0F) / 9.81F))) {

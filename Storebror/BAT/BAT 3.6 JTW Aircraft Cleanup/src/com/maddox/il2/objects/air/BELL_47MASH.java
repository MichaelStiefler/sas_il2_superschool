...
public class BELL_47MASH extends Scheme1 implements TypeScout, TypeTransport, TypeStormovik {
    ...
    protected void moveGear(float f) {
        BELL_47MASH.moveGear(this.hierMesh(), f);
    }
    ...
    protected void moveFan(float f) {
        ...
        if (this.oldProp[j] < 2) {
            ...
            if ((i != this.oldProp[j]) && this.hierMesh().isChunkVisible(Aircraft.Props[j][this.oldProp[j]])) {
                this.hierMesh().chunkVisible(Aircraft.Props[0][this.oldProp[j]], false);
                this.oldProp[j] = i;
        ...
        if (this.oldProp[k] < 2) {
            ...
            if ((i != this.oldProp[k]) && this.hierMesh().isChunkVisible(Aircraft.Props[k][this.oldProp[k]])) {
                this.hierMesh().chunkVisible(Aircraft.Props[k][this.oldProp[k]], false);
                this.oldProp[k] = i;
                this.hierMesh().chunkVisible(Aircraft.Props[k][i], true);
            }
        }
        if (i == 0) {
            this.propPos[k] = (this.propPos[k] + (57.3F * (this.FM.EI.engines[0].getw() * 6F) * f)) % 360F;
        } else {
        ...
        this.hierMesh().chunkSetAngles(Aircraft.Props[k][0], 0.0F, -this.propPos[k], 0.0F);
    }
    ...
    static {
        Class class1 = BELL_47MASH.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "Bell47");
        Property.set(class1, "meshName", "3DO/Plane/Bell47MASH/hier.him");

...
public class Mig_19P extends Mig_19 implements TypeGuidedMissileCarrier, TypeCountermeasure, TypeThreatDetector {
    ...
    public float getFlowRate() {
        return Mig_19P.FlowRate;
    }

    public float getFuelReserve() {
        return Mig_19P.FuelReserve;
    }
    ...
    private boolean RP1() {
        ...
        if (aircraft instanceof Mig_19P) {
            flag1 = true;
        }
        if (d3 < 0.0D) {
            d3 = 0.0D;
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            ...
            if (flag1 && (actor instanceof Aircraft) && (actor.getArmy() != World.getPlayerArmy()) && (actor != World.getPlayerAircraft()) && (actor.getSpeed(vector3d) > 20D)) {
                ...
                if (((double) j1 <= (double) l2) && (j1 > 200D) && (l1 >= -20) && (l1 <= 20) && (Math.sqrt(j2 * j2) <= 60D)) {
                    HUD.log(AircraftHotKeys.hudLogWeaponId, "RP-1: Contact " + s3 + s + ", " + i2 + "m");
                    this.freq = 1;
                } else {
                    this.freq = 7;
                }
    ...
    static {
        Class class1 = Mig_19P.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "MiG-19");
        Property.set(class1, "meshName", "3DO/Plane/Mig-19/hier19P.him");

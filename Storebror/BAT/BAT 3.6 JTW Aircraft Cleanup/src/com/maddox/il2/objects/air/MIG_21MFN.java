...
public class MIG_21MFN extends MIG_21 implements TypeGuidedMissileCarrier, TypeCountermeasure, TypeThreatDetector, TypeLaserSpotter {

    private void hellaLights() {
        if (this.lastTickWhenIDidThis == Time.tickCounter()) {
            return;
        }
        this.lastTickWhenIDidThis = Time.tickCounter();
        if (this.FM.AS.bNavLightsOn) {
            this.hellaTimer++;
            if (this.hellaTimer >= 100) {
                this.hellaTimer = 0;
            }
            Point3d point3d = new Point3d();
            Orient orient = new Orient();
            this.pos.getAbs(point3d, orient);
            if (this.hellaTimer == 0) {
                Eff3DActor eff3dactor = Eff3DActor.New(this, this.findHook("_Hella1"), new Loc(), 1.0F, "3DO/Effects/Fireworks/FlareRed.eff", 0.2F);
                eff3dactor.postDestroy(Time.current() + 200L);
                LightPointActor lightpointactor = new LightPointActor(new LightPointWorld(), new Point3d());
                lightpointactor.light.setColor(1.0F, 0.3F, 0.3F);
                lightpointactor.light.setEmit(1.0F, 5F);
                eff3dactor.draw.lightMap().put("light", lightpointactor);
            }
            if (this.hellaTimer == 30) {
                Eff3DActor eff3dactor1 = Eff3DActor.New(this, this.findHook("_Hella2"), new Loc(), 1.0F, "3DO/Effects/Fireworks/FlareRed.eff", 0.2F);
                LightPointActor lightpointactor1 = new LightPointActor(new LightPointWorld(), new Point3d());
                lightpointactor1.light.setColor(1.0F, 0.3F, 0.3F);
                lightpointactor1.light.setEmit(1.0F, 5F);
                eff3dactor1.draw.lightMap().put("light", lightpointactor1);
                eff3dactor1.postDestroy(Time.current() + 200L);
            }
        }
    }
    ...
    protected void moveGear(float f) {
        ...
        MIG_21MFN.moveGear(this.hierMesh(), f);
    }
    ...
    private int                hellaTimer;
    private int                lastTickWhenIDidThis;
    ...
    static {
        ...
        Property.set(class1, "cockpitClass", new Class[] { CockpitMIG_21MFN.class });

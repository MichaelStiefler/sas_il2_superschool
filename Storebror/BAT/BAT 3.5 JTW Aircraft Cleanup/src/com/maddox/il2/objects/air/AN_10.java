...
public class AN_10 extends AN_10A implements TypeBomber {
    ...
    public void typeBomberAdjDistancePlus() {
        ...
        this.fSightCurDistance = AN_10.toMeters(this.fSightCurAltitude) * (float) Math.tan(Math.toRadians(this.fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f) {
        ...
        if (this.fSightCurReadyness < 1.0F) {
            ...
            this.fSightCurDistance -= AN_10.toMetersPerSecond(this.fSightCurSpeed) * f;
            if (this.fSightCurDistance < 0.0F) {
                this.fSightCurDistance = 0.0F;
                this.typeBomberToggleAutomation();
            }
            this.fSightCurForwardAngle = (float) Math.toDegrees(Math.atan(this.fSightCurDistance / AN_10.toMeters(this.fSightCurAltitude)));
            float calibrationScale[] = null;
            this.calibDistance = AN_10.toMetersPerSecond(this.fSightCurSpeed) * AircraftLH.floatindex(Aircraft.cvt(AN_10.toMeters(this.fSightCurAltitude), 0.0F, 7000F, 0.0F, 7F), calibrationScale);
            if (this.fSightCurDistance < (this.calibDistance + (AN_10.toMetersPerSecond(this.fSightCurSpeed) * Math.sqrt(AN_10.toMeters(this.fSightCurAltitude) * (2F / 9.81F))))) {
    ...
    static {
        Class class1 = AN_10.class;
        new NetAircraft.SPAWN(class1);
        Property.set(class1, "iconFar_shortClassName", "AN_10");
        Property.set(class1, "meshName", "3DO/Plane/AN-10(Multi1)/hier.him");
        Property.set(class1, "PaintScheme", new PaintSchemeBMPar05());
        Property.set(class1, "yearService", 1948.5F);
        Property.set(class1, "yearExpired", 1960.9F);
        Property.set(class1, "cockpitClass", new Class[] { CockpitAN_10.class });

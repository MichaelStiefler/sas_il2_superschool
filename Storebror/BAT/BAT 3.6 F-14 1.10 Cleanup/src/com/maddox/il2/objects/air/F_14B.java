...
public class F_14B extends F_14 {
    ...
    private boolean IRST() {
        ...
        int irstListSize = F_14B.irstPlaneName.length;
        ...
        for (java.util.Map.Entry entry = Engine.name2Actor().nextEntry(null); entry != null; entry = Engine.name2Actor().nextEntry(entry)) {
            Actor actor = (Actor) entry.getValue();
            if (flag1 && (actor instanceof Aircraft) && (actor.getArmy() != World.getPlayerArmy()) && (actor != World.getPlayerAircraft()) && (actor.getSpeed(vector3d) > 20D) && !((Main.cur().clouds != null) && (Main.cur().clouds.getVisibility(actor.pos.getAbsPoint(), aircraft.pos.getAbsPoint()) < 1.0F)) && !(((Mission.curCloudsType() > 3) || ((Mission.curCloudsType() > 1) && (World.Sun().ToSun.z < 0.10F))) && (Mission.curCloudsHeight() > actor.pos.getAbsPoint().z))) {
                this.pos.getAbs(point3d);
                double d4 = Main3D.cur3D().land2D.worldOfsX() + actor.pos.getAbsPoint().x;
                double d5 = Main3D.cur3D().land2D.worldOfsY() + actor.pos.getAbsPoint().y;
                double d6 = Main3D.cur3D().land2D.worldOfsY() + actor.pos.getAbsPoint().z;
                ...
                for (int num = 0; num < irstListSize; num++) {
                    if (((double) j1 <= (double) l2) && (j1 < F_14B.irstMaxDistance[num]) && (l1 >= -20) && (l1 <= 20) && (Math.sqrt(j2 * j2) <= 60D) && ((classnameSection.indexOf(F_14B.irstPlaneName[num])) != -1)) {
                        HUD.log(AircraftHotKeys.hudLogWeaponId, "TV Tracking! " + F_14B.irstPlaneDisplay[num] + " " + s3 + s + ", " + i2 + "m");
                        return true;
                    }
                }
            }

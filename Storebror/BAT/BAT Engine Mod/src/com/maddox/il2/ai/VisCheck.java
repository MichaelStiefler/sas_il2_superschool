// Source File Name:   VisCheck.java
// 2017.Nov.19 , western0221: add checking Anti-Collision lights On/Off to decide visibility
// 2017.Nov.19 , western0221: not only Enemy Aircraft, but also ALL enemy actors can be checked their visibility
...
public class VisCheck
{
    ...
    {
        if(aircraft == null || aircraft1 == null || aircraft == aircraft1)
            return;
        ...
        for(int j = 0; j < aircraft.FM.turret.length; j++)
        {
            ...
            if(turret.bIsAIControlled && turret.bIsOperable)
                switch(turret.obsDir)
                {
                default:
                    break;
        ...
        if(commonCheck(aircraft, aircraft1, f4, f, f2, f3, f1))
            airgroup.bSee = true;
    }
    ...
    private static boolean commonCheck(Aircraft aircraft, Aircraft aircraft1, float f, float f1, float f2, float f3, float f4)
    {
        ...
        f8 *= com.maddox.il2.ai.World.Sun().lightAtAlt((float)aircraft1.pos.getAbsPoint().z);
        if(f8 > 50F)
            f8 = 50F;
        ...
        tmpp1.add(tmpVGr2);
        if(Main.cur().clouds != null && Main.cur().clouds.getVisibility(tmpp1, aircraft1.pos.getAbsPoint()) < 1.0F)
            flag = true;
        tmpVGr2.set(tmpVGr1);
        tmpVGr2.normalize();
        int i = ((Maneuver)aircraft.FM).get_maneuver();
        if(i == 90 || i == 91)
            ...
            aircraft.FM.Or.transformInv(tmpVGr1);
        tmpVGr1.normalize();
        float f9 = World.Rnd().nextFloat(0.0F, 100F);
        boolean flag1 = aircraft1.FM.CT.bAntiColLights || aircraft1.FM.AS.bLandingLightOn || aircraft1.FM.AS.bNavLightsOn || aircraft1.FM.AS.astateSootStates[0] != 0 || aircraft1.FM.AS.astateSootStates[1] != 0 || aircraft1.FM.AS.astateSootStates[2] != 0 || aircraft1.FM.AS.astateSootStates[3] != 0 || aircraft1.FM.AS.astateTankStates[0] > 4 || aircraft1.FM.AS.astateTankStates[1] > 4 || aircraft1.FM.AS.astateTankStates[2] > 4 || aircraft1.FM.AS.astateTankStates[3] > 4 || isShooting(aircraft1) || aircraft1.FM.AS.astateEngineStates[0] > 3 || aircraft1.FM.AS.astateEngineStates[1] > 3 || aircraft1.FM.AS.astateEngineStates[2] > 3 || aircraft1.FM.AS.astateEngineStates[3] > 3 || aircraft1.tmSearchlighted != 0L && Time.current() - aircraft1.tmSearchlighted < 1000L || aircraft1.FM.AS.astateCondensateEffects[0] != null;
        if(flag1)
            f8 *= 10F;
        float f10 = (float)(aircraft.pos.getAbsPoint().z * -1.2960000276507344E-005D);
        float f11 = 1.0F;
        if(tmpVGr2.z < -0.90000000000000002D)
        {
            f11 = 0.1F;
            if(flag)
                f11 = 1.0F;
        ...
        if(tmpVGr1.x > 0.90000000000000002D)
        {
            f7 = f;
            if(tmpVGr2.z >= (double)f10 || flag)
                f11 = 3F;
            else
                f11 = 1.2F;
        } else
        ...
        float f12 = (float)(aircraft.FM.sight + 1) * 0.5F;
        float f13 = (float)aircraft.FM.sight * 0.3F * f11 * f7;
        float f14 = f12 * f11 * f8 * f7;
        float f15 = f13 + f14;
        if(f9 > f15)
            return false;
        if(!visCheck(aircraft.pos.getAbsPoint(), aircraft1.pos.getAbsPoint(), aircraft, aircraft1))
            return false;
    ...
    {
        return visCheck(point3d, point3d1, aircraft, (Actor) aircraft1);
    }

    public static boolean visCheck(Point3d point3d, Point3d point3d1, Aircraft aircraft, Actor actor)
    {
        if(Landscape.rayHitHQ(point3d, point3d1, tmpp1))
            return false;
        ...
        return checkVisibilityFactors(point3d, point3d1, aircraft, actor, 1.0F, false);
    }
    ...
    {
        return checkVisibilityFactors(point3d, point3d1, aircraft, (Actor) aircraft1, f, flag);
    }

    private static boolean checkVisibilityFactors(Point3d point3d, Point3d point3d1, Aircraft aircraft, Actor actor, float f, boolean flag)
    {
        v_1.set(tmpVGr1);
        v_1.normalize();
        if(com.maddox.il2.ai.World.Sun().ToSun.z > 0.0F)
        {
            Vsun.set(com.maddox.il2.ai.World.Sun().ToSun);
            double d = v_1.dot(Vsun);
            d = Math.abs(d);
            if(d > 0.995D)
                return false;
        }
        return checkAmbientPos(point3d, point3d1, aircraft, actor, f, flag);
    }
    ...
    {
        return checkAmbientPos(point3d, point3d1, aircraft, (Actor) aircraft1, f, flag);
    }
    ...
    private static boolean checkAmbientPos(Point3d point3d, Point3d point3d1, Aircraft aircraft, Actor actor, float f, boolean flag)
    {
        ...
        boolean ac_selflightng = false;
        Aircraft aircraft1 = null;
        if(actor instanceof Aircraft)
            aircraft1 = (Aircraft) actor;
        if(aircraft1 != null)
        {
            if(aircraft1.FM.CT.bAntiColLights || aircraft1.FM.AS.bLandingLightOn || aircraft1.FM.AS.bNavLightsOn || aircraft1.FM.AS.astateTankStates[0] > 4 || aircraft1.FM.AS.astateTankStates[1] > 4 || aircraft1.FM.AS.astateTankStates[2] > 4 || aircraft1.FM.AS.astateTankStates[3] > 4 || isShooting(aircraft1) || aircraft1.FM.AS.astateEngineStates[0] > 3 || aircraft1.FM.AS.astateEngineStates[1] > 3 || aircraft1.FM.AS.astateEngineStates[2] > 3 || aircraft1.FM.AS.astateEngineStates[3] > 3 || aircraft1.tmSearchlighted != 0L && Time.current() - aircraft1.tmSearchlighted < 1000L)
            {
                envLighting = 6000F;
                ac_selflightng = true;
            }
    ...
    {
         return visCheckTurret(turret, aircraft, (Actor) aircraft1, flag);
    }

    public static boolean visCheckTurret(Turret turret, Aircraft aircraft, Actor actor, boolean flag)
    {
        ...
        if(Main.cur().clouds != null && Main.cur().clouds.getVisibility(pilotHeadLoc.getPoint(), actor.pos.getAbsPoint()) < 0.05F)
            return false;
        tmpVGr1.sub(actor.pos.getAbsPoint(), pilotHeadLoc.getPoint());
        if(!checkVisibilityFactors(pilotHeadLoc.getPoint(), actor.pos.getAbsPoint(), aircraft, actor, turret.health * (float)aircraft.FM.Skill, flag))
            return false;
        return !checkPlaneBlocking(aircraft, actor, false);
    }

    public static boolean checkDefense(Aircraft aircraft, Aircraft aircraft1)
    {
        if(checkIfOwnPlaneBlocksVisibility(aircraft, aircraft1, true))
            return true;
        tmpVGr1.sub(aircraft1.pos.getAbsPoint(), aircraft.pos.getAbsPoint());
        v_1.set(tmpVGr1);
        v_1.normalize();
    ...
    public static boolean checkLeadShotBlock(Aircraft aircraft, Aircraft aircraft1)
    {
        ...
        if(aircraft1 == null)
            return false;
        boolean flag = false;
        if(aircraft1.FM.EI.getNum() == 1 && aircraft1.hierMesh().chunkFindCheck("Prop1_D0") != -1)
            flag = true;
        ...
        if(f > 0.0F && aircraft.FM.Skill < 2)
            f = Aircraft.cvt(aircraft.FM.Skill, 0.0F, 3F, 0.0F, f);
    ...
    private static boolean checkIfOwnPlaneBlocksVisibility(Aircraft aircraft, Aircraft aircraft1, boolean flag)
    {
        ...
        aircraft.hierMesh().getChunkLocObj(pilotHeadLoc);
        float f = 0.0F;
        if(World.Rnd().nextFloat() < (float)(aircraft.FM.Skill + 2) / 10F)
            f = 0.1F + 0.05F * (float)aircraft.FM.Skill;
        float f1 = World.Rnd().nextFloat(-f, f);
        neckToEyeIncrement.set(0.20000000298023224D, f1, 0.1F + aircraft.getEyeLevelCorrection(), 0.0F, 0.0F, 0.0F);
        pilotHeadLoc.add(neckToEyeIncrement);
        pilotHeadLoc.add(aircraft.pos.getAbs());
    ...
    private static boolean checkPlaneBlocking(Aircraft aircraft, Actor actor, boolean flag)
    {
        tmpV3d.sub(actor.pos.getAbsPoint(), pilotHeadLoc.getPoint());
...
//        float f = 0.0F;
        ...
        if(!flag)
        {
            float f1 = aircraft.hierMesh().detectCollisionLine(aircraft.pos.getAbs(), tmpp1, actor.pos.getAbsPoint());
            if(f1 == -1F)
                return false;
            boolean flag1 = false;
            if(aircraft1 != null)
                flag1 = aircraft1.FM.AS.astateSootStates[0] != 0 || aircraft1.FM.AS.astateSootStates[1] != 0 || aircraft1.FM.AS.astateSootStates[2] != 0 || aircraft1.FM.AS.astateSootStates[3] != 0 || aircraft1.FM.AS.astateTankStates[0] > 4 || aircraft1.FM.AS.astateTankStates[1] > 4 || aircraft1.FM.AS.astateTankStates[2] > 4 || aircraft1.FM.AS.astateTankStates[3] > 4 || aircraft1.FM.AS.astateEngineStates[0] > 3 || aircraft1.FM.AS.astateEngineStates[1] > 3 || aircraft1.FM.AS.astateEngineStates[2] > 3 || aircraft1.FM.AS.astateEngineStates[3] > 3 || aircraft1.FM.AS.astateCondensateEffects[0] != null;
            if(flag1)
            {
                tmpLoc.set(-250D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
                tmpLoc.add(actor.pos.getAbs());
                tmpV3d.sub(tmpLoc.getPoint(), pilotHeadLoc.getPoint());
                tmpV3d.normalize();
                tmpV3d.scale(1.0D);
                tmpp1.set(pilotHeadLoc.getPoint());
            ...
            boolean flag2 = false;
            if(aircraft1 != null)
                flag2 = aircraft1.FM.CT.WeaponControl[0] || aircraft1.FM.CT.WeaponControl[1];
            if(flag2)
            {
                tmpLoc.set(250D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
                tmpLoc.add(actor.pos.getAbs());
        ...
        if(aircraft1 != null && (d < 2000D || flag))
        {
            float f5 = aircraft1.FM.Wingspan / 2.0F;
            tmpLoc.set(0.0D, 1.0D * (double)f5, 0.0D, 0.0F, 0.0F, 0.0F);
            tmpLoc.add(aircraft1.pos.getAbs());
            tmpV3d.sub(tmpLoc.getPoint(), pilotHeadLoc.getPoint());
            tmpV3d.normalize();
            tmpV3d.scale(1.0D);
            ...
            if(f4 == -1F)
                return false;
            tmpLoc.set(0.0D, -1D * (double)f5, 0.0D, 0.0F, 0.0F, 0.0F);
            tmpLoc.add(aircraft1.pos.getAbs());
            tmpV3d.sub(tmpLoc.getPoint(), pilotHeadLoc.getPoint());
            tmpV3d.normalize();
            tmpV3d.scale(1.0D);
    ...
    public static Actor playerVisibilityCheck(Aircraft aircraft, boolean flag, float f)
    {
        if(Main.cur().netServerParams != null && Main.cur().netServerParams.isMaster() && aircraft != World.getPlayerAircraft() && !flag)
        {
            ...
            if(actor != null && (actor instanceof Aircraft))
            {
                ...
                if(aircraft1.getArmy() != aircraft.getArmy())
                {
                    ...
                    AirGroup airgroup1 = ((Pilot)(Pilot)aircraft.FM).Group;
        ...
        {
            if(Config.isUSE_RENDER() && Main3D.cur3D().isViewOutside())
                return null;
            AirGroupList airgrouplist = War.getEnemyGroups(aircraft);
            AirGroup airgroup = ((Pilot)(Pilot)aircraft.FM).Group;
            if(airgrouplist == null || airgroup == null)
                return null;
            ...
            for(int j = 0; j < i; j++)
            {
                ...
                if(airgroup3 != null)
                {
                    ...
                    if(tmpVGr1.length() < (double)(10000F * f))
                    {
                        for(int k = 0; k < airgroup3.nOfAirc; k++)
                        {
                            ...
                            if(!AirGroupList.groupInList(airgroup.enemies[0], airgroup3))
                                AirGroupList.addAirGroup(airgroup.enemies, 0, airgroup3);
                            ...
                            if(flag && flag1 || airgroup.nOfAirc > 1)
                                Voice.speakRoger(aircraft);
                            return aircraft3;
                        }
        ...
        } else
        {
            ...
            if(actor1 != null && (actor1 instanceof Aircraft))
            {
                ...
                if(isVisibilityBlockedByClouds(aircraft.FM, aircraft2.FM, true))
                    return null;
                if(VisibilityChecker.resultAng > 15F * f)
                    return null;
                if(!checkVisibilityFactors(aircraft.pos.getAbsPoint(), aircraft2.pos.getAbsPoint(), aircraft, aircraft2, 3F, false))
                    return null;
    ...
    private static final Loc neckToEyeIncrement = new Loc(0.2D, 0.0D, 0.1D, 0.0F, 0.0F, 0.0F);
    private static final Loc turretIncrement = new Loc(0.3D, 0.0D, 0.2D, 0.0F, 0.0F, 0.0F);

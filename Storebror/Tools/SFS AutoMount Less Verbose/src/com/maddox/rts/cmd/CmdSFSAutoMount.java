...
public class CmdSFSAutoMount extends Cmd {
	...
	private void AutoMountSFSFiles(File theSFSFolder) {
        if (logLevel == SFS_LOGLEVEL_NOT_INITIALIZED) {
            logLevel = Config.cur.ini.get("Mods", "sfsloglevel", SFS_LOGLEVEL_ERROR);
        }
		...
		for (int i = 0; i < theSFSFileList.length; i++) {
			try {
				if (theSFSFileList[i].getName().startsWith("-")) {
				    if ((logLevel & SFS_LOGLEVEL_SUCCESS) != 0) System.out.println("Skipping AutoMount of file " + theSFSFileList[i].getParent() + File.separator
		...
		for (int i = 0; i < theSFSSubfolderList.length; i++) {
			try {
				if (theSFSSubfolderList[i].getName().startsWith("-")) {
				    if ((logLevel & SFS_LOGLEVEL_SUCCESS) != 0) System.out.println("Skipping AutoMount of subfolder " + theSFSSubfolderList[i].getParent() + File.separator
							+ theSFSSubfolderList[i].getName().substring(1) + " (disabled)");
					continue;
				}
				this.AutoMountSFSFiles(theSFSSubfolderList[i]);
			} catch (Exception e) {
			    if ((logLevel & SFS_LOGLEVEL_ERROR) != 0) System.out.println("Automount invocation for subfolder " + theSFSSubfolderList[i].getPath()
	...
	private void AutoUnMountSFSFiles(File theSFSFolder) {
		...
		for (int i = 0; i < theSFSSubfolderList.length; i++) {
			try {
			    ...
			    if ((logLevel & SFS_LOGLEVEL_ERROR) != 0) System.out.println("Autounmount invocation for subfolder " + theSFSSubfolderList[i].getPath()
						+ " failed, see error below:");
			    if ((logLevel & SFS_LOGLEVEL_ERROR) != 0) System.out.println(e.getMessage());
			}
		}

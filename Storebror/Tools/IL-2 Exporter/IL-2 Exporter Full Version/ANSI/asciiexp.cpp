...
{
	//TCHAR* retVal = GetString(IDS_LONGDESC);
	//return retVal;
	return GetString(IDS_LONGDESC);
	//return _T("IL-2 Exporter");
}

const TCHAR * AsciiExp::ShortDesc()
{
	//TCHAR* retVal = GetString(IDS_SHORTDESC);
	//return retVal;
	return GetString(IDS_SHORTDESC);
	//return _T("IL-2 Full Export v1.0 (SAS)");
}

const TCHAR * AsciiExp::AuthorName() 
{
	//TCHAR* retVal = GetString(IDS_AUTHORNAME);
	//return retVal;
	return GetString(IDS_AUTHORNAME);
	//return _T("6S.Maraz + SAS~Storebror");
}

const TCHAR * AsciiExp::CopyrightMessage() 
{
	//TCHAR* retVal = GetString(IDS_COPYRIGHT);
	//return retVal;
...
{
	//TCHAR* retValS = GetString(IDS_VERSION);
	//int retVal = _ttoi(retValS);
	//return retVal;
...
#ifdef DEBUG
       fprintf( pStream, "Exporting node %s\n", FixupName(currNode->GetName()) );
	   fflush( pStream );
#endif     
   ...
   fflush( pStream );
	...
	for (i=0; i< numFacesTot; i++) 
	{
		...
		else if ( nodemtl->IsMultiMtl() )
		{
	        ...
	        strcpy( materialName[ID], FixupName(mtl->GetName()) );
		}
		else
		{
			ID = 0;
			strcpy( materialName[ID], FixupName(nodemtl->GetName()) ); 
        }
#ifdef DEBUG     
...
#ifdef DEBUG
     fprintf( pStream, "Exporting hook object %s\n", FixupName(currNode->GetName() ) );
	 fflush( pStream );
#endif     
...
*************************************************/
	...
	fprintf(pStream,"%s", Format(row*scaleFact));
	fprintf(pStream,"\t// %s\n", nodeName);
//	fprintf(pStream,"%s \n", Format(row*scaleFact));	
...
#ifdef DEBUG
     fprintf( pStream, "Exporting shadow object %s\n", FixupName(currNode->GetName() ) );
	 fflush( pStream );
#endif     
...
#ifdef DEBUG
     fprintf( pStream, "Exporting collision object %s\n", FixupName(currNode->GetName() ) );
	 fflush( pStream );
#endif     
...
int AsciiExp::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options) 
{
	...
	if (EndsWith(name, ".cfg")) {
		...
		if (ptr != NULL)
			strncpy_s(objName, sizeof(objName), ptr + 1, strlen(name) - 5 - (ptr - name));
		else
			strncpy_s(objName, sizeof(objName), name, strlen(name) - 4);
		fprintf(logFile, "**************************************** Exporting single Mesh %s\n", objName);

		DoNodeExport(objName, ip);
		fprintf(logFile, "**************************************** Done\n");
	}
	else {

		// Open project file
		strcpy(prjFileName, name);
		...
		for (k = 0; k < numObjects; k++)
		{
			...
			if (numDamage == 0) // NO DAMAGE e.g. CAPs
			{
				...
				for (j = 0; j < numDamage; j++)
				{
					...
					fprintf(logFile, "**************************************** Exporting object %s (Damage Level: %c%d)\n", objName, dmgChar, j);
					DoNodeExport(objName, ip);
					fprintf(logFile, "**************************************** Done\n");
				}
		}
...
******************************************/
	...
	if( dmgLevel != -1 )
	{
		// part with damage levels
		if (skipDamageSuffix)
			sprintf(outFileName, "%s.msh", name);
		else
			sprintf( outFileName, "%s_%c%d.msh", name, dmgChar, dmgLevel ); 
	}
	else
	{
		// part with damage levels (e.g. caps)
	...
	// Open the stream
	pStream = _tfopen(outFileName,_T("wt"));
	if (!pStream) {
		numErrors++;
		return 1;
	}
	...
	if( dmgLevel != -1 )
	{
		...
		if (skipDamageSuffix)
			...
			sprintf( baseName, "%s_%c%d",name, dmgChar, dmgLevel );
	}
	else
	{
		// part with damage levels (e.g. caps)
    ...
    fprintf( pStream, GetString(IDS_VERSIONSTRING));
	fprintf( pStream, " for 3DS Max ");
	fprintf( pStream, GetString(IDS_VALIDVERSIONS));
	fprintf( pStream, " \n");
  ...
  fprintf( logFile, "Number of collision boxes: %d \n", nCollObjects ); 
	...
	if( dmgLevel == 0 ) //HOOKS only for D0
	{
		...
		parentNode = ip->GetINodeByName( nodeName );
		if( parentNode == NULL )
		{
		...
		for( k = 0; k < nHooks; k++ )
		{
			...
			newNode = ip->GetINodeByName( nodeName );
			if( newNode == NULL )
			{
				fprintf( logFile, "****** ERROR: node %s not found\n", nodeName );
				numErrors++;
	...
	else
	{
		...
		newNode->SetUserPropString(_M("baseName"), MSTR(baseName));
...
/************************ PRINT HIER.HIM INFORMATION *********************/
		...
		if( dmgLevel <= 0 ) //only for D0 and caps
		{
			...
			if( parentNode == NULL || strstr( parentNode->GetName(), "Root" ) != NULL )
				fprintf( hierFile, "Parent _ROOT_\n" );
			else {
				// try to get Parent's baseName
				char* szParentNodeName = (char*)parentNode->GetName();

				MSTR parentBaseName;
				if (parentNode->GetUserPropString(_M("baseName"), parentBaseName)) {
					szParentNodeName = parentBaseName.data();
				} else {
					if( nLODs > 0 ) {
						if( LODStrType == 's' || LODStrType == 'S') {
							parentBaseName = MSTR(parentNode->GetName());
							MSTR lodName = MSTR(LODstr[0]);
							if (EndsWith(parentBaseName.data(), lodName.data())) {
							...
							parentBaseName = MSTR(parentNode->GetName());
							MSTR lodName = MSTR(LODstr[0]);
							if (StartsWith(parentBaseName.data(), lodName.data())) {
...
/************************ PRINT HIER.HIM INFORMATION *********************/
	...
	newNode = ip->GetINodeByName( shadowNodeName );
	if( newNode == NULL )
	{
	...
	for( k = 0; k < nCollObjects ; k++)
	{
		...
		newNode = ip->GetINodeByName( collObject[k] );
		if( newNode == NULL )
		{
...
{
	return((GetFileAttributes(fileName) != INVALID_FILE_ATTRIBUTES) && (GetFileAttributes(fileName) != FILE_ATTRIBUTE_DIRECTORY));
}

...
#define VERSION			0.01			// Version number * 100
...
public:
	...
	int		DoNodeExport(const TCHAR *name, Interface *i ); 
	...
	void	ExportNode( INode* node, char* LODstring );
	//void	ExportHook( INode* node, INode *parentNode );
	void	ExportHook( INode* node, INode *parentNode, char* nodeName );
	void	ExportCollNode( INode *currNode, int k );
	void    ExportShadowNode( INode *currNode, char *LODstring );
...
private:
	...
	char		LODstr[10][10];
	int			nCollObjects;
	char		collObject[30][40];
	char		shadowChar;
	int			dmgLevel;
	char		dmgChar;
	char		shadowBaseName[40];
	int			nHooks;
	char		hooks[30][40];
	int			numErrors;
	int			numWarnings;

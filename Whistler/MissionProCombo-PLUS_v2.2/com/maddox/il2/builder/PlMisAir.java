...
public class PlMisAir extends Plugin
{
    ...
    class _Render3D extends Render
    {
        ...
        public void preRender()
        {
            ...
            if(Actor.isValid(actorMesh[planeIndx]))
            {
                ...
                {
                    ((Actor) (actorMesh[planeIndx])).pos.getAbs(_orient);
                    _orient.set(_orient.azimut() + animateMeshA[planeIndx] * ((GWindowManager) (Main3D.cur3D().guiManager)).root.deltaTimeSec, _orient.tangage() + animateMeshT[planeIndx] * ((GWindowManager) (Main3D.cur3D().guiManager)).root.deltaTimeSec, 0.0F);
                    _orient.wrap360();
                    ((Actor) (actorMesh[planeIndx])).pos.setAbs(_orient);
                    ((Actor) (actorMesh[planeIndx])).pos.reset();
                }
                ((Actor) (actorMesh[planeIndx])).draw.preRender(actorMesh[planeIndx]);
            }
        }
        ...
        public void render()
        {
            if(Actor.isValid(actorMesh[planeIndx]))
            {
                ...
                ((Actor) (actorMesh[planeIndx])).draw.render(actorMesh[planeIndx]);
            }
        }
                ...
                } else
                {
                    ...
                    for(int j5 = 0; j5 < i1; j5++)
                    {
                        ...
                        } else
                        {
                            ...
                            obj = new PAir(pathair, (PPoint)obj, point3d, byte0, d, d1, null);
                            ((PAir)obj).waypointType = k5;
                            if(k5 > 0 && byte0 != 2)
                            {
                                ...
                                case 404: 
                                case 405: 
                                    ((PAir)obj).cycles = numbertokenizer1.next(0);
                                    ((PAir)obj).delayTimer = numbertokenizer1.next(0);
                                    ((PAir)obj).orient = numbertokenizer1.next(0);
                                    ((PAir)obj).baseSize = numbertokenizer1.next(0);
                                    ((PAir)obj).altDifference = numbertokenizer1.next(0);
                                    break;
                                ...
                                case 4: // '\004'
                                case 5: // '\005'
                                    ((PAir)obj).targetTrigger = numbertokenizer1.next(0);
                                    ((PAir)obj).delayTimer = numbertokenizer1.next(0);
                                    ((PAir)obj).takeoffSpacing = numbertokenizer1.next(0);
                                    ((PAir)obj).ignoreAlt = numbertokenizer1.next(0);
                                    break;
                                }
                                j5++;
                            }
                            ...
                            if(s8 != null && s8.equals("&1"))
                                ((PAir)obj).bRadioSilence = true;
                            if(s9 != null)
                                ((PAir)obj).formation = Integer.parseInt(s9);
                        }
                    }
    ...
    public boolean save(SectFile sectfile)
    {
        ...
        for(int j = 0; j < aobj.length; j++)
        {
            ...
            if(actor instanceof PathAir)
            {
                ...
                PAir pair = (PAir)pathair.point(0);
                if(((PPoint) (pair)).time > 0.0D)
                    sectfile.lineAdd(l, "StartTime", "" + (int)Math.round(((PPoint) (pair)).time / 60D));
                boolean flag = true;
                ...
                for(int l2 = 0; l2 < k; l2++)
                {
                    ...
                    Point3d point3d = ((Actor) (pair1)).pos.getAbsPoint();
                    switch(pair1.type())
                    {
                    default:
                        break;
                    ...
                    case 0: // '\0'
                        ...
                        sectfile.lineAdd(k2, "NORMFLY" + s2, fmt(((Tuple3d) (point3d)).x) + " " + fmt(((Tuple3d) (point3d)).y) + " " + fmt(pair1.height) + " " + fmt(pair1.speed) + saveTarget(pair1) + saveRadioSilence(pair1) + saveFormation(pair1));
                    ...
                    case 3: // '\003'
                        ...
                        sectfile.lineAdd(k2, "GATTACK" + s3, fmt(((Tuple3d) (point3d)).x) + " " + fmt(((Tuple3d) (point3d)).y) + " " + fmt(pair1.height) + " " + fmt(pair1.speed) + saveTarget(pair1) + saveRadioSilence(pair1) + saveFormation(pair1));
                    ...
                    case 1: // '\001'
                        ...
                        sectfile.lineAdd(k2, "TAKEOFF" + s4, fmt(((Tuple3d) (point3d)).x) + " " + fmt(((Tuple3d) (point3d)).y) + " 0 0" + saveTarget(pair1) + saveRadioSilence(pair1));
                        if(pair1.waypointType > 0)
                    ...
                    case 2: // '\002'
                        ...
                        sectfile.lineAdd(k2, "LANDING" + s5, fmt(((Tuple3d) (point3d)).x) + " " + fmt(((Tuple3d) (point3d)).y) + " 0 0" + saveTarget(pair1) + saveRadioSilence(pair1));
                        break;
                    }
                }
        ...
        try
        {
            ...
            if(Plugin.builder.selectedPoint() != null)
            {
                ...
                if(pair.type() == 1 && pair.waypointType == 4)
                    k = 1;
                pair1 = new PAir(Plugin.builder.selectedPath(), Plugin.builder.selectedPoint(), point3d, k, defaultHeight, defaultSpeed, pair);
            } else
            {
            ...
            clampSpeed(pair1);
            Plugin.builder.setSelected(pair1);
    ...
    public void changeType()
    {
        ...
        int j0 = j;
        for(; ObjIO.classGetName(class1).equalsIgnoreCase(PlaceholderLabel); class1 = type[i].item[j].clazz)
        {
            if(j >= Plugin.builder.wSelect.comboBox2.size() - 1)
                break;
            j++;
        }
    ...
    public void configure()
    {
        ...
        if(i <= 0)
            throw new RuntimeException("PlMisAir: file '" + super.sectFile + "' is empty");
        type = new Type[i];
        for(int j = 0; j < i; j++)
        {
    ...
    public String[] actorInfo(Actor actor)
    {
        ...
        _actorInfo[1] = "(" + i + ") " + Plugin.timeSecToString(((PPoint) (pair)).time + (double)(int)(World.getTimeofDay() * 60F * 60F));
        return _actorInfo;
    }
    ...
    public void updateSelectorMesh()
    {
        ...
        if(paintscheme != null)
        {
            com.maddox.il2.engine.HierMesh hiermesh = Plugin.builder.wSelect.getHierMesh();
            if(hiermesh != null)
    ...
    private void fillDialogWay()
    {
        ...
        int l = (int)Math.round(((PPoint) (pair)).time / 60D + (double)(World.getTimeofDay() * 60F));
        ...
        if(pair.getTarget() != null)
        {
            if(pair.getTarget() instanceof PPoint)
            {
                if(pair.getTarget() instanceof PAir)
                    ((GWindowDialogControl) (wTarget)).cap.set(((PathAir)pair.getTarget().getOwner()).typedName);
                else
                if(pair.getTarget() instanceof PNodes)
                    ((GWindowDialogControl) (wTarget)).cap.set(Property.stringValue(pair.getTarget().getOwner(), "i18nName", ""));
                else
                    ((GWindowDialogControl) (wTarget)).cap.set(pair.getTarget().getOwner().name());
            } else
            {
                ((GWindowDialogControl) (wTarget)).cap.set(Property.stringValue(pair.getTarget().getClass(), "i18nName", ""));
            }
        } else
        {
            ((GWindowDialogControl) (wTarget)).cap.set("");
        }
    }
    ...
    private void fillDialogWay2()
    {
        ...
        case 1: // '\001'
            wTakeOffType.setSelected(i > 0 ? i - 1 : 0, true, false);
            wTakeoffSpacing.setValue("" + pair.takeoffSpacing, false);
            break;

        case 2: // '\002'
            wLandingType.setSelected(i + (i > 100 ? -100 : 0), true, false);
            break;
    ...
    public void initEditWay()
    {
        ...
        gwindowdialogclient.addControl(wType = new GWindowComboControl(gwindowdialogclient, 9F, 9F, 7F) {
            ...
            public boolean notify(int i, int j)
            {
                if(i == 2)
                {
                    if(_curPointType != super.iSelected)
                    {
                        PAir pair = (PAir)Plugin.builder.selectedPoint();
                        PathAir pathair = (PathAir)Plugin.builder.selectedPath();
                        int k = pathair.pointIndx(pair);
                        ...
                        if(_curPointType == 1 || _curPointType == 2)
                            flag = false;
                        boolean flag1 = true;
                        if(super.iSelected == 1 || super.iSelected == 2)
                            flag1 = false;
                        if(flag != flag1)
                            if(flag1)
                            {
                        ...
                        _curPointType = super.iSelected;
                        pair.setType(super.iSelected);
                        fillDialogWay2();
                        PlMission.setChanged();
                        if(super.iSelected == 1 && k == 0 || super.iSelected == 2)
                        {
                            Airport airport = Airport.nearest(((Actor) (pair)).pos.getAbsPoint(), -1, 7);
                            if(airport != null)
                                if(airport.nearestRunway(((Actor) (pair)).pos.getAbsPoint(), PlMisAir.nearestRunway))
                                    ((Actor) (pair)).pos.setAbs(PlMisAir.nearestRunway.getPoint());
                                else
                                    ((Actor) (pair)).pos.setAbs(((Actor) (airport)).pos.getAbsPoint());
                            pair.setTarget(null);
                            pair.sTarget = null;
                            ((GWindowDialogControl) (wTarget)).cap.set("");
                        } else
                        {
                            ...
                            if(actor != null && (super.iSelected == 0 && !(actor instanceof PAir) || super.iSelected == 3 && (actor instanceof PAir)))
                            {
                                pair.setTarget(null);
                                pair.sTarget = null;
                                ((GWindowDialogControl) (wTarget)).cap.set("");
        ...
        gwindowdialogclient.addLabel(new GWindowLabel(gwindowdialogclient, 1.0F, 11F, 7F, 1.3F, Plugin.i18n("Formation"), null));
        gwindowdialogclient.addControl(wFormations = new GWindowComboControl(gwindowdialogclient, 9F, 11F, 12F) {
        ...
        gwindowdialogclient.addLabel(new GWindowLabel(gwindowdialogclient, 1.0F, 13F, 7F, 1.3F, Plugin.i18n("RadioSilence"), null));
        gwindowdialogclient.addControl(wRadioSilence = new GWindowCheckBox(gwindowdialogclient, 9F, 13F, null) {
        ...
        gwindowdialogclient.addLabel(new GWindowLabel(gwindowdialogclient, 1.0F, 15F, 7F, 1.3F, Plugin.i18n("Target"), null));
        gwindowdialogclient.addLabel(wTarget = new GWindowLabel(gwindowdialogclient, 9F, 15F, 7F, 1.3F, "", null));
        gwindowdialogclient.addControl(wTargetSet = new GWindowButton(gwindowdialogclient, 1.0F, 17F, 5F, 1.6F, Plugin.i18n("&Set"), null) {
        ...
        gwindowdialogclient.addControl(wTargetClear = new GWindowButton(gwindowdialogclient, 9F, 17F, 5F, 1.6F, Plugin.i18n("&Clear"), null) {
    ...
    private void getTimeOut()
    {
        ...
        if(pair.type() != 2)
        {
            ...
            {
                double d5 = ((Actor) (ppoint)).pos.getAbsPoint().distance(((Actor) (pair)).pos.getAbsPoint());
    ...
    private void createMesh(int i)
    {
        ...
        } else
        {
            ...
            ((Actor) (actorMesh[i])).pos.setAbs(new Orient(90F, 0.0F, 0.0F));
            d *= Math.cos(0.26179938779914941D) / Math.sin(((double)camera3D[i].FOV() * 3.1415926535897931D) / 180D / 2D);
            ((Actor) (camera3D[i])).pos.setAbs(new Point3d(d, 0.0D, 0.0D), new Orient(180F, 0.0F, 0.0F));
            ((Actor) (camera3D[i])).pos.reset();
            return;
        }
    }
    ...
    private void initEditSkin()
    {
        for(_planeIndx = 0; _planeIndx < 4; _planeIndx++)
        {
            GWindowDialogClient gwindowdialogclient = (GWindowDialogClient)Plugin.builder.wSelect.tabsClient.create(new GWindowDialogClient() {
                ...
                {
                    super.resized();
                    wSkills[planeIndx].setSize(super.win.dx - ((GWindow) (wSkills[planeIndx])).win.x - lookAndFeel().metric(1.0F), ((GWindow) (wSkills[planeIndx])).win.dy);
                    wSkins[planeIndx].setSize(super.win.dx - ((GWindow) (wSkins[planeIndx])).win.x - lookAndFeel().metric(1.0F), ((GWindow) (wSkins[planeIndx])).win.dy);
                    wNoseart[planeIndx].setSize(super.win.dx - ((GWindow) (wNoseart[planeIndx])).win.x - lookAndFeel().metric(1.0F), ((GWindow) (wNoseart[planeIndx])).win.dy);
                    wPilots[planeIndx].setSize(super.win.dx - ((GWindow) (wPilots[planeIndx])).win.x - lookAndFeel().metric(1.0F), ((GWindow) (wPilots[planeIndx])).win.dy);
                    renders[planeIndx].setSize(super.win.dx - ((GWindow) (renders[planeIndx])).win.x - lookAndFeel().metric(1.0F), super.win.dy - ((GWindow) (renders[planeIndx])).win.y - lookAndFeel().metric(1.0F));
                }
            ...
            gwindowdialogclient.addControl(wSpawnPointClear[_planeIndx] = new GWindowButton(gwindowdialogclient, 15F, 12.5F, 5F, 1.6F, Plugin.i18n("&Clear"), null) {
                ...
                public boolean notify(int i, int j)
                {
                    if(i == 2)
                    {
                        ...
                        ((GWindowDialogControl) (wSpawnPointLabel[k])).cap.set(Plugin.i18n("NotSet"));
                        PlMission.setChanged();
                    }
                    return false;
                }
            ...
            renders[_planeIndx] = new GUIRenders(gwindowdialogclient, 1.0F, 15F, 17F, 7F, true) {
                ...
                public void mouseButton(int i, boolean flag, float f, float f1)
                {
                    ...
                    if(i == 1)
                    {
                        ...
                        if(Actor.isValid(actorMesh[planeIndx]))
                            ((Actor) (actorMesh[planeIndx])).pos.setAbs(new Orient(90F, 0.0F, 0.0F));
                    } else
                    if(i == 0)
                    {
                        f -= super.win.dx / 2.0F;
                        if(Math.abs(f) < super.win.dx / 16F)
                            animateMeshA[planeIndx] = 0.0F;
                        else
                            animateMeshA[planeIndx] = (-128F * f) / super.win.dx;
                        f1 -= super.win.dy / 2.0F;
                        if(Math.abs(f1) < super.win.dy / 16F)
                            animateMeshT[planeIndx] = 0.0F;
                        else
                            animateMeshT[planeIndx] = (-128F * f1) / super.win.dy;
                    }
                }

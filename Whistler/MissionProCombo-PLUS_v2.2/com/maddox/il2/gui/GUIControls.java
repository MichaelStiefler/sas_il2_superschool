...
;
        ...
        {
            if(isKeyFocus() && !(items[indx] instanceof ItemJoy) && messageBox == null && j != 571)
            {
                curKey[0] = curKey[1] = 0;
                doKey(i, true);
                doKey(j, true);
        ...
        public void joyMove(int i, int j, int k)
        {
            ...
            if(isKeyFocus() && (items[indx] instanceof ItemJoy) && messageBox == null)
            {
                ...
                int ai[] = new int[13];
                Joy.adapter().getSensitivity(i - 580, j - 563, ai);
                if(ai[12] > 0)
                    if(flag)
                        ai[12] = 2;
                    else
                        ai[12] = 1;
                Joy.adapter().setSensitivity(i - 580, j - 563, ai);
                int j1 = Config.cur.ini.get("rts", "JoyProfile", 0, 0, 3);
    ...
    public class FixedClient extends GWindowDialogClient
    {
        ...
        public void render()
        {
            float f = ((scrollClient.win.dx - scrollClient.vScroll.win.dx) * 1024F) / root.win.dx;
            for(int i = 0; i < items.length; i++)
            {
                Item item = items[i];
                if(item.control != null)
                    GUISeparate.draw(this, GColor.Gray, x1024(32F), y1024(item.y + 32F), x1024(f - 48F), 1.0F);
            }
        }
    ...
    public class DialogClient extends GUIDialogClient
    {
        ...
        public boolean notify(GWindow gwindow, int i, int j)
        {
            ...
            if(gwindow == bSave)
            {
                new GWindowMessageBox(root, 20F, true, i18n("warning.Warning"), "Override default controls?", 1, 0.0F) {

                    public void result(int i1)
                    {
                        IniFile inifile = new IniFile("users/default.ini", 1);
                        String as[] = UserCfg.nameHotKeyEnvs;
                        for(int k = 0; k < as.length; k++)
                        {
                            inifile.deleteSubject("HotKey " + as[k]);
                            HotKeyEnv.toIni(as[k], inifile, "HotKey " + as[k]);
                        }
                        inifile.saveFile();
                    }
                }
;
            ...
            if(gwindow == bDefault)
            {
                ...
                for(int k = 0; k < as.length; k++)
                {
                    HotKeyEnv.env(as[k]).all().clear();
                    HotKeyEnv.fromIni(as[k], inifile, "HotKey " + as[k]);
                }
        ...
        public void render()
        {
            ...
            setCanvasFont(0);
            draw(x1024(96F), y1024(656F), x1024(224F), y1024(48F), 0, i18n("misc.Apply_Back"));
            draw(x1024(372F), y1024(656F), x1024(272F), y1024(48F), 0, i18n("Save as Default"));
            draw(x1024(648F), y1024(656F), x1024(272F), y1024(48F), 0, i18n("Load Defaults"));
            if(Main.stateStack().getPrevious() == null || Main.stateStack().getPrevious().id() != 53)
                draw(x1024(690F), y1024(656F), x1024(240F), y1024(48F), 2, i18n("ctrl.Input"));
        }
        ...
        public void setPosSize()
        {
            ...
            bSave.setPosC(x1024(332F), y1024(680F));
            bDefault.setPosC(x1024(608F), y1024(680F));
            bInput.setPosC(x1024(970F), y1024(680F));
        }
...
//        client.doResolutionChanged();
    ...
    private void createItems()
    {
        ...
        for(int i = 0; i < as.length; i++)
        {
            ...
            {
                f += 32F;
                item.label = fixedClient.addLabel(new ItemLabel(fixedClient, 0.0F, 0.0F, 1.0F, 1.0F, "", (String)null));
            } else
            {
                f += f == 0.0F ? 32F : 64F;
                item.label = fixedClient.addLabel(new ItemLabel(fixedClient, 0.0F, 0.0F, 1.0F, 1.0F, resName(as[i]), (String)null));
            }
            item.y = f;
            ...
            for(Iterator iterator1 = _sortMap.keySet().iterator(); iterator1.hasNext();)
            {
                ...
                if(hotkeycmd1.name().startsWith("$$$"))
                {
                    ...
                    f += 32F;
                    item1.y = f;
                    item1.label = fixedClient.addLabel(new ItemLabel(fixedClient, 0.0F, 0.0F, 1.0F, 1.0F, "", (String)null));
                    arraylist.add(item1);
                } else
                if(hotkeycmd1.name().startsWith("$$+"))
                {
                    ...
                    f += 64F;
                    item2.y = f;
                    item2.label = fixedClient.addLabel(new ItemLabel(fixedClient, 0.0F, 0.0F, 1.0F, 1.0F, resName(hotkeycmd1.name().substring(3)), (String)null));
                ...
                } else
                {
                    ...
                    ((Item)obj).label = fixedClient.addLabel(new ItemLabel(fixedClient, 0.0F, 0.0F, 1.0F, 1.0F, resName(hotkeycmd1.name()), (String)null));
                    ((Item)obj).control = (ItemControl)fixedClient.addControl(new ItemControl(fixedClient, hotkeycmd1.name(), (String)null));
                    arraylist.add(obj);
    ...
    public GUIControls(GWindowRoot gwindowroot)
    {
        ...
        bInput = (GUIButton)dialogClient.addControl(new GUIButton(dialogClient, gtexture, 0.0F, 192F, 48F, 48F));

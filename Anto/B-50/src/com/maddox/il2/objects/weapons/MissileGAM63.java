...
public class MissileGAM63 extends Missile implements MissileInterceptable
{
	  public MissileGAM63() {
		  ...
		  public void start(float f, int i) {
		    setMesh("3DO/Arms/RASCAL/mono_open.sim");
		    super.start(f, i);
		    noGDelay = -1L;
		  }
  ...
  static
  {
    Class class1 = MissileGAM63.class;
    Property.set(class1, "mesh", "3do/arms/RASCAL/mono.sim");
    Property.set(class1, "sprite", "3DO/Effects/RocketGAM63/GAM63_Flame.eff");
    ...
    Property.set(class1, "force", 46400F); // Rocket Engine Power (in Newton)
    ...
    Property.set(class1, "forceT2", 0.0F); // Time2, i.e. time before Rocket Engine burn time ends (in Seconds), from this time on Rocket Engine power output decreases, 0 disables this feature
    ...
    Property.set(class1, "kalibr", 1.22F);
    Property.set(class1, "massa", 8255.0F);
    Property.set(class1, "massaEnd", 4855.0F); // ~3400kg fuel load
    ...
    Property.set(class1, "engineDelayTime", 1000L); // Rocket Engine Start Delay Time (in milliseconds), negative Value means that Engine is started before Missile leaves it's rail
    ...
    Property.set(class1, "maxFOVfrom", 90.0F); // max Angle offset from launch A/C POV
    Property.set(class1, "maxFOVto", 360.0F); // max Angle offset from target aft POV
    Property.set(class1, "PkMaxFOVfrom", 90.0F); // max Angle offset from launch A/C POV for Pk calculation
    Property.set(class1, "PkMaxFOVto", Float.MAX_VALUE); // max Angle offset from target aft POV for Pk calculation
    Property.set(class1, "PkDistMin", 10000.0F); // min Distance for Pk calculation
    Property.set(class1, "PkDistOpt", 50000.0F); // optimum Distance for Pk calculation
    Property.set(class1, "PkDistMax", 160000.0F); // max Distance for Pk calculation
    ...
    Property.set(class1, "friendlyName", "GAM-63"); // Display Name of this missile
    Property.set(class1, "iconFar_shortClassName", "GAM-63 RASCAL"); // Display Name of this missile
    Spawn.add(class1, new SPAWN());
  }

  public MissileGAM63(Actor actor, NetChannel netchannel, int i, Point3d point3d, Orient orient, float f)
  {
  ...
  static class SPAWN extends Missile.SPAWN
  {
    ...
    {
      new MissileGAM63(actor, netchannel, i, point3d, orient, f);
    }
  }
}
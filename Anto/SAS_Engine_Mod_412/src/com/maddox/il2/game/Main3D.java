...
public class Main3D extends Main {
	...
	class FarActorFilter implements ActorFilter {
		...
		public boolean isUse(Actor actor, double d) {
			...
			if (dotrange.showAltIcon(d))
				s3 = String.valueOf(dotrange.altSymbol());
			if (s3 != null)
	...
	private void drawTaxingPath() {
		if (NetMissionTrack.isPlaying())
			return;
		if (Aircraft.playerTaxingWay == null)
			return;
		if (keyRecord.isPlaying())
			return;
		if (!Mission.isPlaying())
			return;
		if (taxingHelperMesh == null)
			taxingHelperMesh = new Mesh("3DO/Effects/TaxingHelper/mono.sim");
	...
	public void setView(Actor actor, boolean flag) {
		if (viewActor() != actor || flag || bViewFly || bViewEnemy) {
			...
			camera3D.pos.resetAsBase();
			if (flag1 && World.Sun().needsCubeUpdate((float) camera3D.pos.getAbsPoint().z))
				World.land().cubeFullUpdate();
		}
	}
	...
	public String startPlayRecordedMission() {
		...
		else if (!keyRecord.startPlay(playRecordedSect, playRecorderIndx, 12, playRecordedMissionCallback))
	...
	public boolean saveRecordedStates2(InOutStreams inoutstreams) {
		try {
			...
			if (Actor.isValid(cockpitCur) && cockpitCur.isToggleLight())
				i |= 0x80;
			if (Actor.isValid(cockpitCur) && !cockpitCur.isEnableRenderingBall())
				i |= 0x100;
			if (Actor.isValid(cockpitCur)) {
	...
	public void loadRecordedStates0(int i) {
		try {
			...
			if (bOverrideIcons)
				iconTypes = 0;
			if (i >= 104)
				World.cur().userFuzeType = Integer.parseInt(bufferedreader.readLine());
	...
	public void loadRecordedStates2() {
		try {
			...
			if (Actor.isValid(cockpitCur)) {
				...
				if ((i & 0x200) != 0)
					cockpitCur.setReticleBrightness(0.0F);
				else if ((i & 0x400) != 0)
					cockpitCur.setReticleBrightness(0.2F);
				else if ((i & 0x800) != 0)
					cockpitCur.setReticleBrightness(0.4F);
				else if ((i & 0x1000) != 0)
					cockpitCur.setReticleBrightness(0.6F);
				else if ((i & 0x2000) != 0)
					cockpitCur.setReticleBrightness(0.8F);
				else if ((i & 0x4000) != 0)
					cockpitCur.setReticleBrightness(1.0F);
				else if ((i & 0x8000) != 0)
					cockpitCur.setReticleBrightness(1.2F);
				else if ((i & 0x10000) != 0)
					cockpitCur.setReticleBrightness(1.4F);
				if ((i & 0x20000) != 0) {
	...
	private void shadowPairsAdd(ArrayList arraylist) {
		...
		for (int j = 0; j < i; j++) {
			...
			if (!(obj instanceof BigshipGeneric) || (obj instanceof TransparentTestRunway))
				continue;
	...
	private void drawTimeRP() {
		if (!Config.cur.bRec || !NetMissionTrack.isRecording())
			return;
		int i = TextScr.This().getViewPortWidth();
		long l = Time.current();
		...
		if (NetMissionTrack.isPlaying()) {
			if (j > 9)
				TextScr.output(i - TextScr.font().height() * 10, 5, "REC " + k + ":" + j);
			else
				TextScr.output(i - TextScr.font().height() * 10, 5, "REC " + k + ":0" + j);
		} else if (j > 9)
			TextScr.output(i - TextScr.font().height() * 6, 5, "REC " + k + ":" + j);
		else
			TextScr.output(i - TextScr.font().height() * 6, 5, "REC " + k + ":0" + j);
	}
	...
	private void drawRandomSplashScreen() {
		...
		if (f > 1.5F)
			fileNameStart = "wide_" + fileNameStart;
		int maxIndex = 0;
		try {
			while (true) {

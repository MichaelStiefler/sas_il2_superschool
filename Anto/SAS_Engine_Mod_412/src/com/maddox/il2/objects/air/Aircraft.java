			...
			if (shot.chunkName == partNames[43]) {
				...
				if (tmpBonesHit > 0) {
					...
					for (int i = 0; i < tmpBonesHit; i++) {
						...
						if (shot.power > 0.0F) {
							...
							hierMesh();
							point3d.interpolate(point3d_71_, point3d_72_,
									HierMesh
									.collisionDistMulti(i));
							...
							if (!string.startsWith("xx")) {
								...
								if (i == tmpBonesHit - 1)
									...
									hierMesh();
									float f_75_
									= HierMesh.collisionDistMulti(i + 1);
									hierMesh();
									f_74_ = (f_75_
											- HierMesh.collisionDistMulti(i));
								}
									...
									if (i_78_ == 8) {
										if ((double) World.Rnd().nextFloat()
											...
											FM.AS.hitGear(shot.initiator, 1,
													8);
											FM.AS.hitGear(shot.initiator, 0,
													8);
										} else
											FM.AS.hitGear(shot.initiator,
													i_76_, i_78_);
									} else
										FM.AS.hitGear(shot.initiator, i_76_,
												i_78_);
								}
							}
						}
				...
				if (bool) {
					...
					debugprintln("Bullet hit from "
							+ (shot.initiator instanceof Aircraft
									? ((Aircraft) shot.initiator).typedName()
											: shot.initiator.name())

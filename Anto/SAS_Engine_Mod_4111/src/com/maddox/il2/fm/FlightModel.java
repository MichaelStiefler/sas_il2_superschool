...
public class FlightModel extends FlightModelMain
{
    ...
    public void setSkill(int i)
    {
        ...
        World.cur();
        if(actor != World.getPlayerAircraft() && !((Aircraft)actor).isNetPlayer())
            switch(i)
            {
            case 0: // '\0'
                SensPitch *= 0.75F;
            ...
            Aircraft.debugprintln(actor, "Skill adjustment rejected on the Player AI parameters..");
    }
    ...
    private void updateTurret(Turret turret1, int i, float f)
    {
        ...
        if(W.lengthSquared() > 0.25D)
            f3 *= 1.0F - (float)Math.sqrt(W.length() - 0.5D);
        if(getOverload() > 1.0F)
            f3 *= Aircraft.cvt(getOverload(), 1.0F, 5F, 1.0F, 0.0F);
        else
        if(getOverload() < 0.0F)
            f3 *= Aircraft.cvt(getOverload(), -1.5F, 0.0F, 0.0F, 1.0F);
        if(turret1.target != null && (turret1.target instanceof Aircraft) && ((Aircraft)turret1.target).FM.isTakenMortalDamage())
            turret1.target = null;
        ...
        case 0: // '\0'
            ...
            if(Time.current() > turret1.timeNext)
            {
                ...
                if(turret1.target == null)
                {
                    ...
                    if(VisCheck.visCheckTurret(turret1, (Aircraft)this.actor, (Aircraft)turret1.target, true))
                    {
                        turret1.tMode = 1;
                        turret1.timeNext = 0L;
                        if((Aircraft)this.actor == World.getPlayerAircraft() && turret1.health > 0.4F)
                            Voice.speakRearGunEA((Aircraft)this.actor, (Aircraft)turret1.target);
                    } else
                    {
        ...
        case 1: // '\001'
            ...
            if((float)(World.Rnd().nextInt() & 0xff) >= 32F * f3 && f1 >= 148F + 27F * (f3 - 1.0F))
                break;
        ...
        {
            shoot = VisCheck.visCheckTurret(turret1, (Aircraft)this.actor, (Aircraft)turret1.target, false);
            if(!shoot)
                turret1.tMode = 0;
        }
        if(shoot)
    ...
    protected void putScareShpere()
    {
        ...
        if(Landscape.rayHitHQ(Loc, p2, p))
        {
            ...
            if((actor instanceof TypeFighter) && f < 500F && Or.getTangage() < -15F)
            {
                ScareEnemies.set(2);
                Engine.collideEnv().getNearestEnemies(p, 45D, actor.getArmy(), ScareEnemies.enemies());
            }
            ...
            if(AP.way.landingAirport == null)
            {
                ...
                if(AP.way.curr().Action == 2)
                {
                    ...
                    if(actor != null && (actor instanceof BigshipGeneric))
                        AP.way.landingAirport = ((BigshipGeneric)actor).getAirport();
                }
                AP.way.setCur(i);
            }
            if(Actor.isAlive(AP.way.landingAirport) && !AP.way.isLanding())

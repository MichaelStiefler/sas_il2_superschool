...
public class OverLoad extends Render implements MsgGLContextListener {

	public void destroy() {
		com.maddox.il2.engine.Camera camera = getCamera();
		if (Actor.isValid(camera)) camera.destroy();
		super.destroy();
	}
	...
	public OverLoad(int i, float f) {
		...
		useClearDepth(false);
		useClearColor(false);
		if (_indx == 0) setName("renderOverLoad");
		GLContext.getCurrent().msgAddListener(this, null);
		if (i != 0) Main3D.cur3D()._getAspectViewPort(i, viewPort);
	}
	...
	public void render() {
		...
		if (realflightmodel == null) return;
		float f2 = 1.0F;
		if (Main3D.cur3D().cockpitCur != null) f2 = realflightmodel.AS.getPilotHealth(Main3D.cur3D().cockpitCur.astatePilotIndx());
		float f3 = 1.0F - f2;
		if (realflightmodel.saveDeep < 0.02F && realflightmodel.saveDeep > -0.02F && (double) f3 < 0.02D) return;
		Render.clearStates();
		gl.ShadeModel(7425);
		gl.Disable(2929);
		gl.Enable(3553);
		...
		if (realflightmodel.saveDeep >= 0.02F) {
			...
			if ((double) f3 >= 0.02D) renderMinus(f3);
		} else if (realflightmodel.saveDeep <= -0.02F) {

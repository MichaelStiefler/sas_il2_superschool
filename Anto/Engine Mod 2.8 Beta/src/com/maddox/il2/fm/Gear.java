...
//		bSteamCatapult = false;
	...
	public void ground(FlightModel flightmodel, boolean flag, boolean flag1) {
				...
				if (bigshipgeneric.isTowAircraft((Aircraft) flightmodel.actor)) {
					...
					if (v0.x > 0.0D) {
						...
						if (tmpV.x < 0.0D) {
							...
							if (FM.Vmin > 190F) d5 += (FM.Vmin - 190F) * 50F;
							if (FM.M.getFullMass() > 4500F) d5 += FM.M.getFullMass() - 4500F;
							v0.scale(d5 * d3);
							// v0.scale(1000D * d3);
							// TODO: --- CTO Mod 4.12 ---
		...
		D = -Normal.dot(Pn);
		if (!bIsMaster) D -= 0.014999999999999999D;
		if (D > 50D) {
			nearGround = false;
			return;
		}
		...
		for (int i1 = 0; i1 < pnti.length; i1++) {
			...
			if (d < 0.0D) {
				...
				if (d >= 0.10000000000000001D || isWater || bIsGear || !testNonGearCollision(i1)) continue;
				clp[i1] = true;
				nP++;
			}
		...
		if (bIsMaster) {
			...
			if (bigshipgeneric1 != null) {
				if (FM.brakeShoe) {
								...
								if (dCatapultOffsetX[i4] != 0.0D && dCatapultOffsetY[i4] != 0.0D) {
									...
									if (d7 <= 100D) {
										...
										if (bHasBlastDeflector) {
											if (bLogDetailBD) System.out.println("Gear: 876 - bHasBlastDeflector = true , setBlastDeflector(i4=" + Integer.toString(i4) + ", 1)");
					...
					// TODO: +++ CTO Mod 4.12 +++
					if (FM.EI.getCatapult()) FM.Or.setYPR(FM.brakeShoeLastCarrier.pos.getAbsOrient().getYaw() + (float) dCatapultYaw[FM.EI.getCatapultNumber()], FM.Or.getPitch(), FM.brakeShoeLastCarrier.pos.getAbsOrient().getRoll());
					// TODO: --- CTO Mod 4.12 ---
					...
					if (bHasBlastDeflector) {
						if (bLogDetailBD) System.out.println("Gear: 915 - bHasBlastDeflector = true , setBlastDeflector(FM.EI.getCatapultNumber()=" + Integer.toString(FM.EI.getCatapultNumber()) + ", 0)");
	...
	private boolean testGearCollision(int i) {
		...
		if (roughness < 0.20000000298023224D) roughness = 0.20000000298023224D;
		if (i < 2) {
			...
			d1 = Math.max(-9500D * (d - 0.10000000000000001D), -950000D * d);
			if (Pnt[i].x > 0.0F && Fd.dot(Normal) >= 0.0D) d1 *= 0.44999998807907104D;
		...
		case 1: // '\001'
			...
			if (d6 > 0.1D || db > 0.1D) // total brake action as db or d6
			{
				...
				case 3: // diff - toe brakes
					if (da > 0.1D) db += d8 * 2D * (da - 0.1D); // left/right brakes
					// difference alone
					if (da < -0.1D) db += d8 * 2D * (da + 0.1D);
					// if(d6 > 0.1D)
			...
			maxFric = d13 * Math.max(200D * (1.0D - 0.040000000000000001D * d9), 5D) * d5 * d11;
			if (fricF > maxFric) fricF = maxFric;
			if (fricF < -maxFric) fricF = -maxFric;
			double d16 = 0.029999999999999999D;
			if (((Tuple3f) (Pnt[2])).x > 0.0F) d16 = 0.059999999999999998D;
			...
			Tn.add(tmpV);
			if (bIsMaster && NormalVPrj < 0.0D) {
				double d18 = ForwardVPrj * ForwardVPrj * 8.0000000000000007E-005D + RightVPrj * RightVPrj * 0.0067999999999999996D;
				if (((FlightModelMain) (FM)).CT.bHasArrestorControl) d18 += NormalVPrj * NormalVPrj * 0.025000000000000001D;
				else d18 += NormalVPrj * NormalVPrj * 0.070000000000000007D;
				if (d18 > 1.0D) {
					fatigue[i] += 10;
					double d21 = 38000D + (double) ((FlightModelMain) (FM)).M.massEmpty * 6D;
					double d23 = (((Tuple3d) (Tn)).x * ((Tuple3d) (Tn)).x * 0.14999999999999999D + ((Tuple3d) (Tn)).y * ((Tuple3d) (Tn)).y * 0.14999999999999999D + ((Tuple3d) (Tn)).z * ((Tuple3d) (Tn)).z * 0.080000000000000002D) / (d21 * d21);
		...
		case 2: // '\002' // FRONT or REAR WHEEL
			...
			} else if (bFrontWheel) // FRONT WHEEL STEERING
			{
				...
				case 4:
					...
					if (Vs.lengthSquared() > 0.040000000000000001D) {
						...
						if (steerAngleFork.getDeg(0.001F) > -1F && steerAngleFork.getDeg(0.001F) < 1F) steerAngle = steerAngleFork.getDeg(0.001F);
						else if (steerAngleFork.getDeg(0.001F) < -1F) steerAngle = -1F;
						else if (steerAngleFork.getDeg(0.001F) > 1F) steerAngle = 1F;
					}
			...
			if (((FlightModelMain) (FM)).CT.bHasArrestorControl) d20 += NormalVPrj * NormalVPrj * 0.0040000000000000001D;
	...
	public double plateFriction(FlightModel flightmodel) {
		...
		if (!Actor.isValid(actor)) return 0.20000000000000001D;
		if (!World.cur().diffCur.Realistic_Landings) return 0.20000000000000001D;
		float f = 200F;
		actor.pos.getAbs(corn);
		bPlateExist = false;
		bPlateGround = false;
		...
		plateFilterGear = this;
		Engine.drawEnv().getFiltered((AbstractCollection) null, corn.x - (double) f, corn.y - (double) f, corn.x + (double) f, corn.y + (double) f, 1, plateFilter);
		if (bPlateExist) return !bPlateGround ? 0.0D : 0.80000000000000004D;
		...
		if (currentBornPlace != null) {
			...
			if (d1 != -1D) if (zutiHasPlaneSkisOnWinterCamo && d1 > 2.3999999999999999D) return 2.4000000953674316D;
			else return d1;
		}

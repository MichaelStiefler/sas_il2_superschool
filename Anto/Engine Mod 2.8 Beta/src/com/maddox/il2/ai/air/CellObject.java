...
public class CellObject implements Serializable {
	...
	public int getWidth() {
		...
		else return cells.length;
	}

	public int getHeight() {
		if (cells == null) return 0;
		if (cells[0] == null) return 0;
		else return cells[0].length;
	}

	public double getXCoordinate() {
		if (cells == null) return 0.0D;
		if (cells[0][0] == null) return 0.0D;
		else return cells[0][0].getCellX();
	}

	public double getYCoordinate() {
		if (cells == null) return 0.0D;
		if (cells[0][0] == null) return 0.0D;
		else return cells[0][0].getCellY();
	}
	...
	public void setWorldCoordinates(double d, double d1) {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[0].length; j++)
				if (cells[i][j] != null) cells[i][j].setCoordinates(d + (double) i * d2, d1 + (double) j * d2);
	...
	public CellObject getClone() {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[0].length; j++)
				if (cells[i][j] != null) acellobject[i][j] = cells[i][j];
	...
	public void debugPrint() {
		...
		for (int i = 0; i < cells[0].length; i++) {
			for (int j = 0; j < cells.length; j++)
				if (cells[j][i] != null) System.out.print(cells[j][i].getId());
	...
	public void reInitReferences() {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[0].length; j++)
				if (cells[i][j] != null) cells[i][j] = this;

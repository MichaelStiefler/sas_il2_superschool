...
public class Main3D extends Main {
	...
	public static boolean bOverrideIcons = false;
	...
	private Mesh taxingHelperMesh;
	private static String _sLastMusic = "ru";
	protected int viewMirror;
	private int iconTypes;
	public static final String gameHotKeyCmdEnvs[] = { "Console", "hotkeys", "HookView", "PanView", "HeadMove", "SnapView", "pilot", "move", "gunner", "misc", "orders", "aircraftView", "timeCompression", "gui" };
	...
	class CmdScreenSequence extends Cmd {
		...
		public Object exec(CmdEnv cmdenv, Map map) {
			if (screenSequence == null) screenSequence = new ScreenSequence();
	...
	class FarActorFilter implements ActorFilter {
		...
		private void drawPointer(int i, double d, int j, int k) {
			...
			if (p2d.z > 1.0D) if (d2 > 0.0D) d2 = -3.1415926535897931D + d2;
			else d2 = 3.1415926535897931D + d2;
			double d3;
			double d4;
			if (d2 >= 0.0D) {
				if (d2 <= d1) {
				...
				} else if (d2 <= 3.1415926535897931D - d1) {
					d4 = k;
		...
		public boolean isUse(Actor actor, double d) {
			if (!actor.isDrawing()) return false;
			if (actor == iconFarViewActor) return false;
			if (d <= 5D) return false;
			int i = actor.getArmy();
			if (i == 0 && !isBomb(actor)) return false;
			...
			if (d > iconDrawMax) return false;
			actor.pos.getRender(p3d);
			if (!project2d_cam(p3d, p2d)) return false;
			...
			int j2 = l1 | (Main3D.iconFarColor >>> 1 & 0x3f) * i1 + (l >>> 0 & 0xff) * j1 >>> 8 | ((Main3D.iconFarColor >>> 9 & 0x3f) * i1 + (l >>> 8 & 0xff) * j1 >>> 8) << 8
					| ((Main3D.iconFarColor >>> 17 & 0x3f) * i1 + (l >>> 16 & 0xff) * j1 >>> 8) << 16;
			...
			if (d <= dotrange.range()) {
				double d3 = d;
				if (Config.cur.iconUnits > 0) d3 *= 3.2808399200439453D;
			...
			if (dotrange.showAltIcon(d)) s3 = String.valueOf(dotrange.altSymbol());
			if (s3 != null) insertFarActorItem(dotrange.colorIcon(d, i, i1), j, k, (float) (-p2d.z), s3);
			return false;
		}
	...
	public class RenderMap2D extends Render {
		...
		protected void contextResize(int i, int j) {
			...
			if (land2DText != null) land2DText.contextResized();
		}
		...
		public void render() {
			if (land2D != null) land2D.render();
			if (Main.state() != null && Main.state().id() == 18) {
				GUIBuilder guibuilder = (GUIBuilder) Main.state();
				guibuilder.builder.renderMap2D();
			} else if (land2DText != null) land2DText.render();
			renderMap2D();
		}
	...
	public class RenderCockpitMirror extends Render {
		...
		protected void contextResize(int i, int j) {
			setViewPort(new int[] { mirrorX0(), mirrorY0(), mirrorWidth(), mirrorHeight() });
			if (camera != null) ((Camera3D) camera).set(((Camera3D) camera).FOV(), (float) mirrorWidth() / (float) mirrorHeight());
		}

		public boolean isShow() {
			if (viewMirror > 0 && renderCockpit.isShow()) return cockpitCur.isExistMirror();
			else return false;
		}

		public void preRender() {
			if (Actor.isValid(cockpitCur) && cockpitCur.isFocused()) cockpitCur.preRender(true);
		}
	...
	public class RenderCockpit extends Render {
		...
		public void preRender() {
			iRenderIndx = _indx;
			if (Actor.isValid(cockpitCur) && cockpitCur.isFocused()) cockpitCur.preRender(false);
			iRenderIndx = 0;
		}

		public void render() {
			iRenderIndx = _indx;
			if (Actor.isValid(cockpitCur) && cockpitCur.isFocused()) cockpitCur.render(false);
			iRenderIndx = 0;
		}
		...
		public boolean isShow() {
			if (_indx == 0) return super.isShow();
			else return Config.cur.isUse3Renders() && renderCockpit.isShow();
		}
	...
	public class Render2DMirror extends Render {
		...
		protected void contextResize(int i, int j) {
			setViewPort(new int[] { mirrorX0(), mirrorY0(), mirrorWidth(), mirrorHeight() });
			if (camera != null) ((CameraOrtho2D) camera).set(0.0F, mirrorWidth(), 0.0F, mirrorHeight());
		}
		...
		public void render() {
			bRenderMirror = true;
			if (bEnableFog) Render.enableFog(false);
			drawFarActors();
			if (bEnableFog) Render.enableFog(true);
			bRenderMirror = false;
		}
	...
	public class Render2D extends Render {
		...
		public boolean isShow() {
			...
			else return Config.cur.isUse3Renders() && render2D.isShow();
		}
	...
	public class Render3D1 extends Render {
		...
		public boolean isShow() {
			if (_indx == 0) return super.isShow();
			if (Main.state() != null && Main.state().id() == 18) return false;
			else return Config.cur.isUse3Renders() && render3D1.isShow();
		}
	...
	public class Render3D0 extends Render {
		...
		public boolean isShow() {
			if (_indx == 0) return super.isShow();
			if (Main.state() != null && Main.state().id() == 18) return false;
			else return Config.cur.isUse3Renders() && render3D0.isShow();
		}
			...
			if (Engine.land().ObjectsReflections_Begin(0) == 1) {
				...
				if (Landscape.isExistMeshs()) {
					...
					for (int j = 0; j < i; j++) {
						...
						if (actor instanceof ActorLandMesh) drwSolid.add(actor);
					}

					i = drwTranspL.size();
					for (int k = 0; k < i; k++) {
						Actor actor1 = (Actor) drwTranspL.get(k);
						if (actor1 instanceof ActorLandMesh) drwTransp.add(actor1);
					}
	...
	public Main3D() {
		...
		tmpOr = new Orient(0.0F, 0.0F, 0.0F);
		taxingHelperMesh = null;
		viewMirror = 2;
		iconTypes = 3;
	...
	public void setIconsOverride(boolean flag) {
		bOverrideIcons = flag;
	}

	private void drawTaxingPath() {
		if (NetMissionTrack.isPlaying()) return;
		if (Aircraft.playerTaxingWay == null) return;
		if (keyRecord.isPlaying()) return;
		if (!Mission.isPlaying()) return;
		if (taxingHelperMesh == null) taxingHelperMesh = new Mesh("3DO/Effects/TaxingHelper/mono.sim");
		for (int i = 0; i < Aircraft.playerTaxingWay.size() - 1; i++) {
			Point3d point3d = (Point3d) Aircraft.playerTaxingWay.get(i);
			Point3d point3d1 = (Point3d) Aircraft.playerTaxingWay.get(i + 1);
			tmpVector.sub(point3d1, point3d);
			float f = (float) tmpVector.length();
			taxingHelperMesh.setScaleXYZ(f, 1.0F, 1.0F);
			tmpVector.normalize();
			tmpOr.setAT0(tmpVector);
			taxingHelperMesh.setPos(point3d, tmpOr);
			int j = taxingHelperMesh.materialFind("Helper");
			if (j != -1) {
				Mat mat = taxingHelperMesh.material(j);
				mat.setLayer(0);
				mat.set((byte) 13, f * 0.1F);
				mat.set((byte) 11, Aircraft.taxHlpX);
			}
			taxingHelperMesh.render();
		}

	}

	public boolean isDemoPlaying() {
		if (playRecordedStreams != null) return true;
		if (keyRecord == null) return false;
		else return keyRecord.isPlaying();
	}
	...
	public boolean isViewInsideShow() {
		if (!Actor.isValid(cockpitCur)) return true;
		else return !cockpitCur.isNullShow();
	}

	public boolean isEnableRenderingCockpit() {
		if (!Actor.isValid(cockpitCur)) return true;
		else return cockpitCur.isEnableRendering();
	}

	public boolean isViewOutside() {
		if (!Actor.isValid(cockpitCur)) return true;
		else return !cockpitCur.isFocused();
	}

	public boolean isViewPadlock() {
		if (!Actor.isValid(cockpitCur)) return false;
		else return cockpitCur.isPadlock();
	}

	public Actor getViewPadlockEnemy() {
		if (!Actor.isValid(cockpitCur)) return null;
		else return cockpitCur.getPadlockEnemy();
	}

	public void setViewInsideShow(boolean flag) {
		if (isViewOutside() || isViewInsideShow() == flag) return;
	...
	public void setEnableRenderingCockpit(boolean flag) {
		if (isViewOutside() || isEnableRenderingCockpit() == flag) return;
		if (!Actor.isValid(cockpitCur)) {
			return;
		} else {
	...
	private void setViewSomebody(Actor actor) {
		...
		camera3D.pos.resetAsBase();
		if (actor instanceof ActorViewPoint) ((ActorViewPoint) actor).setViewActor(hookViewEnemy.enemy());
	}
	...
	public void setViewPadlock(boolean flag, boolean flag1) {
		if (isViewOutside()) return;
		if (!cockpitCur.existPadlock()) return;
		Aircraft aircraft = World.getPlayerAircraft();
		Actor actor;
		if (World.cur().diffCur.No_Icons) actor = Selector.look(true, flag1, camera3D, -1, -1, aircraft, false);
		else if (flag) actor = Selector.look(true, flag1, camera3D, -1, aircraft.getArmy(), aircraft, false);
		else actor = Selector.look(true, flag1, camera3D, aircraft.getArmy(), -1, aircraft, false);
		if (actor == null || actor == aircraft) return;
		if (!cockpitCur.startPadlock(actor)) return;
		else return;
	}
	...
	public void setViewPadlockForward(boolean flag) {
		if (isViewPadlock()) cockpitCur.setPadlockForward(flag);
	}

	public void setViewInside() {
		if (!isViewOutside() && !isViewPadlock()) return;
		if (!Actor.isValid(cockpitCur)) return;
		if (!cockpitCur.isEnableFocusing()) return;
		endView();
		endViewFly();
		endViewEnemy();
		endViewInside();
		cockpitCur.focusEnter();
		if (World.Sun().needsCubeUpdate((float) camera3D.pos.getAbsPoint().z)) World.land().cubeFullUpdate();
	}
	...
	public void setView(Actor actor, boolean flag) {
		if (viewActor() != actor || flag || bViewFly || bViewEnemy) {
			...
			camera3D.pos.resetAsBase();
			if (flag1 && World.Sun().needsCubeUpdate((float) camera3D.pos.getAbsPoint().z)) World.land().cubeFullUpdate();
		}
	}

	public int cockpitCurIndx() {
		if (cockpits == null || cockpitCur == null) return -1;
		for (int i = 0; i < cockpits.length; i++)
			if (cockpitCur == cockpits[i]) return i;

		return -1;
	}

	public void beginStep(int i) {
		if (!bUseStartLog) if (i >= 0) ConsoleGL0.exclusiveDrawStep(I18N.gui("main.loading") + " " + i + "%", i);
	...
	public boolean beginApp(GLContext glcontext, int i) {
		...
		TTFont.font[4] = TTFont.get("arialRadioChatter");
		...
		if (bUseStartLog) {
			...
			if (Main.showRandomSplash) this.drawRandomSplashScreen();
			else this.drawSingleSplashScreen();
		}
		beginStep(5);
	...
	public void setSaveAspect(boolean flag) {
		if (Config.cur.windowSaveAspect == flag) {
			...
			RendersMain.setSaveAspect(flag);
			return;
		}
	}
	...
	protected int viewSet_Get() {
		...
		i |= (viewMirror & 3) << 2;
		if (!aircraftHotKeys.isAutoAutopilot()) i |= 0x10;
		i |= (HUD.drawSpeed() & 3) << 5;
		return i;
	}
	...
	public void resetGameClear() {
		...
		sunFlareDestroy();
		if (Actor.isValid(spritesFog)) spritesFog.destroy();
		spritesFog = null;
		if (land2D != null) {
			if (!land2D.isDestroyed()) land2D.destroy();
			land2D = null;
		}
		if (land2DText != null) {
			if (!land2DText.isDestroyed()) land2DText.destroy();
			land2DText = null;
		}
		if (cockpits != null) {
			for (int j = 0; j < cockpits.length; j++) {
				if (Actor.isValid(cockpits[j])) cockpits[j].destroy();
				cockpits[j] = null;
			}
	...
	public void resetUserClear() {
		...
		if (cockpits != null) {
			for (int i = 0; i < cockpits.length; i++) {
				if (Actor.isValid(cockpits[i])) cockpits[i].destroy();
				cockpits[i] = null;
			}
	...
	public void sunFlareDestroy() {
		for (int i = 0; i < 3; i++) {
			if (Actor.isValid(_sunFlare[i])) _sunFlare[i].destroy();
			_sunFlare[i] = null;
		}
	...
	public void clearGameTrack(GameTrack gametrack) {
		if (gametrack == gameTrackRecord) gameTrackRecord = null;
		if (gametrack == gameTrackPlay) gameTrackPlay = null;
	}
	...
	public String playRecordedMission(String s, boolean flag) {
		playRecordedFile = s;
		if (playRecordedMissionCallback == null) playRecordedMissionCallback = new KeyRecordCallback() {

			public void playRecordedEnded() {
				if (this != playRecordedMissionCallback) return;
		...
		if (playRecordedNetChannelIn != null) playRecordedNetChannelIn.destroy();
		playRecordedNetChannelIn = null;
		if (InOutStreams.isExistAndValid(new File(s))) return playNetRecordedMission(s, flag);
		String s1 = s;
		...
		if (i >= 0) {
			...
			if (j == 1) playEndBatch = true;
			playBatchCurRecord++;
			if (playBatchCurRecord >= j) playBatchCurRecord = 0;
			s1 = "Records/" + sectfile.line(i, playBatchCurRecord);
			if (InOutStreams.isExistAndValid(new File(s1))) return playNetRecordedMission(s1, flag);
		...
		if (sectfile.vars(i) <= 12) return "Track file '" + s1 + "' is empty";
		int k = Integer.parseInt(sectfile.var(i, 0));
		if (k != 135) return "Track file '" + s1 + "' version is not supported";
		...
		int k1 = Integer.parseInt(sectfile.var(i, 9));
		long l1 = Long.parseLong(sectfile.var(i, 10));
		long l2 = Long.parseLong(sectfile.var(i, 11));
		long l3 = sectfile.fingerExcludeSectPrefix("$$$");
		l3 = Finger.incLong(l3, l);
		l3 = Finger.incLong(l3, f);
		l3 = Finger.incLong(l3, f1);
		l3 = Finger.incLong(l3, f2);
		l3 = Finger.incLong(l3, f3);
		l3 = Finger.incLong(l3, f4);
		l3 = Finger.incLong(l3, i1);
		l3 = Finger.incLong(l3, j1);
		l3 = Finger.incLong(l3, k1);
		l3 = Finger.incLong(l3, l1);
		if (l2 != l3) return "Track file '" + s1 + "' is changed";
		...
		World.cur().diffCur.No_GroundPadlock = false;
		...
		World.cur().userFuzeType = k1;
		FMMath.initSeed(l1);
		viewSet_Set(i1);
		if (!bOverrideIcons) iconTypes = j1;
		else iconTypes = 0;
	...
	private String playNetRecordedMission(String s, boolean flag) {
		try {
			...
			InputStream inputstream = playRecordedStreams.openStream("version");
			BufferedReader bufferedreader = new BufferedReader(new InputStreamReader(inputstream));
			int i = Integer.parseInt(bufferedreader.readLine());
			int j = i;
			if (i >= 107) j = Integer.parseInt(bufferedreader.readLine());
			...
			InputStream inputstream1 = playRecordedStreams.openStream("traffic");
			...
			RTSConf.cur.netEnv.addChannel(playRecordedNetChannelIn);
			playRecordedNetChannelIn.setStateInit(0);
			playRecordedNetChannelIn.userState = 1;
			NetMissionTrack.startPlaying(playRecordedStreams, i, j);
			if (flag) doRecordedPlayFirst();
		} catch (Exception exception) {
			exception.printStackTrace();
			if (playRecordedStreams != null) try {
				playRecordedStreams.close();
	...
	public String startPlayRecordedMission() {
		if (playRecordedStreams != null) keyRecord.startPlay(playRecordedMissionCallback);
		else if (!keyRecord.startPlay(playRecordedSect, playRecorderIndx, 12, playRecordedMissionCallback)) return "Track file '" + playRecordedPlayFile + "' load failed";
		return null;
	}

	public void stopPlayRecordedMission() {
		playRecordedSect = null;
		if (keyRecord.isPlaying()) keyRecord.stopPlay();
		if (playRecordedStreams != null) {
			try {
				playRecordedStreams.close();
			} catch (Exception exception) {
	...
	public void flyRecordedMission() {
		if (keyRecord.isPlaying()) {
			...
			if (Actor.isValid(cockpitCur)) HotKeyCmd.exec("misc", "cockpitEnter" + cockpitCurIndx());
	...
	public boolean saveRecordedMission(String s) {
		...
		if (mission.isDestroyed()) return false;
		if (!keyRecord.isContainRecorded()) return false;
		try {
	...
	public boolean saveRecordedStates2(InOutStreams inoutstreams) {
		try {
			...
			printwriter.println(FOVX);
			int i = 0;
			if (hud.bDrawDashBoard) i |= 1;
			if (isViewInsideShow()) i |= 2;
			if (Actor.isValid(cockpitCur) && cockpitCur.isToggleAim()) i |= 4;
			FlightModel flightmodel = World.getPlayerFM();
			if (flightmodel != null && flightmodel.AS.bShowSmokesOn) i |= 8;
			if (isEnableRenderingCockpit()) i |= 0x10;
			if (Actor.isValid(cockpitCur) && cockpitCur.isToggleUp()) i |= 0x20;
			if (Actor.isValid(cockpitCur) && cockpitCur.isToggleDim()) i |= 0x40;
			if (Actor.isValid(cockpitCur) && cockpitCur.isToggleLight()) i |= 0x80;
			if (Actor.isValid(cockpitCur) && !cockpitCur.isEnableRenderingBall()) i |= 0x100;
			if (Actor.isValid(cockpitCur)) {
				float f = cockpitCur.getReticleBrightness();
				byte byte0 = 9;
				int j = (int) (f / 0.2F);
				int k = (int) Math.pow(2D, byte0 + j);
				i |= k;
			}
			if (Actor.isValid(cockpitCur)) {
				i |= 0x20000;
				printwriter.println(i);
				printwriter.println((int) cockpitCur.aircraft().headingBug);
			}
			printwriter.flush();
			printwriter.close();
			return true;
	...
	public void loadRecordedStates0(int i) {
		try {
			...
			if (bOverrideIcons) iconTypes = 0;
			if (i >= 104) World.cur().userFuzeType = Integer.parseInt(bufferedreader.readLine());
	...
	public void loadRecordedStates1(boolean flag) {
		if (flag == bLoadRecordedStates1Before) try {
	...
	public void loadRecordedStates2() {
		try {
			...
			if (Actor.isValid(cockpitCur)) {
				cockpitCur.doToggleAim(false);
				boolean flag = (i & 4) != 0;
				cockpitCur.doToggleAim(flag);
			}
			FlightModel flightmodel = World.getPlayerFM();
			...
			if (Actor.isValid(cockpitCur)) {
				...
				if ((i & 0x40) != 0 && !cockpitCur.isToggleDim()) cockpitCur.doToggleDim();
				if ((i & 0x80) != 0 && !cockpitCur.isToggleLight()) cockpitCur.doToggleLight();
				cockpitCur.setEnableRenderingBall((i & 0x100) == 0);
				if ((i & 0x200) != 0) cockpitCur.setReticleBrightness(0.0F);
				else if ((i & 0x400) != 0) cockpitCur.setReticleBrightness(0.2F);
				else if ((i & 0x800) != 0) cockpitCur.setReticleBrightness(0.4F);
				else if ((i & 0x1000) != 0) cockpitCur.setReticleBrightness(0.6F);
				else if ((i & 0x2000) != 0) cockpitCur.setReticleBrightness(0.8F);
				else if ((i & 0x4000) != 0) cockpitCur.setReticleBrightness(1.0F);
				else if ((i & 0x8000) != 0) cockpitCur.setReticleBrightness(1.2F);
				else if ((i & 0x10000) != 0) cockpitCur.setReticleBrightness(1.4F);
				if ((i & 0x20000) != 0) {
					int j = Integer.parseInt(bufferedreader.readLine());
					cockpitCur.aircraft().headingBug = j;
				}
			}
			inputstream.close();
		} catch (Exception exception) {
	...
	public boolean project2d_cam(double d, double d1, double d2, Point3d point3d) {
		if (!project2d(d, d1, d2, point3d)) return false;
		if (bRenderMirror) {
	...
	private void shadowPairsAdd(ArrayList arraylist) {
		...
		for (int j = 0; j < i; j++) {
			...
			if (!(obj instanceof BigshipGeneric) || (obj instanceof TransparentTestRunway)) continue;
	...
	private void doPreRender3D(Render render) {
		...
		if (bDrawClouds && clouds != null) clouds.preRender();
		if (bDrawLand) Engine.land().preRender((float) __p.z, false);
	...
	private void doRender3D0(Render render) {
		...
		if (flag && bEnableFog) Render.enableFog(false);
		DrwArray drwarray = bRenderMirror ? drwMirror : drwMaster[iRenderIndx];
		...
		if (flag && bEnableFog) Render.enableFog(true);
		if (bDrawLand) Engine.land().render1(bRenderMirror);
		int i = gl.GetError();
		if (i != 0) System.out.println("***( GL error: " + i + " (render3d0)");
		if (Aircraft.showTaxingWay) drawTaxingPath();
	}
	...
	private void doRender3D1(Render render) {
		...
		if (!bRenderMirror) shadowPairsRender();
	...
	private void plateToRenderArray(ArrayList arraylist, ArrayList arraylist1) {
		...
		for (int j = 0; j < i; j++) {
			...
			if ((actor instanceof ActorMesh) && (((ActorMesh) actor).mesh() instanceof MeshShared)) {
				...
				if (!((MeshShared) ((ActorMesh) actor).mesh()).putToRenderArray(__l)) actor.draw.render(actor);
			} else {
	...
	public void reloadClearColor() {
		render3D0.setClearColor(GWindowLookAndFeel.getUIBackgroundColor());
		renderMap2D.setClearColor(GWindowLookAndFeel.getUIBackgroundColor());
	}

	private void drawTime() {
		if (!hud.bDrawAllMessages) return;
	...
	private void drawTimeRP() {
		if (!Config.cur.bRec || !NetMissionTrack.isRecording()) return;
		int i = TextScr.This().getViewPortWidth();
		long l = Time.current();
		...
		if (NetMissionTrack.isPlaying()) {
			if (j > 9) TextScr.output(i - TextScr.font().height() * 10, 5, "REC " + k + ":" + j);
			else TextScr.output(i - TextScr.font().height() * 10, 5, "REC " + k + ":0" + j);
		} else if (j > 9) TextScr.output(i - TextScr.font().height() * 6, 5, "REC " + k + ":" + j);
		else TextScr.output(i - TextScr.font().height() * 6, 5, "REC " + k + ":0" + j);
	}
	...
	protected void drawFarActors() {
		...
		for (int l = 0; l < k; l++) {
			...
			if (d3 < 25000D) farActorFilter.isUse(actor, d3);
		}
	...
	public void initHotKeys() {
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "ScreenShot") {
			...
			public void begin() {
				if (scrShot == null) scrShot = new ScrShot("grab");
				if (Mission.isNet()) {
					long l = Time.real();
					if (lastTimeScreenShot + 10000L < l) lastTimeScreenShot = l;
					else return;
				}
				scrShot.grab();
			}
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "land") {
			...
			public void begin() {
				if (RTSConf.cur.console.getEnv().levelAccess() == 0) setDrawLand(!isDrawLand());
			}
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "clouds") {
			...
			public void begin() {
				if (Mission.isSingle() && RTSConf.cur.console.getEnv().levelAccess() == 0) bDrawClouds = !bDrawClouds;
			}
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmd(true, "showTime") {
			...
			public void begin() {
				if (RTSConf.cur.console.getEnv().levelAccess() == 0) bShowTime = !bShowTime;
			}
	...
	private void drawRandomSplashScreen() {
		float f = RendersMain.getViewPortWidth() / RendersMain.getViewPortHeight();
		String fileNameStart = "background";
		if (f > 1.5F) fileNameStart = "wide_" + fileNameStart;
		int maxIndex = 0;
		try {
			while (true) {
				SFSInputStream sfsinputstream = new SFSInputStream("gui/" + fileNameStart + maxIndex + ".mat");
				sfsinputstream.close();
				maxIndex++;
			}
		} catch (Exception exception) {
			System.out.println("" + maxIndex + " Splashscreens available.");
			maxIndex--;
		}
		if (maxIndex < 0) return;
		RangeRandom rangerandom = new RangeRandom(System.currentTimeMillis());

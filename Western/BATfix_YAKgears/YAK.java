...
// By western, rework compatibility around gears on 16th/Dec./2018*/
    ...
    implements TypeFighter
{
    ...
    // +++++ get rid of 4.12.2m imcompatible old gear codes +++++
...
//    protected void moveGear(float f) {}
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("p1"))
                    getEnergyPastArmor((double)World.Rnd().nextFloat(5F, 12.7F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else
                if(s.endsWith("p2"))
                    getEnergyPastArmor((double)World.Rnd().nextFloat(0.0F, 12.7F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else
                if(s.endsWith("g1"))
                {
                    getEnergyPastArmor((double)World.Rnd().nextFloat(20F, 30F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
    ...
    static 
    {
        Class class1 = com.maddox.il2.objects.air.YAK.class;

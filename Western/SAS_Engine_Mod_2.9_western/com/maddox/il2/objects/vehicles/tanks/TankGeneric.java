// Source File Name:   TankGeneric.java from stock 4.13.4m , importing to "4.12.2m based" Engine mod 2.9.x .
// first imported and edited on 30th/Sep./2020
...
//          tankproperties.meshSummer = getS(sectfile, s, "MeshSummer");
            ...
            for(int j = 0; j < i; j++)
            {
                ...
                if(f1 > -1F)
                    tankproperties.gunProperties[j].NUMBER_TRUNKS = (int)f1;
                else
                    tankproperties.gunProperties[j].NUMBER_TRUNKS = 1;
                tankproperties.gunProperties[j].ATTACK_FAST_TARGETS = false;
                float f2 = sectfile.get(s, "FireFastTargets", -9865.345F);
                if(f2 != -9865.345F)
                    tankproperties.gunProperties[j].ATTACK_FAST_TARGETS = f2 > 0.5F;
                // +++ By western, expanded for flags Intercept missiles and Radar use
                tankproperties.gunProperties[j].ATTACK_MISSILES = false;
                float f9 = sectfile.get(s, "InterceptMissiles", -9865.345F);
                if(f9 != -9865.345F)
                    tankproperties.gunProperties[j].ATTACK_MISSILES = f9 > 0.5F;
                ...
                tankproperties.gunProperties[j].ATTACK_MAX_DISTANCE = getF(sectfile, s, "AttackMaxDistance" + s4, 6F, 25000F);
                tankproperties.gunProperties[j].ATTACK_MAX_RADIUS = getF(sectfile, s, "AttackMaxRadius" + s4, 6F, 25000F);
                tankproperties.gunProperties[j].ATTACK_MAX_HEIGHT = getF(sectfile, s, "AttackMaxHeight" + s4, 6F, 16000F);
                // ---
                ...
                if(sectfile.varExist(k, "HeadYawHalfRange" + s4))
                {
                    ...
                    tankproperties.gunProperties[j].HEAD_YAW_RANGE.set(-f3, f3);
                ...
                } else
                {
                    ...
                    tankproperties.gunProperties[j].HEAD_YAW_RANGE = new AnglesRange(-1F, 1.0F);
                    tankproperties.gunProperties[j].HEAD_YAW_RANGE.set(f4, f7);
                    tankproperties.gunProperties[j].HEAD_STD_YAW = f6;
                }
                ...
                float f5 = sectfile.get(s, "FastTargetsAngleError" + s4, -9865.345F);
                if(f5 <= 0.0F)
                    f5 = 0.0F;
                else
                if(f5 >= 45F)
                    f5 = 45F;
                tankproperties.gunProperties[j].FAST_TARGETS_ANGLE_ERROR = f5;
            }
...
// 4.12.2m doesn't support these 4.13.4m's newer codes.
//            String as[] = {
//                tankproperties.meshSummer, tankproperties.meshWinter, tankproperties.meshDesert
//            };
            Property.set(class1, "iconName", "icons/" + getS(sectfile, s, "Icon") + ".mat");
//            Property.set(class1, "meshName", as);
            Property.set(class1, "meshName", tankproperties.meshSummer);  // roll back to 4.12.2m code
        ...
        public boolean tick()
        {
            ...
            if(flag)
            {
                ...
                if(collisionStage == 0)
                {
                    ...
                    {
                        TankGeneric.p.x = TankGeneric.Rnd(-0.30D, 0.30D);
                        TankGeneric.p.y = TankGeneric.Rnd(-0.30D, 0.30D);
                ...
                } else
                {
                    ...
                    vector2d1.scale((double)prop.AFTER_COLLISION_DIST * TankGeneric.Rnd(0.20D, 0.60D));
            ...
            {
                mov.rotatCurTime--;
                float f2 = 1.0F - (float)mov.rotatCurTime / (float)mov.rotatTotTime;
                TankGeneric.o.setYaw(mov.angles.getDeg(f2));
                flag1 = true;
                if(mov.rotatCurTime <= 0L)
                {
                    mov.rotatCurTime = -1L;
...
// Keep 4.12.2m based codes, instead of using new 4.13.4m codes
//        f *= 1.1F;
...
//        if(f1 <= 3.33F)
    ...
    private void MakeCrush()
    {
        dying = 2;
        Point3d point3d = pos.getAbsPoint();
        long l = (long)((point3d.x % 2.10D) * 221D + (point3d.y % 3.10D) * 211D * 211D);
    ...
    private TankGeneric(TankProperties tankproperties, Actor actor)
    {
        ...
        for(int k = 0; k < arms.length; k++)
        {
            ...
            try
            {
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 1)
                    arms[k].gun0 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 2)
                    arms[k].gun1 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 3)
                    arms[k].gun2 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 4)
                    arms[k].gun3 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 5)
                    arms[k].gun4 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 6)
                    arms[k].gun5 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 7)
                    arms[k].gun6 = (Gun)prop.gunProperties[k].gunClass.newInstance();
                if(prop.gunProperties[k].NUMBER_TRUNKS >= 8)
                    arms[k].gun7 = (Gun)prop.gunProperties[k].gunClass.newInstance();
            }
            catch(Exception exception)
            {
                System.out.println(exception.getMessage());
            ...
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 1)
            {
                arms[k].gun0.set(this, "Gun" + s);
                arms[k].gun0.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 2)
            {
                arms[k].gun1.set(this, "Gun" + s + "a");
                arms[k].gun1.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 3)
            {
                arms[k].gun2.set(this, "Gun" + s + "b");
                arms[k].gun2.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 4)
            {
                arms[k].gun3.set(this, "Gun" + s + "c");
                arms[k].gun3.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 5)
            {
                arms[k].gun4.set(this, "Gun" + s + "d");
                arms[k].gun4.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 6)
            {
                arms[k].gun5.set(this, "Gun" + s + "e");
                arms[k].gun5.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 7)
            {
                arms[k].gun6.set(this, "Gun" + s + "f");
                arms[k].gun6.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
            if(prop.gunProperties[k].NUMBER_TRUNKS >= 8)
            {
                arms[k].gun7.set(this, "Gun" + s + "g");
                arms[k].gun7.loadBullets(isNetMirror() ? -1 : prop.gunProperties[k].MAX_SHELLS);
            }
    ...
    static Point3d unpackPos(byte abyte0[])
    {
        ...
        return new Point3d((double)i * 0.050D, (double)j * 0.050D, (double)k * 0.10D);
    }
    ...
    public float getReloadingTime(Aim aim)
    {
        ...
        for(int i = 0; i < arms.length; i++)
            if(aim.equals(arms[i].aim))
            {
                if(arms[i].gun0.haveBullets())
                    return prop.gunProperties[i].DELAY_AFTER_SHOOT;
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 1)
                    arms[i].gun0.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 2)
                    arms[i].gun1.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 3)
                    arms[i].gun2.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 4)
                    arms[i].gun3.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 5)
                    arms[i].gun4.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 6)
                    arms[i].gun5.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 7)
                    arms[i].gun6.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                if(prop.gunProperties[i].NUMBER_TRUNKS >= 8)
                    arms[i].gun7.loadBullets(prop.gunProperties[i].MAX_SHELLS);
                return 120F;
            }
    ...
    public int targetGun(Aim aim, Actor actor, float f, boolean flag)
    {
        ...
        if(arms[i].gun0 != null && (arms[i].gun0 instanceof CannonMidrangeGeneric))
        {
            int k = ((Prey)actor).chooseBulletType(arms[i].gun0.prop.bullet);
            if(k < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun0).setBulletType(k);
        }
        if(arms[i].gun1 != null && (arms[i].gun1 instanceof CannonMidrangeGeneric))
        {
            int l = ((Prey)actor).chooseBulletType(arms[i].gun1.prop.bullet);
            if(l < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun1).setBulletType(l);
        }
        if(arms[i].gun2 != null && (arms[i].gun2 instanceof CannonMidrangeGeneric))
        {
            int i1 = ((Prey)actor).chooseBulletType(arms[i].gun2.prop.bullet);
            if(i1 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun2).setBulletType(i1);
        }
        if(arms[i].gun3 != null && (arms[i].gun3 instanceof CannonMidrangeGeneric))
        {
            int j1 = ((Prey)actor).chooseBulletType(arms[i].gun3.prop.bullet);
            if(j1 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun3).setBulletType(j1);
        }
        if(arms[i].gun4 != null && (arms[i].gun4 instanceof CannonMidrangeGeneric))
        {
            int k1 = ((Prey)actor).chooseBulletType(arms[i].gun4.prop.bullet);
            if(k1 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun4).setBulletType(k1);
        }
        if(arms[i].gun5 != null && (arms[i].gun5 instanceof CannonMidrangeGeneric))
        {
            int l1 = ((Prey)actor).chooseBulletType(arms[i].gun5.prop.bullet);
            if(l1 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun5).setBulletType(l1);
        }
        if(arms[i].gun6 != null && (arms[i].gun6 instanceof CannonMidrangeGeneric))
        {
            int i2 = ((Prey)actor).chooseBulletType(arms[i].gun6.prop.bullet);
            if(i2 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun6).setBulletType(i2);
        }
        if(arms[i].gun7 != null && (arms[i].gun7 instanceof CannonMidrangeGeneric))
        {
            int j2 = ((Prey)actor).chooseBulletType(arms[i].gun7.prop.bullet);
            if(j2 < 0)
                return 0;
            ((CannonMidrangeGeneric)arms[i].gun7).setBulletType(j2);
        }
        boolean flag1 = ((Prey)actor).getShotpointOffset(arms[i].aim.shotpoint_idx, p1);
        if(!flag1)
            return 0;
        float f1 = f * Rnd(0.8F, 1.2F);
        if(!Aimer.aim((BulletAimer)arms[i].gun0, actor, this, f1, p1, arms[i].fireOffset))
            return 0;
        Point3d point3d = new Point3d();
        ...
        if(f1 > 0.001F)
        {
            ...
            if(d2 > 0.001D)
            {
                ...
                int l2 = Mission.curCloudsType();
                if(l2 >= 3)
                {
                    float f11 = l2 < 5 ? 500F : 250F;
        ...
        if(!((BulletAimer)arms[i].gun0).FireDirection(point3d1, point3d, vector3d))
            return 0;
        float f3;
        float f4;
        float f5;
        ...
        int k2 = aim.setRotationForTargeting(this, orient1, point3d1, arms[i].headYaw, arms[i].gunPitch, vector3d, f2, f1, prop.gunProperties[i].HEAD_YAW_RANGE, prop.gunProperties[i].GUN_MIN_PITCH, prop.gunProperties[i].GUN_MAX_PITCH, f3, f5, f4);
        if(i == 0 && !Head360() && k2 != 0 && aim.bodyRotation)

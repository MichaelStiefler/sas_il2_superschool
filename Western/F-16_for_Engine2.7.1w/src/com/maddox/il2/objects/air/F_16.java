...
public class F_16 extends Scheme1
    implements TypeSupersonic, TypeFighter, TypeFighterAceMaker, TypeAcePlane, TypeGSuit, TypeFastJet, TypeRadar, TypeBomber, TypeX4Carrier, TypeLaserSpotter, TypeStormovikArmored, TypeSemiRadar, TypeGroundRadar, TypeFuelDump, TypeDockable
{
...
label0:
    ...
    private boolean Flaps ()
     {
                    Polares polares = (Polares)Reflection.getValue(FM, "Wing");
                    if(FM.EI.engines[0].getThrustOutput() > 1.001F && (double)calculateMach() >= 0.18D && (double)calculateMach() < 0.32D && !Flaps)
            {
                    polares.Cy0_1 += 5.3F;
            Flaps = true;
                    }
     ...
     {
                    Polares polares = (Polares)Reflection.getValue(FM, "Wing");
                    if(FM.EI.engines[0].getThrustOutput() > 1.001F && (double)calculateMach() >= 0.18D && (double)calculateMach() < 0.32D && !Flaps2)
            {
                    polares.CyCritH_0 += 3.5F;
            Flaps2 = true;
                    }
                    if(FM.EI.engines[0].getThrustOutput() < 1.001F && (double)calculateMach() < 0.18D && (double)calculateMach() > 0.32D && Flaps2)
                    {
    ...
    private void receivingRefuel(float f)
    {
        ...
        if(typeDockableIsDocked())
        {
            ...
            } else
            {
                ...
                if(FM.M.fuel < FM.M.maxFuel - 20F)
                {
                    float getFuel = ((TypeTankerBoom) (queen_)).requestRefuel((Aircraft) this, 18.0F, f);
                    FM.M.fuel += getFuel;
                }

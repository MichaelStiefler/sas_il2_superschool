// Source File Name: MissileR60M_gn16.java
// Author:           western0221
...
public class MissileR60M_gn16 extends Missile {
	static class SPAWN extends Missile.SPAWN {
		...
		public void doSpawn(Actor actor, NetChannel netchannel, int i, Point3d point3d, Orient orient, float f) {
			new MissileR60M_gn16(actor, netchannel, i, point3d, orient, f);
		}
	}

	static {
		Class class1 = MissileR60M_gn16.class;
		Property.set(class1, "mesh", "3do/arms/R60M_gn16/mono.sim");
		...
		Property.set(class1, "timeLife", 28F); // Rocket life time in Seconds
		Property.set(class1, "timeFire", 4.3F); // Rocket Engine Burn time in Seconds
		Property.set(class1, "force", 10500F); // Rocket Engine Power (in Newton)
		...
		Property.set(class1, "power", 0.35F); // RL Data: 3.5kg HE warhead, for realism reduced to 1/10th of it's RL weight
		Property.set(class1, "radius", 7.5F); // Radius of Explosion Effect
		Property.set(class1, "kalibr", 0.12F); // Diameter of Missile Body
		Property.set(class1, "massa", 44.0F); // Missile Weight at Launch time
		Property.set(class1, "massaEnd", 33.0F); // Missile Weight when Booster burned out
		...
		Property.set(class1, "timeForNextLaunchAI", 5000L); // time between two Missile launches for AI
		...
		Property.set(class1, "PkDistMin", 200.0F); // min Distance for Pk calculation
		Property.set(class1, "PkDistOpt", 3500.0F); // optimum Distance for Pk calculation
		...
		Property.set(class1, "maxGForce", 35.0F); // max turning rate G-Force
		Property.set(class1, "stepsForFullTurn", 7); // No. of ticks (1 tick = 30ms) for full control surface turn, higher value means slower reaction and smoother flight, lower value means higher agility
		Property.set(class1, "fxLock", "weapon.R60.lock"); // prs file for Lock Tone
		Property.set(class1, "fxLockVolume", 1.0F); // Volume for Lock Tone, "1" is default
		Property.set(class1, "fxNoLock", "weapon.R60.nolock"); // prs file for No Lock Tone
		Property.set(class1, "fxNoLockVolume", 1.0F); // Volume for No Lock Tone, "1" is default
		Property.set(class1, "smplLock", "R60_lock.wav"); // wav file for Lock Tone
		Property.set(class1, "smplNoLock", "R60_no_lock.wav"); // wav file for No Lock Tone
		Property.set(class1, "friendlyName", "R-60M"); // Display Name of this missile
		Spawn.add(class1, new SPAWN());
	}

	public MissileR60M_gn16() {
	}

	public MissileR60M_gn16(Actor actor, NetChannel netchannel, int i, Point3d point3d, Orient orient, float f) {

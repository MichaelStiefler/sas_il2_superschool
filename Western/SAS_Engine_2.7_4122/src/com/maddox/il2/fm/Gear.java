    ...
    public Gear()
    {
      ...
      //TODO: +++ CTO Mod 4.12 +++
        ...
        if(Mission.cur().sectFile().get("Mods", "StandardDeckCVL", 0) == 1)
            bStandardDeckCVL = true;
        if(Config.cur.ini.get("Mods", "OldCatapultPowerCode", 1) == 0)
            bUseOldCatapultPowerProcedure = false;
        if(Config.cur.ini.get("Mods", "DebugCatapult", 0) == 1)
            bDebugCatapult = true;
      //TODO: --- CTO Mod 4.12 ---
...
//                            FM.producedAMM.set(0D, 0D, 0D);
			...
			float Vfly;
			if(FM.Vmin < 56F)  // 250km/h static when Vmin < 200km/h
                Vfly = 69.5F;
			else if(FM.Vmin < 61.1F)  // Vmin + 50km/h when Vmin < 220km/h
                Vfly = FM.Vmin + 13.9F;
			else
                Vfly = FM.Vmin * 1.25F;
...
////                }
        ...
        if(bCatapultHookExist[i])
        {
            ...
            if(catapultPower == 0.0F)
                bForceAutoCalculate = true;
        }
        else
            dist = 46F;
        float massX = FM.M.getFullMass();  //By PAL and western, empiric factor
        ...
        if(vTarget > 85F)
            vTarget = 85F;   // Limitter for 306km/h on the catapult's end
    ...
    public boolean setCatapultOffset(BigshipGeneric bigshipgeneric, SectFile theSectFile)
    {
        ...
        if(!flag2)
        {
            ...
            else
            {
                ...
                if(catapultPowerJets == -1.0F)
                    catapultPowerJets = catapultPower;
        ...
        for(; k < 4; k++)
        {
            if(findHook(carrierhiermesh, "_SCat" + (k + 1) + "_start", catapults[k][0]))
            {
                ...
                {
                    System.out.println("Gear - 2274 : Hooks mismatching _SCat" + (k + 1) + "_start and _SCat" + k + 1 + "_end");
                    break;
                }
            }
            else if(findHook(carrierhiermesh, "_Cat" + (k + 1) + "_start", catapults[k][0]))
            {
                ...
                {
                    System.out.println("Gear - 2288 : Hooks mismatching _Cat" + (k + 1) + "_start and _Cat" + k + "_end");
                    break;
                }
            }
            else

...
public class A_6fuelReceiver extends A_6
    implements TypeDockable
{

    public A_6fuelReceiver()
    {
        bNeedSetup = true;
        ...
        if(typeDockableIsDocked())
        {
            if(!(super.FM instanceof RealFlightModel) || !((RealFlightModel)super.FM).isRealMode())
            {
                ...
                if(FM.AP.way.curr().Action != 3)
                    ((FlightModelMain) ((Maneuver)super.FM)).AP.way.setCur(((FlightModelMain) ((((Aircraft)queen_)).FM)).AP.way.Cur());

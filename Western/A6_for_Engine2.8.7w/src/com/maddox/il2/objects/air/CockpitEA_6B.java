...
public class CockpitEA_6B extends CockpitPilot
{
    protected boolean doFocusEnter()
    {
        if(super.doFocusEnter())
        {
            ((A_6)aircraft()).startCockpitSounds();
            return true;
        }
        else
        {
    ...
    {
        stopSoundEffects();
        ((A_6)aircraft()).stopCockpitSounds();
        super.doFocusLeave();
    }
...
//            radioaltnow = radioaltnow / (float)Math.cos(Math.toRadians(Math.abs(fm.Or.getKren()))) / (float)Math.cos(Math.toRadians(Math.abs(fm.Or.getTangage())));
    ...
    public CockpitEA_6B()
    {
        super("3DO/Cockpit/A6E/EA6B.him", "he111");
        ...
        cockpitNightMats = (new String[] {
            "instr7_EA6", "Gauges_10", "Needles", "Needles2"
        });
        setNightMats(false);
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        if(!fm.CT.bMoveSideDoor)
            mesh.chunkSetAngles("Blister", 0.0F, 0.0F, 35F * fm.CT.getCockpitDoor());
        ...
        mesh.chunkSetAngles("Z_LiquidOxygen", cvt(((A_6)aircraft()).OxygenMiliLitter, 0F, 40000F, 0F, 180F), 0.0F, 0.0F);

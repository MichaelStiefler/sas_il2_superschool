...
//            CockpitGunner, A_6, Cockpit, 
//            TypeLaserDesignator

public class CockpitA_6FLIR extends CockpitGunner
{
    ...
    {
        super.doFocusLeave();
        ((A_6)aircraft()).FLIR = false;
        ((A_6)aircraft()).laserTimer = Time.current() + 180000L;  // Laser Auto OFF after 180 sec.
    }
    ...
    private void instrument(Orient orient)
    {
        ...
        ((A_6)aircraft()).pos.getAbs(point3d, orient2);
    ...
    public void laserTrack(Orient orient)
    {
        ...
        if(((A_6)aircraft()).hold)
        {
            ...
            laser.sub(point3d);
            double d = ((Tuple3d) laser).y;
            double d1 = ((Tuple3d) laser).x;
            double d2 = ((Tuple3d) laser).z;
            double radius = Math.abs(Math.sqrt(d * d + d1 * d1));
            ...
            if(f3 < 0.0F)
                f3 += 360F;
            float f4 = f3;
            float t = 270.8F - (float)Math.toDegrees(Math.atan(radius / d2));
            float te = 0.0F;
            ...
            float y = 0.0F;
            if(f4 > 90F && f4 <= 180F)
                y = -(float)Math.toDegrees(Math.atan(d / d1)) + orient2.getYaw();
            else
            if(f4 > 180F && f4 <= 270F)
                y = -(float)Math.toDegrees(Math.atan(d / d1)) + te;
            else
            if(f4 > 270F && f4 <= 360F)
                y = -(float)Math.toDegrees(Math.atan(d / d1)) + te;
            else
        ...
        else
        {
            ...
            ((A_6)aircraft()).pos.getRender(Actor._tmpLoc);
    ...
    static
    {
        Property.set(com.maddox.il2.objects.air.CockpitA_6FLIR.class, "weaponControlNum", 10);
        Property.set(com.maddox.il2.objects.air.CockpitA_6FLIR.class, "astatePilotIndx", 0);
    }
}
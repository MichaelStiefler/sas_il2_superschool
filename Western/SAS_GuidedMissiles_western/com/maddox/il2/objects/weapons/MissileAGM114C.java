...
public class MissileAGM114C extends Missile {
	...
	static {
		Class class1 = MissileAGM114C.class;
		Property.set(class1, "mesh", "3do/arms/AGM114/mono.sim");
		...
		Property.set(class1, "force", 8000F); // Rocket Engine Power (in Newton)
		Property.set(class1, "timeSustain", 10F); // Rocket Sustained Motor Burn time in Seconds (0 for single stage rockets)
		Property.set(class1, "forceSustain", 1000F); // Rocket Sustained Power (in Newton)
		...
		Property.set(class1, "dragCoefficient", 0.30F); // Aerodynamic Drag Coefficient
		Property.set(class1, "powerType", 0); // Type of Explosion. 0="Splash", 1="Splinters", 2="Napalm"
		Property.set(class1, "power", 1.8F); // RL Data: 9kg HE warhead, for realism reduced to 1/5th of it's RL weight
		Property.set(class1, "radius", 12.93F); // Radius of Explosion Effect
		Property.set(class1, "kalibr", 0.178F); // Diameter of Missile Body
		Property.set(class1, "massa", 48.5F); // Missile Weight at Launch time
		Property.set(class1, "massaEnd", 36.5F); // Missile Weight when Booster burned out
		...
		Property.set(class1, "attackDecisionByAI", 2); // let AI decide whether or not to attack. Usually "1" for short range missiles and "0" for all others.
		...
		Property.set(class1, "PkDistOpt", 2500.0F); // optimum Distance for Pk calculation
		...
		Property.set(class1, "fxLock", "weapon.AGM114.lock"); // prs file for Lock Tone
		Property.set(class1, "fxLockVolume", 1.0F); // Volume for Lock Tone, "1" is default
		Property.set(class1, "fxNoLock", "weapon.AGM114.nolock"); // prs file for No Lock Tone
		Property.set(class1, "fxNoLockVolume", 1.0F); // Volume for No Lock Tone, "1" is default
		Property.set(class1, "smplLock", "AGM114_lock.wav"); // wav file for Lock Tone
		Property.set(class1, "smplNoLock", "AGM114_no_lock.wav"); // wav file for No Lock Tone
		Property.set(class1, "friendlyName", "AGM-114C"); // Display Name of this missile
		Spawn.add(class1, new SPAWN());
	}

	public MissileAGM114C() {
	}

	public MissileAGM114C(Actor actor, NetChannel netchannel, int i, Point3d point3d, Orient orient, float f) {

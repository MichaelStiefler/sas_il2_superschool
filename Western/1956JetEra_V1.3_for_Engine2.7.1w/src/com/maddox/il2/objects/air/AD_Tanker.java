...
public abstract class AD_Tanker extends Scheme1
    implements TypeTransport, TypeZBReceiver
{
    ...
    public static void moveGear(HierMesh hiermesh, float f, FlightModel flightmodel)
    {
        float f1 = 10F * f;
        if(flightmodel != null)
            f = 10F * Math.max(f, ((FlightModelMain) (flightmodel)).CT.getAirBrake());
        else
            f = f1;
    ...
    {
        resetYPRmodifier();
        Aircraft.xyz[1] = Aircraft.cvt(((FlightModelMain) (super.FM)).Gears.gWheelSinking[0], 0.0F, 0.231F, 0.0F, 0.231F);
        hierMesh().chunkSetLocate("GearL7_D0", Aircraft.xyz, Aircraft.ypr);
        Aircraft.xyz[1] = Aircraft.cvt(((FlightModelMain) (super.FM)).Gears.gWheelSinking[1], 0.0F, 0.231F, 0.0F, -0.231F);
    ...
    public void update(float f)
    {
        ...
        if(bNeedSetup)
        {
            cGear = ((FlightModelMain) (super.FM)).CT.GearControl;
            bNeedSetup = false;
        }
        cGear = filter(f, ((FlightModelMain) (super.FM)).CT.GearControl, cGear, 999.9F, ((FlightModelMain) (super.FM)).CT.dvGear);
        if(prevGear2 > cGear)
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("g1"))
                    getEnergyPastArmor(9.96F / (1E-005F + (float)Math.abs(((Tuple3d) (Aircraft.v1)).x)), shot);
                else
                if(s.endsWith("g2"))
                    getEnergyPastArmor(World.Rnd().nextFloat(30F, 50F) / (1E-005F + (float)Math.abs(((Tuple3d) (Aircraft.v1)).x)), shot);
            } else
            ...
            if(s.startsWith("xxeng1"))
            {
                ...
                if(s.endsWith("cyl") && getEnergyPastArmor(0.45F, shot) > 0.0F && World.Rnd().nextFloat() < ((FlightModelMain) (super.FM)).EI.engines[0].getCylindersRatio() * 1.75F)
                {

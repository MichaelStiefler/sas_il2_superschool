//By PAL, implemented into CUP Gear (Last taken from JTW 2015/07/18) !!!!!!!!!!!!!!!!!!
...
//            if((Mission.isCoop() || Mission.isDogfight()) && !Mission.isServer() && Actor.isAlive(FM.brakeShoeLastCarrier) && Time.current() < 60000L)
                                ...
                                if(dCatapultOffsetX[k1] != 0.0D && dCatapultOffsetY[k1] != 0.0D)
                                {
                                    ...
                                    {                                   	
                                        ...
                                        if(bHasBlastDeflector)
                                        {
 			                        	...
 			                        	//By PAL, to indicate if Catapult is set
			                        	bCatapultHooked = true;                                        
                                        break;
                                    }
                                }
                                k1++;
	        ...
	        //By PAL, to indicate if Catapult is set
	       	if(!onGround)
	        	bCatapultHooked = false; 	        	                       
        }
        
	    //By PAL, show Chocks and Catapult gear in currier.
		showChocks(bUnderDeck, (bCatapultHooked || FM.EI.getCatapult()/*|| getCatapultAI()*/) && onGround);
		showCatapult(bUnderDeck, (bCatapultHooked || FM.EI.getCatapult()/*|| getCatapultAI()*/) && onGround);        
...
//		System.out.println("Brakes: " + d6 + ", db1: " + db1 + ", db2: " + db2 + ", Rudder: " + d7);
		...
		// PAS--
		//By PAL, siwtching which wheel
		switch (i)
		{
		case 0: // '\0' // MAIN WHEELS ( LEFT or RIGHT )
		case 1: // '\001'
			double d8 = 1.2D;
			if (i == 0)
				d8 = -1.2D;
		...
		case 2: // '\002' // FRONT or REAR, AUXILIARY WHEEL
			...
			if(bFrontWheel) // FRONT WHEEL STEERING
			{
				// TODO: PAS++
				...
				default:
					...
					Tn.add(tmpV);
					break;
				case 1: // Passive steering, normalized by PAL: limits for Steering Angle have to be set on MoveSteering(f) method in Plane Classes
				case 2:
				case 3:
				case 4:
					// if(msg2)
...
//							steerAngle = 1F;
    ...
    private void drawEffects()
    {
		...
		//By PAL, CatapultSteam introduced originally by PAL
...
// --------------------------------------------------------
	//By PAL, Chocks MOD
	private boolean bCatapultHooked = false;	
	...
	//Orient oCh = new Orient();
	//static private String fName = "Test/LocValues.ini";
	//static private IniFile inifile = new IniFile(fName, 0);
	//	static private float Az, Ta, Kr;	
// --------------------------------------------------------		
...
//		if((isCarrier && isCatapult) == bCatHook)
//			return;
//			
//		bCatHook = (isCarrier && isCatapult);
		...
		System.out.println(bCatHook ? "Catapult Set!" : "Catapult Released...");
...
//	        if(!Actor.isValid(actorhmesh))
//	        {
//	        	System.out.println("Not valid Actor Mesh for Catapult Hook");
//	        	return;
//	        }        	
            ...
            try
            {
		        ...
		        {	        
					...
					locCatH.set(bFrontWheel ? PosCatHx : (PosLx + PosRx) / 2,
								...
								0F, -Pitch, 0F);
					Point3d tempP = new Point3d(bFrontWheel ? -1D : 1D, 0D, 0D);
					Orient tempO = new Orient(0F, Pitch, 0F);
					tempO.transformInv(tempP);
					locCatH.add(tempP);
	            	...
	            	CatH.pos.setBase(FM.actor, hookc, false);//.setAbs(locL);
	            	CatH.pos.setAbs(locCatH);
	            	CatH.pos.changeHookToRel();
				    CatH.pos.resetAsBase();
				    CatH.mesh().setScaleXYZ(1F, 1F, 1.5F);		            		
		        }
            }
	        catch (Exception e){}  	        		                   			       	                	
        }	                   			       	                	
    ...
    private void showChocks(boolean isCarrier, boolean isCatapult)
    {
		...
		//SectFile inifile = new SectFile(fName, 0);			        		        
...
//            pnti[1] = HM.hookFind("_ClipRGear");
//            pnti[2] = HM.hookFind("_ClipCGear");
//            for(int k = 3; k < pnti.length; k++)
//                pnti[k] = HM.hookFind("_Clip" + s(k - 3));
		...
		{				
			...
			if(ChR != null)
			{
				if(!isCarrier)
				{
			        ...
			        //By PAL, for single engines, some degrees in advance	
			        if(FM.EI.engines.length == 1)	        			        
			        	otemp.increment(-30F, 0F, 0F); otemp.wrap360();
			        locR.set(otemp);
...
//			        double PosCatHz = M4.m23;	        		
	            ...
	            try
	            {
			        ...
			        {	        
//					    //By PAL, the wheel could be: hiermesh.chunkFind("GearL4_D0") / "GearL3_D0" / "GearL2_D0" / "GearL1_D0"
//					    System.out.println("Chunk Landing Gear: '" + hookl.chunkName() + "'");
//					    //By PAL, set the one where the Hook is		
//				        hiermesh.setCurChunk(hookl.chunkNum());
				        
				        int il = hiermesh.chunkFind("GearL2_D0");
				        hiermesh.setCurChunk(il);			        
				        hiermesh.getChunkLocObj(locL0);
				                    	
	//		        int il = hiermesh.chunkFind("GearL1_D0");//hookParentChunk("_ClipLGear");//chunkFind("GearL2_D0");//"GearL3_D0");
	//		        if (il >= 0)
	//		        {		        			        	
	//			        hiermesh.setCurChunk(il);		        			        
	//			        hiermesh.getChunkLocObj(locL0);																        
	//			        locL.add(FM.actor.pos.getCurrent()); //FM.actor.pos.getAbs()			        
	//    				Vector3d normal = new Vector3d();			        
	//			        Engine.land().N(locL.getX(), locL.getY(), normal);
	//			        Orient o = new Orient();
	//			        o.orient(normal);
	//			        o.set(FM.actor.pos.getCurrent().getAzimut() + 180F, o.tangage(), o.kren()); //By PAL, added + 180F
	
						Point3f po1 = new Point3f();
						Point3f po2 = new Point3f();					
						hiermesh.getChunkCurVisBoundBox(po1, po2);
						//double zF = locL.getZ() - Math.abs(po1.z - po2.z) / 2D;
			        	double zAx = locL0.getZ();					
			        	//double zGr = PosLz; //Engine.land().HQ(locL.getX(), locL.getY());			    
					    //double z = locL.getZ() - H / 2D; // Engine.land().HQ(llr.getX(), llr.getY());									    				
						//double dZ = (Math.abs(po1.x - po2.x) / 2F) - Math.abs(zAx - zGr); //By PAL, difference between radio and radio (height over ground)
					    ...
					    //locL.set(new Vector3d(locL.getX(), locL.getY(), zGr/* + gWheelSinking[0]*/), o);
					    
	//				    locL.set(xHl, yHl, zHl, 180F, Pitch, 0F); //by PAL, 180 rotados en el Chock izquierdo.
	//				    Vector3d sink = new Vector3d(0D, 0D, gWheelSinking[0]);
	//				    (new Orient(0F, Pitch, 0)).transform(sink); //By PAL, Pitch in position of Kren, to produce projection in y axis (normally Tangage).
	//				    System.out.println("Sink x=" + sink.x + ", y=" + sink.y + ", z=" + sink.z);			    
	//				    locL.add(sink);
	//					locL0.set(locL);  //By PAL, set the Plane relative position for later update.				    
...
//					locL.add(tempP);
		            	...
		            	//Engine.land().HQ(locL0.getX(), locL0.getY();
		            	//Engine.land().h					
						
						//By PAL, set it to ground
						//locL.add(tempP);
						//By PAL, convert in absolute
						locL.add(FM.actor.pos.getCurrent());
						
//			tempLoc.set(Pnt[i].x, Pnt[i].y, Pnt[i].z + 0.05F, 0.0F, 0.0F, 0.0F);
//            MiscEffects.gearMarksSnow(FM, tempLoc, i, pnti[i]);
					    		    				    		    					    				    				    	
		            	ChL = new ActorSimpleMesh("3DO/Arms/Chocks/mono.sim");
		            	ChL.pos.setBase(FM.actor, hookl, false);//.setAbs(locL);
		            	ChL.pos.setAbs(locL);
		            	ChL.pos.changeHookToRel();
					    ChL.pos.resetAsBase();		            	
		            	//ChL.pos.setUpdateEnable(true);
		            	//ChL.setOwner(FM.actor);
		            	//ChL.mesh().setPos(locL);//  "3DO/Arms/Chock/mono.sim", false, locL, 0F, p1, p1 /*+ 2F*/, 0F, 1F, p2);
				...
				try
				{
			        ...
			        {	        
//					    //By PAL, the wheel could be: hiermesh.chunkFind("GearR4_D0") / "GearR3_D0" / "GearR2_D0" / "GearR1_D0"
//					    System.out.println("Chunk Landing Gear: '" + hookr.chunkName() + "'");
//					    //By PAL, set the one where the Hook is		
//				        hiermesh.setCurChunk(hookr.chunkNum());			        
//				        hiermesh.getChunkLocObj(locR0);
				        ...
				        hiermesh.getChunkLocObj(locR0);
				        //locR.set(lr);																		        
	//			        locR.add(FM.actor.pos.getCurrent()); //FM.actor.pos.getAbs()			        		        
	//    				Vector3d normal = new Vector3d();			        
	//			        Engine.land().N(locR.getX(), locR.getY(), normal);
	//			        Orient o = new Orient();
	//			        o.orient(normal);
	//			        o.set(FM.actor.pos.getCurrent().getAzimut(), o.tangage(), o.kren());	
	...
	//				    locR.set(new Vector3d(locR0.getX(), locR0.getY(), zGr/* + gWheelSinking[1]*/), o);	
		            	
		            	//System.out.println("WheelSinking= " + gWheelSinking[1] + ", Delta= " + dZ);
		            	...
		            	ChR = new ActorSimpleMesh("3DO/Arms/Chocks/mono.sim");
        ...
        //ActorCrater("3do/Effects/Explosion/Crater.sim", lr, p1);
        //System.out.println("Snaps to Land: " + ActorSnapToLand.countAll());
        }
    }

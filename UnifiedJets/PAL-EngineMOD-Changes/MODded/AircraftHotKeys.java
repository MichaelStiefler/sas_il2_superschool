...
;
    ...
    //By PAL, why bHasFlapsControlSwitch
			...
			if (!RFM.CT.bHasFlapsControlRed) {
				...
				else
				{
					if (RFM.CT.FlapStage != null && RFM.CT.FlapStageMax != -1.0F) {
						if (RFM.CT.FlapsControl > RFM.CT.FlapStage[flapIndex] + 0.05F) { //By PAL, added 0.05F to avoid imprecissions
							RFM.CT.FlapsControl = RFM.CT.FlapStage[flapIndex];
							//By PAL, use of round instead of floor to avoid rounding errors and precision on 10F and not 100F
							HUD.log("Flaps: " + (Math.round((double)RFM.CT.FlapStage[flapIndex] * RFM.CT.FlapStageMax * 10F) / 10F) + " deg.");
							break;
						}
	...
	public boolean SetVarWingHotKeys (int direction, RealFlightModel RFM)
	{
		...
		case 0:
			if(RFM.CT.VarWingStage != null && RFM.CT.VarWingStageMax != -1.0F){
				if(RFM.CT.VarWingControl > RFM.CT.VarWingStage[varWingIndex] + 0.05F) //By PAL, added 0.05F to avoid imprecissions
				{
					RFM.CT.VarWingControl = RFM.CT.VarWingStage[varWingIndex];
					//By PAL, use of round instead of floor to avoid rounding errors and precision on 10F and not 100F
					HUD.log("Wings: "+(Math.round((double)RFM.CT.VarWingStage[varWingIndex] * RFM.CT.VarWingStageMax * 10F) / 10F)+" deg.");
					break;
				}

 ...
 *
 * <p>
 * Enum names of this enum themselves are represented by <a href="http://en.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a> code (2-letter lower-case alphabets).
 * </p>
 *
...
public enum LanguageCode {
     ...
     * @see <a href="http://i18n.skolelinux.no/localekoder.txt" >Spr&aring;kkoder for POSIX locale i Norge</a>
     * @see <a href="https://bugzilla.redhat.com/show_bug.cgi?id=532487" >Red Hat Bugzilla – Bug 532487 Legacy Norwegian locale (no_NO) should be removed from language selection menu</a>
     */
    no() {
        @Override
        public LanguageAlpha3Code getAlpha3() {
     ...
     *
     * <p>
     * In most cases, this method creates a new {@code Locale} instance every time it is called, but some {@code LanguageCode} instances return their corresponding entries in {@code Locale} class. For example, {@link #ja LanguageCode.ja} always returns
     ...
     *
     * <p>
     * Most languages have just one corresponding ISO 639-2 code, but some languages have two ISO 639-2 codes. They are known as "bibliographic" code (ISO 639-2/B code) and "terminological" code (ISO 639-2/T code). This method returns ISO 639-2/T code. If
     * you want to get ISO 639-2/B code, write like below.
     * </p>
     *
     ...
     *
     * <p>
     * The above code does no harm for most {@code LanguageCode}s that have just one ISO 639-2 code. {@link LanguageAlpha3Code#getAlpha3B()
     * getAlpha3B()} of such {@code LanguageAlpha3Code} instances just return themselves (= <code>this</code> object).
     * </p>
     *
     * <p>
     ...
     *            If {@code false}, this method internally canonicalizes the given code by {@link String#toLowerCase()} and then performs search. For example, {@code getByCode("JA", true)} returns {@code null}, but on the other hand,
     ...
     *
     * <ol>
     * <li>If the given code is {@code null} or an empty string, {@code null} is returned.
     * <li>Otherwise, if the given code matches one of three legacy language code ("iw", "ji" and "in"), its official counterpart ("he", "yi" and "id", respectively) is returned. Note that {@code String.equals(Object)} is used for comparison if
     * {@code  caseSensitive} is {@code true} and that {@code  String.equalsIgnoreCase(String)} is used if {@code caseSensitive} is {@code false}.
     * <li>Otherwise, if {@code caseSensitive} is {@code true}, the given code is returned as is.

...
public class Explosions {
	...
	private static HookNamed getRandomHook(ActorHMesh actorhmesh) {
		...
		do {
			...
			if (j == 0) s = "_Engine1Smoke";
			else if (j == 1) s = "_Engine2Smoke";
			else if (j == 2) s = "_Engine3Smoke";
			else if (j == 3) s = "_Engine4Smoke";
			else if (j == 4) s = "_Tank1Burn";
			else if (j == 5) s = "_Tank2Burn";
			else if (j == 6) s = "_Tank3Burn";
			else if (j == 7) s = "_Tank4Burn";
			int k = actorhmesh.mesh().hookFind(s);
			if (k >= 0) return new HookNamed(actorhmesh, s);
		} while (++i < 6);
		return null;
	}
	...
	public static void runByName(String s, ActorHMesh actorhmesh, String s1, String s2, float f, Actor actor) {
		...
		if (s.equalsIgnoreCase("Tank")) Tank_Explode(actorhmesh.pos.getAbsPoint());
			...
			if (f > 0.0F) new MsgAction(1.2D, new MydataForSmoke(actorhmesh, f)) {
	...
	private static void ExplodeSurfaceWave(int i, float f, float f1) {
		...
		else if (i == 1) new ActorSnapToLand("3do/Effects/Explosion/WaterRing.sim", true, l, 2.0F, f, 1.0F, 0.0F, f1);
	}
		...
		if (i == 0) {
			...
			if (bEnableActorCrater) {
				...
				for (j = 64; j >= 2 && f < (float) j; j /= 2)
					;
				if (j >= 2) new ActorCrater("3do/Effects/Explosion/Crater" + j + "/Live.sim", l, f1);
			}
		}
	}
	...
	private static void fontain(Point3d point3d, float f, float f1, int i, int j, boolean flag, boolean flag1) {
		...
		case 0: // '\0'
			...
			String s1 = "Land";
			if (flag1) s1 = "Land_dig_in";
			if (World.rnd().nextFloat() < 0.5F) s1 = s1 + "_2";
			...
			float f7 = 80F;
			if (flag && !Mission.isCoop() && !Mission.isDogfight()) if (j == 0) f7 *= Main.cur().mission.zutiMisc_BombsCat2_CratersVisibilityMultiplier;
			else if (j == 2) f7 *= Main.cur().mission.zutiMisc_BombsCat3_CratersVisibilityMultiplier;
			else f7 *= Main.cur().mission.zutiMisc_BombsCat1_CratersVisibilityMultiplier;
		...
		case 1: // '\001'
			...
			if (j == 3) ExplodeSurfaceWave(i, 15F, 2.75F);
			else ExplodeSurfaceWave(i, 45F, 5F);
			break;
	...
	public static void AirDrop_Land(Point3d point3d) {
		...
		if (bEnableActorCrater) {
			...
			for (i = 64; i >= 2 && f1 < (float) i; i /= 2)
				;
			if (i >= 2) new ActorCrater("3do/Effects/Explosion/Crater" + i + "/Live.sim", l, 1500F);
		}
	...
	public static void Bullet_Land(Point3d point3d, float f, float f1) {
		...
		if (World.rnd().nextFloat() < 0.2F) Eff3DActor.New(l, 1.0F, "effects/Explodes/Bullet/Land/Flash.eff", f);
	}
	...
	public static void Cannon_Land(Point3d point3d, float f, float f1) {
		...
		if (World.rnd().nextFloat() < 0.3F) Eff3DActor.New(l, 1.0F, "effects/Explodes/Cannon/Land/Flash.eff", f);
	}
	...
	public static void generateSound(Actor actor, Point3d point3d, float f, int i, float f1) {
		if (Config.isUSE_RENDER()) if (actor == null) SfxExplosion.shell(point3d, 1, f, i, f1);
		else if (Engine.land().isWater(point3d.x, point3d.y)) SfxExplosion.shell(point3d, 2, f, i, f1);
		else SfxExplosion.shell(point3d, 0, f, i, f1);
	}
	...
	public static void generate(Actor actor, Point3d point3d, float f, int i, float f1, boolean flag, boolean flag1, int k) {
		...
		if (actor != null) {
			...
			if (actor instanceof ActorLand) {
				if (k > 0) {
					if (k == 1) if (Engine.land().isWater(point3d.x, point3d.y)) bombFatMan_water(point3d, -1F, 1.0F);
					else bombFatMan_land(point3d, -1F, 1.0F);
				} else

				if (f < 15F) {
					if (Engine.land().isWater(point3d.x, point3d.y)) Explode10Kg_Water(point3d, 4F, 1.0F);
					else Explode10Kg_Land(point3d, 4F, 1.0F);
				} else if (f < 50F) {
					if (Engine.land().isWater(point3d.x, point3d.y)) RS82_Water(point3d, 4F, 1.0F);
					else RS82_Land(point3d, 4F, 1.0F, flag, flag1);
				} else if (f < 450F) {
					if (Engine.land().isWater(point3d.x, point3d.y)) BOMB250_Water(point3d, 4F, 1.0F);
					else BOMB250_Land(point3d, 4F, 1.0F, flag, flag1);
				} else if (f < 3000F) {
					if (Engine.land().isWater(point3d.x, point3d.y)) BOMB1000a_Water(point3d, 4F, 1.0F);
					else BOMB1000a_Land(point3d, 4F, 1.0F, flag, flag1);
				} else if (Engine.land().isWater(point3d.x, point3d.y)) bomb1000_water(point3d, -1F, 1.0F);
				else bomb1000_land(point3d, -1F, 1.0F, flag, flag1);
			} else if (f < 50F) {
				if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 5D) if (Engine.land().isWater(point3d.x, point3d.y)) RS82_Water(point3d, 4F, 1.0F);
				else RS82_Land(point3d, 4F, 1.0F, flag, flag1);
				bomb50_land(point3d, -1F, 1.0F);
			} else if (f < 450F) {
				if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 10D) if (Engine.land().isWater(point3d.x, point3d.y)) BOMB250_Water(point3d, 4F, 1.0F);
				else BOMB250_Land(point3d, 4F, 1.0F, flag, flag1);
				bomb50_land(point3d, -1F, 2.0F);
			} else if (f < 3000F) {
				if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 20D) if (Engine.land().isWater(point3d.x, point3d.y)) BOMB1000a_Water(point3d, 4F, 1.0F);
				else BOMB1000a_Land(point3d, 4F, 1.0F, flag, flag1);
				bomb50_land(point3d, -1F, 2.0F);
			} else {
				if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 50D) if (Engine.land().isWater(point3d.x, point3d.y)) bomb1000_water(point3d, -1F, 1.0F);
				else bomb1000_land(point3d, -1F, 1.0F, flag, flag1);
	...
	public static void generateMidAirBombExp(Point3d point3d, float f, int i, float f1, boolean flag, int x) {
		...
		generateSound(actor, point3d, f, i, f1);
		int j = 0;
		if (Engine.land().isWater(point3d.x, point3d.y)) j = 1;
		o.set(0.0F, 90F, 0.0F);
		l.set(point3d, o);
		float f2;
		float f3;
		...
		o.set(0.0F, 0.0F, 0.0F);
		l.set(point3d, o);
		if (Engine.land().HQ_Air(point3d.x, point3d.y) >= point3d.z - (double) f1) ExplodeSurfaceWave(j, f2, f3);
		o.set(0.0F, 90F, 0.0F);
		l.set(point3d, o);
	...
	public static void generateShot(Actor actor, Shot shot1) {
		...
		rel.sub(l);
		if (World.cur().isArcade() && !(actor instanceof Aircraft)) Eff3DActor.New(actor, null, rel, 0.75F, "3DO/Effects/Fireworks/Sprite.eff", 30F);
		if (!(actor instanceof ActorLand)) {
			Eff3DActor.New(actor, null, rel, 1.0F, "3DO/Effects/Fireworks/Debris1A.eff", -1F);
			if (World.rnd().nextFloat() < 0.05F) switch (World.rnd().nextInt(1, 3)) {
			case 1: // '\001'
		...
		case 0: // '\0'
			...
			if (f < 50F) RS82_Land(shot1.p, 4F, 1.0F, false, true);
			else BOMB250_Land(shot1.p, 4F, 1.0F, false, true);
			break;
		...
		case 1: // '\001'
			...
			if (f < 24.2F) RS82_Water(shot1.p, 4F, 1.0F);
			else BOMB250_Water(shot1.p, 4F, 1.0F);
			break;
		...
		case 2: // '\002'
			...
			if (f < 24.2F) RS82_Object(shot1.p, shot1.v, 0.5F, 1.0F);
			else BOMB250_Object(shot1.p, shot1.v, 0.5F, 1.0F);
			break;
		...
		case 3: // '\003'
			...
			if (f < 24.2F) RS82_Object(shot1.p, shot1.v, 1.0F, 1.0F);
			else BOMB250_Object(shot1.p, shot1.v, 1.0F, 1.0F);
			break;
		}
	}
	...
	public static void generateExplosion(Actor actor, Point3d point3d, float f, int i, float f1, double d) {
		...
		if (actor == null) {
			...
			if (f < 0.0019F) {
				if (flag) Bullet_Explode_Water(point3d, 4F, 1.0F);
				else Bullet_Explode_Land(point3d, 4F, 1.0F);
			} else if (f < 0.005F) {
				if (flag) Cannon_Explode_Water_2(point3d, 4F, 1.0F);
				else Cannon_Explode_Land_2(point3d, 4F, 1.0F);
			} else if (f < 0.05F) {
				if (flag) Cannon_Explode_Water(point3d, 4F, 1.0F);
				else Cannon_Explode_Land(point3d, 4F, 1.0F);
			} else if (f < 1.0F) {
				if (flag) CannonBigExp_Water(point3d, 4F, 1.0F);
				else CannonBigExp_Land(point3d, 4F, 1.0F);
			} else if (f < 15F) {
				if (flag) Explode10Kg_Water(point3d, 4F, 1.0F);
				else Explode10Kg_Land(point3d, 4F, 1.0F);
			} else if (f < 50F) {
				if (flag) RS82_Water(point3d, 4F, 1.0F);
				else RS82_Land(point3d, 4F, 1.0F, false, true);
			} else if (flag) BOMB250_Water(point3d, 4F, 1.0F);
			else BOMB250_Land(point3d, 4F, 1.0F, false, true);
		} else if (f < 0.002F) {
			...
			if (World.rnd().nextFloat() < 0.1F) Eff3DActor.New(actor, null, rel, 1.0F, "3DO/Effects/Fireworks/20_DebrisBigA.eff", -1F);

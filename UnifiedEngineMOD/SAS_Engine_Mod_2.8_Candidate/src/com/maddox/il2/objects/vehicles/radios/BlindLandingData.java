...
//		bSingleDirection = flag1;
		...
		if (Math.abs(blindLandingAzimuthBP) < 10F && f2 > (f6 - markerFanLength) + runwayLength && f2 < f6 + markerFanLength + runwayLength) isOnInner2Marker = true;
		else isOnInner2Marker = false;
		if (Math.abs(blindLandingAzimuthBP) < 10F && f2 > (f5 - markerFanLength) + runwayLength && f2 < f5 + markerFanLength + runwayLength) isOnInnerMarker = true;
		else isOnInnerMarker = false;
		if (Math.abs(blindLandingAzimuthBP) < 10F && f2 > (f4 - markerFanLength) + runwayLength && f2 < f4 + markerFanLength + runwayLength) isOnOuterMarker = true;
		else isOnOuterMarker = false;
	}

...
public class FlightModelMain extends FMMath {
	...
	private static ArrayList diffFmLogHashes = null;
	...
	public void load(String s) {
		...
		if (CT.nFlapStages == -1) CT.nFlapStages = j - 1;
		float f = sectfile.get(s2, "CFlapStageMax", -1F);
		if (f > 0F) CT.FlapStageMax = f;
		if (CT.nFlapStages != -1 && CT.FlapStageMax != 1) {
			if (CT.FlapStage == null && CT.nFlapStages != -1) CT.FlapStage = new float[CT.nFlapStages];
			if (CT.FlapStage.length > 0) for (int k = 0; k < CT.FlapStage.length; k++) {
		...
		if (CT.nVarWingStages != -1 && CT.VarWingStageMax != 1) {
			if (CT.VarWingStage == null && CT.nVarWingStages != -1) CT.VarWingStage = new float[CT.nVarWingStages];
			if (CT.VarWingStage.length > 0) for (int k = 0; k < CT.VarWingStage.length; k++) {
	...
	public void update(float f) {
		...
		if (CT.bHasBlownFlaps) {
			if (CT.BlownFlapsType != null) if (CT.BlownFlapsControl > 0.0F && CT.FlapsControl > 0.0F && EI.getThrustOutput() > 0.2F) {
	...
	private static void writeDiffFmToLog(String logLine) {
		if (diffFmLogHashes == null) diffFmLogHashes = new ArrayList();
		Integer diffFmLogHash = new Integer(logLine.hashCode());;
		if (diffFmLogHashes.contains(diffFmLogHash)) return;
		diffFmLogHashes.add(diffFmLogHash);
	...
	public static SectFile sectFile(String s) {
		...
		if (i > -1) // It is not valid in first position either
		{
			...
			writeDiffFmToLog("FM called '" + s + "' is being loaded from File: '" + s2 + "'");
		}
		...
		if (prButtons != null && prButtons.length() > 0) {
			...
			else s3 = prButtons + s2;
			// By PAL, if it exists, then replace original
			if (exisstFile(s3)) {
				s2 = s3;
				// System.out.println("s2 = " + s2 + ", s3 = " + s3);
				writeDiffFmToLog("FM called '" + s + "' is being loaded from Alternative File: '" + s2 + "'");
			}
		}
		...
		try {
			...
			if (obj != null) {
				...
				if (fmFile == null) {
				...
				} else if (!s2.equalsIgnoreCase(fmLastFName)) {
					fmFile = null;
					...
					for (int j = 0; j < fmFNames.size(); j++) {
						...
						if (s2.equalsIgnoreCase(s3)) {
							...
							if (bPrintFM) System.out.println("Getting FM from Previous File: '" + s2 + "'");
							fmLastFName = s2;
							break;
						}
					}
					if (fmFile == null) // By PAL, it wasn't loaded before
					{
						if (exisstFile(s2)) {
							...
							fmFiles.add(fmFile); // BY PAL, from DiffFM
							fmFNames.add(s2); // BY PAL, from DiffFM
							if (bPrintFM) System.out.println("Opening FM from New File: '" + s2 + "'");
							fmLastFName = s2;
						} else {
				...
				if (fmFile == null) {
					...
					if (s1.endsWith(".emd")) return sectFile("FlightModels/DB-600_Series.emd"); // By PAL; open PlaceHolder DM to avoid 60% CTD!!!
				...
				if (fmFile != null) {
					...
					if (inputstream == null) inputstream = fmFile.openStream("" + Finger.Int(s1 + "d2wO"));
					// By PAL, if it didn't work with 4.101 neither with 4.111 then HSFX6 Expert Encoding:
					if (inputstream == null) inputstream = fmFile.openStream("" + Finger.Int(s1 + "d2w5"));
				}

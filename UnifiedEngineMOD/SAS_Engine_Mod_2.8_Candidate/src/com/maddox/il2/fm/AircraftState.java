...
public class AircraftState {
	...
	private void doSetOilState(int i, int j) {
		...
		case 1: // '\001'
			...
			if (World.Rnd().nextFloat() < 0.25F) aircraft.FM.setReadyToReturn(true);
			break;
		}
	}
	...
	private void doHitEngine(Actor actor1, int i) {
		...
		if (astateEngineStates[i] == 3) {
			...
			if (actor instanceof Scheme1) {
				...
				aircraft.FM.setTakenMortalDamage(true, actor1);
				aircraft.FM.setCapableOfACM(false);
			} else if (World.Rnd().nextFloat(0.0F, 100F - f) < 50F - (float) aircraft.FM.subSkill * 3F) {
				aircraft.FM.setReadyToDie(true);
				aircraft.FM.setTakenMortalDamage(true, actor1);
				aircraft.FM.setCapableOfACM(false);
			...
			if (actor instanceof TypeFuelDump) {
				for (int j = 0; j < 2; j++) {
					...
					case 0:
						...
						if (astateDumpFuelEffects[0] != null) break;
					case 1:
	...
	public void update(float f) {
		...
		if (bIsMaster) {
			...
			for (int j1 = 0; j1 < 4; j1++)
				...
				case 4: // '\004'
					if (World.Rnd().nextFloat(0.0F, 1.0F) < 0.06F) hitTank(aircraft, j1, 0);
			...
			for (int k1 = 0; k1 < aircraft.FM.EI.getNum(); k1++) {
				...
				case 3: // '\003'
					if (l1 == 6 && World.Rnd().nextFloat() < Aircraft.cvt(aircraft.FM.EI.engines[k1].getEngineLoad(), 1.0F, 3F, 0.015F, 0.0F)) {
						...
						if (World.Rnd().nextFloat() < 0.2F) aircraft.FM.EI.engines[k1].setEngineDies(actor);
					} else if (l1 != 6 && (double) World.Rnd().nextFloat() < 0.0074999999999999997D) repairEngine(k1);
					break;
				...
				case 4: // '\004'
					...
					if (aircraft.FM.EI.engines[k1].getType() == 1 || aircraft.FM.EI.engines[k1].getType() == 0) {
						if (l1 == 6) {
							...
							if (World.Rnd().nextFloat() < f3) if (actor instanceof Scheme1) {
							...
							if (World.Rnd().nextFloat() < 0.3F) aircraft.FM.EI.engines[k1].setEngineDies(actor);
						} else if (l1 != 6 && World.Rnd().nextFloat() < 0.15F) {
							repairEngine(k1);
							Aircraft.debugprintln(aircraft, "Engine " + k1 + " cuts fire..");
						}
					} else if ((actor instanceof Scheme1) && World.Rnd().nextFloat() < 0.05F) {
						Aircraft.debugprintln(aircraft, "Engine 0 detonates and explodes, fatal damage level forced..");
						aircraft.msgCollision(actor, "CF_D0", "CF_D0");
					}
					break;
			...
			if (!((actor instanceof TypeFighter) && aircraft.FM.CT.bHasBayDoorControl)) {
				...
				bailout();
				if (World.Rnd().nextFloat() < 0.25F && !aircraft.FM.CT.saveWeaponControl[3]) if (aircraft.FM.isPlayers() && (aircraft.FM instanceof RealFlightModel) && ((RealFlightModel) aircraft.FM).isRealMode()) {
				...
				bailout();
			} else if (!aircraft.FM.isPlayers() && World.Rnd().nextFloat() < 0.125F && !aircraft.FM.CT.saveWeaponControl[3] && (!(actor instanceof TypeBomber) || aircraft.FM.AP.way.curr().Action != 3)) aircraft.FM.CT.BayDoorControl = 0.0F;
		}
	}

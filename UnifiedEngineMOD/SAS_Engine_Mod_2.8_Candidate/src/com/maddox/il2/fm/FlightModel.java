...
public class FlightModel extends FlightModelMain {
		...
		case 0: // '\0'
			...
			if (Time.current() > turret1.timeNext) {
				...
				if (turret1.target == null) {
					...
					else if (VisCheck.visCheckTurret(turret1, (Aircraft) this.actor, (Aircraft) turret1.target, true)) {
						turret1.tMode = 1;
						turret1.timeNext = 0L;
						if ((Aircraft) this.actor == World.getPlayerAircraft() && turret1.health > 0.4F) Voice.speakRearGunEA((Aircraft) this.actor, (Aircraft) turret1.target);
					} else {
	...
	public void update(float f) {
		if (actor.isNetMirror()) ((com.maddox.il2.objects.air.NetAircraft.Mirror) actor.net).fmUpdate(f);
		else FMupdate(f);
	}
	...
	private boolean isComingFromTheSun(com.maddox.il2.engine.Actor actor) {
		if ((actor instanceof com.maddox.il2.objects.air.Aircraft) && com.maddox.il2.ai.World.Sun().ToSun.z > 0.0F) {
			if (com.maddox.il2.game.Mission.curCloudsType() > 3 && actor.pos.getAbs().getZ() < (double) (com.maddox.il2.game.Mission.curCloudsHeight() + 200F)) return false;

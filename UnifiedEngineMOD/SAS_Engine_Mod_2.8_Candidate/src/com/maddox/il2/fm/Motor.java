...
public class Motor extends FMMath {
	...
	// Ensure not to cause exzessive startup smoke
	private long lastStartupSmokeEffectsGenerated;
	private static final long TIME_BETWEEN_STARTUP_SMOKE_EFFECTS = 3000L;
	...
	public Motor() {
		...
		starter = 0;
		this.lastStartupSmokeEffectsGenerated = 0L;
	}
	...
	public void load(FlightModel flightmodel, String s, String s1, int i) {
		...
		if (type == 0 || type == 1 || type == 7 || type == 9) initializeInline(((FlightModelMain) (flightmodel)).Vmax);
		if (type == 2) initializeJet(((FlightModelMain) (flightmodel)).Vmax);
		if (type == 10)
	...
	private void resolveFromFile(SectFile sectfile, String s) {
		...
		if (s1 != null) if (s1.endsWith("Inline")) type = 0;
		else if (s1.endsWith("Radial")) type = 1;
		else if (s1.endsWith("Jet")) type = 2;
		else if (s1.endsWith("RocketBoost")) type = 4;
		else if (s1.endsWith("Rocket")) type = 3;
		else if (s1.endsWith("Tow")) type = 5;
		else if (s1.endsWith("PVRD")) type = 6;
		else if (s1.endsWith("Unknown")) type = 8;
		else if (s1.endsWith("Azure")) type = 8;
		else if (s1.endsWith("HeloI")) type = 7;
		else if (s1.endsWith("Rotary")) type = 9;
		else if (s1.endsWith("Turboprop")) type = 10;
		...
		if (s1 != null) if (s1.endsWith("Left")) propDirection = 0;
		else if (s1.endsWith("Right")) propDirection = 1;
		...
		if (f != -99999F) propReductor = f;
		// TODO: New engine types
		// --------------------------------------------------------
		if (type == 0 || type == 1 || type == 7 || type == 9) {
			f = sectfile.get(s, "HorsePowers", -99999F);
			if (f != -99999F) horsePowers = f;
			int j = sectfile.get(s, "Carburetor", 0xfffe7961);
			if (j != 0xfffe7961) engineCarburetorType = j;
			f = sectfile.get(s, "Mass", -99999F);
			if (f != -99999F) engineMass = f;
			else engineMass = horsePowers * 0.6F;
		} else {
			f = sectfile.get(s, "Thrust", -99999F);
			if (f != -99999F) thrustMax = f * 9.81F;
		}
		// --------------------------------------------------------
		...
		if (f != -99999F) engineBoostFactor = f;
		f = sectfile.get(s, "WEPBoostFactor", -99999F);
		if (f != -99999F) engineAfterburnerBoostFactor = f;
		if (type == 2) {
		...
		if (f != -99999F) FuelConsumptionP0 = f;
		f = sectfile.get(s, "FuelConsumptionP05", -99999F);
		if (f != -99999F) FuelConsumptionP05 = f;
		f = sectfile.get(s, "FuelConsumptionP1", -99999F);
		if (f != -99999F) FuelConsumptionP1 = f;
		f = sectfile.get(s, "FuelConsumptionPMAX", -99999F);
		if (f != -99999F) FuelConsumptionPMAX = f;
		...
		if (k != 0xfffe7961) if (k == 0) bIsAutonomous = false;
		else if (k == 1) bIsAutonomous = true;
		k = sectfile.get(s, "cThrottle", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasThrottleControl = false;
		else if (k == 1) bHasThrottleControl = true;
		k = sectfile.get(s, "cAfterburner", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasAfterburnerControl = false;
		else if (k == 1) bHasAfterburnerControl = true;
		k = sectfile.get(s, "cProp", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasPropControl = false;
		else if (k == 1) bHasPropControl = true;
		k = sectfile.get(s, "cMix", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasMixControl = false;
		else if (k == 1) bHasMixControl = true;
		k = sectfile.get(s, "cMagneto", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasMagnetoControl = false;
		else if (k == 1) bHasMagnetoControl = true;
		k = sectfile.get(s, "cCompressor", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasCompressorControl = false;
		else if (k == 1) bHasCompressorControl = true;
		k = sectfile.get(s, "cFeather", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasFeatherControl = false;
		else if (k == 1) bHasFeatherControl = true;
		k = sectfile.get(s, "cRadiator", 0xfffe7961);
		if (k != 0xfffe7961) if (k == 0) bHasRadiatorControl = false;
		else if (k == 1) bHasRadiatorControl = true;
		k = sectfile.get(s, "Extinguishers", 0xfffe7961);
		if (k != 0xfffe7961) {
			extinguishers = k;
			if (k != 0) bHasExtinguisherControl = true;
			else bHasExtinguisherControl = false;
		}
		f = sectfile.get(s, "PropDiameter", -99999F);
		if (f != -99999F) propDiameter = f;
		propr = 0.5F * propDiameter * 0.75F;
		f = sectfile.get(s, "PropMass", -99999F);
		if (f != -99999F) propMass = f;
		...
		if (f != -99999F) propAngleChangeSpeed = f;
		f = sectfile.get(s, "PropAnglerMinParam", -99999F);
		if (f != -99999F) {
			propAngleDeviceMinParam = f;
			if (propAngleDeviceType == 6 || propAngleDeviceType == 5) propAngleDeviceMinParam = (float) Math.toRadians(propAngleDeviceMinParam);
			if (propAngleDeviceType == 1 || propAngleDeviceType == 2 || propAngleDeviceType == 7 || propAngleDeviceType == 8 || propAngleDeviceType == 9) propAngleDeviceMinParam = toRadianPerSecond(propAngleDeviceMinParam);
		}
		f = sectfile.get(s, "PropAnglerMaxParam", -99999F);
		if (f != -99999F) {
			propAngleDeviceMaxParam = f;
			if (propAngleDeviceType == 6 || propAngleDeviceType == 5) propAngleDeviceMaxParam = (float) Math.toRadians(propAngleDeviceMaxParam);
			if (propAngleDeviceType == 1 || propAngleDeviceType == 2 || propAngleDeviceType == 7 || propAngleDeviceType == 8 || propAngleDeviceType == 9) propAngleDeviceMaxParam = toRadianPerSecond(propAngleDeviceMaxParam);
			if (propAngleDeviceAfterburnerParam == -999.9F) propAngleDeviceAfterburnerParam = propAngleDeviceMaxParam;
		}
		...
		if (f != -99999F) {
			...
			if (wWEP != wMax) afterburnerChangeW = true;
			if (propAngleDeviceType == 6 || propAngleDeviceType == 5) propAngleDeviceAfterburnerParam = (float) Math.toRadians(propAngleDeviceAfterburnerParam);
			if (propAngleDeviceType == 1 || propAngleDeviceType == 2 || propAngleDeviceType == 7 || propAngleDeviceType == 8 || propAngleDeviceType == 9) propAngleDeviceAfterburnerParam = toRadianPerSecond(propAngleDeviceAfterburnerParam);
		} else {
			wWEP = wMax;
		}
		...
		if (f != -99999F) {
			...
			if (propPhi < propPhiMin) propPhi = propPhiMin;
			if (propTarget < propPhiMin) propTarget = propPhiMin;
		}
		f = sectfile.get(s, "PropPhiMax", -99999F);
		if (f != -99999F) {
			propPhiMax = (float) Math.toRadians(f);
			if (propPhi > propPhiMax) propPhi = propPhiMax;
			if (propTarget > propPhiMax) propTarget = propPhiMax;
		}
		f = sectfile.get(s, "PropAoA0", -99999F);
		if (f != -99999F) propAoA0 = (float) Math.toRadians(f);
		k = sectfile.get(s, "CompressorType", 0xfffe7961);
		if (k != 0xfffe7961) compressorType = k;
		f = sectfile.get(s, "CompressorPMax", -99999F);
		if (f != -99999F) compressorPMax = f;
		k = sectfile.get(s, "CompressorSteps", 0xfffe7961);
		if (k != 0xfffe7961) {
			compressorMaxStep = k - 1;
			if (compressorMaxStep < 0) compressorMaxStep = 0;
		}
		if (compressorAltitudes != null) if (compressorAltitudes.length == compressorMaxStep + 1)
		;
		...
		if (compressorAltitudes.length > 0) {
			for (int l = 0; l < compressorAltitudes.length; l++) {
				...
				if (f != -99999F) compressorAltMultipliers[l] = f;
				f = sectfile.get(s, "CompressorBaseMultiplier" + l, -99999F);
				if (f != -99999F) compressorBaseMultipliers[l] = f;
				else compressorBaseMultipliers[l] = 0.8F;
			}
		...
		if (f != -99999F) compressorRPMtoCurvature = f;
		...
		if (f != -99999F) compressorSpeedManifold = f;
		f = sectfile.get(s, "CompressorPAt0", -99999F);
		if (f != -99999F) compressorPAt0 = f;
		f = sectfile.get(s, "Voptimal", -99999F);
		if (f != -99999F) Vopt = f * 0.277778F;
		boolean flag = true;
		float f1 = 2000F;
		float f2 = 1.0F;
		int i1 = 0;
		do {
			...
			if (flag) insetrPoiInCompressorPoly(f1, f2);
			i1++;
			if (nOfCompPoints > 15 || i1 > 15) flag = false;
		} while (true);
		k = sectfile.get(s, "AfterburnerType", 0xfffe7961);
		if (k != 0xfffe7961) afterburnerType = k;
		k = sectfile.get(s, "MixerType", 0xfffe7961);
		if (k != 0xfffe7961) mixerType = k;
		f = sectfile.get(s, "MixerAltitude", -99999F);
		if (f != -99999F) mixerLowPressureBar = Atmosphere.pressure(f) / Atmosphere.P0();
		f = sectfile.get(s, "EngineI", -99999F);
		if (f != -99999F) engineI = f;
		f = sectfile.get(s, "EngineAcceleration", -99999F);
		if (f != -99999F) engineAcceleration = f;
	...
	private void initializeJet(float f) {
		if (type == 10) propSEquivalent = 0.26F * propr * propr;
		else propSEquivalent = ((float) (cylinders * cylinders) * (2.0F * thrustMax)) / (getFanCy(propAoA0) * Atmosphere.ro0() * wMax * wMax * propr * propr);
	...
	private void insetrPoiInCompressorPoly(float f, float f1) {
		int i = 0;
		do {
			if (i >= nOfCompPoints) break;
			if (compressorRPM[i] >= f) {
				if (compressorRPM[i] == f) return;
				break;
			}
			i++;
		} while (true);
	...
	private void calcAfterburnerCompressorFactor() {
		if (afterburnerType == 1 || afterburnerType == 7 || afterburnerType == 8 || afterburnerType == 10 || afterburnerType == 11 || afterburnerType == 6 || afterburnerType == 5 || afterburnerType == 9 || afterburnerType == 4) {
			...
			if ((double) f3 < 0.10000000000000001D) f2 = Atmosphere.pressure((float) reference.Loc.z) * _1_P0;
	...
	public float getATA(float f) {
		...
		int j = 1;
		float f1 = 1.0F;
		if (nOfCompPoints < 2) return 1.0F;
		if ((double) f < 0.10000000000000001D) f1 = Atmosphere.pressure((float) reference.Loc.z) * _1_P0;
	...
	public void update(float f) {
		...
		if (controlThrottle > 1.0F && engineBoostFactor == 1.0F) {
			...
			if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()) HUD.log(AircraftHotKeys.hudLogPowerId, "Power", new Object[] { new Integer(100) });
		}
		computeForces(f);
		computeStage(f);
		if (stage > 0 && stage < 6) engineForce.set(0.0F, 0.0F, 0.0F);
		else if (stage == 8) rpm = w = 0.0F;
		if (reference.isPlayers()) {
			if (bIsMaster && (reference instanceof RealFlightModel)) {
				computeTemperature(f);
				if (World.cur().diffCur.Reliability) computeReliability(f);
			}
			if (World.cur().diffCur.Limited_Fuel) computeFuel(f);
		} else {
			computeFuel(f);
			if (bIsMaster && reference.isTick(32, 0)) computeTemperature(f * 32F);
		}
		old_engineForce.set(engineForce);
		old_engineTorque.set(engineTorque);
		updateLast = 0.0F;
		float f1 = 0.5F / (Math.abs(aw) + 1.0F) - 0.1F;
		if (f1 < 0.025F) f1 = 0.025F;
		if (f1 > 0.4F) f1 = 0.4F;
		if (f1 < updateStep) updateStep = 0.9F * updateStep + 0.1F * f1;
		else updateStep = 0.99F * updateStep + 0.01F * f1;
	}

	public void netupdate(float f, boolean flag) {
		computeStage(f);
		if ((double) Math.abs(w) < 1.0000000000000001E-005D) propPhiW = 1.570796F;
		else propPhiW = (float) Math.atan(reference.Vflow.x / (double) (w * propReductor * propr));
		...
		computeForces(f);
		if (flag) compressorManifoldPressure = f3;
		w = f1;
		propPhi = f2;
		rpm = toRPM(w);
	}
	...
	public void setEngineStarts(Actor actor) {
		if (!bIsMaster || !Actor.isAlive(actor)) return;
	...
	public void doSetEngineStarts() {
		...
		if (stage == 0) {
			...
			} else if ((type == 0 || type == 1 || type == 7) && bRan) {
				if (w > 20) {
					stage = 5;
				} else if (w > 150) {
					stage = 6;
				}
			} else ((FlightModelMain) (reference)).CT.setMagnetoControl(3);
			setControlMagneto(3);
			stage = 1;
			timer = Time.current();
		}
	...
	public void doSetEngineDies() {
		if (stage < 7) {
			...
			if (i != 0) {
				...
				if (f < 0.7F) reference.setReadyToReturn(true);
				if (f < 0.3F) reference.setReadyToDie(true);
			}
			stage = 7;
			if (reference.isPlayers()) HUD.log("FailedEngine");
			timer = Time.current();
		}
	}
	...
	public void doSetEngineRunning() {
		...
		reference.CT.setMagnetoControl(3);
		setControlMagneto(3);
		if (reference.isPlayers()) HUD.log("EngineI1");
		w = wMax * 0.75F;
		...
		propTarget = propPhi;
		if (isnd != null) isnd.onEngineState(stage);
	}
	...
	public void doSetCyliderKnockOut(int i) {
		cylindersOperable -= i;
		if (cylindersOperable < 0) cylindersOperable = 0;
		if (bIsMaster) if (getCylindersRatio() < 0.12F) setEngineDies(reference.actor);
		else if (getCylindersRatio() < getReadyness()) setReadyness(reference.actor, getCylindersRatio());
	}
	...
	public void doSetMagnetoKnockOut(int i) {
		bMagnetos[i] = false;
		if (i == controlMagneto) setEngineStops(reference.actor);
	}
	...
	public void doSetEngineStuck() {
		...
		if (stage != 8) {
			setReadyness(0.0F);
			if (reference.isPlayers() && stage != 7) HUD.log("FailedEngine");
			stage = 8;
			timer = Time.current();
		}
	}
	...
	public void setControlAfterburner(boolean flag) {
		if (bHasAfterburnerControl) {
			if (afterburnerType == 1 && !controlAfterburner && flag && controlThrottle > 1.0F && World.Rnd().nextFloat() < 0.5F && reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()
					&& World.cur().diffCur.Vulnerability) setCyliderKnockOut(reference.actor, World.Rnd().nextInt(0, 3));
			controlAfterburner = flag;
		}
		if (afterburnerType == 4 || afterburnerType == 8 || afterburnerType == 10) controlAfterburner = flag;
	}
	...
	public void setControlProp(float f) {
		if (bHasPropControl) controlProp = f;
	}

	public void setControlPropAuto(boolean flag) {
		if (bHasPropControl) bControlPropAuto = flag && isAllowsAutoProp();
	}
	...
	public void setControlMix(float f) {
		if (bHasMixControl) switch (mixerType) {
		case 0: // '\0'
			controlMix = f;
			break;
		...
		case 1: // '\001'
			controlMix = f;
			if (controlMix < 1.0F) controlMix = 1.0F;
			break;
	...
	public void setControlMagneto(int i) {
		if (bHasMagnetoControl) {
			controlMagneto = i;
			if (i == 0) setEngineStops(reference.actor);
		}
	}

	public void setControlCompressor(int i) {
		if (bHasCompressorControl) controlCompressor = i;
	}

	public void setControlFeather(int i) {
		if (bHasFeatherControl) {
			controlFeather = i;
			if (reference.isPlayers()) HUD.log("EngineFeather" + controlFeather);
		}
	}

	public void setControlRadiator(float f) {
		if (bHasRadiatorControl) controlRadiator = f;
	}
	...
	public void setExtinguisherFire() {
		...
		if (bHasExtinguisherControl) {
			...
			if (reference.AS.astateEngineStates[number] > 2) reference.AS.setEngineState(reference.actor, number, World.Rnd().nextInt(1, 2));
			else if (reference.AS.astateEngineStates[number] > 0) reference.AS.setEngineState(reference.actor, number, 0);
		}
	}

	public void doSetExtinguisherFire() {
		if (!bHasExtinguisherControl) return;
		if (reference.AS.bIsAboutToBailout) return;
		extinguishers--;
		if (extinguishers == 0) bHasExtinguisherControl = false;
		reference.AS.doSetEngineExtinguisherVisuals(number);
		if (bIsMaster) {
			if (reference.AS.astateEngineStates[number] > 1 && World.Rnd().nextFloat() < 0.56F) reference.AS.repairEngine(number);
		...
		if (reference.isPlayers()) HUD.log("ExtinguishersFired");
	}
	...
	private void computeStage(float f) {
		...
		if (stage > 0 && stage < 6 && l > given) {
			...
			if ((type == 0 || type == 7) && starter != 1) {
				...
				if (starter == 2 && stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 2F) {
					stage = 3;
				}
				if ((starter == 0 || starter == 3 || starter == 5) && stage > 3) {
					if (World.Rnd().nextFloat(0.0F, 10.0F) < 0.25F && reference.isStationedOnGround()) stage = 0;
					else if (World.Rnd().nextFloat(0.0F, 10.0F) < 3F) stage = 3;
				}
			}
			// Radial engines
			if (type == 1 && starter != 1) {
				if (starter == 2) {
					if (stage == 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 2F) stage = 2;
					else if (stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 3F) stage = 3;
				}
				if (starter == 0 || starter == 3 || starter == 5) {
					if (stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 0.25F && reference.isStationedOnGround()) stage = 0;
					else if (stage == 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 1F) stage = 2;
					else if (stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 3F) stage = 3;
				}
			}
			// Rotary and hand-cranked engines
			if ((starter == 1) || (type == 9)) {
				if (stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 2F && reference.isStationedOnGround()) stage = 0;
				else if (stage == 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 2F) stage = 2;
				else if (stage > 3 && World.Rnd().nextFloat(0.0F, 10.0F) < 3F) stage = 3;
			}
		...
		if (stage > 0 && stage < 6) setControlThrottle(0.2F);
		switch (stage) {
		case 0: // '\0'
			if (bTFirst) {
				given = 0x3fffffffffffffffL;
		...
		case 1: // '\001'
			if (bTFirst) {
				...
				if (type == 3 || type == 4 || type == 6) {
					stage = 5;
					if (reference.isPlayers()) HUD.log("Starting_Engine");
					return;
				}
				// TODO:
				if (type == 0 || type == 1 || type == 7 || type == 9) {
					...
					if (!bIsAutonomous) {
						...
						if (reference.isStationedOnGround()) {
							...
							doSetEngineStops();
							if (reference.isPlayers()) HUD.log("EngineI0");
							return;
						}
					} else if (reference.isPlayers()) HUD.log("Starting_Engine");
				...
				} else given = (long) (1000F * World.Rnd().nextFloat(1.0F, 2.0F));
			}
			if (isnd != null) isnd.onEngineState(stage);
			reference.CT.setMagnetoControl(3);
			setControlMagneto(3);
			if (starter == 5) w = 0;
			else w = (0.1047F * ((20F * (float) l) / (float) given));
		...
		case 2: // '\002'
			if (bTFirst) {
				...
				} else given = (long) (8000F * World.Rnd().nextFloat(1.0F, 2.0F));
				if (bRan) {
					given = (long) (100F + ((tOilOutMaxRPM - tOilOut) / (tOilOutMaxRPM - f1)) * 7900F * World.Rnd().nextFloat(2.0F, 4.2F));
					if (given > 9000L) given = World.Rnd().nextLong(7800L, 9600L);
		...
		case 3: // '\003'
			if (bTFirst) {
				...
				} else given = (long) (100F * World.Rnd().nextFloat(1.0F, 2.0F));
				if (bIsMaster && World.Rnd().nextFloat() < 0.12F && (tOilOutMaxRPM - tOilOut) / (tOilOutMaxRPM - f1) < 0.75F) reference.AS.setEngineStops(number);
			}
			w = 0.1047F * (60F + (60F * (float) l) / (float) given);
			setControlThrottle(0.0F);
			if (reference == null || type == 2 || type == 3 || type == 4 || type == 6 || type == 5 || type == 10) break;
			if (Time.current() - this.lastStartupSmokeEffectsGenerated < TIME_BETWEEN_STARTUP_SMOKE_EFFECTS) break;
			this.lastStartupSmokeEffectsGenerated = Time.current();
			for (int i = 1; i < 32; i++) {
				try {
					Hook hook = reference.actor.findHook("_Engine" + (number + 1) + "EF_" + (i >= 10 ? "" + i : "0" + i));
					if (hook != null) Eff3DActor.New(reference.actor, hook, null, 1.0F, "3DO/Effects/Aircraft/EngineStart" + World.Rnd().nextInt(1, 3) + ".eff", -1F);
				} catch (Exception exception) {
				}
			}
			// TODO: Engine Mod 2.7 Bugfix, "break" was missing so there's been a fallthrough and self-callback from case 4
			break;
			// ---

		case 4: // '\004'
			if (bTFirst) if (bTFirst) if (type == 9) {
				given = (long) (25F * World.Rnd().nextFloat(1.0F, 2.0F));
			} else if (type == 0 || type == 7) {
				if (starter == 0) {
					given = (long) (100F * World.Rnd().nextFloat(1.0F, 2.0F));
		...
		case 5: // '\005'
			if (bTFirst) {
				...
				} else given = (long) (500F * World.Rnd().nextFloat(1.0F, 2.0F));
				if (bRan && (type == 0 || type == 1 || type == 7 || type == 9)) {
					if ((tOilOutMaxRPM - tOilOut) / (tOilOutMaxRPM - f1) > 0.75F) if (type == 0 || type == 7) {
						if (bIsMaster && getReadyness() > 0.75F && World.Rnd().nextFloat() < 0.25F) setReadyness(getReadyness() - 0.05F);
					} else
					// TODO: Stops random engine deaths on startup
					// if(type == 1 && bIsMaster && World.Rnd().nextFloat() < 0.1F)
					// reference.AS.setEngineDies(reference.actor, number);
						if (bIsMaster && World.Rnd().nextFloat() < 0.1F) reference.AS.setEngineStops(number);
				}
				bRan = true;
			}
	...
	private void computeFuel(float f) {
		...
		if (stage == 6) {
			...
			case 7: // '\007'
				...
				if (d < (double) horsePowers * 0.050000000000000003D) d = (double) horsePowers * 0.050000000000000003D;
				break;

			default:
				d = thrustMax * (f1 = getPowerOutput());
				if (d < (double) thrustMax * 0.050000000000000003D) d = (double) thrustMax * 0.050000000000000003D;
				break;
			}
			if (f1 < 0.0F) f1 = 0.0F;
			double d1;
			...
			case 3: // '\003'
				...
				if (reference.actor instanceof MXY_7) tmpF = 0.5F * getPowerOutput() * f;
				else tmpF = 2.5777F * getPowerOutput() * f;
				break;
		...
		if (controlAfterburner) switch (afterburnerType) {
		case 3: // '\003'
		case 6: // '\006'
		case 7: // '\007'
		case 8: // '\b'
		...
		case 4: // '\004'
			if (reference.M.requestNitro(0.044872F * f)) break;
			reference.CT.setAfterburnerControl(false);
			if (reference.isPlayers()) {
				Main3D.cur3D().aircraftHotKeys.setAfterburnerForAutoActivation(false);
				HUD.logRightBottom(null);
	...
	private void computeReliability(float f) {
		...
		if (engineBoostFactor > 1.0F) f1 *= 0.9090909F;
		switch (type) {
		default:
			zatizeni = f1;
		...
		case 7: // '\007'
			...
			if (d <= World.Rnd().nextDouble(0.0D, 1.0D)) break;
	...
	private void computeTemperature(float f) {
		...
		if (!reference.isPlayers()) f3 *= 1.1F;
		float f4 = f3;
		float f5 = 0.8F + 60F / (50F + f1);
		float f6 = 0.45F;
		if (engineBoostFactor > 1.0F) f4 = (f3 + Math.max(f3 - 1.0F, 0.0F) * (engineBoostFactor - 1.1F) * 10F) / engineBoostFactor;
		if (!controlAfterburner && engineAfterburnerBoostFactor > 1.0F && afterburnerType != 1 && afterburnerType != 9 && afterburnerType != 4) f4 /= engineAfterburnerBoostFactor;
		float f7 = f4;
		if (type == 0 || type == 1 || type == 7) f6 = 0.45F / Atmosphere.density((float) reference.Loc.z);
		float f8 = coolMult * f4;
		if (World.cur().diffCur.ComplexEManagement && reference.isPlayers() && ((RealFlightModel) reference).isRealMode()) f7 = w / wWEP;
		if (!reference.isPlayers()) f7 = (w * 1.02F) / wWEP;
		if (stage == 6) {
			...
			if (type == 2) f6 = 0.35F + 0.1F / Atmosphere.density((float) reference.Loc.z);
	...
	public void updateRadiator(float f) {
		if ((reference.actor instanceof GLADIATOR) || (reference.actor instanceof DXXI_DK) || (reference.actor instanceof DXXI_DU) || (reference.actor instanceof DXXI_SARJA3_EARLY) || (reference.actor instanceof DXXI_SARJA3_LATE)
				|| (reference.actor instanceof DXXI_SARJA3_SARVANTO)) {
			controlRadiator = 0.0F;
			return;
		}
		if ((reference.actor instanceof P_51) || (reference.actor instanceof P_38) || (reference.actor instanceof YAK_3) || (reference.actor instanceof YAK_3P) || (reference.actor instanceof YAK_9M) || (reference.actor instanceof YAK_9U)
			...
			if (tOilOut > tOilOutMaxRPM) {
				...
				if (controlRadiator > 1.0F) controlRadiator = 1.0F;
			} else {
				controlRadiator = 1.0F - reference.getSpeed() / reference.VmaxH;
				if (controlRadiator < 0.0F) controlRadiator = 0.0F;
			}
			return;
		}
		...
		case 8: // '\b'
			if (type == 0) {
				...
				if (tOilOut >= tOilOutMaxRPM - 10F) break;
				controlRadiator -= 0.1F * f;
				if (controlRadiator < 0.0F) controlRadiator = 0.0F;
				break;
			}
	...
	private void computeForces(float f) {
		...
		case 9: // '\009'
		case 7: // '\007'
			if (Math.abs(w) < 1E-005F) propPhiW = 1.570796F;
			else if (type == 7) propPhiW = (float) Math.atan(Math.abs(reference.Vflow.x) / (double) (w * propReductor * propr));
			else propPhiW = (float) Math.atan(reference.Vflow.x / (double) (w * propReductor * propr));
			propAoA = propPhi - propPhiW;
			if (type == 7) computePropForces(w * propReductor, (float) Math.abs(reference.Vflow.x), propPhi, propAoA, reference.getAltitude());
			else computePropForces(w * propReductor, (float) reference.Vflow.x, propPhi, propAoA, reference.getAltitude());
			switch (propAngleDeviceType) {
			case 3: // '\003'
			case 4: // '\004'
				...
				if (isPropAngleDeviceOperational()) {
					if (bControlPropAuto) propTarget = propPhiW + propAoA0;
					else propTarget = propPhiMax - controlProp * (propPhiMax - propPhiMin);
				} else if (propAngleDeviceType == 3) propTarget = 0.0F;
				else propTarget = 3.141593F;
				break;

			case 9: // '\t'
				if (bControlPropAuto) {
					float f15 = propAngleDeviceMaxParam;
					if (controlAfterburner) f15 = propAngleDeviceAfterburnerParam;
					controlProp += ((float) controlPropDirection * f) / 5F;
					if (controlProp > 1.0F) controlProp = 1.0F;
					else if (controlProp < 0.0F) controlProp = 0.0F;
					...
					if (isPropAngleDeviceOperational()) {
						...
						if (stage == 6 && propTarget < propPhiW - 0.12F) {
							...
							if (propPhi < propTarget) propPhi += 0.2F * f;
						}
					} else {
						propTarget = propPhi;
					}
					...
					propTarget = propPhi;
					if (isPropAngleDeviceOperational()) if (controlPropDirection > 0) propTarget = propPhiMin;
					else if (controlPropDirection < 0) propTarget = propPhiMax;
				}
				break;

			case 1: // '\001'
			case 2: // '\002'
				if (bControlPropAuto) if (engineBoostFactor > 1.0F) controlProp = 0.75F + 0.227272F * controlThrottle;
				else controlProp = 0.75F + 0.25F * controlThrottle;
				float f16 = propAngleDeviceMaxParam;
				if (controlAfterburner && (!bWepRpmInLowGear || controlCompressor != compressorMaxStep)) f16 = propAngleDeviceAfterburnerParam;
				...
				} else if (propAngleDeviceType == 1) propTarget = 0.0F;
				else propTarget = 1.5708F;
				break;

			case 7: // '\007'
				float f22 = controlThrottle;
				if (engineBoostFactor > 1.0F) f22 = 0.9090909F * controlThrottle;
				float f17 = propAngleDeviceMaxParam;
				if (controlAfterburner) if (afterburnerType == 1) {
					if (controlThrottle > 1.0F) f17 = propAngleDeviceMaxParam + 10F * (controlThrottle - 1.0F) * (propAngleDeviceAfterburnerParam - propAngleDeviceMaxParam);
				} else {
				...
				if (isPropAngleDeviceOperational()) if (bControlPropAuto) {
					if (w < f2) {
					...
					if (propTarget < propPhiMin + (float) Math.toRadians(3D)) propTarget = propPhiMin + (float) Math.toRadians(3D);
				} else {
					propTarget = (1.0F - f * 0.1F) * propTarget + f * 0.1F * (propPhiMax - controlProp * (propPhiMax - propPhiMin));
					if (w > 1.02F * wMax) wMaxAllowed = (1.0F - 4E-007F * (w - 1.02F * wMax)) * wMaxAllowed;
					if (w > wMax) {
						float f26 = w - wMax;
						f26 *= f26;
			...
			case 8: // '\b'
				...
				if (engineBoostFactor > 1.0F) f23 = 0.9090909F * controlThrottle;
				float f18 = propAngleDeviceMaxParam;
				if (controlAfterburner) if (afterburnerType == 1) {
					if (controlThrottle > 1.0F) f18 = propAngleDeviceMaxParam + 10F * (controlThrottle - 1.0F) * (propAngleDeviceAfterburnerParam - propAngleDeviceMaxParam);
				} else {
			...
			case 6: // '\006'
				...
				if (isPropAngleDeviceOperational()) if (bControlPropAuto) {
					...
					if (controlProp < 0.0F) controlProp = 0.0F;
					if (controlProp > 1.0F) controlProp = 1.0F;
				} else {
			...
			case 5: // '\005'
				...
				if (bControlPropAuto) if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()) {
					if (World.cur().diffCur.ComplexEManagement) controlProp = -controlThrottle;
					else controlProp = -Aircraft.cvt(reference.getSpeed(), reference.Vmin, reference.Vmax, 0.0F, 1.0F);
				} else {
			...
			if (controlFeather == 1 && bHasFeatherControl && isPropAngleDeviceOperational()) propTarget = 1.55F;
			...
			if (propTarget > propPhiMax) propTarget = propPhiMax;
			else if (propTarget < propPhiMin) propTarget = propPhiMin;
			if (propPhi > propPhiMax && controlFeather == 0) propPhi = propPhiMax;
			else if (propPhi < propPhiMin) propPhi = propPhiMin;
			engineMoment = getN();
			...
			if (aw > 0.0F) aw *= engineAcceleration;
			oldW = w;
			w += aw * f;
			if (w < 0.0F) w = 0.0F;
			if (w > wMaxAllowed + wMaxAllowed) w = wMaxAllowed + wMaxAllowed;
			if (oldW == 0.0F) {
				if (w < 10F * fricCoeffT) w = 0.0F;
			} else if (w < 2.0F * fricCoeffT) w = 0.0F;
			if (reference.isPlayers() && World.cur().diffCur.Torque_N_Gyro_Effects && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()) {
				...
				if (propDirection == 1) propIW.x = -propIW.x;
			...
			if (addVside < 0.0D) addVside = 0.0D;
			break;
		...
		case 2: // '\002'
			...
			if (aw > 0.0F) aw *= engineAcceleration;
			w += aw * f;
			if (w < -wMaxAllowed) w = -wMaxAllowed;
			if (w > wMaxAllowed + wMaxAllowed) w = wMaxAllowed + wMaxAllowed;
			engineForce.set(engineVector);
			engineForce.scale(propForce);
		...
		case 10: // '\002'
			...
			if (Math.abs(w) < 1E-005F) propPhiW = 1.570796F;
			else propPhiW = (float) Math.atan(((Tuple3d) (((FlightModelMain) (reference)).Vflow)).x / (double) (w * propReductor * propr));
			propAoA = propPhi - propPhiW;
		...
		case 4: // '\004'
			...
			if (w < wMin || w < 0.0F || reference.M.fuel == 0.0F || stage != 6) w = 0.0F;
		...
		case 6: // '\006'
			w = wMin + (wMax - wMin) * controlThrottle;
			if (w < wMin || w < 0.0F || stage != 6) w = 0.0F;
			float f34 = reference.getSpeed() / 94F;
			if (f34 < 1.0F) w = 0.0F;
			else f34 = (float) Math.sqrt(f34);
			propForce = (w / wMax) * thrustMax * f34;
			propForce *= getStageMultiplier();
			float f7 = (float) reference.Vwld.length();
			if (f7 > 208.333F) if (f7 > 291.666F) propForce = 0.0F;
			else propForce *= (float) Math.sqrt((291.666F - f7) / 83.33299F);
			engineForce.set(engineVector);
			engineForce.scale(propForce);
			engineTorque.cross(enginePos, engineForce);
			rpm = toRPM(w);
			if (!(reference instanceof RealFlightModel)) break;
			RealFlightModel realflightmodel = (RealFlightModel) reference;
			f7 = Aircraft.cvt(propForce, 0.0F, thrustMax, 0.0F, 0.21F);
			if (realflightmodel.producedShakeLevel < f7) realflightmodel.producedShakeLevel = f7;
			break;
	...
	public float getPowerOutput() {
		if (stage == 0 || stage > 6) return 0.0F;
		else return controlThrottle * readyness;
	}
	...
	private float getPropAngleDeviceSpeed() {
		if (isPropAngleDeviceHydroOperable) return propAngleChangeSpeed;
		else return propAngleChangeSpeed * 10F;
	}
	...
	public float getElPropPos() {
		float f;
		if (bControlPropAuto) f = controlProp;
		else f = (propPhiMax - propPhi) / (propPhiMax - propPhiMin);
	...
	public boolean isAllowsAutoProp() {
		if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()) if (World.cur().diffCur.ComplexEManagement) switch (propAngleDeviceType) {
		case 0: // '\0'
			return false;
		...
		case 1: // '\001'
		case 2: // '\002'
			return (reference.actor instanceof SPITFIRE9E) || (reference.actor instanceof SPITFIRE9ECLP) || (reference.actor instanceof SPITFIRE9EHF) || (reference.actor instanceof SPITFIRE925LBSCW) || (reference.actor instanceof SPITFIRE8)
	...
	public boolean isAllowsAutoRadiator() {
		if (World.cur().diffCur.ComplexEManagement) {
			if ((reference.actor instanceof P_51) || (reference.actor instanceof P_38) || (reference.actor instanceof YAK_3) || (reference.actor instanceof YAK_3P) || (reference.actor instanceof YAK_9M) || (reference.actor instanceof YAK_9U)
					|| (reference.actor instanceof YAK_9UT) || (reference.actor instanceof SPITFIRE8) || (reference.actor instanceof SPITFIRE8CLP) || (reference.actor instanceof SPITFIRE9) || (reference.actor instanceof P_63C)) return true;
			switch (propAngleDeviceType) {
			case 7: // '\007'
				return true;
	...
	private float getFanCy(float f) {
		if (f > 34F) f = 34F;
		if (f < -8F) f = -8F;
		if (f < 16F) return -0.004688F * f * f + 0.15F * f + 0.4F;
		float f1 = 0.0F;
		if (f > 22F) {
			f1 = 0.01F * (f - 22F);
			f = 22F;
	...
	private float getFanCx(float f) {
		if (f < -4F) f = -8F - f;
		if (f > 34F) f = 34F;
		if ((double) f < 16D) return 0.00035F * f * f + 0.0028F * f + 0.0256F;
		float f1 = 0.0F;
		if (f > 22F) {
			f1 = 0.04F * (f - 22F);
			f = 22F;
	...
	public float forcePropAOA(float f, float f1, float f2, boolean flag, float f3) {
		...
		case 7: // '\007'
			...
			boolean flag3;
			{
				...
				w *= f3;
				controlThrottle = f2;
				if ((double) engineBoostFactor <= 1.0D && controlThrottle > 1.0F) controlThrottle = 1.0F;
				if (afterburnerType > 0 && flag) controlAfterburner = true;
				stage = 6;
				fastATA = true;
				...
				float f7 = -1E+008F;
				flag3 = false;
				if ((Aircraft) (Aircraft) reference.actor instanceof TypeTwoPitchProp) flag3 = true;
				while (propAngleDeviceType == 0 || flag3) {
					...
					do {
						...
						maxW = w;
						if (j > 32 || (double) f10 <= 1.0000000000000001E-005D) break;
					...
					if (!flag3) break;
					if (f6 != propPhiMin) break;
					f7 = propForce;
					f6 = propPhiMax;
				}
				if (f7 > propForce) propForce = f7;
			}
			controlThrottle = f4;
			...
			fastATA = false;
			w = 0.0F;
			if (flag3 || propAngleDeviceType == 0) return propForce;
			float f9 = 1.5F;
			float f11 = -0.06F;
			int k = 0;
			do {
				float f14 = 0.5F * (f9 + f11);
				if (flag && (!bWepRpmInLowGear || controlCompressor != compressorMaxStep)) computePropForces(wWEP * f3 * propReductor, f, 0.0F, f14, f1);
				else computePropForces(wMax * f3 * propReductor, f, 0.0F, f14, f1);
				if ((propForce <= 0.0F || Math.abs(propMoment - maxMoment) >= 1E-005F) && k <= 32) {
					if (propForce > 0.0F && propMoment > maxMoment) f9 = f14;
					else f11 = f14;
					k++;
				} else {
					return propForce;
		...
		case 10:
			...
			do {
				...
				if (flag) computePropForces(wWEP * propReductor, f, 0.0F, f13, f1);
				else computePropForces(wMax * propReductor, f, 0.0F, f13, f1);
				if ((propForce <= 0.0F || Math.abs(propMoment - f5) >= 1E-005F) && kx <= 32) {
					if (propForce > 0.0F && propMoment > f5) f7 = f13;
					else f9x = f13;
					kx++;
				} else {
					// TODO
		...
		case 2: // '\002'
			...
			float f18 = 1.0F - 0.006F * (Atmosphere.temperature(f1) - 290F);
	...
	private void overrevving() {
		if ((reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode() && World.cur().diffCur.ComplexEManagement && World.cur().diffCur.Engine_Overheat && w > wMaxAllowed && bIsMaster) {
			...
			if (getReadyness() < 0.2F) setEngineDies(reference.actor);
		}
	}
	...
	public float getN() {
		if (stage == 6) {
			...
			case 0: // '\0'
				...
				if (getControlThrottle() > 1.0F) tmpF *= engineBoostFactor;
				overrevving();
				break;
			...
			case 3: // '\003'
				...
				if (reference.getOverload() < 0.0F && neg_G_Counter >= 0.0F) {
					...
					tmpF *= f10;
					if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode() && bIsMaster && neg_G_Counter > World.Rnd().nextFloat(5F, 8F)) setEngineStops(reference.actor);
			...
			case 2: // '\002'
				...
				float f6;
				if (controlAfterburner) f6 = w / (wWEP * f2);
				else f6 = w / (wNom * f2);
				tmpF = f8 * (2.0F * f6 - 1.0F * f6 * f6);
				if (getControlThrottle() > 1.0F) tmpF *= 1.0F + (getControlThrottle() - 1.0F) * 10F * (engineBoostFactor - 1.0F);
				overrevving();
				break;
			...
			case 4: // '\004'
				...
				if (controlAfterburner) {
					f7 = w / (wWEP * f3);
					if (f3 >= 0.95F) bFullT = true;
					else bFullT = false;
				} else {
					f7 = w / (wNom * f3);
					bFullT = false;
					if ((reference.actor instanceof SPITFIRE5B) && f3 >= 0.95F) bFullT = true;
				}
				...
				if (reference.getOverload() < 0.0F && neg_G_Counter >= 0.0F) {
					...
					if (bFullT && neg_G_Counter < 0.5F) {
						...
						producedDistabilisation += 10F + 5F * neg_G_Counter;
						tmpF *= f11;
						if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode() && bIsMaster && neg_G_Counter > World.Rnd().nextFloat(7.5F, 9.5F)) setEngineStops(reference.actor);
					}
				} else if (reference.getOverload() >= 0.0F && neg_G_Counter > 0.0F) {
					neg_G_Counter -= 0.03F;
					if (!bFullT) {
			...
			if (controlAfterburner) if (afterburnerType == 1) {
				if (controlThrottle > 1.0F && reference.M.nitro > 0.0F) tmpF *= engineAfterburnerBoostFactor;
			} else if (afterburnerType == 8 || afterburnerType == 7) {
				if (controlCompressor < compressorMaxStep) tmpF *= engineAfterburnerBoostFactor;
			} else {
				tmpF *= engineAfterburnerBoostFactor;
			}
			if (engineDamageAccum > 0.0F) engineDamageAccum -= 0.01F;
			if (engineDamageAccum < 0.0F) engineDamageAccum = 0.0F;
			if (tmpF < 0.0F) tmpF = Math.max(tmpF, -0.8F * w * _1_wMax * engineMomentMax);
			return tmpF;
		}
	...
	private float getDistabilisationMultiplier() {
		if (engineMoment < 0.0F) return 1.0F;
		float f = 1.0F + World.Rnd().nextFloat(-1F, 0.1F) * getDistabilisationAmplitude();
		if (f < 0.0F && w < 0.5F * (wMax + wMin)) return 0.0F;
		else return f;
	}
	...
	private float getCompressorMultiplier(float f) {
		...
		case 1: // '\001'
			...
			if (fastATA) {
				for (controlCompressor = 0; controlCompressor <= compressorMaxStep; controlCompressor++) {
					...
					if (f2 > f5) {
						float f46 = 1.0F - f5;
						if (f46 < 0.0001F) f46 = 0.0001F;
						float f52 = 1.0F - f2;
						float f57 = 1.0F;
						for (int k = 1; k <= controlCompressor; k++)
							if (compressorAltMultipliers[controlCompressor] >= 1.0F) f57 *= compressorBaseMultipliers[controlCompressor];
							else f57 *= compressorBaseMultipliers[controlCompressor] * compressorAltMultipliers[controlCompressor];
					...
					if (compressor2ndThrottle > 1.0F) compressor2ndThrottle = 1.0F;
					compressorManifoldPressure *= compressor2ndThrottle;
					compressor1stThrottle = f18 / compressorRPMtoWMaxATA;
					if (compressor1stThrottle > 1.0F) compressor1stThrottle = 1.0F;
				...
				float f37 = f18;
				if (controlAfterburner) f37 *= afterburnerCompressorFactor;
				do {
					...
					if (f2 > f6) {
						float f53 = 1.0F - f6;
						if (f53 < 0.0001F) f53 = 0.0001F;
						float f58 = 1.0F - f2;
						float f62 = 1.0F;
						for (int l = 1; l <= controlCompressor; l++)
							if (compressorAltMultipliers[controlCompressor] >= 1.0F) f62 *= compressorBaseMultipliers[controlCompressor];
							else f62 *= compressorBaseMultipliers[controlCompressor] * compressorAltMultipliers[controlCompressor];
					...
					if (!compressorStepFound) {
						controlCompressor++;
						if (controlCompressor == compressorMaxStep + 1) compressorStepFound = true;
					}
				...
				if (fastATA) compressor2ndThrottle = f20;
				else compressor2ndThrottle -= 3F * f * (compressor2ndThrottle - f20);
				if (compressor2ndThrottle > 1.0F) compressor2ndThrottle = 1.0F;
				compressorManifoldPressure *= compressor2ndThrottle;
				compressor1stThrottle = f18 / compressorRPMtoWMaxATA;
				if (compressor1stThrottle > 1.0F) compressor1stThrottle = 1.0F;
			...
			if (w <= 20F && w < 150F) compressorManifoldPressure = Math.min(compressorManifoldPressure, f2 * (0.4F + (w - 20F) * 0.04F));
			if (w < 20F) compressorManifoldPressure = f2 * (1.0F - w * 0.03F);
			if (mixerType == 1 && stage == 6) compressorManifoldPressure *= getMixMultiplier();
			return f26;
		...
		case 2: // '\002'
			...
			if (fastATA) {
				...
				for (controlCompressor = 0; controlCompressor <= compressorMaxStep; controlCompressor++) {
					...
					float f65 = 1.0F - f3;
					if (f3 > f8) {
						if (f63 < 0.0001F) f63 = 0.0001F;
						for (int i1 = 1; i1 <= controlCompressor; i1++)
							if (compressorAltMultipliers[controlCompressor] >= 1.0F) f59 *= compressorBaseMultipliers[controlCompressor];
							else f59 *= compressorBaseMultipliers[controlCompressor] * compressorAltMultipliers[controlCompressor];
				...
				float f39 = f18;
				if (controlAfterburner) f39 *= afterburnerCompressorFactor;
				float f44 = 1.0F;
				float f49 = 1.0F - f3;
				...
				if (f60 < f39 && compressorMaxStep == 1) {
					...
					float f68 = 1.0F - f9;
					if (f68 < 0.0001F) f68 = 0.0001F;
					float f70 = (f3 * (compressorPAt0 + (1.0F - compressorPAt0)) * w * _1_wMax * compressorRPMtoWMaxATA) / f9 + 0.001F;
					if (f70 > f39) {
						if (compressorAltMultipliers[1] >= 1.0F) f44 *= compressorBaseMultipliers[1];
						else f44 *= compressorBaseMultipliers[1] * compressorAltMultipliers[1];
			...
			if (w <= 20F && w < 150F) compressorManifoldPressure = Math.min(compressorManifoldPressure, f3 * (0.4F + (w - 20F) * 0.04F));
			if (w < 20F) compressorManifoldPressure = f3 * (1.0F - w * 0.03F);
			return f27;
		...
		case 3: // '\003'
			...
			if (f4 > f11) {
				float f50 = 1.0F - f11;
				if (f50 < 0.0001F) f50 = 0.0001F;
				float f56 = 1.0F - f4;
				if (f56 < 0.0F) f56 = 0.0F;
				float f61 = 1.0F;
			...
			if (fastATA) compressor2ndThrottle = f23;
			else compressor2ndThrottle -= 3F * f * (compressor2ndThrottle - f23);
			if (compressor2ndThrottle > 1.0F) compressor2ndThrottle = 1.0F;
			compressorManifoldPressure *= compressor2ndThrottle;
			compressor1stThrottle = f18 / compressorRPMtoWMaxATA;
			if (compressor1stThrottle > 1.0F) compressor1stThrottle = 1.0F;
			...
			f28 *= f35;
			if (w <= 20F && w < 150F) compressorManifoldPressure = Math.min(compressorManifoldPressure, f4 * (0.4F + (w - 20F) * 0.04F));
			if (w < 20F) compressorManifoldPressure = f4 * (1.0F - w * 0.03F);
			return f28;
		}
		return 1.0F;
	}
	...
	private float getMixMultiplier() {
		...
		case 1: // '\001'
			if (controlMix == 1.0F) {
				if (bFloodCarb) reference.AS.setSootState(reference.actor, number, 1);
				else reference.AS.setSootState(reference.actor, number, 0);
				return 1.0F;
			}
			// fall through
		...
		case 2: // '\002'
			if (reference.isPlayers() && (reference instanceof RealFlightModel) && ((RealFlightModel) reference).isRealMode()) {
				...
				if (f < pressureExtBar - f4) {
					if (f < 0.03F) setEngineStops(reference.actor);
					if (bFloodCarb) reference.AS.setSootState(reference.actor, number, 1);
					else reference.AS.setSootState(reference.actor, number, 0);
			...
			if (f1 < pressureExtBar - f4 && f1 < 0.03F) setEngineStops(reference.actor);
			return 1.0F;
	...
	private float getFrictionMoment(float f) {
		...
		if (!bPropHit && (bIsInoperable || stage == 0 || controlMagneto == 0)) {
			...
			if ((((Interpolate) (reference)).actor instanceof BF_109) || (((Interpolate) (reference)).actor instanceof BF_110) || (((Interpolate) (reference)).actor instanceof JU_87) || (((Interpolate) (reference)).actor instanceof JU_88)
					|| (((Interpolate) (reference)).actor instanceof JU_88NEW) || ((((Interpolate) (reference)).actor instanceof FW_190) && type == 0)) {
				...
				fricCoeffT = 0.25F;
			} else if ((((Interpolate) (reference)).actor instanceof P_51) || (((Interpolate) (reference)).actor instanceof P_47) || (((Interpolate) (reference)).actor instanceof P_38) || (((Interpolate) (reference)).actor instanceof P_39)
					|| (((Interpolate) (reference)).actor instanceof P_40) || (((Interpolate) (reference)).actor instanceof P_40SUKAISVOLOCH) || (((Interpolate) (reference)).actor instanceof SPITFIRE)
					|| (((Interpolate) (reference)).actor instanceof Hurricane) || ((((Interpolate) (reference)).actor instanceof FW_190) && type == 1) || type == 1) {
				fricCoeffT += 0.08F * f;
				if (fricCoeffT > 0.2F) fricCoeffT = 0.2F;
			} else if (type == 9) // TODO: Spin-down behaviour for rotaries. Great rotating inertial mass, hence longer spin-down
			{
				fricCoeffT += 0.07F * f;
				if (fricCoeffT > 0.05F) fricCoeffT = 0.05F;
			} else {
				fricCoeffT += 0.09F * f;
				if (fricCoeffT > 0.3F) fricCoeffT = 0.3F;
			}
			float f2 = w * _1_wMax;
			...
			fricCoeffT += 0.2F * f;
			if (fricCoeffT > 2.0F) fricCoeffT = 2.0F;
			float f2 = w * _1_wMax;
			f1 = (-fricCoeffT * (6F + 3.8F * f2) * (propI + engineI)) / f;
			float f3 = (-0.99F * w * (propI + engineI)) / f;
			if (f1 < f3) f1 = f3;
	...
	private float getJetFrictionMoment(float f) {
		float f1 = 0.0F;
		if (bIsInoperable || stage == 0) f1 = (-0.002F * w * (propI + engineI)) / f;
		return f1;
	}

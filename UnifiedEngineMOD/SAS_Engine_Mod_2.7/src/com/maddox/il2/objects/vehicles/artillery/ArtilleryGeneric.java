...
public abstract class ArtilleryGeneric extends ActorHMesh
    implements MsgCollisionRequestListener, MsgExplosionListener,
	       MsgShotListener, Predator, Obstacle, ActorAlign, HunterInterface
{
	...
	private static float getF(SectFile sectfile, String string,
		...
		if (f_2_ == -9865.345F)
		    System.out.println("Artillery: Parameter [" + string
				       + "]:<" + string_0_ + "> "
				       + "not found");
		else
		    System.out.println("Artillery: Value of [" + string + "]:<"
				       + string_0_ + "> (" + f_2_ + ")"
	...
	private static String getS(SectFile sectfile, String string,
		...
		System.out.print("Artillery: Parameter [" + string + "]:<"
				 + string_3_ + "> ");
		System.out
		    .println(string_4_ == null ? "not found" : "is empty");
		throw new RuntimeException("Can't set property");
	    }
	    return string_4_;
	}
	...
	private static ArtilleryProperties LoadArtilleryProperties
	    (SectFile sectfile, String string, Class var_class) {
	    ArtilleryProperties artilleryproperties
		= new ArtilleryProperties();
	    String string_8_ = getS(sectfile, string, "PanzerType", null);
	    if (string_8_ == null)
		string_8_ = "Tank";
	    artilleryproperties.fnShotPanzer
		= TableFunctions.GetFunc2(string_8_ + "ShotPanzer");
	    artilleryproperties.fnExplodePanzer
		= TableFunctions.GetFunc2(string_8_ + "ExplodePanzer");
	    artilleryproperties.PANZER_TNT_TYPE
		= getF(sectfile, string, "PanzerSubtype", 0.0F, 100.0F);
	    artilleryproperties.meshSummer
		= getS(sectfile, string, "MeshSummer");
	    artilleryproperties.meshDesert
		= getS(sectfile, string, "MeshDesert",
		       artilleryproperties.meshSummer);
	    artilleryproperties.meshWinter
		= getS(sectfile, string, "MeshWinter",
		       artilleryproperties.meshSummer);
	    artilleryproperties.meshSummer1
		= getS(sectfile, string, "MeshSummerDamage", null);
	    artilleryproperties.meshDesert1
		= getS(sectfile, string, "MeshDesertDamage",
		       artilleryproperties.meshSummer1);
	    artilleryproperties.meshWinter1
		= getS(sectfile, string, "MeshWinterDamage",
		       artilleryproperties.meshSummer1);
	    int i = ((artilleryproperties.meshSummer1 == null ? 1 : 0)
		     + (artilleryproperties.meshDesert1 == null ? 1 : 0)
		     + (artilleryproperties.meshWinter1 == null ? 1 : 0));
	    if (i != 0 && i != 3) {
		System.out.println
		    ("Artillery: Uncomplete set of damage meshes for '"
		     + string + "'");
		throw new RuntimeException("Can't register artillery object");
	    }
	    artilleryproperties.PANZER_BODY_FRONT
		= getF(sectfile, string, "PanzerBodyFront", 0.0010F, 9.999F);
	    if (sectfile.get(string, "PanzerBodyBack", -9865.345F)
		== -9865.345F) {
		artilleryproperties.PANZER_BODY_BACK
		    = artilleryproperties.PANZER_BODY_FRONT;
		artilleryproperties.PANZER_BODY_SIDE
		    = artilleryproperties.PANZER_BODY_FRONT;
		artilleryproperties.PANZER_BODY_TOP
		    = artilleryproperties.PANZER_BODY_FRONT;
	    } else {
		artilleryproperties.PANZER_BODY_BACK
		    = getF(sectfile, string, "PanzerBodyBack", 0.0010F,
			   9.999F);
		artilleryproperties.PANZER_BODY_SIDE
		    = getF(sectfile, string, "PanzerBodySide", 0.0010F,
			   9.999F);
		artilleryproperties.PANZER_BODY_TOP
		    = getF(sectfile, string, "PanzerBodyTop", 0.0010F, 9.999F);
	    }
	    if (sectfile.get(string, "PanzerHead", -9865.345F) == -9865.345F)
		artilleryproperties.PANZER_HEAD
		    = artilleryproperties.PANZER_BODY_FRONT;
	    else
		artilleryproperties.PANZER_HEAD
		    = getF(sectfile, string, "PanzerHead", 0.0010F, 9.999F);
	    if (sectfile.get(string, "PanzerHeadTop", -9865.345F)
		== -9865.345F)
		artilleryproperties.PANZER_HEAD_TOP
		    = artilleryproperties.PANZER_BODY_TOP;
	    else
		artilleryproperties.PANZER_HEAD_TOP
		    = getF(sectfile, string, "PanzerHeadTop", 0.0010F, 9.999F);
	    float f = Math.min(Math.min(artilleryproperties.PANZER_BODY_BACK,
					artilleryproperties.PANZER_BODY_TOP),
			       Math.min(artilleryproperties.PANZER_BODY_SIDE,
					artilleryproperties.PANZER_HEAD_TOP));
	    artilleryproperties.HITBY_MASK = f > 0.015F ? -2 : -1;
	    artilleryproperties.explodeName
		= getS(sectfile, string, "Explode", "Artillery");
	    int i_9_ = 0;
	    for (;;) {
		if (i_9_ != 0) {
		...
		String string_14_ = ("com.maddox.il2.objects.weapons."
				     + getS(sectfile, string, string_12_));
		try {
		    artilleryproperties.gunProperties[i_11_].gunClass
			= Class.forName(string_14_);
		} catch (Exception exception) {
		    System.out.println("Artillery: Can't find gun class '"
				       + string_14_ + "'");
		    throw new RuntimeException
			      ("Can't register artillery object");
		}
		artilleryproperties.gunProperties[i_11_].WEAPONS_MASK
		    = (Gun.getProperties
		       (artilleryproperties.gunProperties[i_11_].gunClass)
		       .weaponType);
		if (artilleryproperties.gunProperties[i_11_].WEAPONS_MASK
		    == 0) {
		    System.out.println
			("Artillery: Undefined weapon type in gun class '"
			 + string_14_ + "'");
		    throw new RuntimeException
			      ("Can't register artillery object");
		}
		artilleryproperties.gunProperties[i_11_].ATTACK_FAST_TARGETS
		    = true;
		float f_15_
		    = sectfile.get(string, "FireFastTargets" + string_13_,
				   -9865.345F);
		if (f_15_ != -9865.345F)
		    artilleryproperties.gunProperties[i_11_]
			.ATTACK_FAST_TARGETS
			= f_15_ > 0.5F;
			...
			.ATTACK_FAST_TARGETS
			= false;
		artilleryproperties.gunProperties[i_11_].ATTACK_MAX_DISTANCE
		    = getF(sectfile, string, "AttackMaxDistance" + string_13_,
			   6.0F, 60000.0F);
		artilleryproperties.gunProperties[i_11_].ATTACK_MAX_RADIUS
		    = getF(sectfile, string, "AttackMaxRadius" + string_13_,
			   6.0F, 60000.0F);
		artilleryproperties.gunProperties[i_11_].ATTACK_MAX_HEIGHT
		    = getF(sectfile, string, "AttackMaxHeight" + string_13_,
			   6.0F, 18000.0F);
		int i_16_ = sectfile.sectionIndex(string);
		if (sectfile.varExist(i_16_,
				      "HeadYawHalfRange" + string_13_)) {
		    float f_17_
			= getF(sectfile, string,
			       "HeadYawHalfRange" + string_13_, 0.0F, 180.0F);
		    artilleryproperties.gunProperties[i_11_].HEAD_YAW_RANGE
			= new AnglesRange(-1.0F, 1.0F);
		    artilleryproperties.gunProperties[i_11_].HEAD_YAW_RANGE
			.set(-f_17_, f_17_);
		    artilleryproperties.gunProperties[i_11_].HEAD_STD_YAW
			= 0.0F;
		} else {
		    float f_18_
			= getF(sectfile, string, "HeadMinYaw" + string_13_,
			       -180.0F, 180.0F);
		    float f_19_
			= getF(sectfile, string, "HeadStdYaw" + string_13_,
			       -180.0F, 180.0F);
		    float f_20_
			= getF(sectfile, string, "HeadMaxYaw" + string_13_,
			       -180.0F, 180.0F);
		    artilleryproperties.gunProperties[i_11_].HEAD_YAW_RANGE
			= new AnglesRange(-1.0F, 1.0F);
		    artilleryproperties.gunProperties[i_11_].HEAD_YAW_RANGE
		...
		artilleryproperties.gunProperties[i_11_].GUN_MIN_PITCH
		    = getF(sectfile, string, "GunMinPitch" + string_13_,
			   -15.0F, 85.0F);
		artilleryproperties.gunProperties[i_11_].GUN_STD_PITCH
		    = getF(sectfile, string, "GunStdPitch" + string_13_,
			   -15.0F, 89.9F);
		artilleryproperties.gunProperties[i_11_].GUN_MAX_PITCH
		    = getF(sectfile, string, "GunMaxPitch" + string_13_, 0.0F,
			   89.9F);
		artilleryproperties.gunProperties[i_11_].HEAD_MAX_YAW_SPEED
		    = getF(sectfile, string, "HeadMaxYawSpeed" + string_13_,
			   0.1F, 999.0F);
		artilleryproperties.gunProperties[i_11_].GUN_MAX_PITCH_SPEED
		    = getF(sectfile, string, "GunMaxPitchSpeed" + string_13_,
			   0.1F, 999.0F);
		artilleryproperties.gunProperties[i_11_].DELAY_AFTER_SHOOT
		    = getF(sectfile, string, "DelayAfterShoot" + string_13_,
			   0.0F, 999.0F);
		artilleryproperties.gunProperties[i_11_].CHAINFIRE_TIME
		    = getF(sectfile, string, "ChainfireTime" + string_13_,
			   0.0F, 600.0F);
		float f_21_ = sectfile.get(string, "FastTargetsAngleError",
					   -9865.345F);
		if (f_21_ <= 0.0F)
		    f_21_ = 0.0F;
		else if (f_21_ >= 45.0F)
		    f_21_ = 45.0F;
		artilleryproperties.gunProperties[i_11_]
		    .FAST_TARGETS_ANGLE_ERROR
		    = f_21_;
	    }
	    Property.set(var_class, "iconName",
			 "icons/" + getS(sectfile, string, "Icon") + ".mat");
	...
	public SPAWN(Class var_class) {
		...
		proper = LoadArtilleryProperties(Statics.getTechnicsFile(),
						 string_23_, var_class);
	    } catch (Exception exception) {
	...
	public Actor actorSpawn(ActorSpawnArg actorspawnarg) {
		...
		exception.printStackTrace();
		System.out.println
		    ("SPAWN: Can't create Artillery object [class:"
		     + cls.getName() + "]");
		return null;
	    }
	...
	public boolean netInput(NetMsgInput netmsginput) throws IOException {
		...
		case 73: {
			...
			NetMsgGuaranted netmsgguaranted
			    = new NetMsgGuaranted(netmsginput, 0);
			...
			for (int i_25_ = 1; i_25_ < arms.length; i_25_++) {
				...
				if (dying != 1) {
				    ...
				    ArtilleryGeneric.this
					.setGunAngles(i_25_, f, f_24_);
				}
			    } else if (dying != 1)
				ArtilleryGeneric.this.setGunAngles(i_25_, f,
								   f_24_);
			}
		    }
		    return true;
		...
		case 68: {
		    if (isMirrored()) {
			NetMsgGuaranted netmsgguaranted
			    = new NetMsgGuaranted(netmsginput, 1);
			post(netmsgguaranted);
		    }
		    short i = netmsginput.readShort();
		    float f = netmsginput.readFloat();
			...
			com.maddox.rts.NetObj netobj
			    = netmsginput.readNetObj();
			Actor actor = (netobj == null ? null
				       : ((ActorNet) netobj).actor());
			while (netmsginput.available() > 0) {
			    i_27_++;
			    f = netmsginput.readFloat();
			    f_26_ = netmsginput.readFloat();
			    ArtilleryGeneric.this.setGunAngles(i_27_, f,
							       f_26_);
			}
		...
		Actor actor
		    = netobj == null ? null : ((ActorNet) netobj).actor();
		int i = netmsginput.readUnsignedByte();
		int i_28_ = 0;
		if (netmsginput.available() > 0)
		    i_28_ = netmsginput.readInt();
		...
		Actor actor
		    = netobj == null ? null : ((ActorNet) netobj).actor();
		float f = netmsginput.readFloat();
		float f_29_ = (0.0010F * (float) (Message.currentGameTime()
						  - Time.current())
			       + f);
		int i = netmsginput.readUnsignedByte();
		int i_30_ = 0;
		if (netmsginput.available() > 0)
		    i_30_ = netmsginput.readInt();
	...
	public boolean tick() {
		...
		if (access$610(ArtilleryGeneric.this) <= 0L) {
		    ...
		    dying = 0;
		    hideTmr = 0L;
		    if (!ArtilleryGeneric.this.isNetMirror()
			&& RADIUS_HIDE > 0.0F)
			hideTmr = -1L;
		    ArtilleryGeneric.this.setDiedFlag(false);
		    for (int i = 0; i < arms.length; i++)
	    ...
	    for (int i = 0; i < arms.length; i++)
		arms[i].aim.tick_();
	    if (RADIUS_HIDE > 0.0F && hideTmr >= 0L
		&& !ArtilleryGeneric.this.isNetMirror()) {
		for (int i = 0; i < arms.length; i++) {
		    if (arms[i].aim.getEnemy() != null)
			hideTmr = 0L;
		    else if (access$804(ArtilleryGeneric.this)
			     > ArtilleryGeneric.delay_hide_ticks)
			hideTmr = -1L;
		}
	    }
	    return true;
	...
	for (int i_33_ = 0; i_33_ < i; i_33_++) {
	    AirGroup airgroup_34_
		= AirGroupList.getGroup(airgroup.enemies[0], i_33_);
	...
	if (dying == 0 && !(shot.power <= 0.0F)
	    && (!isNetMirror() || !shot.isMirage())) {
	    if (shot.powerType == 1) {
		if (!RndB(0.15F))
		    Die(shot.initiator, (short) 0, true);
	    } else {
		float f = Shot.panzerThickness(pos.getAbsOrient(), shot.v,
					       shot.chunkName
						   .equalsIgnoreCase("Head"),
					       prop.PANZER_BODY_FRONT,
					       prop.PANZER_BODY_SIDE,
					       prop.PANZER_BODY_BACK,
					       prop.PANZER_BODY_TOP,
					       prop.PANZER_HEAD,
					       prop.PANZER_HEAD_TOP);
		f *= Rnd(0.93F, 1.07F);
		float f_38_ = prop.fnShotPanzer.Value(shot.power, f);
		if (f_38_ < 1000.0F && (f_38_ <= 1.0F || RndB(1.0F / f_38_)))
		    Die(shot.initiator, (short) 0, true);
	    }
	}
    }
	...
	if (dying == 0 && (!isNetMirror() || !explosion.isMirage())
		...
		if (TankGeneric.splintersKill(explosion, prop.fnShotPanzer,
					      Rnd(0.0F, 1.0F), Rnd(0.0F, 1.0F),
					      this, 0.7F, 0.25F,
					      prop.PANZER_BODY_FRONT,
					      prop.PANZER_BODY_SIDE,
					      prop.PANZER_BODY_BACK,
		...
		f *= Rnd(0.95F, 1.05F);
		float f_39_
		    = prop.fnExplodePanzer.Value(f, prop.PANZER_TNT_TYPE);
		if (f_39_ < 1000.0F && (f_39_ <= 1.0F || RndB(1.0F / f_39_)))
		    Die(explosion.initiator, (short) 0, true);
	    }
	}
    }
	...
	fs[1] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(4.0F, 9.0F));
	fs[2] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(4.0F, 9.0F));
	fs[3] = -arms[i_40_].gunPitch + rangerandom.nextFloat(-15.0F, 15.0F);
	fs[4] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(2.0F, 5.0F));
	fs[5] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(5.0F, 9.0F));
	fs[6] = 0.0F;
	fs[7] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(4.0F, 8.0F));
	fs[8] = ((rangerandom.nextBoolean() ? 1.0F : -1.0F)
		 * rangerandom.nextFloat(7.0F, 12.0F));
	...
	if (dying == 0) {
		...
		if (arms[i_41_].aim != null)
		    arms[i_41_].aim.forgetAiming();
	    }
	    float f = 0.0F;
		...
		if (i_42_ != 0)
		    string = "_" + i_42_;
		hierMesh().chunkSetAngles("Head" + string, fs[0], fs[1],
					  fs[2]);
	...
	if (i != 0)
	    string = "_" + i;
	hierMesh().chunkSetAngles("Head" + string, arms[i].headYaw, 0.0F,
				  0.0F);
	hierMesh().chunkSetAngles("Gun" + string, -arms[i].gunPitch, 0.0F,
				  0.0F);
	...
	for (int i_45_ = 0; i_45_ < arms.length; i_45_++)
	    setGunAngles(i_45_, prop.gunProperties[i_45_].HEAD_STD_YAW,
			 prop.gunProperties[i_45_].GUN_STD_PITCH);
	Align();
    }
    ...
    private ArtilleryGeneric(ArtilleryProperties artilleryproperties,
			     ActorSpawnArg actorspawnarg) {
	...
	if (actorspawnarg.timeLenExist) {
	    startDelay
		= (long) (actorspawnarg.timeLen * 60.0F * 1000.0F + 0.5F);
	    if (startDelay < 0L)
		startDelay = 0L;
	}
	...
	for (int i = 0; i < arms.length; i++) {
	    ...
	    try {
		arms[i].gun
		    = (Gun) prop.gunProperties[i].gunClass.newInstance();
	    } catch (Exception exception) {
		System.out.println(exception.getMessage());
		exception.printStackTrace();
		System.out.println("Artillery: Can't create gun '"
				   + prop.gunProperties[i].gunClass.getName()
				   + "'");
	    }
	    String string = "";
	    if (i != 0)
	...
	if (isNetMaster()) {
		...
		netmsgguaranted
		    .writeNetObj(actor == null ? (ActorNet) null : actor.net);
	...
	if (isNetMaster() && net.isMirrored()
	    && (Actor.isValid(actor) && actor.isNet())) {
	    i &= 0xff;
	    if (f < 0.0F) {
		try {
	...
	if (isNetMirror()
	    && !(net.masterChannel() instanceof NetChannelInStream)) {
	    try {
		...
		net.postTo(Time.current(), net.masterChannel(),
			   netmsgfiltered);
	    } catch (Exception exception) {
		System.out.println(exception.getMessage());
		exception.printStackTrace();
	...
	for (int i = 0; i < arms.length; i++) {
	    if (aim.equals(arms[i].aim))
		return (prop.gunProperties[i].CHAINFIRE_TIME <= 0.0F ? 0.0F
			: prop.gunProperties[i].CHAINFIRE_TIME * Rnd(0.75F,
								     1.25F));
	}
	return (prop.gunProperties[0].CHAINFIRE_TIME <= 0.0F ? 0.0F
		: prop.gunProperties[0].CHAINFIRE_TIME * Rnd(0.75F, 1.25F));
    }
    
    public float probabKeepSameEnemy(Actor actor) {
	if (nearAirfield || isNetMirror() || actor == null
	    || !(actor instanceof Aircraft)
	    || Math.abs(time_lastCheckShoot - Time.current()) < 12000L
	    || (float) actor.getSpeed(null) < 10.0F) {
	    if (isSpotterAcGuided)
		return 0.98F;
	    return 0.825F;
	}
	...
	for (int i = 0; i < arms.length; i++) {
	    if (aim.equals(arms[i].aim)) {
		arms[i].aim.setRotationForParking
		    (arms[i].headYaw, arms[i].gunPitch,
		     prop.gunProperties[i].HEAD_STD_YAW,
		     prop.gunProperties[i].GUN_STD_PITCH,
		     prop.gunProperties[i].HEAD_YAW_RANGE,
		     prop.gunProperties[i].HEAD_MAX_YAW_SPEED,
	...
	if (isSpotterAcGuided && spotterAc == null) {
	    spotterAc
		= War.getNearestSpotter(this, AttackMaxRadius(aim) * 1.5F);
	    if (spotterAc != null)
	...
	if (prop.gunProperties[i].ATTACK_FAST_TARGETS)
	    ...
	    NearestEnemies.set(prop.gunProperties[i].WEAPONS_MASK, -9999.9F,
			       KmHourToMSec(100.0F));
	if (!isSpotterAcGuided)
	    actor
		= NearestEnemies.getAFoundEnemy(pos.getAbsPoint(),
						(double) AttackMaxRadius(aim),
						getArmy());
	if (actor == null)
	    return null;
	...
	if (!nearAirfield && !isNetMirror() && actor instanceof Aircraft
	    && !((float) actor.getSpeed(null) < 10.0F)) {
	    if (Math.abs(time_lastCheckShoot - Time.current()) < 12000L) {
		if (dontShoot)
		    return null;
	    } else if (friendPlanesAreNear((Aircraft) actor))
		return null;
	}
	BulletProperties bulletproperties = null;
	if (arms[i].gun.prop != null) {
	    int i_52_
		= ((Prey) actor).chooseBulletType(arms[i].gun.prop.bullet);
	    if (i_52_ < 0)
		return null;
	...
	aim.setAimingError(World.Rnd().nextFloat(-1.0F, 1.0F),
			   World.Rnd().nextFloat(-1.0F, 1.0F), 0.0F);
	...
	if (i == 1 && hideTmr < 0L) {
	    float f_54_ = (float) actor.pos.getAbsPoint()
				      .distanceSquared(pos.getAbsPoint());
	    if (f_54_ > RADIUS_HIDE * RADIUS_HIDE)
		return false;
	    hideTmr = 0L;
	}
	int i_55_ = 0;
	...
	if (!isNetMirror())
	    send_FireCommand(actor, arms[i_55_].aim.shotpoint_idx,
			     i == 0 ? -1.0F : f, i_55_);
	return true;
    }
	...
	if (dying != 1 && actor != null && arms[i].aim != null)
	    arms[i].aim.passive_StartFiring(0, actor, i_57_, 0.0F);
    }
	...
	boolean bool_61_
	    = ((Prey) actor).getShotpointOffset(arms[i].aim.shotpoint_idx, p1);
	if (!bool_61_)
	    return 0;
	float f_62_ = f * Rnd(0.8F, 1.2F);
	if (!Aimer.aim((BulletAimer) arms[i].gun, actor, this, f_62_, p1,
		       arms[i].fireOffset))
	    return 0;
...
//TODO: ZP's gunner scare code	
	...
	if (f_62_ > 0.0010F) {
		...
		double d_70_ = (point3d_66_.x * point3d_66_.x
				+ point3d_66_.y * point3d_66_.y
				+ point3d_66_.z * point3d_66_.z);
		if (d_70_ > 0.01) {
		...
		if (i_73_ >= 3 && d_65_ > (double) Mission.curCloudsHeight())
		    f_69_ *= 1.25F;
		f_64_ += f_69_;
	    }
	}
	...
	} else if (aim.getAimingError().length()
		   > 0.011 * (double) skillErrCoef) {
	    if (spotterAc == null)
	...
	if (World.Sun().ToSun.z < -0.15F) {
		...
		f_76_ = 1.0F;
	    if (actor instanceof Aircraft
		&& Time.current() - ((Aircraft) actor).tmSearchlighted < 1000L)
		f_76_ = 0.0F;
	    f_64_ += 12.0F * f_76_;
	}
	...
	if (!((BulletAimer) arms[i].gun).FireDirection(point3d1, point3d,
						       vector3d))
	    return 0;
	float f_79_;
	float f_80_;
	...
	int i_81_
	    = (arms[i].aim.setRotationForTargeting
	       (this, orient, point3d1, arms[i].headYaw, arms[i].gunPitch,
		vector3d, f_64_, f_62_, prop.gunProperties[i].HEAD_YAW_RANGE,
		prop.gunProperties[i].GUN_MIN_PITCH,
		prop.gunProperties[i].GUN_MAX_PITCH, f_79_, f_80_, 0.0F));
	return i_81_;
    }
